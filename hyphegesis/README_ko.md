...
**기질 위의 지휘(Conduction over Substrate)**: 둘 이상의 이동이 비자명한 지휘를 요구할 때, Hyphegesis는 식별한 이동들 위에서 지휘 토폴로지를 **먼저 초안으로 채웁니다** — 방법 전체를 한 번에, 각 값을 그것이 밀어낸 대안들과 나란히, 영향/레버리지 순으로, 지도 한 장에 — 답할 때마다 무엇이 바뀌었는지 변경 장부와 함께 다시 그리고, 방법 계획을 핸드오프합니다. 미루는 것은 판단 근거가 아직 존재하지 않는 결정, 또는 사용자만 줄 수 있어 계획이 미리 내다본 것(설정할 비밀 정보, 런타임에 맡기는 배포)뿐이고, 그것만 세션 내 체크포인트로 갑니다. 그 밖에는 방법이 끝난 뒤 하나의 정리된 요약으로 한 번 돌려줍니다. 이동을 실행하지 않으며, 실현 불가능한 기질에 결코 묶지 않습니다. 단일 이동 작업은 지휘하지 않고 그 한 프로토콜로 relay합니다.
...
Hyphegesis는 아무것도 묻기 전에 방법 전체를 **지도 한 장**으로 펼칩니다 — 작업 브리프, 후보 이동들(크거나 누적 그래프에 걸친 집합은 **세션 목표에 대해 salience-ranked로** 제시해, 이동 집합 확정이 사용자가 더 이상 머릿속에 두지 못하는 그래프로부터의 recall이 아니라 누적된 형태의 recognition이 되도록 함), 제안된 덩어리 가르기, 그 위의 모든 축(순서, 독립성, 화해, 종료, routing)이 근거를 단 값·나머지 값들의 이름·계획이 갈리는 지점의 차등 함의를 함께 실은 채로 — 이동은 덩어리 아래에 이름을 단 개요로(여러 선행 이동이 합쳐지는 곳에만 연결선을 그림), 축 값은 각 값이 사용자의 것인지 초안의 것인지 표시한 표로 한 장에 펼칩니다. 사용자는 어느 층이든 틀린 곳을 고치거나 그대로 받습니다. 답할 때마다 지도를 다시 그리고 변경 장부(사용자가 고친 것 먼저, 그로 인해 초안이 바꾼 값과 그 이유)를 붙여, 앞을 고치면 뒤의 초안이 따라 채워지되 사용자가 정한 값은 사용자의 것으로 남습니다. 브리프, 이동, 토폴로지를 차례로 확정하는 방식은 나머지 방법이 아직 없는 상태에서 각각에 답하게 했고, 지도 한 장이 그것을 없앱니다. 각 값을 대안과 나란히 보이는 것이 사용자를 승인하는 자리가 아니라 구성하는 자리에 남게 합니다. 둘 이상의 이동이 실제로 갈릴 때만 지휘가 유지되며(단일 이동 작업은 relay로 빠짐), 기질 실현가능성은 방법을 받기 전에 관찰해 지도에 보이고, 받는 모든 값이 근거와 함께 보였을 때 세션 내 체크포인트와 함께 방법 계획을 핸드오프합니다. 결과가 종합(synthesis)으로 화해되고 그 출력이 융합 형태를 재유도할 수 없는 소비자(사용자, 또는 `handoff_to_span`을 통한 미래 span)로 가는 영역에는 **종합 체크포인트**가 등록되고, 컴파일된 Recognition brief가 함께 실립니다 — 이동별 산출물, 수렴, 발산, 비공개-맥락 gap 슬롯, 융합/출력-형태 후보를 기질이 융합 시점에 제시해, 사용자가 융합 선택을 떠올리는(recall) 대신 인식(recognize)하게 합니다.
