...
Output all analysis, evidence, and rationale as text before presenting via Cognitive Partnership Move (Constitution). The question contains only the essential question; options contain only option-specific differential implications. Scope includes both Phase 2 gates: the draft — every axis·region's value, its ground, its named alternatives, and the differential where the plan turns — is the pre-gate text, and the draft gate carries only the question of which slots are wrong; an opened axis gate's per-value trade-off prose is likewise pre-gate text, and that gate carries only the question and per-option (per-value and per-composite) differential implications.
