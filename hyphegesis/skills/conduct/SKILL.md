...
Checkpoint = an in-session conductor re-entry point (plan | exec | verify, or a finer sub-topology point) where Constitution may re-fire, carrying brief: Option(CheckpointBrief)  -- in-session ONLY; /compact and /clear are the span wall, never checkpoints. brief populates at Phase 3 CompileCheckpointBrief and is read only after the ConductedMethod handoff completes; a registered synthesis checkpoint with brief = None before then is a valid in-protocol interim state, not a contract for the substrate
