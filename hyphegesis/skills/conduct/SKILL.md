...
Phase 1 PresentOwedReapportionment (extension) → TextPresent+Proceed (fires on the |MS| < 2 ∧ withdrawn_units ≠ ∅ relay exit at A_s = Confirm: present {owed_reapportionment(w) | (w, r) ∈ Λ.withdrawn_units}, together with Λ.carried_locator when it is Some, so the owed resolver can re-reach the canonical plan record in the session that owns it. At |MS| = 1 this rides in the SAME relay text that routes to the surviving move's protocol; at |MS| = 0 this presentation IS the relay text on its own)
