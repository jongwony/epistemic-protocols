...
Phase 3 AssembleTraceContract (track)    → internal Λ update (ASSEMBLE the cross-cutting disclosure overlay: populate Λ.trace_contract from Λ.residuals + Λ.degradations + coverage caps derived from the resolved topology via the source→bound functor (bounded_rounds → no_retry, until_dry_ceiling → top_n, region sampling/selection → sampling/top_n, boundary scope reduction → scope_cut; see CoverageLimit); an INVARIANT aggregation, never gated — never-silent has no alternative value to choose, so no AxisGate fires)
