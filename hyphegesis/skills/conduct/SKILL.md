...
checkpoint_set(WP, CT) = { Checkpoint(r, SynthesisOutputShape, None) | CT[reconciliation][r] contains synthesis ∧ CT[routing][r] ∈ {return_to_user, handoff_to_span} }
compile_checkpoint_brief(c, WP, CT, MS) = SynthesisBrief compiled from current CT + MS when c.decision = SynthesisOutputShape
...
TerminationGround = { region: MoveRegion, ground ∈ {protocol_contract(ref), stated_condition(ref), resolution_required(resolver)} }  -- recorded per region whose termination resolved until_goal_met: the referenceable ground that makes "goal met" determinate. The constructors carry a fixed precedence (relay, never separately gated), resolved at assembly: a stated/compiled condition first; then a resolution_required the incoming plan's own UnitConditionBinding CARRIED for this region — that is a reference, not an absence, because the incoming plan asserted it rather than the assembly reaching it for want of anything else, so it outranks inferring a contract the region's units never compiled; when the plan arrived through a navigation block, carried_locator additionally travels on the handoff so the substrate can re-reach the record that resolver owes work against; then the assigned protocols' own convergence contracts; else resolution_required as the fallback when no reference exists at all. Both resolution_required payloads follow `Resolver`'s own per-move rule, conjoined across the region's moves
...
HandoffLocator = { record: the durable identity of E, session: the id of the session that emitted E }  -- restated from merismos; the session half is required because E lives in a session-scoped instrument and its record identity alone does not resolve from here
...
I = LivePlan(plan) → Some(plan) | Navigation(N) → DereferencePlan(N) → ReadPlan(E) → Some(plan) |
    NoPlan → None. DereferencePlan reads N.canonical_locator's record identity within the session that locator
    names; a locator missing either half is not dereferenceable. A failed dereference or support-integrity
    check stops and deactivates; it never selects None. On the Navigation arm ONLY, DereferencePlan also
    retains that locator as Λ.carried_locator — the LivePlan and NoPlan arms reach no navigation block, so
    both leave it None

── PHASE TRANSITIONS ──
Phase 0: WP → BindPlanInput(WP) → PI → [LivePlan(plan): bind_I(Some(plan)) | Navigation(N): DereferencePlan(N) → (unreachable ∨ support-integrity failure: relay(handoff unreadable) → deactivate | E → ReadPlan(E) → plan → bind_I(Some(plan)) → retain_locator(track: Λ.carried_locator := Some(N.canonical_locator))) | NoPlan: bind_I(None)] → MethodBrief(WP) → guard[relay-test, anti-self-application] → warrant? → [warrant=relay: relay_route(extension) → deactivate | warrant=warranted: Qc(brief, conduction-warrant) → Stop → continue]   [Tool]
...
conducted(WP) = dom(move_assignment) = MS
              ...
              ∧ (I ≠ None → carried_plan = Some(I))
              ∧ (I = None → carried_plan = None)
              ∧ carried_locator = (the canonical_locator Phase 0 DereferencePlan retained, or None when Phase 0 took the LivePlan or NoPlan arm)   -- DereferencePlan is its SOLE producer; no later step writes it, and Phase 3 CarryPlan carries it here unread
...
Phase 0 BindPlanInput (sense)        → Internal analysis (classify the incoming carrier as LivePlan, Navigation, or NoPlan)
Phase 0 DereferencePlan (observe)    → TaskGet, Read (when PI = Navigation(N): follow N.dereference_instruction at N.canonical_locator — reading its record identity within the session that locator names, since the record is session-scoped — honor its snapshot anchor when present, and run the grounding instruction; an unreachable source, an absent session half, or an unsupported load-bearing premise surfaces handoff unreadable and deactivates. On a successful dereference this step also WRITES Λ.carried_locator := Some(N.canonical_locator): the locator is retained here, not consumed, because it is the only point at which it is in scope and the executing substrate needs it to re-reach the canonical record after handoff)
...
Phase 3 AssembleTraceContract (track)    → internal Λ update (ASSEMBLE the cross-cutting disclosure overlay: populate Λ.trace_contract from Λ.residuals + Λ.degradations + derived_coverage_limits(CT, unit_condition_bindings) + derived_termination_grounds(CT, MS, move_assignment, I, unit_condition_bindings); an INVARIANT aggregation, never gated. Reading the bindings here is what makes the overlay report the same ground the bindings carry — a region whose units compiled no completion condition discloses resolution_required with its owed resolver, never a convergence contract standing in for one)
