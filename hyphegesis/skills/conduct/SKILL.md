...
CoverageLimit = { region: MoveRegion, bound ∈ {top_n, no_retry, sampling, scope_cut, emergent}, dropped, reason }  -- a coverage cap the resolved topology imposes (what the method does NOT cover); surfaced explicitly — the "no silent caps" discipline. Derived from the topology at assembly (a until_dry_ceiling ceiling, a bounded_rounds bound, a sampling/top-N cut), never separately gated
...
Phase 3 AssembleTraceContract (track)    → internal Λ update (ASSEMBLE the cross-cutting disclosure overlay: populate Λ.trace_contract from Λ.residuals + Λ.degradations + coverage caps derived from the resolved topology (until_dry_ceiling ceiling, bounded_rounds bound, sampling/top-N cut); an INVARIANT aggregation, never gated — never-silent has no alternative value to choose, so no AxisGate fires)
