...
10. **Re-entry produces fresh activation**: Same-session re-invocation is permitted but treated as a distinct activation — each cycle through Phases 0-3 produces a fresh `ResolvedEndpoint`, with the prior endpoint becoming session evidence rather than auto-cycling input. Prevents silent reuse where a previously resolved endpoint masquerades as the current cycle's starting state.
