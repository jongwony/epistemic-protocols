...
ResolvedEndpoint = I' where user_judges_resolved(I') ∨ user_dismiss(I'); residual includes any projection still in Λ.deferred at convergence, each wrapped as a DeferredResidual (reason derived from its certificate.status + substrate basis) — surfaced, never silently dropped
