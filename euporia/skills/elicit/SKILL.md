...
9. **PCME alignment** (Tool-Aligned Cognitive Compromise family): Cycle-emergent dimension surfacing is the round-scale realization. Per-cycle dimension count is governed by cognitive-cost homogeneity and dependency boundary across surfaced coordinates, not by fixed cardinality. A single coordinate carrying disproportionately high cost relative to peers defers to a subsequent cycle as a focused round (protocol-synthesis fork). Coordinates whose values are derivable from substrate without user constitution route through autonomous propagation at the integration step.
