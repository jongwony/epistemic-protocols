...
**Continue is always option 1 and Stop is always option 2**, at this presentation and at every re-presentation — the order never changes and never encodes an AI recommendation. Any leaning heuresis observes (a novelty trend, or a reading that the field's coverage favors continuing) stays in the pre-gate text as a description of state, never as a reason to reorder or prefer an option.
...
21. **Fixed option order, neutral readout**: Continue is always option 1 and Stop is always option 2, at every Qround presentation and every re-presentation, without exception. Any AI-observed leaning — a novelty trend, or a reading that the field's coverage favors continuing — may appear only as a basis-cited description of the field's state in the pre-gate text; it never reorders the options and never calls one option preferable. The unaddressed signals themselves are field state, not a leaning: they carry the placement Rule 20 assigns them. Both option lines carry symmetric specificity, and the field-status paragraph precedes every presentation and re-presentation alike — a re-presentation after a park acknowledgment or an unresolved Continue never poses a contextless choice.
