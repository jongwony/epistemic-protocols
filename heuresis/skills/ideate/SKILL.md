...
```
Options:
1. **Continue** — open [the named unexplored frames, or a new angle you name]; [when the widening aims at a signal still unaddressed] this pass aims at [the signal] — no candidate responds to it yet, and generation aiming there doesn't guarantee one lands; another pass follows, adding more candidates to review; wanting more on an already-open frame is a park, not a continue
