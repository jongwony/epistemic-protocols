...
Phase 0 extract_signals (sense)       → Internal analysis (Signals — concerns, weaknesses, requirements — read from the same bound utterance + named ChainRef only, same `Euporia boundary — utterance-only input` boundary as bind; tagged Utterance or Chain by source; surfaced once via the classify_relay emission; feeds unaddressed(Λ) each round, never scored or ranked)
...
finding      (extension)       → TextPresent+Proceed (fires at any Constitution checkpoint of this protocol while it is active: a `↗ /command — reason` line present in accumulated context — placed there by this protocol's own detection or by /route in monitor mode — is presented beside that checkpoint's options as a free-response pathway, never as a peer option. Answering it constitutes this protocol's own termination path with residual folded, the same path a Dismiss or an equivalent free-response exit takes; nothing transfers control before that answer, and no router terminates this protocol. Timing is the checkpoint, never the arrival: a finding arriving between checkpoints waits for the next one, and the loop continues)
Seam transition to declared next protocol (extension) → TextPresent+Proceed (fires at deactivation, after converge, at either terminal — DiverseCandidateField or EarlyExit: a user-declared chain naming the next protocol settles the next move — proceed directly to it, citing that settling source. This protocol declares no wired outbound continuation edge — the field is endpoint-neutral by contract (an issue tracker, /preview, a chained /ideate), so the second trigger is vacuously absent. What crosses the seam is the assembled terminal whole, with every origin tag, parked follow-up, and unaddressed signal intact — the seam never selects, ranks, or trims among candidates (Divergence over Selection holds through the handoff; selection is the next protocol's act). The seam is the outbound counterpart of `Chain semantics`: a chained /ideate re-reads the field as a ChainRef under the inbound rule, and any other next protocol reads it as its own input. Parked follow-ups are NOT routed here — their durable externalization stays the host-side handoff `converge` declares, unless the user's chain names a later /ideate on them. Every Constitution gate inside this protocol and inside the next protocol fires unchanged — the seam exists only after the user's own Stop and never pre-empts it)
...
- **User-initiated, zero entry questions**: `/ideate` activates only on direct invocation (Layer 1); `Entry` (Blank vs. Seeded) is inferred from the utterance alone — never asked. Phase 0's `classify_entry` is Extension/relay, not a gate.
...
- **User-bounded field**: `Stop` remains available from the first gate onward; it returns `DiverseCandidateField` only with candidates and otherwise the fully declared `EarlyExit`.
...
- **Chain semantics**: A named `ChainRef` folds material in as seeds, preserving existing origin tags and using `User` only for untagged material. Read `references/chain-reference.md` after classification and before first-pass promotion; the ownership trade-off remains declared under Known Limitations.
...
- **Depth parks, width continues**: A request for more inside an already-open frame parks as a source-cited `ParkedFollowUp`, is declared at either terminal, and leaves `Qround` open; deepening chains from the assembled field later.
