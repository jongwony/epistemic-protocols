...
aporia_signal(K) = (∀ u ∈ K: spent(u)) ∨ (∃ u ∈ K: contradiction(u))            -- K = the Phase 2 cluster; the frame itself is unanswerable, or the intent is split
                 -- one exhausted item is a fact the user lacks; a whole cluster exhausted is an axis the substrate cannot fix — /elicit's deficit
-- Layer 1 (epistemic)
...
Phase 2: Qs(classify_result + Uₑ + Uᵢ''[cluster], framing) → Stop → A          -- uncertainty surfacing [Tool]; cluster = one coherent cluster (size ≤ 4)
         [if aporia_signal(cluster)] Qs carries finding(↗ /elicit — reason)     -- this protocol's own detector for the finding slot; the answer routes through sufficiency or Dismiss below, never through an automatic transition
...
Taking a finding at Phase 2 (`↗ /elicit`, or any finding a monitor placed) is one such declaration: every uncertainty still in Λ.remaining moves to Λ.dismissed carrying the finding as its recorded reason, the run converges as InformedExecution with that residual, and the named protocol is what the turn after reaches — nothing here invokes it.
...
aporia_signal (extension)      → TextPresent+Proceed (fires at Phase 2 when aporia_signal(cluster) holds: every item in the cluster is promotion-spent, or a coherence item is a contradiction no evidence settles. Relay: the finding `↗ /elicit — reason` is placed beside the cluster, naming the spent items and the channels each tried, or the contradiction quoted from the utterance — that list is the substrate signal /elicit's activation reads. It is this protocol's own detector for the finding slot below; the slot, not this entry, says how the finding is presented and answered)
...
- **Finding slot**: A `↗` line at Phase 2 — this protocol's own aporia signal or a finding /route placed while this protocol was active — is a free-response pathway beside the cluster, never a peer option. Taking it is a sufficiency declaration with the finding as the recorded reason; nothing switches before the user takes it.
