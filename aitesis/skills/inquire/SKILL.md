...
**Commonly-missed context categories** (cross-session enrichment heuristic): While scanning, prime the uncertainty inference against categories that users (especially those with lower prompt-literacy for this domain) reliably omit from initial framing. Recurring blind-spot categories include: (a) **domain constraints** — regulatory, compliance, or policy boundaries governing the prospect's action space; (b) **non-functional requirements** — performance budgets, availability targets, security posture, observability expectations; (c) **stakeholder perspectives** — parties affected by the action whose viewpoint is absent from the stated prospect (downstream consumers, operators, auditors, end users); (d) **operational lifecycle** — rollout, rollback, migration, and deprecation considerations beyond the immediate build step; (e) **failure and edge modes** — behavior under partial failure, unusual inputs, or boundary conditions not in the stated happy path. This heuristic adjusts detection sensitivity — categories in this list that remain unaddressed in the prospect become higher-priority uncertainty candidates during Scan. This is a runtime priming input only; it does not introduce a new phase step, tool call, or gate — PHASE TRANSITIONS and TOOL GROUNDING are unchanged. Subject to the pollution caveat in `architectural-principles.md` §Cross-Session Knowledge Composition: priming may bias detection toward previously observed categories and must not substitute for genuine prospect-specific scanning. Grounding references for the category list are curated in `docs/failure-mode-catalog.md`.
