...
Phase 1: one pass over the whole live set
         ...
         [the pass changed nothing ∨ a further pass is not worth reaching for] → Phase 2   -- nothing is left unlanded either way: every item in W landed at Step₂ of the pass that opened it, so an item opened but not pushed carries the landing the material shows, with its basis saying collection stopped there
