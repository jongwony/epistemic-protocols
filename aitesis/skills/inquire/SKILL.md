...
16. **PCME alignment** (Tool-Aligned Cognitive Compromise family): When multiple context-gap questions surface, bundle confidence-homogeneous gaps per round (gaps the user can answer with similar effort and substrate access). Heterogeneous-cost gaps route as separate rounds; substrate-resolvable gaps flow through autonomous propagation. Round determination follows cognitive-cost homogeneity and dependency boundary, not fixed cardinality. Composes with Rule 10 (One coherent cluster) and Rule 11 (Divergence-bounding) — PCME provides the cost-homogeneity criterion that determines admissible cluster composition.
