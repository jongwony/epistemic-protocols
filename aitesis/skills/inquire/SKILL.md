...
- **Co-fire disambiguation** (Step 1 entry sub-step): When the prospect utterance fires BOTH the empty_intention trigger (subjective recall of prior session content) AND a Prior-decision implication signal (objective historical investigation) from the same utterance, the Routing distinction above does not auto-resolve — both conditions hold simultaneously and silent precedence selection (e.g., "Anamnesis first because empty_intention dominates" or "Aitesis first because the Prior-decision signal is more specific") exercises constitutive authority without surfacing the routing choice. Present a Cognitive Partnership Move (Constitution) Qc gate before Ctx collection proceeds, with three routing options: **RouteRecall** (invoke `/recollect` first; recalled context narrows subsequent Aitesis Phase 1 Ctx via the cross-session enrichment path), **RouteInquiry** (bypass `/recollect`; proceed with Aitesis Phase 1 Ctx including version-control history scan as the Routing distinction prescribes), **RouteBoth** (sequential — `/recollect` first, then Aitesis Phase 1 Ctx with recalled context as input). The Qc answer is currently an untyped routing selection presented as text options; formal `RoutingMode` coproduct in TYPES is deferred pending variation-stable observed use.
...
3. **Evidence over Inference over Detection** (Aitesis Core Principle): Three-level hierarchy. Lower boundary (Inference > Detection): infer the highest-gain question rather than detecting via fixed checklist. Upper boundary (Evidence > Inference): when a factual uncertainty is empirically observable, observe directly rather than infer from reasoning alone — Cite-or-observe is the structural guard. Partial evidence covering a subset of the claim scope is inference for the uncovered portion; verify scope ⊇ claim before treating as resolved, and classify the uncovered portion separately. **Inquiry above Evidence in cost**: facts discoverable by AI through Evidence (codebase, memory, observation, canonical external) are resolved at Phase 1; Phase 2 surfaces only judgment-requiring uncertainties.
...
15. **Co-fire routing disambiguation** (Detection with Authority): When the prospect utterance fires both the empty_intention trigger (subjective recall) and a Prior-decision implication signal (objective investigation) from the same utterance, present routing choice via Cognitive Partnership Move (Constitution) before Phase 1 Ctx collection — silent precedence selection (Anamnesis-first or Aitesis-first) exercises constitutive authority without surfacing the choice. Routing options (RouteRecall / RouteInquiry / RouteBoth) are presented as untyped text options per Phase 1 Step 1 Co-fire disambiguation prose; formal `RoutingMode` coproduct in TYPES deferred pending variation-stable observed use.
