...
-- A = Unknown(Partial) re-enters the loop through the promotion arc while an unattempted named source remains, and surfaces the spent ladder at Phase 2 once none does; the loop is bounded by that count (detail in PHASE TRANSITIONS)
...
A        = User answer ∈ {Provide(context), Point(location), Dismiss, Unknown(Partial)}
             -- Unknown(Partial) = user declines certainty; Phase 3 auto-promotes via Cite-or-observe tiebreaker to next_source(v, u) — the
             -- lowest-cost UNATTEMPTED base source — and re-enters Phase 1 for reclassification; when sources_exhausted(u) the promotion has
             -- no target and the spent ladder is surfaced at Phase 2 instead; both routing arcs formalized in PHASE TRANSITIONS
...
EarlyExit = X' where early_exit  -- non-convergent early exit: X' as of exit (original prospect + any partial integration), partial transformation trace over resolved uncertainties, remaining declared as unresolved residual (morphism not completed). The exit REASON — user Esc, or the automatic evidence ladder spent with the item left unresolved at Qs_ladder_spent — rides the partial trace as provenance, not as a second constructor: both reasons hand back the same shape (partial X', partial trace, declared residual), so what a downstream reader needs is which uncertainties are open and why the run stopped reaching for them, and that is a field of the trace rather than a fork in the type. Λ.attempted_sources supplies the ladder half of that reason without any new terminal
-- Layer 1 (epistemic)
...
base : ℘(EvidenceSource) → ℘(EvidenceSource)
  base(S) = S \ Emergent(EvidenceSource)   -- the finite NAMED elements of a ValidSources image; the open tail is excluded by construction
               -- Emergent(EvidenceSource) is deliberately outside base: a channel unvalidated by definition cannot be a target the AI
               --   selects on its own, so the open tail is entered ONLY by explicit user designation at Phase 2 Qs_emergent_channel /
               --   Qs_ladder_spent. That exclusion is exactly what makes exhaustion decidable over a set that is otherwise open —
               --   |base(ValidSources(v))| ≤ 4 for every v, while ValidSources(v) itself admits no such bound
next_source : Verifiability × Uncertainty → EvidenceSource
  next_source(v, u) = min_cost(base(ValidSources(v)) \ Λ.attempted_sources[u])   -- lowest-cost UNATTEMPTED base source under the
               --   cost-ordering above; UNDEFINED when that difference is empty, which is the condition sources_exhausted names.
               --   Partial by construction rather than by omission: the caller (Phase 3 backward arc) is guarded on ¬sources_exhausted(u),
               --   so the function is never invoked outside its domain
sources_exhausted(u) ≡ base(ValidSources(v)) \ Λ.attempted_sources[u] = ∅   where classify(u) = (Factual, (v, _))
               -- the automatic ladder has no target left for u. μ(u) = |base(ValidSources(v)) \ Λ.attempted_sources[u]| is the decreasing
               --   measure: Phase 1 Step 2 records every SELECTED source into Λ.attempted_sources[u], so the source in play is already
               --   spent when the arc is evaluated, μ(u) strictly decreases at every traversal, and it reaches 0 in at most
               --   |base(ValidSources(v))| steps. The arc is therefore well-founded and the loop it closes is bounded — a source that
               --   yielded nothing is still spent, which is what stops the ladder from re-selecting it
...
Phase 1: Uᵢ → Step₁ Ctx(Uᵢ) → (Uᵢ', Uᵣ) →                    -- Step 1: context collection [Tool]
         ...
         [on selecting EvidenceSource s for u] Λ.attempted_sources[u] := Λ.attempted_sources[u] ∪ {s}  -- (track) the ledger is written where a source is SELECTED, initial classification and promotion alike, so the source currently in play is already spent when Phase 3 evaluates sources_exhausted(u); recording only promotions would leave the initial source unspent and let the arc re-select the very source that just returned Unknown(Partial)
...
Phase 3: A → integrate(A, X) → X'                               -- prospect update (track: mutates Λ.X)
         [if A = Unknown(Partial) ∧ ¬sources_exhausted(u)] auto_promote(u, next_source(v, u)) → goto Phase 1  -- backward arc (T2): user declines certainty → re-enter classification with the next-preferred UNATTEMPTED base source, which Step 2 records on selection. μ(u) therefore decreases on every traversal whether or not the promoted source yields anything — a source that came back empty has still been spent, which is what stops the arc from re-selecting it
         [if A = Unknown(Partial) ∧ sources_exhausted(u)] → Phase 2 Qs_ladder_spent  -- backward arc (T2) terminates: the automatic ladder holds no further target for u. The disposition is surfaced for the user to constitute, never classified by the AI — an item the evidence channels could not reach is not thereby dismissible on the user's behalf
...
User can exit at Phase 2 (early_exit), and can declare the context sufficient at any Phase 2 as a free response (sufficiency_declared) — the latter converges rather than exits.
When sources_exhausted(u) holds, Phase 2 discloses the spent ladder (Qs_ladder_spent) and the user's answer decides which of the three the run takes: a designated or supplied channel returns u to Phase 1 and the loop continues, a recorded default moves u to Λ.dismissed and the loop continues toward convergence, and leaving u unresolved satisfies early_exit.
Continue until: informed(X') OR user ESC OR the ladder is spent for some uncertainty the user leaves unresolved.
Convergence evidence: At remaining = ∅, present transformation trace — for each u ∈ (Λ.context_resolved ∪ Λ.read_only_resolved ∪ Λ.empirically_observed ∪ Λ.user_responded), show (ContextInsufficient(u) → resolution(u)). Where convergence was reached through a sufficiency declaration, show each u ∈ Λ.dismissed with the declaration recorded against it, so the trace reports what was accepted unresolved instead of presenting it as resolved. Convergence is demonstrated, not asserted.
On early_exit (EarlyExit, not InformedExecution): present the same partial transformation trace restricted to uncertainties already resolved, declare `remaining` as explicit unresolved residual, and name the exit reason — user Esc, or the ladder spent for a named uncertainty, citing Λ.attempted_sources for that item so the residual carries how far the automatic reach got.
...
early_exit = user_esc ∨ ∃ u : sources_exhausted(u) ∧ unresolved_at_ladder_spent(u)   -- the two exit-flavoured events, both non-convergent: the user withdraws, or the automatic ladder is spent for some u and the user leaves it unresolved at Qs_ladder_spent. Named to match the type it types — EarlyExit = X' where early_exit — which the prior binding of this predicate to a sufficiency declaration did not
unresolved_at_ladder_spent(u) ≡ the user's answer at Qs_ladder_spent for u is Unknown(Partial)   -- the one Qs_ladder_spent answer that neither supplies a channel nor records a default, so u stays in Λ.remaining with no arc left to take
...
Phase 2 Qs_ladder_spent (constitution) → present (specialization of Phase 2 Qs: fires when sources_exhausted(u) — the automatic promotion ladder holds no unattempted base source for u. The classify summary discloses Λ.attempted_sources[u] (which base channels were spent, in the order spent) and states that automatic promotion has no target left, so the user recognizes the ladder's end rather than recalling how far it got. The answer rides the parent A coproduct unchanged — Point(location) designates a channel outside the base set, which is the ONLY entry into Emergent(EvidenceSource) and re-enters Phase 1 with that channel routed through Qs_emergent_channel; Provide(context) supplies the fact directly; Dismiss records the stated default; Unknown(Partial) leaves u unresolved and satisfies early_exit, terminating the run as EarlyExit rather than re-entering the arc. No new constructor is introduced: what the disclosure changes is which future each existing constructor now names, which is the recognition the user needs to dispose of u)
...
esc          (extension)       → TextPresent+Proceed (partial transformation trace + unresolved residual declaration + the exit reason — user Esc, or the ladder spent for an uncertainty left unresolved at Qs_ladder_spent, citing Λ.attempted_sources for that item; terminate as EarlyExit, not InformedExecution)
...
Λ = { phase: Phase, X: Prospect, uncertainties: Set(Uncertainty),
      ...
      attempted_sources: Map(Uncertainty, Set(EvidenceSource)),  -- per-uncertainty ledger of base sources already spent on u; written at Phase 1 Step 2 wherever a source is selected — initial classification and promotion alike — and NEVER reset by reclassification, since a reclassified item carries its spent sources forward and the measure bounding the backward arc is what the ledger holds. This is what makes sources_exhausted(u) decidable and the Qs_ladder_spent disclosure citable
      active: Bool,
      cause_tag: String }
...
-- Note: observation_skips, source_choice_overrides and attempted_sources are audit logs orthogonal to the partition —
...
--       attempted_sources: logged at every Unknown(Partial) promotion; a LEDGER keyed by uncertainty, not a partition bucket — an uncertainty
--         appears in exactly one partition set regardless of how many sources its ledger holds, so the pairwise-disjoint invariant above is untouched
...
All uncertainties resolved (context, read-only, observed, or user) → proceed with updated prospect. All remaining uncertainties dismissed → proceed with original prospect + defaults. User declares the context sufficient at any Phase 2 → every remaining uncertainty is dismissed with the declaration recorded against it → proceed with updated prospect (InformedExecution): a declaration of sufficiency accepts the residual rather than abandoning the inquiry, so it lands where the per-item Dismiss it generalizes already lands. Automatic evidence ladder spent for an uncertainty and the user leaves it unresolved at that disclosure → EarlyExit (not InformedExecution): present partial transformation trace + declare `remaining` as unresolved residual, citing the attempted-source ledger for the named uncertainty, then return to normal operation. User Esc key → EarlyExit (not InformedExecution): present partial transformation trace + declare `remaining` as unresolved residual, then return to normal operation.
...
- **EvidenceSource selection** (Layer 2, Factual dimension — `Fiber(Factual) = Σ(v: Verifiability). {s | s ∈ ValidSources(v)}`):
  ...
  - **Default**: lowest-cost valid source per cost-ordering in TYPES (ascending: `CodeDerivable < CanonicalExternal < Instrumentation < UserTacit`), restricted to the named sources not yet spent on this uncertainty.
  - **Record the selection**: every selected source is added to `Λ.attempted_sources[u]` at the moment it is selected, on initial classification and on promotion alike. This ledger is what bounds the `Unknown(Partial)` promotion arc and what the spent-ladder disclosure cites; it is never reset when an uncertainty is reclassified, since a source that failed under one classification has still been spent.
...
- **Classification summary** (each remaining uncertainty shows how the AI plans to resolve it, with the basis cited; each line is one case the user may see):
  ...
  - a factual question every automatic channel has now been spent on — the channels tried are listed in the order they were tried, and the disposition is yours: name another channel, supply the fact, record a default, or leave it open
...
Option 4 is the typed `Unknown(Partial)` constructor (TYPES `A`): Phase 3 records the source just spent, auto-promotes to the next-preferred unattempted named source, and re-enters Phase 1 classification via the backward arc (PHASE TRANSITIONS). This is a type-preserving materialization — not gate mutation — since the TYPES coproduct already admits `Unknown(Partial)` as a constructor.

At the spent-ladder disclosure the same four constructors are presented, and only their stated implications specialize — which is materialization for the same reason: the coproduct classifies the response identically before and after. Option 2 there designates a channel outside the automatic set and is the only route into it; option 4 there no longer promises another attempt, because none remains, and instead leaves the uncertainty open as declared residual. Presenting option 4 with its ordinary implication at that gate would state a future the protocol can no longer deliver, which is the recall the disclosure exists to spare the user.
...
4. **Unknown(Partial)**: Record the source just attempted into the attempted-source ledger, then promote to the next-preferred unattempted base source and re-enter Phase 1 — or, when no base source is left, surface the spent ladder at Phase 2 with the channels already tried, for the user to dispose
...
| Spent ladder disclosure | When automatic promotion has no unattempted base source left for an uncertainty, Phase 2 shows which channels were tried and hands the disposition to the user | The automatic ladder cannot loop without end, and an item the evidence channels could not reach is never disposed of on the user's behalf |
...
| Free-response escape | Phase 2 Constitution interaction accepts `Unknown(Partial)` as typed answer constructor (TYPES `A`) — Phase 3 records the attempted source, auto-promotes the uncertainty to the next-preferred unattempted base source, and re-enters Phase 1 classification via the backward arc (PHASE TRANSITIONS); when the base set is spent the arc terminates at the spent-ladder disclosure instead | User need not simulate certainty for unknowns; routing degrades gracefully through a typed, formally-tracked arc that is bounded by the base set rather than open-ended |
...
6. **Convergence persistence**: Mode active until one of the termination paths is reached — all identified uncertainties resolved (context, read-only, observed, user-responded) or dismissed, converging as InformedExecution; the user declaring the context sufficient, which dismisses every remaining uncertainty with the declaration recorded and likewise converges as InformedExecution; user Esc, terminating as EarlyExit; or the automatic evidence ladder spent for an uncertainty the user then leaves unresolved, terminating as EarlyExit with the exit reason and the attempted-source ledger named. Convergence demonstrated via per-uncertainty transformation trace before declaring remaining = ∅; a non-convergent path presents the partial trace and declares the residual instead.
...
12. **Cite-or-observe** (Evidence-Inquiry boundary structural guard): When a Factual uncertainty has a cheaper EvidenceSource available than UserTacit, resolve via cheaper source OR cite explicit dominance basis (logged to `Λ.source_choice_overrides`). Reclassification of EmpiricallyObservable to UserDependent is legitimate only under empirically verifiable escape conditions evaluated BEFORE observation (env mutation / 30s bound / elevated risk; logged to `Λ.observation_skips`). Observation outputs always proceed to Phase 2 via `Uₑ` — positive (differentiating evidence) or negative (null-signal); pre-observation categorical classification is not a substitute. On the ReadOnly direct-resolve path, support_integrity is the structural test for the same upper boundary: evidence that is present (and current) but not coupled to the source × claim pair or behavior it asserts (`support_integrity_unverified`) is inference, not evidence, and reclassifies to EmpiricallyObservable rather than auto-resolving; admissibility on this path requires confirming provenance coupling and evidence→behavior coupling, beyond freshness. Verifiability-tier dominance basis detail in Phase 1 Step 2.
12a. **Bounded promotion ladder** (Cite-or-observe termination guard): Automatic promotion on `Unknown(Partial)` ranges over the unattempted NAMED sources of `ValidSources(v)` only, and every source selection — initial classification and promotion alike — records its source in `Λ.attempted_sources[u]`, so the unattempted count strictly decreases and the arc is well-founded. `Emergent(EvidenceSource)` stays outside that range: the open tail is reached only by explicit user designation, since a channel unvalidated by definition is not a target the AI may select on its own. When no named source is left, the spent ladder is surfaced at Phase 2 with the channels already tried and the disposition is the user's to constitute — an uncertainty the evidence channels could not reach is carried to the user as a recognition, never auto-dismissed and never re-promoted.
...
18. **Seam relay on declared continuation**: when a user-declared chain or a composition edge this SKILL.md declares (the CrossDomain/Relevance/Emergent deficit-matched routing targets in Phase 1 Step 2) names the next protocol, the between-protocol seam after Mode Deactivation is relay (Extension) — proceed directly, citing the settling source (the chain declaration or the named routing edge). This governs only the seam BETWEEN protocols; every Constitution gate inside this protocol and the next fires unchanged, and the user can redirect at any turn.
