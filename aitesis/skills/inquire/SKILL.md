...
Convergence evidence: At remaining = ∅, present transformation trace — for each u ∈ (Λ.context_resolved ∪ Λ.read_only_resolved ∪ Λ.empirically_observed ∪ Λ.user_responded), show (ContextInsufficient(u) → resolution(u)). Convergence is demonstrated, not asserted. The trace additionally declares every u ∈ Λ.non_factual_detected as detected-but-outside-scope: these resolve nowhere, so no transformation pair exists for them, and the trace is the only convergence-path surface that carries them. This declaration is unconditional — the all-non-actionable path (actionable(Λ) = ∅) converges without reaching a Phase 2 question, so a classify summary rendered only alongside a question would leave the detections invisible at exactly the moment nothing else surfaces them, returning InformedExecution over unsurfaced cross-domain and relevance findings. Declaring is what this path owes; it does not additionally gate. The declaration stays in the session record for the user to act on whenever they choose, and stopping on every out-of-scope detection would reinstate the frequent interruption that classifying them non-blocking exists to avoid — so an honest record, not a halt, is what carries them.
