...
classify   = Uᵢ' → Σ(d: Dimension). Fiber(d)
             ...
             --   support_integrity: link(evidence → asserted behavior/current reality) is verified, not silently desynced
             --                      -- undercutting axis: does the evidence actually track what it asserts?
             ...
             --   failure of either axis (¬coverage = scope_gap; ¬support_integrity = support_integrity_unverified) → reclassify EmpiricallyObservable
...
Phase 1: Uᵢ → Step₁ Ctx(Uᵢ) → (Uᵢ', Uᵣ) →                    -- Step 1: context collection [Tool]
         ...
         Step₃ ReadOnlyVerify(Uᵣ') →                           -- Step 3: read-only verification (CodeDerivable + CanonicalExternal) [Tool]
           [if support_integrity_unverified(u) ∨ scope_gap(u)] reclassify(u, EmpiricallyObservable) → goto Step₂  -- backward arc (T4): support-integrity/coverage failure re-enters classification (staleness = temporal sub-case of support_integrity_unverified)
...
- **Verifiability assessment** (Layer 2, Factual dimension — Observability sub-modes guide classification):
  - ReadOnlyVerifiable: fact exists in environment (StaticObservation or BeliefVerification) and is observable with current tools, AND both admissibility axes pass — **coverage** (evidence scope ⊇ claim scope) AND **support_integrity** (the evidence→behavior link is verified, not silently desynced) → resolve directly via extended context lookup
    - support_integrity operational test: *"If the behavior this evidence asserts changed right now, would the evidence break or fail (no longer compile / fail a test / no longer evaluate / be contradicted by current state) — or would it silently drift?"* Breaks-on-change → support-linked (admissible); silently drifts → support_integrity fails. Currency (freshness) is the temporal sub-case: a current artifact still fails this test when nothing couples it to the behavior it claims (e.g., a comment or doc asserting behavior with no enforcement channel).
    ...
    - When **support_integrity** fails (current/present but not coupled to the asserted behavior): reclassify the uncertainty as EmpiricallyObservable and re-enter classification (existing backward arc) — observe the behavior rather than trusting the artifact; if observation is not bounded/non-destructive, it routes onward to user inquiry per the standard EmpiricallyObservable path
...
- **Classification summary** (each remaining uncertainty shows how the AI plans to resolve it, with the basis cited; each line is one case the user may see):
  ...
  - a factual question whose codebase evidence is present and current but not coupled to the behavior it asserts (e.g., a comment or doc with no enforcement channel) — routed to a test/observation rather than trusted as-is
...
5. **Context collection precedes classification precedes inquiry**: Phase 1 Step 1 (codebase + memory + history collection) → Step 2 (dimension + verifiability + EvidenceSource classification) → Step 3 (read-only verification) → Step 4 (empirical observation) before Phase 2 inquiry. Memory and version-control evidence is staleness-guarded — tagged `source: "memory:{path}"` or `source: "history:{ref}"` and NOT eligible for Factual/ReadOnly direct-resolve; verify against current state or escalate via classify summary with `staleness:unverified` flag. Procedural detail in Phase 1 Step 1-2.
