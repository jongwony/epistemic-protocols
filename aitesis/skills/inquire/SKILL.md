...
6. **Convergence persistence**: as typed by the closed deactivation-condition set in Mode Deactivation, together with the `Convergence evidence` statement in the LOOP block.
...
18. **Seam relay on declared continuation**: as typed by the "seam (extension)" entry in TOOL GROUNDING.
