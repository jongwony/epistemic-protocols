...
- When a target account and a candidate mapping are already in play and the conclusions licensed by that mapping are uncertain, use `/ground` to audit their evidence and limits.
- When a familiar architecture pattern is applied to an existing system, state the conclusions at stake before constructing correspondences; check the structural claims those conclusions depend on.
