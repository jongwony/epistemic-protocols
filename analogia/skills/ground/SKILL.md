...
```
── FLOW ──
Analogia(R) → attempts := 0 → Detect(R) → [¬uncertain: Qc(zero_gap_finding) → Stop → (Confirm: deactivate | Reopen(q): reopen_seed := q, proceed)] → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → [(Sₐ, Sₜ) changed: attempts := 0 ; Λ.self_grounding := self_grounding(Sₐ, Sₜ) ; Λ.partition_reading := None] → re-enter derive_focus_candidates)] → [focus_change_requires_disposition(Λ, φ'): Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ)))] → Λ.focus := φ' → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → carry_over(M) → AssessFit(M, Sₐ, Sₜ) → [self_grounding(Sₐ, Sₜ): PartitionRead(F, Sₜ)] → [M = ∅: surface(no correspondence constructed along φ', F.missing) → re-enter the focus checkpoint | M ≠ ∅ ∧ Λ.remaining = ∅: declare_fit_disposition(F, ∅) → D_f → R' → terminalize | M ≠ ∅ ∧ Λ.remaining ≠ ∅: c ∈ Λ.remaining → I(M, F, Sₜ) → V → D_f → R'] → (loop until terminalized ∨ attempts_exhausted(Λ))
...
Phase 1: uncertain → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → [(Sₐ, Sₜ) changed: attempts := 0 ; Λ.self_grounding := self_grounding(Sₐ, Sₜ) ; Λ.partition_reading := None] → re-enter derive_focus_candidates, recheck focus_settled)] → [focus_change_requires_disposition(Λ, φ'): surface(Λ.focus, φ', affected(Λ)) → Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ)))] → Λ.focus := φ' → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → carry_over(M) → AssessFit(M, Sₐ, Sₜ) → F → [reopen_seed = Some(q): F.open := F.open ∪ {q}; reopen_seed := None] → [self_grounding: PartitionRead(F, Sₜ) → PartitionReading] → [M = ∅: surface(no correspondence constructed along φ', F.missing) → re-enter the focus checkpoint — an empty construction is not a validated mapping, and attempts_exhausted bounds the retry | M ≠ ∅ ∧ Λ.remaining = ∅: declare_fit_disposition(F, ∅) → D_f → R' → terminalize, skipping Phase 2 — carry_over may leave nothing awaiting judgment, and re-gating an already-addressed correspondence is not a valid way to reach D_f | M ≠ ∅ ∧ Λ.remaining ≠ ∅: proceed to Phase 2]  -- settle_focus binds a provisional φₚ BEFORE the settlement guard reads it (per-field over relation ∧ purpose, NOT source_scope/target_scope alone — see Rule 3b); Reframe re-enters decomposition instead of falling through; the focus-change checkpoint fires only when terminal judgments stand under an outgoing focus; the attempt counter is charged at the Map/AssessFit reconstruction, never at the focus checkpoints; then fit map (Reopen seed folded into F.open, then cleared — track); derived split-vs-trim reading in the self-grounding case (relay), via partition_reading() [Tool]
Phase 2: (M, F) → c ∈ Λ.remaining → I(M, F, Sₜ) → [surface Λ.focus as pre-gate relay: the settled comparison focus this instantiation is scoped by] → [self_grounding: surface PartitionReading + routing recommendation as pre-gate relay] → Qs(I, F, φ) → Stop → V  -- instantiation + validation; mapping focus and (when self-grounding) the partition reading are surfaced as relay before the gate; the prior untyped `framing` argument is now the typed MappingFocus φ [Tool]
...
── CONVERGENCE ──
terminalized(R', F, D_f) = all_addressed(R') ∧ fit_disposition_declared(F, D_f)
progress(Λ) = 1 if |mappings| = 0 else 1 - |remaining| / |mappings|   -- mappings = ∅ is fully converged, not undefined, in exactly one case: Phase 0 zero-gap trivial convergence, where Confirm deactivates before Phase 1 ever constructs M. It is NOT convergence when Phase 1 constructed M and got ∅ — that routes to EmptyConstruction and re-enters the focus checkpoint, since a mapping with nothing in it was never validated
...
Phase 1 EmptyConstruction (extension) → TextPresent+Proceed (conditional: M = ∅ — relay that no correspondence was constructed along φ', citing F.missing as the basis, then re-enter the focus checkpoint; an empty construction never reaches terminalize, so Λ.remaining = ∅ alone is not convergence)
