...
carry_over(M') = confirmed := confirmed ∩ M' ; dismissed := dismissed ∩ M' ; remaining := M' \ (confirmed ∪ dismissed)  -- reconstruction carry-over, applied when Map rebuilds the correspondence set: still-standing judgments keep their addressed status for correspondences the new mapping retains, correspondences the new mapping drops leave the sets, and genuinely new correspondences enter remaining. Without this the partition invariant would not survive a reconstruction
...
Phase 1: uncertain → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → [(Sₐ, Sₜ) changed: attempts := 0] → re-enter derive_focus_candidates, recheck focus_settled)] → [focus_change_requires_disposition(Λ, φ'): surface(Λ.focus, φ', affected(Λ)) → Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ)))] → Λ.focus := φ' → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → carry_over(M) → AssessFit(M, Sₐ, Sₜ) → F → [reopen_seed = Some(q): F.open := F.open ∪ {q}; reopen_seed := None] → [self_grounding: PartitionRead(F, Sₜ) → PartitionReading]  -- settle_focus binds a provisional φₚ BEFORE the settlement guard reads it (per-field over relation ∧ purpose, NOT source_scope/target_scope alone — see Rule 3b); Reframe re-enters decomposition instead of falling through; the focus-change checkpoint fires only when terminal judgments stand under an outgoing focus; the attempt counter is charged at the Map/AssessFit reconstruction, never at the focus checkpoints; then fit map (Reopen seed folded into F.open, then cleared — track); derived split-vs-trim reading in the self-grounding case (relay), via partition_reading() [Tool]
