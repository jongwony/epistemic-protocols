...
Analogia(R) → attempts := 0 ; superseded_by := None ; refuted_focuses := ∅ ; zero_gap_confirmed := false ; fit_map := None ; assessment := None ; focus := None ; inferences := None ; mappings := ∅ ; warrant := {} ; preference := {} ; cited := {} ; checks := ∅ ; verdicts := {} ; partition_reading := None ; reopen_seed := None → Detect(R) → [¬uncertain: ZeroGapRelay(finding) → proceed] → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ) ∧ φₚ ∉ Λ.refuted_focuses: FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ) ∨ φₚ ∈ Λ.refuted_focuses: Qc(candidates) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): [¬pair_committed(Λ): (Sₐ, Sₜ) := decompose(R, d, context) ; self_grounding := self_grounding(Sₐ, Sₜ) → re-enter derive_focus_candidates(Sₐ, Sₜ, d) | pair_committed(Λ) ∧ d replaces Sₐ or Sₜ: Λ.superseded_by := Some(d) → DomainSuperseded | pair_committed(Λ) ∧ ¬replaces: re-enter derive_focus_candidates(Sₐ, Sₜ, d)])] → [Λ.focus = Some(φ) ∧ φ' ≠ φ: invalidate_derived(Λ)] → Λ.focus := φ' → K := settle_inferences(R, φ', context) ; Λ.inferences := Some(K) → [attempts_exhausted(Λ): Inconclusive(cap)] → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → [M ≠ ∅: carry_over(M) | M = ∅: Λ.mappings := ∅] → AssessFit(M, Sₐ, Sₜ) → F → Λ.fit_map := Some(F) → CheckRead(F, K) → Λ.checks := checks(F, K) → RunChecks(Λ) → Λ.warrant := warrant_of(Λ) → Judge(K, Λ) → Λ.verdicts := A → [self_grounding: PartitionRead(F, Sₜ) → Λ.partition_reading := Some(partition_reading(F, Sₜ))] →
  ...
  [a later utterance addresses the assessment] interpret_turn(u, S, context) → T → integrate_turn(T, Λ) → [T = Cite(_) ∨ T leaves the basis: re-enter Phase 1 from CheckRead] → …
...
R
  ...
  → run_checks(checks, cited, context) -- carry out the checks this activation can reach, take up the grounds the user cited as checks they carried out, and record what each one met
...
run_checks(checks, cited, context) = for each check on x: Failed(g) when a ground g cited for x bears Defeats, or an AIReachable run of it failed on g ; Survived(g) when a ground g cited for x bears Supports, or an AIReachable run of it survived on g ; Unmet otherwise
             -- a cited ground is a check carried out by the user's reach, so it lands in the same field as one this activation ran; Defeats prevails where grounds of both bearings were cited for one claim, because a decisive defeater is not outweighed by support. A UserHeld check is met only this way
...
warrant_of(Λ) : Map(FitClaim → Warrant) = for each x ∈ domain(Λ.checks):
  ...
  Open(what the unmet check would take)  otherwise
             -- read off check state alone; a Cite reaches warrant only through run_checks, so there is exactly one path from a ground to a warrant
...
integrate_turn : T × Λ → Λ  -- Cite writes its Ground and bearing into Λ.cited under the named claim and re-enters assessment from CheckRead, which re-runs the checks on it, re-reads warrant and re-judges K; Adopt and Withdraw write Λ.preference; Explore writes nothing, and where its answer needs evidence outside the current basis it writes the question into Λ.reopen_seed and re-enters from CheckRead too, so the question becomes an Unmet Check this activation tries to reach. An utterance replacing Sₐ or Sₜ sets Λ.superseded_by and ends the activation
invalidate_derived(Λ) = mappings := ∅ ; fit_map := None ; checks := ∅ ; verdicts := {} ; warrant := {} ; assessment := None ; partition_reading := None  -- everything derived from (Sₐ, Sₜ, φ), discarded when the focus changes. There is no disposition gate over it: warrant is read from evidence rather than established by the user, so a comparison the run has left simply recomputes. Λ.preference and Λ.cited survive, being records of what the user said rather than readings of the evidence; a cited ground whose claim the new comparison does not raise is simply not consumed
...
Phase 3: a later utterance addressing the assessment → interpret_turn(u, S, context) → T → integrate_turn(T, Λ) → [T = Cite(g, bearing, about): Λ.cited[about] ∪= {(g, bearing)} → re-enter Phase 1 assessment from CheckRead, re-running the checks, re-reading warrant and re-judging K | T = Adopt(A): Λ.preference[c] := Adopted for c ∈ A | T = Withdraw(A, reason): Λ.preference[c] := Withdrawn for c ∈ A, the reason recorded ; no warrant moves and no verdict changes | T = Explore(q): [q answerable from the current basis: answer, Λ unchanged | q needs evidence outside it: Λ.reopen_seed := Some(q) → re-enter Phase 1 assessment from CheckRead] ; [q replaces Sₐ or Sₜ: Λ.superseded_by := Some(q) → DomainSuperseded]]
...
A pass is followed by another when a Cite supplies a ground, when an Explore needs evidence outside the current basis, or when a focus change invalidates the derived assessment. The first two re-enter from CheckRead and reconstruct nothing; only the third reconstructs, and it runs while the reconstruction budget holds.
Max 3 reconstructions per ACTIVATION — `attempts := 0` on activation and never refunded, incremented immediately before each Map/AssessFit reconstruction, which is the one site where the cap is tested. Neither focus checkpoint consumes an attempt, a pass re-entered from CheckRead consumes none, and a focus change does not refund the count.
...
Phase 1 RunChecks (observe) → artifact read, artifact search, external fetch, environment run (the AIReachable checks this activation can carry out, including exercising an artifact whose behavior the claim turns on; a ground the user cited for a claim is taken up here as a check they carried out; a UserHeld check nobody has cited for stays Unmet and is surfaced as the question it names)
...
Phase 3 TurnApply (track) → Internal state update (Cite writes its ground into Λ.cited and re-enters assessment from CheckRead; Adopt and Withdraw write preference only; Explore writes nothing unless its answer leaves the basis, in which case it seeds Λ.reopen_seed and re-enters the same way)
...
Λ = { phase: Phase, R: Text, Sₐ: Domain, Sₜ: Domain,
      ...
      inferences: Option(K),   -- None until settle_inferences runs for the current focus, which happens before construction; re-settled when the focus changes, because what a comparison is asked to license is scoped by the comparison
      ...
      reopen_seed: Option(String),   -- a question an Explore raised that the current basis could not answer; written by Phase 3, folded in as an Unmet Check at the next CheckRead, and cleared at Phase 2 entry
      ...
      cited: Map(FitClaim → Set(Ground × Bearing)),   -- the grounds the user cited, under the claim each was cited for. Written only by Cite, consumed by RunChecks, and the only way a UserHeld check is met
...
-- Invariant: warrant[x] = Supported(g) ∨ Defeated(g) ⟹ g is the ground of a check on x that was carried out, by this activation's own reach or by a Cite the user made, and an assenting utterance alone never supplies one. This is the invariant the whole re-founding turns on: agreement cannot promote a claim, and disagreement cannot defeat one without a ground
