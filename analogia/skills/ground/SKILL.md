...
```
── FLOW ──
Analogia(R) → attempts := 0 ; basis_dirty := false ; refuted_focuses := ∅ → Detect(R) → [¬uncertain: Qc(zero_gap_finding) → Stop → (Confirm: deactivate | Reopen(q): reopen_seed := q, proceed)] → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ) ∧ φₚ ∉ Λ.refuted_focuses: FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ) ∨ φₚ ∈ Λ.refuted_focuses: Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): reframe_basis(d) → re-enter derive_focus_candidates)] → [focus_change_requires_disposition(Λ, φ'): Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ)))] → Λ.focus := φ' ; Λ.basis_dirty := false → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → carry_over(M) → AssessFit(M, Sₐ, Sₜ) → [self_grounding(Sₐ, Sₜ): PartitionRead(F, Sₜ)] → [M = ∅: surface(no correspondence constructed along φ', F.missing) → Λ.refuted_focuses := Λ.refuted_focuses ∪ {φ'} ; Λ.partition_reading := None → derive_focus_candidates(Sₐ, Sₜ, F.missing) → re-enter focus settlement (attempts NOT reset — the failed Map already charged one, which is what bounds this retry; the domain basis is unchanged, so basis_dirty stays as it was) | M ≠ ∅ ∧ Λ.remaining = ∅: declare_fit_disposition(F, ∅) → D_f → R' → terminalize | M ≠ ∅ ∧ Λ.remaining ≠ ∅: c ∈ Λ.remaining → I(M, F, Sₜ) → V → D_f → R'] → (loop until terminalized ∨ attempts_exhausted(Λ))
...
Phase 1 FocusReadback (extension) → TextPresent+Proceed (conditional: focus_settled(φₚ) already holds — checked PER FIELD, relation ∧ purpose independently determined — from explicit user language or a citable standing rule, AND φₚ ∉ Λ.refuted_focuses; relay φₚ, binding φ' := φₚ, no gate. The refuted clause is what stops a focus that already produced an empty mapping from being relayed straight back into the same empty construction)
...
5. Branch on what the reconstruction left, rather than entering Phase 2 unconditionally:
   - **`M` is empty** — construction produced no correspondence along this focus. Relay that finding with `F.missing` as its basis, mark the focus refuted so it cannot be relayed straight back, seed the next candidate derivation with `F.missing`, and re-enter focus settlement. Do not reset the attempt count here: the failed construction already charged one, and that charge is the only thing bounding this retry. An empty mapping was never validated, so it is not convergence
