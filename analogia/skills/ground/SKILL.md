...
```
── FLOW ──
Analogia(R) → attempts := 0 → Detect(R) → [¬uncertain: Qc(zero_gap_finding) → Stop → (Confirm: deactivate | Reopen(q): reopen_seed := q, proceed)] → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → [(Sₐ, Sₜ) changed: attempts := 0] → re-enter derive_focus_candidates)] → [focus_change_requires_disposition(Λ, φ'): Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ)))] → Λ.focus := φ' → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → AssessFit(M, Sₐ, Sₜ) → [self_grounding(Sₐ, Sₜ): PartitionRead(F, Sₜ)] → I(M, F, Sₜ) → V → D_f → R' → (loop until terminalized ∨ attempts = max)
...
FocusChangeAnswer = ParkPriorJudgments ∪ RevalidatePriorJudgments  -- user's answer to the focus-change checkpoint; occasion-local, neither constructor establishes a standing policy. ParkPriorJudgments retains the affected records and their addressed status under each record's own focus_snapshot; RevalidatePriorJudgments returns the affected correspondences to remaining and removes their prior terminal judgments
affected(Λ) = { r ∈ Λ.validations | Λ.focus = Some(r.focus_snapshot) ∧ r.correspondence ∈ (Λ.confirmed ∪ Λ.dismissed) }  -- the already-terminal judgments standing on the OUTGOING focus; Λ.focus still holds that outgoing value at the checkpoint, since Λ.focus := φ' runs only after the disposition resolves. Takes Λ alone, so no unwrapping of Option(MappingFocus) is left to the call site; ∅ when Λ.focus = None
focus_change_requires_disposition(Λ, φ') ≡ ∃ φ : Λ.focus = Some(φ) ∧ φ' ≠ φ ∧ affected(Λ) ≠ ∅  -- the checkpoint's guard: a stored focus exists, the successor differs, and terminal judgments stand under the outgoing one. False on the initial settle path (Λ.focus = None), so the checkpoint does not fire there
...
retain(A) = identity over confirmed, dismissed, remaining, validations  -- ParkPriorJudgments state update: the affected records keep their addressed status, each still carrying its own focus_snapshot
...
Phase 1: uncertain → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → [(Sₐ, Sₜ) changed: attempts := 0] → re-enter derive_focus_candidates, recheck focus_settled)] → [focus_change_requires_disposition(Λ, φ'): surface(Λ.focus, φ', affected(Λ)) → Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ)))] → Λ.focus := φ' → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → AssessFit(M, Sₐ, Sₜ) → F → [reopen_seed = Some(q): F.open := F.open ∪ {q}; reopen_seed := None] → [self_grounding: PartitionRead(F, Sₜ) → PartitionReading]  -- settle_focus binds a provisional φₚ BEFORE the settlement guard reads it (per-field over relation ∧ purpose, NOT source_scope/target_scope alone — see Rule 3b); Reframe re-enters decomposition instead of falling through; the focus-change checkpoint fires only when terminal judgments stand under an outgoing focus; the attempt counter is charged at the Map/AssessFit reconstruction, never at the focus checkpoints; then fit map (Reopen seed folded into F.open, then cleared — track); derived split-vs-trim reading in the self-grounding case (relay), via partition_reading() [Tool]
...
Continue until one of the following, in this precedence: terminalized(R', F, D_f) → ValidatedMapping ; ¬terminalized(R', F, D_f) ∧ attempts = max ∧ a further Map/AssessFit reconstruction would be required → AttemptExhausted ; user ESC → EarlyExit. Success takes precedence over the cap, so a run that terminalizes on its last attempt converges rather than exhausting; and because the cap counts reconstructions rather than judgments, correspondences still remaining from the mapping already built continue through Phase 2 on the current attempt instead of tripping the cap.
...
Phase 1 FocusReadback (extension) → TextPresent+Proceed (conditional: focus_settled(φₚ) already holds — checked PER FIELD, relation ∧ purpose independently determined — from explicit user language or a citable standing rule; relay φₚ, binding φ' := φₚ, no gate)
Phase 1 FocusSelector (constitution) → present (conditional: ¬focus_settled(φₚ); candidate MappingFocus options — Select(MappingFocus) or Reframe(description); fires once per domain pair before Map — this is the initial focus SELECTION only; the separate focus-CHANGE disposition (Phase 1 FocusChange below) fires conditionally, whenever a revision would strand judgments already terminal under the outgoing focus; Esc key → loop termination at LOOP level, not a FocusAnswer)
Phase 1 FocusStore (track) → Internal state update (Λ.focus := φ' once settled via FocusReadback or FocusSelector, after any focus-change disposition has resolved; a changed (Sₐ, Sₜ) domain pair clears Λ.focus, resets attempts := 0, and re-triggers the checkpoint)
Phase 1 FocusChange (constitution) → present (conditional: focus_change_requires_disposition(Λ, φ') — the outgoing focus, the successor focus, and the affected already-terminal judgments as pre-gate relay; ParkPriorJudgments / RevalidatePriorJudgments; Esc key → loop termination at LOOP level, not a FocusChangeAnswer)
...
exhausted    (extension)       → TextPresent+Proceed (conditional: attempts = max with a further reconstruction required; partial transformation trace + unresolved-correspondence declaration; terminate as AttemptExhausted, not ValidatedMapping)
...
7. **Validation respected**: User validation or dismissal is final for that correspondence under its recorded focus_snapshot in the current session, unless the user returns it to pending at the focus-change checkpoint
