...
exhaustion_residual(Λ) = Λ.fit_map.missing ∪ Λ.remaining when Λ.fit_map = Some(F), else the declaration that no mapping was constructed for the current (Sₐ, Sₜ) within its attempt budget  -- the exit describes the pair it is exiting, never the one it last left: after a Reframe restores an exhausted pair's count, the fit map and remainder of the previous pair are gone (reframe_basis clears them), so there is nothing from a departed pair to report. Λ.remaining survives an empty construction (carry_over is skipped there), but that remainder belongs to the SAME pair and is genuinely unresolved, so it belongs in the residual rather than being stale
declare_exhaustion_disposition(Λ) = declare_fit_disposition(F, ∅) when Λ.fit_map = Some(F), else { status: Bounded, issues: exhaustion_residual(Λ) }  -- the exhaustion exit's D_f, total over both fit-map states. declare_fit_disposition consumes an F, and the pre-Map cap test genuinely fires with none: reframe_basis clears the fit map on a pair change, and the arriving pair's restored count may already sit at max, so a run can exhaust having built nothing for this pair. Routing that case through declare_fit_disposition leaves D_f unconstructible; there the declaration IS the residual. Read at every exhaustion site so the two cannot drift
...
Λ = { phase: Phase, R: Text, Sₐ: Domain, Sₜ: Domain,
      ...
      cause_tag: String }
-- Invariant: mappings = confirmed ∪ dismissed ∪ remaining (pairwise disjoint) — holds in steady state, suspended across the reconstruction window a pair-changing reframe_basis opens: it clears mappings and remaining with the pair that produced them while confirmed/dismissed keep standing, because whether those judgments survive a basis change is the focus-change checkpoint's to decide, not this transition's. carry_over(M) closes the window at the next non-empty Map. The window is safe to exit from: all_addressed requires M ≠ ∅ and reads mappings = ∅ here, so it can never be mistaken for convergence, and both exits report from validations and exhaustion_residual rather than from mappings
