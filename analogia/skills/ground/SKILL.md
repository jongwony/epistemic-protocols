...
```
── FLOW ──
Analogia(R) → attempts := 0 ; basis_dirty := false → Detect(R) → [¬uncertain: Qc(zero_gap_finding) → Stop → (Confirm: deactivate | Reopen(q): reopen_seed := q, proceed)] → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → [(Sₐ, Sₜ) changed: attempts := 0 ; Λ.basis_dirty := true ; Λ.self_grounding := self_grounding(Sₐ, Sₜ) ; Λ.partition_reading := None] → re-enter derive_focus_candidates)] → [focus_change_requires_disposition(Λ, φ'): Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ)))] → Λ.focus := φ' ; Λ.basis_dirty := false → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → carry_over(M) → AssessFit(M, Sₐ, Sₜ) → [self_grounding(Sₐ, Sₜ): PartitionRead(F, Sₜ)] → [M = ∅: surface(no correspondence constructed along φ', F.missing) → re-enter the focus checkpoint | M ≠ ∅ ∧ Λ.remaining = ∅: declare_fit_disposition(F, ∅) → D_f → R' → terminalize | M ≠ ∅ ∧ Λ.remaining ≠ ∅: c ∈ Λ.remaining → I(M, F, Sₜ) → V → D_f → R'] → (loop until terminalized ∨ attempts_exhausted(Λ))
