...
── CONVERGENCE ──
terminalized(R', F, D_f) = all_addressed(R') ∧ fit_disposition_declared(F, D_f)
zero_gap_converged(Λ) ≡ Λ.mappings = ∅ ∧ Λ.phase = 0 ∧ ¬Λ.active  -- the ONE empty-mapping state that is convergence: Phase 0 detected no uncertainty and the user's Confirm deactivated the protocol before Phase 1 ever constructed M. Deactivation is what distinguishes it — every run also SITS in Phase 0 while detection is still running, and that state has converged nothing
...
Λ = { phase: Phase, R: Text, Sₐ: Domain, Sₜ: Domain,
      ...
      refuted_focuses: Set(MappingFocus),   -- focuses already constructed an empty mapping along; a refuted focus never relays via FocusReadback, so the retry reaches a different comparison rather than replaying the failure. Cleared on activation ONLY: a MappingFocus carries its own source_scope/target_scope, so a focus refuted under one domain pair cannot collide with one derived under another, and clearing on pair change would let an A→B→A Reframe replay a comparison already shown to produce nothing
