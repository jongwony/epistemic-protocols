...
misfit_instances : F × Sₜ → Set(MemberInstance)  -- DERIVED projection from the fit map onto the member set: m ∈ Sₜ is misfit iff (∃ c ∈ F.overextended : m violates c's added target constraint) ∨ (∃ facet ∈ F.missing : the abstraction asserts facet of all members but m does not exhibit it). Projects correspondence-level (F.overextended ⊆ M) and component-level (F.missing ⊆ Sₐ) fit onto Sₜ's members; the source-side cells stay in basis, never in the member set — so the reading is genuinely over instances and leaves fit_partition(F, M) intact
...
partition_reading(F, Sₜ) =   -- invoked ONLY under self_grounding (guarded in FLOW / PHASE TRANSITIONS); returns a full PartitionReading RECORD, never a bare verdict token and never the Option None
  let mis = misfit_instances(F, Sₜ), clusters = rival_clusters(mis) in
  { verdict        = Hold if mis = ∅ ; Split if clusters ≠ ∅ ; Trim otherwise,
    rival_essences = clusters,                                    -- ∅ when verdict ∈ {Hold, Trim}; pairwise disjoint
    trim_outliers  = mis \ ⋃ { c.members | c ∈ clusters },        -- misfits in no rival cell: = ∅ when Hold, = mis when Trim, the scattered remainder when Split (mixed Split+scattered)
    core_remainder = Sₜ \ mis,                                    -- the members that genuinely FIT (= Sₜ when Hold); NEVER contains a misfit
    basis          = F }
  -- verdict = Split → wrong fusion: route to /conduct (cells = core + rivals; trim_outliers narrowed-out or placed at the checkpoint); Trim → all misfits scattered: narrow in place (/induce Narrow); Hold → fusion holds, no partition action
...
-- Invariant: partition_reading = None ⟺ ¬self_grounding; under self_grounding it is Some(PartitionReading) with verdict ∈ {Split, Trim, Hold} (Hold = no-misfit, distinct from the Option None — finding-2 disambiguation)
...
4b. **Derive the partition reading** (self-grounding case only): when `self_grounding` holds, compute `partition_reading(F, Sₜ)` over the misfit members — the `misfit_instances(F, Sₜ)` projection: members `m ∈ Sₜ` implicated by `F.overextended` (members that violate an overextended facet's added constraint) or by `F.missing` (members lacking a facet the abstraction asserts of all members). The reading is **derived** from `F` (relay, no gate) and classifies the misfit set:
   - **Split** — the misfit members cluster into ≥1 *coherent rival essence*: a sub-group that consistently supports a *different* abstraction, not just isolated outliers. The reading partitions `Sₜ` into three pairwise-disjoint groups: `rival_essences` (the InstanceClusters, each = member instances + the essence they support — pairwise disjoint), `core_remainder` (the members the original abstraction *genuinely fits* — `Sₜ` minus *all* misfits, so never a misfit), and `trim_outliers` (scattered/ambiguous misfits in no rival cell). The cell-candidate partition handed to `/conduct` is the rival cells **plus** the core cell (covering every fitting member); `trim_outliers` is surfaced for the checkpoint to narrow-out or place, never folded into the core. This is wrong fusion — the member set carries ≥2 essences forced under one form, so the recovery is **decompose**. Route recommendation: the `/conduct` decompose-recovery recipe. Analogia *evidences* the split boundary and the full candidate partition; it does **not** constitute the cell assignment — that is the user's constitutive judgment at the recipe's boundary-checkpoint.
   - **Trim** — the misfits are *scattered* (no coherent rival cluster): a few outliers around an otherwise-sound core. The recovery is to **narrow in place**, not split. Route recommendation: `/induce` Narrow (a single in-`/induce` move, no decompose composition).
