...
```
── FLOW ──
Analogia(R) → attempts := 0 → Detect(R) → [¬uncertain: Qc(zero_gap_finding) → Stop → (Confirm: deactivate | Reopen(q): reopen_seed := q, proceed)] → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → attempts := 0 → re-enter derive_focus_candidates)] → [focus_change_requires_disposition(Λ, φ'): Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ.focus, Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ.focus, Λ)))] → Λ.focus := φ' → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → AssessFit(M, Sₐ, Sₜ) → [self_grounding(Sₐ, Sₜ): PartitionRead(F, Sₜ)] → I(M, F, Sₜ) → V → D_f → R' → (loop until terminalized ∨ attempts = max)
...
affected(φ, Λ) = { r ∈ Λ.validations | r.focus_snapshot = φ ∧ r.correspondence ∈ (Λ.confirmed ∪ Λ.dismissed) }  -- the already-terminal judgments standing on the outgoing focus
focus_change_requires_disposition(Λ, φ') ≡ ∃ φ : Λ.focus = Some(φ) ∧ φ' ≠ φ ∧ affected(φ, Λ) ≠ ∅  -- the checkpoint's guard: a stored focus exists, the successor differs, and terminal judgments stand under the outgoing one. False on the initial settle path (Λ.focus = None), so the checkpoint does not fire there
...
Phase 1: uncertain → (Sₐ, Sₜ) → derive_focus_candidates(Sₐ, Sₜ) → candidates → settle_focus(candidates, R, context) → φₚ → [focus_settled(φₚ): FocusReadback(φₚ) → φ' := φₚ | ¬focus_settled(φₚ): Qc(candidate_focuses) → Stop → FocusAnswer → (Select(φₛ): φ' := φₛ | Reframe(d): (Sₐ, Sₜ) := decompose(R, d, context) → attempts := 0 → re-enter derive_focus_candidates, recheck focus_settled)] → [focus_change_requires_disposition(Λ, φ'): surface(Λ.focus, φ', affected(Λ.focus, Λ)) → Qc(focus_change) → Stop → FocusChangeAnswer → (ParkPriorJudgments: retain(affected(Λ.focus, Λ)) | RevalidatePriorJudgments: return_pending(affected(Λ.focus, Λ)))] → Λ.focus := φ' → attempts := attempts + 1 → Map(Sₐ, Sₜ, φ') → M → AssessFit(M, Sₐ, Sₜ) → F → [reopen_seed = Some(q): F.open := F.open ∪ {q}; reopen_seed := None] → [self_grounding: PartitionRead(F, Sₜ) → PartitionReading]  -- settle_focus binds a provisional φₚ BEFORE the settlement guard reads it (per-field over relation ∧ purpose, NOT source_scope/target_scope alone — see Rule 3b); Reframe re-enters decomposition instead of falling through; the focus-change checkpoint fires only when terminal judgments stand under an outgoing focus; the attempt counter is charged at the Map/AssessFit reconstruction, never at the focus checkpoints; then fit map (Reopen seed folded into F.open, then cleared — track); derived split-vs-trim reading in the self-grounding case (relay), via partition_reading() [Tool]
...
Continue until one of the following, in this precedence: terminalized(R', F, D_f) → ValidatedMapping ; ¬terminalized(R', F, D_f) ∧ attempts = max → AttemptExhausted ; user ESC → EarlyExit. Success takes precedence over the cap, so a run that terminalizes on its last attempt converges rather than exhausting.
...
Phase 1 FocusChangeApply (track) → Internal state update (ParkPriorJudgments → identity over confirmed/dismissed/remaining/validations; RevalidatePriorJudgments → return_pending(affected(Λ.focus, Λ)))
