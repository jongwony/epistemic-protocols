...
Codex에는 `skill-installer`가 이미 포함되어 있으므로, 권장 경로는 Codex에게 현재 Codex 지원 스킬 세트를 설치하라고 요청하는 것입니다:
...
설치 후 Codex를 재시작하세요. 그다음 `$onboard`부터 시작하면 됩니다.

이 one-shot 경로는 현재 `frame`, `gap`, `clarify`, `grasp`, `goal`, `inquire`, `bound`, `ground`, `attend`, `contextualize`, `onboard`를 설치합니다.
`/report`, `/dashboard`, `/preferences`, `reflexion`, `write`는 포함하지 않으며, GitHub release의 `bundle.zip`도 Codex 설치 경로는 아닙니다.

Codex 기준 설치 소스 오브 트루스는 이 README입니다. 하나만 설치하고 싶다면 위와 같은 `skill-installer` 패턴에 단일 path만 넣어 요청하면 됩니다. 고급 사용자는 `install-skill-from-github.py` 를 직접 호출할 수 있지만, 그 경로는 quick start가 아니라 fallback입니다.
...
| 플러그인 | 명령어 | 용도 |
|----------|--------|------|
| [Epistemic Cooperative](./epistemic-cooperative) | `/onboard`, `/report`, `/dashboard`, `/preferences` | 프로토콜 학습, 사용 분석, 커버리지 대시보드, 설정 |
