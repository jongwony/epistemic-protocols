...
- 11개 프로토콜 + `onboard`를 설치합니다. `/report`, `/dashboard`, `/write`는 미포함.
...
각 프로토콜은 human-AI 협업이 어긋날 수 있는 특정 결정 지점을 다룹니다. 공개 문서는 plan-level hook를 앞세우고, contributor 문서는 planning, execution, verification, recall, comprehension까지 포괄하는 더 넓은 machinery를 설명합니다. 두 층을 잇는 설명은 [docs/mission-bridge.md](./docs/mission-bridge.md), 아키텍처와 설계 철학의 상세 내용은 [CLAUDE.md](./CLAUDE.md)를 참조하세요.
