...
유틸리티 플러그인 둘은 opt-in이라 위 한 줄은 건너뜁니다. `epistemic-cooperative`는 학습·결핍 인식(`/onboard`, `/probe`)과 컨트리뷰터 도구를, `route`는 세션 훅들 — 매 프롬프트 라우팅 지시문, 세션 시작에 설치된 프로토콜의 결핍 테이블과 그 아래 [premise](./premise) 색인, 그리고 지시 표면을 편집하는 시점에 그 순간의 premise 항목 — 을 담습니다. 필요한 쪽을 따로 추가하세요:
