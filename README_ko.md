...
Devin은 `.agents/skills/` 경로에서 스킬을 탐색합니다. 이 레포지토리는 모든 프로토콜/유틸리티 스킬을 해당 경로에 미리 노출시켜 둡니다(플러그인 원본을 가리키는 상대 심링크 — 파일 중복 없음). Devin이 레포를 클론하면 자동으로 인식되며, 별도 인스톨러 단계는 필요 없습니다.
...
> 참고: 스킬 디스커버리는 검증되었으나, `PHASE TRANSITIONS` / `TOOL GROUNDING`에서 참조하는 Claude Code 전용 도구의 런타임 동작은 Devin 환경에서 다를 수 있습니다. Devin 호환성은 Stage 2 사용 증거 누적 전까지 Stage 1 추정으로 다루세요.
