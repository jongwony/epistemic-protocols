...
Merismos(G) → Probe(G) → goal_plan_uncompiled? →
  ...
  uncompiled: ReadObligations(G) → O_G → VelocityFilter(O_G) → oos → init_loop_state: U=∅, residual=O_G \ {d.obligation | d∈oos}, K=∅, R=∅, P=∅,
    ...
    Phase 1 Scan(G, residual) [per-cycle re-scan] → seams(residual) → Pack(seams, horizon) → (Anchor, draft_unit) → fit(draft_unit, horizon) → SpanFit → qualify(that cut) → Seam → complete_unit(draft_unit, SpanFit, Seam) → proposed_unit →
      ...
      Anchor empty ∧ residual ≠ ∅:  → autonomous_pack(residual) (track) → U'' → ∀u∈U'' with SpanFit = Fits: integrate_unit(u) → u' → U := U ∪ {u'}, residual := residual \ u'.obligations → [∃u∈U'' with SpanFit ≠ Fits: → Qu(u's Anchor, u, its SpanFit, its Seam, U_snapshot) over the first such unit, reason surfaced as relay first → Stop → Aᵤ → the same Aᵤ dispatch | none: residual = ∅ → surface (extension) → Phase 2]   -- the fitting siblings are integrated FIRST, then the overflows are settled one per cycle; autonomous_pack completes EVERY obligation it touches into a ProposedUnit, the unplaceable remainder included, so the gate always receives a produced four-tuple, and residual strictly shrinks per settled overflow, so re-entering Scan on the same exhausted seam evidence reaches this branch with strictly less left
      SpanFit = Fits ∧ Pack's search finds no second Fits-worthy cut for Anchor: → relay(AcceptUnit) (extension) → integrate_unit → loop  -- option-set relay test
        ...
        Aᵤ = Sufficient     → autonomous_pack(residual) (track) → U'' → ∀u∈U'' with SpanFit = Fits: integrate_unit(u) → u' → U := U ∪ {u'}, residual := residual \ u'.obligations → [∃u∈U'' with SpanFit ≠ Fits: → Qu(u's Anchor, u, its SpanFit, its Seam, U_snapshot) over the first such unit, reason surfaced as relay first → Stop → Aᵤ → the same Aᵤ dispatch | none: residual = ∅ → surface (extension) → Phase 2]
...
Phase 1: (G, residual) → Scan [Tool] → seams → Pack(seams, H) → (Anchor, draft_unit) → fit(draft_unit, H) → SpanFit → qualify(that cut) → Seam → complete_unit(draft_unit, SpanFit, Seam) → proposed_unit   -- Pack searches, fit judges the horizon, and qualify produces the Seam disposition, so the four-tuple the gate receives has a named producer for each component; complete_unit is what WRITES that fit and seam onto the draft, since ProposedUnit declares both as required fields and nothing before this step has filled them. apportionment loop (sense); residual/oos enter this phase either freshly seeded or as Reopen left them — never re-seeded on entry
           Anchor empty ∧ residual = ∅ ∧ U = ∅ ∧ oos = ∅ → relay(goal's scope too thin to read any obligation — route to /inquire) (extension) → deactivate
           Anchor empty ∧ residual = ∅ ∧ (U ≠ ∅ ∨ oos ≠ ∅) → Phase 2
           Anchor empty ∧ residual ≠ ∅ → autonomous_pack(residual) (track) → U'' → ∀u∈U'' with SpanFit = Fits: integrate(u) → u' → U := U ∪ {u'}, residual := residual \ u'.obligations → [nothing overflows ∧ residual = ∅: surface (extension) → Phase 2 | else Qu over the first overflowing unit, carrying its own SpanFit and Seam → Stop → Aᵤ → the Aᵤ dispatch below] → Phase 1
           SpanFit = Fits ∧ no second Fits-worthy cut exists for Anchor → relay(AcceptUnit) (extension) → integrate → Phase 1  -- option-set relay test
             ...
             Aᵤ = Sufficient   → autonomous_pack(residual) (track) → U'' → ∀u∈U'' with SpanFit = Fits: integrate(u) → u' → U := U ∪ {u'}, residual := residual \ u'.obligations → [nothing overflows ∧ residual = ∅: surface (extension) → Phase 2 | else Qu over the first overflowing unit, carrying its own SpanFit and Seam → Stop → Aᵤ → the same Aᵤ dispatch] → Phase 1
...
1. **User-initiated, AI-apportioned**: User declares autonomous execution intent via `/apportion`; AI reads the obligations, searches the apportionment, and derives the conditions; the user settles each unit at Phase 1 — except the single-dominant-cut case Rule 17 relays as Extension, and the fitting units autonomous packing integrates when the user has declared the apportionment sufficient or the goal's seam evidence is exhausted, each surfaced as relay with its heuristic declaration and every overflow still carried to the gate — and the conditioned plan at Phase 2 via Cognitive Partnership Move (Constitution).
