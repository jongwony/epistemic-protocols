...
Phase 0 → deactivate: ¬autonomous_intent(G) ∨ condition_bearing(G) ∨ (locator in scope ∧ (¬dereferenceable ∨ support-integrity failure))   -- relay the scan result; no activation. "Locator in scope" is EITHER locator this phase can hold: the goal's navigation block, or the one an owed re-apportionment carries
...
Phase 0 ReadObligations (observe)   → TaskGet, Read (construct O_G once as G.obligations plus, when owed_unit(G), the obligations read at the owed unit's ref by dereferencing the record that owed edge's own locator names — an unreachable record or a ref resolving to no such unit takes the SAME unreadable-handoff arm the goal's navigation block takes, rather than yielding a short O_G that coverage_complete would then pass over. The ref points into this protocol's own parked record, so what crosses back over the seam is an identity and a locator, nothing more; a local read, never a G mutation. Produces the set VelocityFilter, residual seeding, and coverage_complete consume)
