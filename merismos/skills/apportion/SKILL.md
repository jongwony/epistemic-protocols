  ...
  uncompiled: ReadObligations(G) → O_G → VelocityFilter(O_G) → oos → init_loop_state: U=∅, residual=O_G \ {d.obligation | d∈oos}, K=∅, R=∅, P=∅,
    ...
    Phase 1 Scan(G, residual) [per-cycle re-scan] → seams(residual) → Pack(seams, horizon) → (Anchor, draft_unit) → fit(draft_unit, horizon) → SpanFit → qualify(that cut) → Seam → complete_unit(draft_unit, SpanFit, Seam) → proposed_unit →
      ...
      SpanFit = Fits ∧ Pack's search finds no second Fits-worthy cut for Anchor: → relay(AcceptUnit) (extension) → integrate_unit → loop  -- option-set relay test
        ...
        Aᵤ = Sufficient     → autonomous_pack(residual) (track) → ∀u∈U'' Fits-worthy: integrate_unit(u) → u' → U := U ∪ {u'} → [any packed unit with SpanFit ≠ Fits, or residual ≠ ∅ after packing: next cycle's Anchor → loop | otherwise residual := ∅ → surface (extension) → Phase 2]
...
Phase 1 autonomous_pack (track)     → Internal state update (extension — pack the remainder at Heuristic seams when seam evidence is exhausted or the user declares the apportionment sufficient; judges fit and records the Heuristic seam per packed unit and runs complete_unit on each draft, so what it integrates are completed ProposedUnit values — the constructor is the same one the anchored path uses, never a second inhabitant; otherwise exactly as Pack does, capability requirements and feasibility notes included; each packed unit re-checks fit and is surfaced as relay with its heuristic declaration; a packed unit whose fit is not Fits does NOT enter U — it re-enters as the next cycle's anchor for Qu, where the override is the user's to record; a still-non-empty residual likewise re-anchors; every Fits-worthy packed unit is routed through integrate before entering U)
