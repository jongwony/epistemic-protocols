...
Merismos(G) → Probe(G) → goal_plan_uncompiled? →
  ...
  uncompiled: ReadObligations(G) → O_G → VelocityFilter(O_G) → oos → init_loop_state: U=∅, residual=O_G \ {d.obligation | d∈oos}, K=∅, R=∅, P=∅,
    ...
    Phase 1 Scan(G, residual) [per-cycle re-scan] → seams(residual) → Pack(seams, horizon) → (Anchor, draft_unit, SpanFit) → qualify(that cut) → Seam → complete_unit(draft_unit, SpanFit, Seam) → proposed_unit →
