...
Merismos(G) → Probe(G) → goal_plan_uncompiled? →
  ...
  uncompiled: ReadObligations(G) → O_G → VelocityFilter(O_G) → oos → init_loop_state: U=∅, residual=O_G \ {d.obligation | d∈oos}, K=∅, R=∅, P=∅,
    ...
    Phase 1 Scan(G, residual) [per-cycle re-scan] → seams(residual) → Pack(seams, horizon) → (Anchor, draft_unit) → fit(draft_unit, horizon) → SpanFit → qualify(that cut) → Seam → complete_unit(draft_unit, SpanFit, Seam) → proposed_unit →
      ...
      Anchor empty ∧ residual ≠ ∅:  → autonomous_pack(residual) (track) → ∀u∈U'' Fits-worthy: integrate_unit(u) → u' → U := U ∪ {u'} → [any packed unit with SpanFit ≠ Fits, or residual ≠ ∅ after packing: → Qu(that unit — or, when only residual remains, the remaining obligations as their own Anchor — with its SpanFit, its Seam and the cut-set snapshot; reason surfaced as relay first) → Stop → Aᵤ | otherwise residual := ∅ → surface (extension) → Phase 2]   -- the overflow does NOT re-enter Scan: Anchor is Pack's own output, so looping on the exhausted seam evidence that made autonomous_pack fire would reproduce this same branch and never reach Qu   -- the fitting siblings are integrated FIRST, then the remainder re-anchors; a mixed pack never discards work that already fits
      SpanFit = Fits ∧ Pack's search finds no second Fits-worthy cut for Anchor: → relay(AcceptUnit) (extension) → integrate_unit → loop  -- option-set relay test
        ...
        Aᵤ = Sufficient     → autonomous_pack(residual) (track) → ∀u∈U'' Fits-worthy: integrate_unit(u) → u' → U := U ∪ {u'} → [any packed unit with SpanFit ≠ Fits, or residual ≠ ∅ after packing: → Qu(that unit — or, when only residual remains, the remaining obligations as their own Anchor — with its SpanFit, its Seam and the cut-set snapshot; reason surfaced as relay first) → Stop → Aᵤ | otherwise residual := ∅ → surface (extension) → Phase 2]
...
Phase 1: (G, residual) → Scan [Tool] → seams → Pack(seams, H) → (Anchor, draft_unit) → fit(draft_unit, H) → SpanFit → qualify(that cut) → Seam → complete_unit(draft_unit, SpanFit, Seam) → proposed_unit   -- Pack searches, fit judges the horizon, and qualify produces the Seam disposition, so the four-tuple the gate receives has a named producer for each component; complete_unit is what WRITES that fit and seam onto the draft, since ProposedUnit declares both as required fields and nothing before this step has filled them. apportionment loop (sense); residual/oos enter this phase either freshly seeded or as Reopen left them — never re-seeded on entry
           Anchor empty ∧ residual = ∅ ∧ U = ∅ ∧ oos = ∅ → relay(goal's scope too thin to read any obligation — route to /inquire) (extension) → deactivate
           Anchor empty ∧ residual = ∅ ∧ (U ≠ ∅ ∨ oos ≠ ∅) → Phase 2
           Anchor empty ∧ residual ≠ ∅ → autonomous_pack(residual) (track) → ∀u∈U'' (Fits-worthy packed units): integrate(u) → u' → U := U ∪ {u'} → [nothing overflows ∧ residual = ∅: surface (extension) → Phase 2 | else Qu over the overflowing unit, or over the remaining obligations as their own Anchor → Stop → Aᵤ] | re-anchor → Phase 1
           SpanFit = Fits ∧ no second Fits-worthy cut exists for Anchor → relay(AcceptUnit) (extension) → integrate → Phase 1  -- option-set relay test
             ...
             Aᵤ = Sufficient   → autonomous_pack(residual) (track) → ∀u∈U'' (Fits-worthy packed units): integrate(u) → u' → U := U ∪ {u'} → [nothing overflows ∧ residual = ∅: surface (extension) → Phase 2 | else Qu over the overflowing unit, or over the remaining obligations as their own Anchor → Stop → Aᵤ] | re-anchor → Phase 1
...
Phase 1 autonomous_pack (track)     → Internal state update (extension — pack the remainder at Heuristic seams when seam evidence is exhausted or the user declares the apportionment sufficient; judges fit and records the Heuristic seam per packed unit and runs complete_unit on each draft, so what it integrates are completed ProposedUnit values — the constructor is the same one the anchored path uses, never a second inhabitant; otherwise exactly as Pack does, capability requirements and feasibility notes included; each packed unit re-checks fit and is surfaced as relay with its heuristic declaration; a packed unit whose fit is not Fits does NOT enter U — it is presented AT Qu IN THIS SAME CYCLE as its own anchor, where the override is the user's to record; a still-non-empty residual is presented the same way, its remaining obligations serving as the anchor. Neither re-enters Scan: Anchor is Pack's output, and the exhausted seam evidence that made this step fire would only reproduce the empty-Anchor branch; every Fits-worthy packed unit is routed through integrate before entering U)
...
1. **User-initiated, AI-apportioned**: User declares autonomous execution intent via `/apportion`; AI reads the obligations, searches the apportionment, and derives the conditions; the user settles each unit at Phase 1 — except the single-dominant-cut case Rule 17 relays as Extension — and the conditioned plan at Phase 2 via Cognitive Partnership Move (Constitution).
