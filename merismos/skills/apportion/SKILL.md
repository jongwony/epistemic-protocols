...
Merismos(G) → Probe(G) → goal_plan_uncompiled? →
  ...
  navigation block in scope ∧ ¬dereferenceable: → relay(handoff unreadable — locator unreachable or missing its session half) (extension) → deactivate
  ...
  uncompiled: ReadObligations(G) → O_G → VelocityFilter(O_G) → oos → init_loop_state: U=∅, residual=O_G \ {d.obligation | d∈oos}, K=∅, R=∅, P=∅,
    ...
    Phase 3 Emit(U, K, R, P, oos, unbounded_approved) → E [TaskCreate] → package(E) → record_handoff(E) → N → converge(apportionment trace) → ConditionBearingUnitPlan
...
locator(E)     = HandoffLocator { record: the record identity the substrate assigned this emission at Emit — TaskCreate's own product, resolved back by TaskGet at dereference, so how the substrate groups E's entries under one identity is delegated to it; session: the id of the session running record_handoff }
...
handoff_recorded(N, E) ≡ record_handoff presented N in the handoff output ∧ N.purpose_frame ≠ "" ∧ N.canonical_locator = locator(E) ∧ N.canonical_locator.session ≠ ""   -- the emission IS the text
── PHASE TRANSITIONS ──
...
Phase 3: (U, K, R, P, oos, unbounded_approved) → Emit → E [Tool: TaskCreate] → package(E) → record_handoff(E) → N (extension) → converge(apportionment trace) (extension) → ConditionBearingUnitPlan
...
Phase 3 record_handoff (extension)  → TextPresent+Proceed (emit N in the fixed navigation-block shape with canonical_locator = locator(E) — its record half read from what Emit's TaskCreate returned, its session half this session's own id — a dereference instruction, the optional snapshot anchor only when exact-state determinacy is needed, and the grounding instruction; entry points only, never a re-authored plan. This emitted block is what handoff_recorded(N, E) reads)
