...
Merismos(G) → Probe(G) → goal_plan_uncompiled? →
  ...
  navigation block in scope ∧ ¬dereferenceable: → relay(handoff unreadable — locator unreachable or missing its session half) (extension) → deactivate   -- the canonical record is LOST, not absent; recompiling here would silently rebuild a second plan blind to the first's accepted residuals and owed obligations
...
AutonomousGoal × ExecutionHorizon
  ...
  → record_handoff(E) → N              -- emits the fixed-shape navigation block a later session dereferences to read E back
...
BindPlanRequirements(P, U) = { p with dischargeable_when := plan_terminal(|U|) when p.scope = WholeGoalAcceptance; p unchanged otherwise | p ∈ P }
...
HandoffLocator = { record: the durable identity of the emitted record set E, session: the id of the session that emitted E }  -- both halves are required: E is created through a session-scoped instrument (TaskCreate), so the record identity alone does not resolve from a later session
locator        = E → HandoffLocator   -- reads the emitting session's own id for the session half; neither half is inferred by the reader
...
handoff_recorded(N, E) ≡ record_handoff presented N in the handoff output ∧ N.purpose_frame ≠ "" ∧ N.canonical_locator = locator(E) ∧ N.canonical_locator.session ≠ ""   -- the emission IS the text; locator(E) is the step that produces both halves
── PHASE TRANSITIONS ──
Phase 0: G → Probe(G) [Tool] → goal_plan_uncompiled?                   -- activation checkpoint (observe): dereferences a prior navigation block when one is in scope, else internal analysis
           ...
           navigation block in scope ∧ ¬dereferenceable → relay → deactivate   -- handoff unreadable; the uncompiled arm is NOT taken (extension)
...
Phase 3 record_handoff (extension)  → TextPresent+Proceed (emit N in the fixed navigation-block shape with canonical_locator = locator(E) — carrying BOTH E's record identity and this session's own id, since a session-scoped record does not resolve from either half alone — a dereference instruction, the optional snapshot anchor only when exact-state determinacy is needed, and the grounding instruction; entry points only, never a re-authored plan. This emitted block is what handoff_recorded(N, E) reads)
...
**Declared Seam over Asserted Joint**: A cut cites the seam it sits on — a dependency edge, a deliverable boundary, a verification point, an ownership change — or declares itself heuristic. Seam quality is a declared disposition rather than an invariant: the plan is honest about which cuts are grounded and which are judgment calls, and the user judges the heuristic ones with that fact visible.
...
goal_plan_uncompiled(G) ≡ autonomous_intent(G) ∧ ¬condition_bearing(G)
  ...
  condition_bearing(G)  ≡ (G already carries units whose conditions are compiled — each closed by a determinate completion
                          ...
                          navigation block rather than off the goal's utterance, or supplied by the user as an explicit
...
Scan, in priority order: prior protocol output in this session (a boundary map, or a conducted method's unresolved autonomous region, is the richest input), `/apportion` arguments, then conversation context. Cite the cue for autonomous intent. When the scan turns up a prior `/apportion` navigation block, dereference it at its locator — record identity within the session that locator names — and decide `condition_bearing` against the record read back, not against the goal's wording; a unit the record closes by recorded acceptance is carried, the same as one closed by a predicate. A block that is present but will not dereference — the locator unreachable, or missing its session half — stops the protocol: surface that the canonical record is unreachable and deactivate. Recompiling instead would rebuild a second plan from heuristic context, blind to the first plan's accepted residuals and owed obligations. Only the absence of any block leaves the uncompiled path correct.
...
4. **Present** the proposed unit with its fit verdict, its seam quality and basis, the current cut-set, and the cycle counter, via Cognitive Partnership Move (Constitution) — unless a single dominant unit fits, in which case accept it as relay.
...
4. **Emit the handoff navigation block** and deactivate: purpose/frame, E's canonical locator — its record identity together with this session's own id, because the record is parked in a session-scoped instrument and a later reader cannot reach it from the record identity alone — the dereference instruction, a snapshot anchor only when exact-state determinacy is needed, and the grounding instruction to run `/inquire` where available (or equivalent grounding) and stop on an unreachable source or unsupported load-bearing premise. This is a pointer to E, never a re-authored plan.
...
12. **Stateless compile, but never blind**: Emission is terminal. No session approvals, no per-action classification, no mid-execution checkpoint; a re-invocation carries no invocation-local state forward and recompiles rather than resuming. What it does not do is recompile past a record it should have read: a prior plan reached through its navigation block is already condition-bearing and relays, and a navigation block that is present but will not dereference — locator unreachable, or missing its session half — stops the protocol instead of falling through to the uncompiled path. Statelessness is about not resuming Λ; it is not a licence to rebuild a second plan blind to a first one that still exists.
