  ...
  uncompiled: ReadObligations(G) → O_G → VelocityFilter(O_G) → oos → init_loop_state: U=∅, residual=O_G \ {d.obligation | d∈oos}, K=∅, R=∅, P=∅,
    ...
    Phase 2 ∀u∈U, ¬derived_already(u,K,R): Derive(u) → (Set(κ), Set(ρ)) → K:=K∪κs, R:=R∪ρs ∥ [¬Λ.plan_conditions_derived: DerivePlan(G, U) → P; Λ.plan_conditions_derived := ⊤] →
      ...
      Qc(U, K, R, P, InvariantStatus, oos) → Stop → V →
        V = Adjust(d)  → rederive(K, R, P, d) → (K, R, P) := (K', R', P') → Λ.plan_conditions_stale := ⊥ → [¬acceptance_present(P') → Qt(K', P') → Stop → Vₜ → P' updated as at Phase 2 entry] → [acceptance_present(P') ∧ Λ.unbounded_approved: Λ.unbounded_approved := ⊥] → BindPlanRequirements(P', U) → P' := Pᵦ' → check(U, K', R', Pᵦ', oos) → Λ.invariant_status := InvariantStatus → Qc(...)   -- over the SAME U: K' ∪ R' spans every obligation of every unit; no removal — a withdrawn condition becomes a residual
...
Phase 0: G → Probe(G) [Tool] → goal_plan_uncompiled?                   -- activation checkpoint (observe): dereferences a prior navigation block when one is in scope, else internal analysis
           ...
           uncompiled            → ReadObligations(G) → O_G → VelocityFilter(O_G) → oos → init_loop_state (U=∅, residual=O_G \ {d.obligation | d∈oos}, K=∅, R=∅, P=∅, plan_conditions_derived=⊥, plan_conditions_stale=⊥, fit_overrides=∅, invariant_status=⊥, accepted=∅, unbounded_approved=⊥) → Phase 1   -- ONE-TIME init, fired only on this edge; Phase 2's Reopen re-enters Phase 1 without re-running it. ReadObligations is this edge's formal producer for an owed unit's obligations entering the LOCAL compilation set, and the same step SUBTRACTS the host's standing procedural contract, so residual is seeded from what THIS goal generated; G remains unchanged
...
--   either: ReadObligations subtracts it before O_G exists, so the partition never has to place it
