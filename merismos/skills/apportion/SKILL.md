...
RecordRef      = the durable identity of one emitted record entry, as the harness names it
HandoffLocator = { record: the durable identity of the emitted record set E, session: the id of the session that emitted E }
locator(E)     = HandoffLocator { record: the durable identity of E's plan-envelope entry, through which every other entry of E is reachable; session: the id of the session running record_handoff }
...
apportioned(G) = emitted(E) ∧ handoff_recorded(N, E)
                 ...
                 ∧ (∀e∈E: e is PlanEnvelopeEntry → e.entry_refs = { ref(x) | x ∈ E, x ≠ e })   -- the envelope indexes every other entry of this emission; this is what makes locator(E)'s single record identity resolve all of E
