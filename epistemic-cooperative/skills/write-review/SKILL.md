...
4. **Feedback consumption is single-shot with latest-timestamp dedup** — each `feedback-{slug}.jsonl` is read once and archived to prevent stale loops; for entries sharing the same `(anchor, context_before, context_after)` tuple, the revision consumer keeps only the latest-timestamp entry. This supports in-browser comment editing (click an existing 💬 mark to edit) without breaking the append-only JSONL invariant.
