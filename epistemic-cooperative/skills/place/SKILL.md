...
One example can answer to more than one kind — a filled invocation shown with its result pins an exact shape and teaches a procedure at the same time. Where the kinds it matches disagree about the destination, each kind contributes its argument only once its own condition holds, and the tier ordering decides among the arguments that survive: a schema or grammar that fully specifies the contract removes the specimen's claim on the loaded surface, while a need arising before the on-demand surface loads keeps the load-bearing part at the earlier tier. A preference the table states is an argument for a destination and never a route on its own. Where an example matches no kind at all, what the route describes in place of a label is what the example does, because a label taken for closeness carries a standing-surface rule the example was never measured against.

Where a specimen's condition cannot be checked — nothing records whether prose alone has failed — and no other kind it matches independently requires an earlier tier, route it to the on-demand surface and record the missing evidence. Granting a standing place on an unverified condition is how the exception becomes the rule; letting unverified evidence about one kind's condition override an earlier need another kind established is how the fallback overrides a settled fact.
...
- **The required action follows every disposition the route names.** A relocating clause routinely names two: the extraction that let it move is what leaves it needing to stand alone, so `move` and `rewrite` co-occur and each discharges its own obligation — whichever label the route leads with settles nothing about the other. For `rewrite`, the exact restatement. For `move`, the destination binding, or an explicitly unresolved binding with the reason; where the destination is the on-demand surface, also the outbound pointer on the source surface that reaches it, or a note that one already exists. A clause moved to a reference nothing points at is not relocated but lost — the on-demand surface is defined by being pointed to, so naming the outbound pointer is a condition of the route working rather than a courtesy after it. The inbound direction carries the same condition and a different obligation: the outbound pointer is an edge the route creates and already knows both ends of, while an inbound pointer is one that exists elsewhere and has to be found. Before emitting any `move`, `rewrite`, or `delete` whose application would change what the unit is, where it sits, or whether it remains, search the reference-search boundary for pointers at it. The required action names each one found together with the edit that keeps it reaching what it names; where none is found, it names the boundary searched and where the pass established that. A pointer needs an edit exactly where application would stop it reaching what it names, so one naming a file the route leaves in place needs none while one naming what moved, what a rewrite renamed, or what a delete removed does. Where the boundary or the match between a pointer and its referent cannot be bounded, the required action is unresolved and names the search or the owner still to be reached, rather than reporting reachability as preserved. For `delete` caused by ownership, also the external owner.
...
**Two known gaps in the pass.** Delete's criterion weighs a clause read alone, so a rule stated more than once on one surface is operative in each instance and each instance routes to the tier it already occupies — intra-surface duplication reaches the report as a cross-route finding, never as a route. And no step audits a pointer against its referent generally, so a disagreement already sitting on the surface — an enumeration that no longer matches the table it points at — is found while executing the pass rather than by it. Both gaps are known and unclosed; closing either would widen the method past placement, which is a separate decision. The route-local reference check above is not that widening and does not close this gap: it asks only whether applying this pass's own action would stop an existing pointer reaching what it names, which is a condition of the emitted action working rather than a review of what the surface already says.
