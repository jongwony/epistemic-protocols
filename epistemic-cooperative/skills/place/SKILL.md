...
The classifiers are five: ownership, the destinations, the example kinds, the enforcement axis, and the finding kinds. Naming them is what makes the requirement above checkable, since a requirement quantified over an open set has no state in which it is done, and completing the classifiers one happens to notice always leaves the ones one does not. A classifier added to this pass is added to this list, where its absence would be visible. Closing the list bounds this audit's own machinery and not the categories it routes: each of the five carries its own no-match path, so the members of each stay working hypotheses.
