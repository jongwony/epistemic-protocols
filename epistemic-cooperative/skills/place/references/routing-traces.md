...
These traces narrate the pass and summarize where each unit lands. Their route lines carry what the trace is showing rather than every element a block requires, so they are not emitted route blocks and reading one as a template would model an incomplete output. The inbound-reference result is the element this costs most visibly: these surfaces are named by kind rather than as a bounded working tree, so no reference-search boundary exists to search and the result is unresolved wherever an illustrated action would change what a unit is, where it sits, or whether it remains. A live audit names its boundary and its result instead. The one filled block in this file is the specimen in the report fragment below. The route-block contract itself is defined in `SKILL.md` §The route block, and where anything here disagrees with that section, the section governs. This file states no conformance guarantee of its own: a certification nothing checks reads as verification while tracking the contract only as well as its last hand-edit, which is the failure the audit routes and would be committing here.
...
1. *Extract.* The sentence splits. *"Node.js 22 or later is required"* names a threshold and is operative. *"the continuous integration pipeline pins Node 22"* names no action a reader takes; it reports which runtime the pipeline selects for its own jobs, and is residue.
...
4. *Enforcement.* The requirement binds behavior and is checkable, so the question is whether a guard already covers it. Read the pin at the pipeline rather than at the sentence asserting it — the sentence is the claim under audit and cannot be its own evidence — and what the pin does is select the runtime the pipeline's own jobs run on. It reads no contributor's runtime and rejects nothing; the package manifest declares no version constraint either, so nothing converts a wrong local runtime into a stop. A guard is warranted and none covers this: the axis names the guard to add — a check that reads the running version and fails below the threshold — and names the static-check channel the host binds, because a pass over sources is a place such a check can run. The residue binds no behavior — it reports which runtime the pipeline selects rather than requiring anything — so its axis says no norm is asserted, and that the pinned version is mechanically readable does not change that.
5. *Ledger.* The residue fails the positive record test. It states present configuration, not the rationale, provenance, trade-off, or rejected alternative a future reader needs to reconstruct why the surface carries the requirement; the pipeline file already states it, and a copy here drifts when that file changes. Having no runtime relevance is what sent the residue to this step and is not by itself admission to it.
6. *Delete.* Steps 2, 3, and 5 leave nothing — no action selection, no loading decision, no record value — so the residue routes here. Ownership settled local at step 2, so no external owner is named.
7. *Transformations.* None fire. Neither unit names a path to avoid, and neither carries a pre-assembled command.

**Routes:** two, sharing this sentence as `parent`. The requirement → `tier0`, disposition `stay`, the axis naming the guard to add with the static-check channel it would run in; the residue → `delete`, disposition `delete`, its axis `no norm asserted`.

**Why it is worth tracing:** the salience step is a rewrite *test*, not a presumption in either direction. Asserting that a positive restatement would lose the condition — without writing the restatement out and reading it — is how a prohibition gets kept for the wrong reason. Sentence B shows what reading the axis at its channel is guarding against once the channel is reached: a pin selects the environment a job runs in, while a guard reads the governed value and rejects a mismatch, so configuration that is present, current, and mechanically readable is still not a guard. The two sentences then separate the two missing-guard states the axis keeps apart — Sentence A's guard has no bound channel that could carry it, while Sentence B's has one that simply does not carry it yet — and only the second yields a channel to name.
