...
**This is an orchestration utility, not a runtime executor and not a new epistemic protocol.** Reduced-Space Test introduces no new interaction deficit. It realizes a known composite — decompose the target↔surrogate equivalence claim into verifiable facets, then project `/bound`'s source-grounded result into a synchronized test space and coverage complement ∘ `/inquire` for evidence inside it → scoped resolution + carried complement. It is sibling to `/forge`: a thin composition over existing protocols, kept outside the core protocol set because it owns no deficit of its own.
