...
|---|---|---|
| No usable answer — the advisor failed, timed out, returned nothing readable, or returned something readable that is neither a verdict nor a report of members it could not clear | **failed** | On a first pass, present the original gate labeled as unchecked, with what went wrong. On a recheck, the original was already found defective, so show both it and the rebuilt set with neither as the answer. The cycle ends in failure either way, and the originating decision still gets one |
...
2. **A change of answer kind routes back.** If the repair needs a different answer kind — a relocated axis that asks for one, a member fix that cannot be made without one — or belongs to a different phase of the work, return control to wherever the original gate was constructed. Do not present the replacement as though it were a sharpening of the original. Routing back is a landing, not a silent exit: the cycle ends in `indeterminate (routed back)` like any other, and the originating gate still fires, labeled with what could not be rebuilt here.
