...
1. **Bind the gate.** With an explicit draft argument, use it. Without one, take the most recently drafted gate still awaiting an answer — usually the current turn's, and the one just before it when a gate has already been put to you and its answer settled nothing. With neither an argument nor a gate to reach for, there is nothing to check: say so and stop, naming what would give the skill a target. An invocation with no gate is a landing like any other, not a silent no-op.
2. **Screen for eligibility.** Four exclusions send a gate out of the check, and two more conditions mean it should not have come: a decision a citable source already settles is relayed rather than adjudicated, and a set already down to one option has no option set to judge. Any of the six stops the cycle here: report which one applies and let the gate fire on its own terms.
...
| A cited ground is missing, says something other than what was attributed to it, or the answer cites nothing at all | **indeterminate (uncitable)** | Unfold everything; the user decides with both readings in view |
...
| The rebuild cannot be done here — it would need a different kind of answer, or would remove an option only the originating definition can remove | **indeterminate (routed back)** | Present the original gate carrying what the check established it cannot represent — the answer kind it has no option for, or the alternative only its definition can remove — and where that has to be changed instead. The gate fires because the decision still needs an answer; it fires disqualified rather than silently, because answering through a set already shown unable to carry the answer is the failure this landing detected |
...
- **The observer stays armed.** Once invoked, every later drafted gate enters the screen without re-invocation. Entering is not optional — the screen is where a gate leaves, by one of the four exclusions, and it leaves reported. "Candidate for checking" left the choice with the side whose gates are being checked, which is the discretion this skill exists to remove; a gate that goes unchecked because nobody looked is indistinguishable afterwards from one nobody needed to.
