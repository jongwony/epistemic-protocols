...
The repairs that survive verification are a different set from the ones the adjudicator sent, because dropping some of them changed it. Who wrote the originals settles nothing about the set that is left: an adjudicated set with three members removed was never adjudicated, and authorship does not transfer across a filter. So: if anything was dropped, the surviving set goes back for adjudication as the set it now is; only when nothing was dropped does the adjudicator's own verdict still cover what is left. That re-adjudication is bounded at one on its own account, and spending it leaves the rebuilt gate's own single recheck untouched — they judge different things at different moments, so one bound cannot stand for both. A survivor set that cannot be re-adjudicated — its own attempt is spent, the advisor is unreachable — still reaches you inside the same `unmapped` unfold, saying plainly that it was not adjudicated, with each drop named and what it failed on; an unadjudicated set you can see the gaps in is worth more than a repair you never got to choose. What is not available is choosing between those two by preference. Treating the advisor's authorship as covering the remainder would put the drafting side back in the seat this whole skill exists to empty, only with someone else's name on the chair.
