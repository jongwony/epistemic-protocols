...
| The rebuild cannot be done here — it would need a different kind of answer, or would remove an option only the originating definition can remove | **indeterminate (routed back)** | Present the original gate, labeled with what the rebuild could not do and where that has to be decided instead. The originating decision still needs an answer, so the gate fires |
...
The choice among viable repairs is not a newly drafted gate and does not route back through a fresh check of its own. Its options came from the adjudicator and were verified before they reached you, so the drafter is not judging a set it wrote. What the drafting side did here was drop the repairs that failed verification, and each drop is named with what it failed on — that is the one place to look if you suspect the choice was shaped on the way to you.
