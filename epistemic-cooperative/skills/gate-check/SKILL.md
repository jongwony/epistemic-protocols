...
1. **Bind the gate.** With an explicit draft argument, use it. Without one, take the most recently drafted gate whose decision is still open — usually the current turn's, and the one before it where a gate already fired and its answer settled nothing, which leaves that decision open whatever was said. With neither an argument nor a gate to reach for, there is nothing to check: say so and stop, naming what would give the skill a target. An invocation with no gate is a landing like any other, not a silent no-op.
2. **Screen for eligibility** against the screen above, in full. Anything it catches stops the cycle here: report which condition applied and let the gate fire on its own terms.
...
4. **Capture a substrate anchor** — as a value the later check can re-read and compare against, never as a description of one. A commit SHA is such a value; "the current working tree" is not, and where the tree carries uncommitted changes on the surface the check will read, what is dirty there is captured alongside the SHA. Everything downstream is evaluated against this one anchor, and the movement check later asks whether the substrate moved away from it — a question answerable only if what was captured can be held up against what is there now.
...
Where none of the three fits — the set stands on the right axis and still is not the set to ask, because a member is off-axis, missing, dead, or already travelling another's road — ask it to say so rather than round to the nearest verdict. What it owes then is the same in kind: which members it could not clear and the ground for each, the repairs available for them, and what each repair changes about the set you would end up answering. A verdict rounded to from a set that fit none of them is worse than no verdict, because it arrives labeled as checked.
...
|---|---|---|
| No usable answer — the advisor failed, timed out, returned nothing readable, or returned something readable that is neither a verdict nor a report of members it could not clear | **failed** | On a first pass, present the original gate labeled as unchecked, with what went wrong. On a recheck, show both the original and the rebuilt set with what the check found against each, and the gate fires over the pair — neither arrives as the checked one, and choosing between them with both findings visible is what is left to do. The cycle ends in failure either way, and the originating decision still gets an answer |
...
| A cited ground is missing, says something other than what was attributed to it, or the answer cites nothing at all | **indeterminate (uncitable)** | Unfold everything. On a first pass that means the set beside the advisor's uncited reading of it, and you decide with both in view; on a recheck it means the original and the rebuilt set with what the check found against each, since by then "both" names two sets rather than two readings of one |
| Grounds located, but something the answer owed never arrived — an axis reading, a falsifier, a collapse target, a relocation axis — or an entailment to any conclusion it does assert is contestable or was never offered, a material premise is interpretive, a source the precommit named went untouched, a criterion the precommit set goes unmet, a cited source says what was attributed to it but tracks nothing that would keep it true, a proposed repair arrives without what it changes, or the stated falsifier could not have defeated the reading | **indeterminate (audit)** | Unfold everything, naming what the answer did not reach |
...
| Grounds check out, falsifier absent, verdict `genuine` | **genuine** | On a first pass, present the original gate, now checked, with the axis reading. On a recheck it is the rebuilt set that was adjudicated, so it is the rebuilt set that is presented — showing the original here would resurface the very set the first pass found defective |
...
| The rebuild cannot be done here — it would need a different kind of answer, it would remove an option only the originating definition can remove, or the rebuilt set came back caught by the screen | **indeterminate (routed back)** | Present the original gate carrying what the check established — the answer kind it has no option for, the alternative only its definition can remove, or the screen condition the relocation landed on — and where that has to be changed instead. The gate fires because the decision still needs an answer; it fires disqualified rather than silently, because answering through a set already shown unable to carry the answer is the failure this landing detected |

**An unmapped answer settles its repair before anything is rebuilt.** Where a citable source determines which repair applies — a documented convention, a rule belonging to whatever defined the gate, a decision already made in this conversation — it is relayed with that citation and the rebuild runs on it; asking you to pick where the answer is already written is the over-gating this skill screens for elsewhere. Where more than one repair stays genuinely viable, they reach you as the choice, each with the set it would produce and what that changes downstream, and the rebuild waits. Either way the settled repair enters the same rebuild and the same single recheck a relocation does, because a set quietly changed and handed over unchecked is the one thing no landing here allows. This is a first-pass path only: an `unmapped` landing on the recheck ends the cycle the way every other non-clean recheck landing does, since the repetition is bounded at one whichever way the second answer falls.

The repairs that survive verification are a different set from the ones the adjudicator sent, because dropping some of them changed it. Who wrote the originals settles nothing about the set that is left: an adjudicated set with three members removed was never adjudicated, and authorship does not transfer across a filter. So: if anything was dropped, the surviving set goes back for adjudication as the set it now is; only when nothing was dropped does the adjudicator's own verdict still cover what is left. That re-adjudication is a dispatch like any other and carries what every dispatch carries: bounded at one on its own account — spending it leaves the rebuilt gate's own single recheck untouched, since they judge different things at different moments and one bound cannot stand for both — and preceded by its own precommit, launched first and recorded second. A dispatch exempted from the ordering because it is small is the ordering exempted. A survivor set that cannot be re-adjudicated — its own attempt is spent, the advisor is unreachable — still reaches you inside the same `unmapped` unfold, saying plainly that it was not adjudicated, with each drop named and what it failed on; an unadjudicated set you can see the gaps in is worth more than a repair you never got to choose. What is not available is choosing between those two by preference. Treating the advisor's authorship as covering the remainder would put the drafting side back in the seat this whole skill exists to empty, only with someone else's name on the chair.
...
**The ground-by-ground detail folds to one line.** Something like *"nine of ten cited grounds confirmed; one citation pointed at the wrong location though its claim holds elsewhere; the answer turned on both sources the precommit named and met the criteria it set; the falsifier could have landed and did not."* The precommit comparison rides that line either way, on both of its halves — stating that the answer did reach what was expected, on the terms set before it arrived, is what makes the converged case distinguishable from an unasked question. The full detail stays available and is produced on request. It folds because it is what you read to decide whether to *trust* the conclusion, not to reach it — a later and rarer need, and one that keeps its own place.
...
4b. **A disqualification travels inside the gate, not beside it.** What the check established about the set — the answer kind it has no option for, the members it could not clear, the adjudication that no longer covers it — is what you are deciding with, so it belongs where the options are and not in a note above them. Each option carries what choosing it means given the finding. A label set apart from the choice is read after the choice as often as before it, and a finding read afterwards did not reach the decision it was for.
...
11. **Every repetition is bounded at one, each on its own account, and the repeat is a full pass.** The bound attaches to each thing that can repeat rather than to a shared pool — the rebuild's recheck, the anchor's one re-anchoring, the repair menu's one re-adjudication — because they answer different questions at different moments, and a single budget shared across them would let whichever fired first silently spend the others. The replacement's answer re-enters ground verification and lands on exactly one disposition, as the first answer did — the rebuilt gate presented on a checked `genuine`, a relay on a checked `collapse`. Every other landing ends the cycle with a surfaced diagnosis and both attempts. A substrate that moved is re-anchored once on the same terms, and movement after that lands on `indeterminate (anchor)`. A third attempt at either would be confidence manufactured by repetition.
