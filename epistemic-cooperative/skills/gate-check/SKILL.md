...
The repairs that survive verification are a different set from the ones the adjudicator sent, because dropping some of them changed it. Who wrote the originals settles nothing about the set that is left: an adjudicated set with three members removed was never adjudicated, and authorship does not transfer across a filter. And on this path there is no earlier verdict to fall back on even when nothing was dropped: an answer reaching no verdict is what put the cycle here, so the repairs arrived authored but never adjudicated as a set. A repair menu you are asked to choose from goes for adjudication as the set it is, dropped members or not — the filter changes which set that is, it does not change whether one is owed. That re-adjudication is a dispatch like any other and carries what every dispatch carries: bounded at one on its own account — spending it leaves the rebuilt gate's own single recheck untouched, since they judge different things at different moments and one bound cannot stand for both — preceded by its own precommit, launched first and recorded second, and followed by the same ground verification, landing its answer on one of the same dispositions the first answer could. A dispatch exempted from the ordering because it is small is the ordering exempted, and an answer exempted from the verification because its question was narrow is the verification exempted. A survivor set that cannot be re-adjudicated — its own attempt is spent, the advisor is unreachable — still reaches you inside the same `unmapped` unfold, saying plainly that it was not adjudicated, with each drop named and what it failed on; an unadjudicated set you can see the gaps in is worth more than a repair you never got to choose. What is not available is choosing between those two by preference. Treating the advisor's authorship as covering the remainder would put the drafting side back in the seat this whole skill exists to empty, only with someone else's name on the chair.
...
11. **Every repetition is bounded at one, each on its own account, and the repeat is a full pass.** The bound attaches to each thing that can repeat rather than to a shared pool — the rebuild's recheck, the anchor's one re-anchoring, the repair menu's one re-adjudication — because they answer different questions at different moments, and a single budget shared across them would let whichever fired first silently spend the others. The replacement's answer re-enters ground verification and lands on exactly one disposition, as the first answer did — the rebuilt gate presented on a checked `genuine`, a relay on a checked `collapse`. Every other landing ends the cycle with a surfaced diagnosis and both attempts. A substrate that moved is re-anchored once on the same terms, and movement after that lands on `indeterminate (anchor)`. A third attempt at any of them would be confidence manufactured by repetition.
