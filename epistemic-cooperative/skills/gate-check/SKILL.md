...
advisor : { codex }                     -- optional; the independent adjudicator behind the
                                        ...
                                        --   → { verdict, axis reading, falsifier, and a ground
                                        --      cited for every conclusion asserted — including
                                        --      what a collapse reduces to, or what axis a
                                        --      malformed relocates onto } interface
```
...
| Grounds located, but an entailment to any conclusion the answer asserts is contestable or was never offered, a material premise is interpretive, a source the precommit named went untouched, a criterion the precommit set goes unmet, or the stated falsifier could not have defeated the reading | **indeterminate (audit)** | Unfold everything, naming what the answer did not reach |
...
A `malformed` verdict means the options stand on the wrong axis. The advisor supplied the axis to relocate onto and the ground for it, and Phase 2 checked that ground like any other; the replacement option set is built here, where the live context is. A relocation that did not survive that check never reaches this phase — it routed to an indeterminate audit, because a rebuilt set standing on an unverified axis is the original defect one move over. That makes this a synthesis step, not a relay — it selects and composes — so it carries its own guards:
