...
| Unattended-run directives in firstPrompt ("work through", "go through all", "handle the whole", "run until done") | firstPrompt keyword match: hand-off-and-leave directives | **Merismos** `/apportion` | Autonomous interval intended — the keyword match tests only autonomous_intent(G); it does not test whether a unit plan already exists, so a goal already condition-bearing (a prior /apportion plan, or explicit units+conditions the user supplied) can still surface here |
