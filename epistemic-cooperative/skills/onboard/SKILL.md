...
- **Incorrect** (reasoning inquiry → targeted correction):
  ...
  2. **Targeted correction**: Using the user's stated reasoning, explain the distinction through the design axis that separates the confused pair. Directly address the reasoning — e.g., "You mentioned timing — that's the right axis. The key difference is *direction*: `/inquire` collects what the AI can reach and hands back the user's unknowns *before* execution (User→AI), while `/contextualize` checks context fit *after* (AI→User)."
