...
| Diylisis `/distill` | Cross-cutting | Certifying an existing substrate-owned record's portability for a declared recipient role, repairing resolution gaps in place | Task description / delegation prompt / commit message headed to a fresh zero-memory session, with undefined jargon, deictic anchors, or dangling task ids |
