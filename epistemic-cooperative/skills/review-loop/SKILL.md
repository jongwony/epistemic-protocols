...
Capture the **resolved base SHA** (the merge-base or PR base commit the diff is taken against) and, for an explicitly numbered PR, the **PR head SHA** (`gh pr view {N} --json headRefOid`) — when the local checkout is not that head, fetch it first (`git fetch origin pull/{N}/head`) rather than letting a diff against an unrelated `HEAD` silently review the wrong patch — plus the changed-files list: this base SHA + head + file list is the **pointer** the codex prompt passes; codex re-derives the diff locally with its own git (read-only sandbox, no network), so the full diff content is not inlined. Also capture diff stats for context.
