...
A source-agnostic, convergence-paced review-resolve loop for code/PR diffs: it drives a pluggable review source until every finding has reached a landed repair or a handover, auto-applying mechanical fixes, gating the judgment calls, and unfolding a conflict between governing surfaces instead of settling it.
...
- Check the same finding against the artifact **as of the base captured at Phase 0**: does the asserted issue also hold there? This types the finding `introduced` — the change brought it — or `pre-existing` — the change only put it in view — or `indeterminate`, which is an answer rather than a failure to reach one: a finding about the interaction between changed and unchanged material holds at neither end alone, and pushing it to a side would manufacture a boundary the evidence does not draw. The base is already in hand from Phase 0, so this is relay exactly as the check above it is: a read against a named ref, with the ref cited. Where the artifact did not exist at that base, every finding on it is `introduced`.

Phase 3's scope-expansion axis and Rule 1 both branch on whether a defect is pre-existing, and the check above is what produces that value. Before it the distinction was read off whatever the round happened to know — which is to say off nothing the trace could show afterwards. The classification therefore rides on the finding's trace entry (Convergence section), so a round's boundary stays legible once the session that drew it is gone. What the classification does not do is decide anything by itself: `pre-existing` licenses no repair and forecloses none. It is the evidence the boundary question is asked with, and Phase 3 is where that question gets put. `indeterminate` resolves fail-closed there: it is not evidence that the change introduced the defect, so where the boundary decides something it goes with the pre-existing side and reaches the user, rather than being folded in on a classification that was never established.
...
**The goal as criterion (once a round, over the verified set).** Ask where the artifact works against what the project says it is for, naming the specific goal a finding runs counter to and how — load raised where the project says it is lowered, a deficit closed where the symptom shows rather than where it comes from, one shared term carrying different meanings in different places. This runs here rather than at Phase 0 because it needs a verified set to run over, and its output is findings: each goes back through Phase 2 verification like any other before it is classified, since a reading formed against a goal is as capable of being wrong about the code as a reading formed against a convention. Finding nothing is a result of asking, not a failure to ask.
...
| `code-review` | single, Claude-native (built-in) | Call via `Skill("code-review", ...)` passing the resolved diff pointer (base…head, or the captured working-tree base — never a raw PR number, which would re-review the possibly stale remote state) **and the design-intent bundle**; it runs its own multi-angle finder fan-out and returns a findings JSON array (`{ file, line, summary, failure_scenario }`, ranked most-severe-first, capped at 15) with **no verdict line** — the adapter derives the verdict (`[]` → approve, otherwise needs-attention) and maps each finding to `[severity] file:line — description`, assigning severity by assessing each finding's own summary and failure scenario against the mission-anchored severity calibration (the same steer every source receives) and using rank order only for ordering or tie-breaking. Its fixed output shape carries no direction, and the adapter leaves that slot empty rather than filling it: a direction the adapter composed would be the loop's own reading wearing the source's name, and Phase 3 already reads the verified set on the loop's behalf. That same fixed shape carries no channel for a reach report either, and the adapter never fills the slot with an inference about what the source probably examined. Because this is a standing property of the source rather than a per-round outcome, it is declared once at Phase 0 among that source's coverage trade-offs and surfaced once more at convergence; the adapter does not stamp a round-level "did not say" onto every round, which would add a trace line that never varies and train the reader to skip past the very slot that has to stay legible for sources which can report. The two absent slots differ in kind and are handled accordingly: an absent direction costs the loop an account it can form for itself, whereas reach is not the loop's to form at all. Rounds run on this source therefore carry no reach evidence, and Phase 5 weighs their quiet against that standing fact rather than against a claim the round made. Claude-native and reading the same repo, it can resolve `CLAUDE.md` and code comments itself; the bundle still directs it to the changed-surface `.claude/rules/*.md` it would not otherwise weight. It needs no external CLI, so it is available whenever the running harness provides the built-in; the source-designation ask offers only sources the current harness can invoke. |
...
Round {k} — source: {source} — verdict: {verdict}
  ...
  Relay:   {findings the loop dispositioned autonomously — Extension}
             each entry: finding [provenance] → [applied | dropped: basis | carried: reason]   (a side-effect rides inline as [side-effect: …] on an applied entry; an applied entry also carries the predicate its fix instantiates)
  Gated:   {findings that needed your judgment — Constitution, from a Judgment disposition or an orthogonal risk screen}
             each entry: finding [provenance] → [applied | dropped: basis | carried: reason]
```
...
Two other things reach past the loop for the ledger's own reason — their reach already exceeds the diff — while failing its admission criterion, that an entry be a decision already constituted. Both take the same offer. A **conflict left unsettled** travels as the conflict itself: each side with the surface it rests on, the point they disagree about, and the authority relation between them or the stated absence of one. Carrying a verdict nobody gave would be worse than carrying nothing, since the next reader cannot tell a recorded settlement from an assumed one. And the **far side of a boundary the user drew** travels as an entry point: each finding with where it holds and what establishes it, so whoever picks the work up starts from the finding instead of rediscovering it. That side is the run's deliverable rather than its remainder — a run can be complete and still hand most of what it found onward, and a record shaped as leftovers misreports the run to the person who has to act on it.
...
4. **FULL re-review each round** — re-call the source over the updated diff; declare convergence only after a full re-review pass over the updated diff. A finding handed to a successor is dispositioned and is not new on the rounds that re-surface it, so a run ending with every finding at a landed repair or a handover converges on the zero-new arm whatever the last verdict said; the last repair to land still owes its full pass first, because an edit moves the artifact past the verdict that preceded it.
5. **Verify before apply** — a finding that fails support-integrity is dropped with its cited basis; only support-integrity-passing findings proceed to apply. Verification also checks the finding against the artifact as of the base captured at Phase 0, typing it `introduced`, `pre-existing`, or `indeterminate`, and that type rides on its trace entry. This is relay, and it decides nothing on its own: the scope-expansion axis in Rule 1 and the trajectory reading in Rule 11 both branch on whether a defect is pre-existing, and this is the step that produces the value they branch on. Fit against the surrounding work-flow is no longer checked per finding here — it is checked once, at Phase 4, over the whole applied bundle, immediately before hand-forward.
...
10. **A constituted decision outlives the loop; its record is offered, never demanded** —
    ...
    home. Two other things reach past the loop for that same reason while failing the
    ledger's criterion of being already constituted, and both take the same offer: a
    conflict left unsettled travels as the conflict — each side with the surface it rests
    on, the point of disagreement, and the authority relation or its stated absence, never
    a verdict nobody gave — and the far side of a boundary the user drew travels as an
    entry point, each finding with where it holds and what establishes it. That far side
    is the run's deliverable rather than its remainder.
11. **The driving session reads its own trajectory by regime and by re-flagging, never by
    ...
    tier-escalation signal; a rise the pre-existing side carries is the review reaching past
...
12. **A shared-cause reading is fix-side, provisional, and analytical** — reading one
    ...
    fix's trace entry and read when a later round's fixes are briefed; its falsifier is an
    earlier round's own sweep, since a later instance at a site that sweep did not reach
    falsifies its enumeration and reopens the root reading at the wider shape. Every other
