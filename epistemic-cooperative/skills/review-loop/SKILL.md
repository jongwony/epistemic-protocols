...
**Recurrence escalation.** Settled policy absorbs the FIRST recurrence — consistent application, no re-gate. But when the same finding returns again on a later full re-review, after its disposition was applied and (for design decisions) conveyed in the design-intent bundle, the repetition is itself evidence: either the disposition is genuinely incomplete (the source sees something the fix did not resolve) or the disagreement is design-level. Do not silently re-drop it a second time — escalate to a Constitution design gate presenting the recurrence history, and let the user constitute the resolution. The constituted decision enters the design-decision ledger (Phase 0 harvest, source 4) and re-enters the bundle at the next re-harvest — recurrence ends through constitution, never through suppression.
...
2. Launch via `Bash(run_in_background: true, timeout: 300000)`. `--color never` + splitting the streams (stdout to the events file, `2>` to a separate warn file) keeps the events file pure JSONL — the codex banner and any stderr warnings ride their own warn file. `--cd {repo_root}` points codex's own git/Read at the repo so it re-derives the diff against the orchestrator-supplied base SHA; `--sandbox read-only` is kept because `git diff` is a local read needing no network. If codex's git cannot resolve that base SHA the extraction comes back empty — the step-3 "empty extraction = codex failed" guard surfaces it:
   ```bash
   codex exec --ephemeral --json --color never --skip-git-repo-check --cd {repo_root} -m gpt-5.6-sol --config model_reasoning_effort="xhigh" --sandbox read-only < /tmp/review_loop_codex_${SUFFIX}.txt > /tmp/review_loop_codex_events_${SUFFIX}.jsonl 2>/tmp/review_loop_codex_warn_${SUFFIX}.txt
   ```
...
9. **Recurrence escalates; suppression never converges** — a finding that returns after
   its disposition was applied and conveyed escalates on its second recurrence to a
   Constitution design gate with the recurrence history presented; the constituted
   decision enters the design-decision ledger and re-enters the source's bundle at the
   next re-harvest. Convergence is earned through constitution — a manufactured verdict
   (a source instructed to approve when findings match a ledger) is not convergence.
