...
9. **Recurrence escalates; suppression never converges** — when a finding returns beyond
   ...
   source saw the disposition's outcome, is absorbed once by settled policy, while a
   return made with the constituted decision already conveyed awaits diagnosis
   immediately), diagnose the repetition: if the prior disposition was an apply and Phase
