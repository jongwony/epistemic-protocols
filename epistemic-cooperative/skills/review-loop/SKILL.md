...
**How a source is actually driven, once designated, is in `references/source-adapters.md`** — the prompt an adapter composes, the call it makes, how it collects the result and maps it onto this interface. None of that bears on which source to designate, so it loads at the point it binds: after Phase 0 has settled the source, and for that source alone. What stays here is what the designation gate weighs and what every adapter answers to.
