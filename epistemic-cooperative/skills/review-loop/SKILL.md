...
/review-loop [source?] [scope?]
  ...
  Phase 1  : review    — source(diff, intent) → { findings[], verdict, exercised, direction? }
                          ...
                          (exercised: what the source ran vs only read this call — required, so silence stays readable)
...
The loop is the skill's identity; the review source is a parameter behind it. Loop control (verify → classify → apply → re-review) is fixed; the source that produces `{ findings[], verdict, exercised }` is swappable.
...
| `code-review` | single, Claude-native (built-in) | Call via `Skill("code-review", ...)` passing the resolved diff pointer (base…head, or the captured working-tree base — never a raw PR number, which would re-review the possibly stale remote state) **and the design-intent bundle**; it runs its own multi-angle finder fan-out and returns a findings JSON array (`{ file, line, summary, failure_scenario }`, ranked most-severe-first, capped at 15) with **no verdict line** — the adapter derives the verdict (`[]` → approve, otherwise needs-attention) and maps each finding to `[severity] file:line — description`, assigning severity by assessing each finding's own summary and failure scenario against the mission-anchored severity calibration (the same steer every source receives) and using rank order only for ordering or tie-breaking. Its fixed output shape carries no direction, and the adapter leaves that slot empty rather than filling it: a direction the adapter composed would be the loop's own reading wearing the source's name, and Phase 3 already reads the verified set on the loop's behalf. That same fixed shape reports nothing about what the source ran, so the adapter fills the exercised slot with exactly that — the source did not say — and never with an inference about what it probably did. The two slots are handled differently on purpose: an absent direction costs the loop an account it can form itself, whereas an absent exercised report is the unreadable silence, so leaving this one blank would reintroduce the very ambiguity the slot was added to remove. A round run on this source therefore carries no execution-grounded silence, and Phase 5 weighs its quiet accordingly. Claude-native and reading the same repo, it can resolve `CLAUDE.md` and code comments itself; the bundle still directs it to the changed-surface `.claude/rules/*.md` it would not otherwise weight. It needs no external CLI, so it is available whenever the running harness provides the built-in; the source-designation ask offers only sources the current harness can invoke. |
...
Round {k} — source: {source} — verdict: {verdict}
  Exercised: {what the source ran against the artifact; what it reported it could not run}
...
| Source returns no exercised report | Treat the round's silence as unexercised across the board rather than as clearance: the axes it did not account for surface as annotated residual, and the gap in the source's own reporting is surfaced with them |
...
12. **A shared-cause reading is fix-side, provisional, and analytical** — reading one
    ...
    unwound, since every finding was verified on its own before any reading grouped them. **Analytical**: it is weighed on the verified set before dispositions are
...
14. **A source reports what it exercised, and unexercised silence is residual, not
    clearance** — every source returns a report naming what it put to the test against the
    artifact this call and what it could only read; the slot admits an empty answer but
    never an absent one, since *nothing is wrong here* and *nothing here could be run* are
    precisely the two readings it exists to separate. A source's reach inside fixed
    constraints varies between calls, so an axis going unexercised is a fact about that
    call and not a property of the source — which is why the report is asked per call and
    never inferred from what the constraints allow in principle. A convergence resting on a
    round's quiet reaches only as far as that round exercised: an unrun axis surfaces as
    annotated residual at convergence like any carried deferral, blocking neither
    convergence nor triggering a further re-review, because the loop cannot grant a reach
    the source lacks. What it can refuse is to record the gap as a clearance.
