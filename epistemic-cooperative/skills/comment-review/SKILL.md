...
2. Classify each line:
   **A line linked to a finding (`[task: <id>]`) is classified by what that finding's SOURCE PROTOCOL requires of a judgment, never by tag shape alone** — the tag records what was typed, and the protocol says what counts as an answer. Tag shape is a detail inside that classification, not the axis of it.
   ...
   - **Linked to a CLOSED-VARIANT protocol's finding** (`/gap`, `/contextualize`): the `[disposition: <variant>]` tag IS the judgment, and a free comment with no tag is Address-equivalent. Close the task per the finding-lifecycle rule, and where the variant implies an edit (e.g. `/gap`'s `Address`) translate its semantics into a concrete Edit/Write call.
