...
Take any markdown or HTML artifact from drafted state to fixation-ready through four reviews — factual verifiability, source vetting, decision quality, application fit — bound to a **channel-first feedback loop with TaskList-backed sidepanel finding visibility**. The render substrate is keyed off the file extension: markdown (`.md`) renders through marked; HTML (`.html`/`.htm`) serves the raw file directly through a Shadow DOM. On invocation the skill opens a browser preview of the rendered artifact (Vorverständnis layer) and surfaces the loop's branch gate; the user reads the rendered preview first, then chooses how to advance each round. Every markdown-mode loop iteration presents a 2-option branch gate (`apply + scan` / `apply`) before the user returns to the browser for the next round. Both options apply this round's queued JSONL comments as edits NOW; `apply + scan` additionally runs the sub-protocol audit, materializing each finding (with its disposition affordance) into the TaskList for the NEXT round's sidepanel. `apply` skips the scan; both modes translate the unambiguous comments and defer the ambiguous ones alike. The judgment venue for sub-protocol findings moves from in-round chat to next-round sidepanel — Constitution semantics preserved, timing shifted across the round boundary. (HTML artifacts are the exception: HTML mode suppresses the sidepanel and the sub-protocol scan finding-flow, so an HTML round is effectively apply-only and its gate opens no round-mode axis — review is in-place anchored commenting; see Channel Modality.) "Wait and add more comments" is implicit in not yet answering the gate. Termination is a free-response pathway available at any time.
...
**Trajectory differential** — both options translate this round's queued JSONL into edits NOW, under the same source-protocol classification (so the hermeneutic cycle stays seamless), but differ in whether the scan step also runs (in **HTML mode** the scan step has no venue — the sidepanel and sub-protocol finding-flow are suppressed — so both round modes reduce to apply-only for HTML, and the round-mode axis therefore does not open there at all: the two would be one option under two names, and neither leaves the queue in a different state, both deferring the untranslatable lines identically. An HTML round runs as `apply` with that basis stated rather than chosen, and what it defers waits on the user's own next comment rather than on a scan round that will not come; see Channel Modality § HTML Anchoring):
- ① `apply + scan` — APPLY: queued JSONL consumed and archived EXCEPT the untranslatable lines, which stay queued exactly as under `apply` — step 2's classification is the same in both modes and the scan is not a second chance to translate a comment; clear comments translated into edits, except a response to an open-verdict protocol's finding, which is the user's judgment and is recorded rather than executed; sidepanel disposition signals (`[disposition: Withdraw] [task: …]` etc.) translated to TaskUpdate(completed) — subject to the finding-lifecycle rule's two conditions on closing, which leave an incomplete judgment or an owed `Revisit`/`HandOff` task open — plus, where the variant implies an edit, the corresponding edit; browser auto-reloads after edits. SCAN: `/inquire`+`/sublate`+`/gap`+`/contextualize` audit runs over the post-apply artifact; each finding is materialized as a TaskList entry whose description carries the source protocol's own judgment shape (e.g., /sublate's free-text verdict plus at most one instruction — Withdraw, Revisit(condition) or HandOff(deficit); /gap's Address/Dismiss/Probe). No audit-finding edits this round; the user judges per finding via the next round's sidepanel.
...
Within each `apply + scan` round's scan step, `/inquire` runs first as Step 1 against the artifact's factual surface — named people, project names, citations, statistics, technical claims about external systems, links. `/inquire` Phase 1 dispatches to the right verification track (CodeDerivable, CanonicalExternal via WebFetch, Instrumentation, UserTacit) per claim. `/inquire`'s own Phase 2 Constitution gates do not fire as in-round chat prompts here; instead, each surfaced uncertainty is materialized as a TaskList entry carrying `/inquire`'s OWN Phase 2 answer shape — `Provide(context)`, `Point(location)`, `Dismiss`, or `Unknown(Partial)` — as the disposition affordance for the next round's sidepanel. The verification track is not what that gate asks: the track is `/inquire`'s own classification of the uncertainty, and `Unknown(Partial)` is precisely the answer it auto-promotes to the next untried track WITHOUT asking. An affordance offering the track would put the user's name on a choice the protocol makes for itself and would never present the one it actually owes them.
...
- **Constitution venue shift**: `/contextualize`'s per-mismatch answer does not fire as an in-round chat gate; it is rendered as the disposition affordance attached to each mismatch finding's TaskList entry, judged by the user in the next round's sidepanel. That answer is TWO-AXIS — a judgment of the mismatch together with a disposition drawn from `Adapt(direction)`, `Keep`, or `Discard(replacement)` — and both axes are carried, since materializing one axis would answer the other on the user's behalf. This is the same two-axis answer named below where the tag-shape branches decline to invent a taxonomy over it
...
- **Close without responding** → ask in chat ("mark task &lt;id&gt; done"); the AI calls `TaskUpdate(status=completed)` directly. For an open-verdict protocol's finding this closes the task WITHOUT a judgment having been given, so record it as such and name it in the round report — that protocol's own convergence condition still quantifies over every audit it admitted, and a finding closed unanswered is one it never received. WHICH TERMINAL THAT LEAVES IT AT IS ITS OWN CONDITION TO ANSWER and is not concluded here — this channel delivers the fact that no judgment arrived, and the cross-round Constitution rule's terminal clause is where that fact is evaluated against the protocol's own condition
...
- **Mid-chain invalidation**: an earlier scan step's finding invalidating a later scan step's finding → replay forward from the affected step (not backward compensation). Examples: Step 1 factual finding invalidating a Step 2 source-vetting finding → replay forward from Step 2; a Step 2 judgment withdrawing a source as authority for the claim a Step 3 gap finding rested on → replay forward from Step 3, which happens at the NEXT round's materialization since `/sublate`'s judgments arrive there. `/sublate` revises no source of its own — its source list is read-only to it and the scan step lands no edits — so a revision is not an event this branch can wait for. Replay produces revised TaskList entries before round-boundary materialization.
...
- **Anchor-not-found on apply**: When a prior edit removes or substantially rewrites the anchor span of a queued comment or a referenced TaskList finding, the apply step cannot locate the target. Behavior: (a) attempt fuzzy-match only when both `context_before` and `context_after` match within edit distance 5 — if matched, proceed as located; (b) otherwise leave the line in the queue tagged `status="anchor-missing"` (and, for disposition signals against a TaskList task, leave the task open with the anchor-missing state written into its own `description`, which is the only durable per-finding surface this channel owns — a sub-protocol's mode state is that protocol's and ends with its run, so nothing here can write to it) so the user can judge whether the intent still applies — do not silently drop.
...
10. **Cross-round Constitution preservation** — sub-protocol Phase 2 Constitution gate option sets are not elided; they are materialized as TaskList disposition affordances and judged by the user in the next round's sidepanel. Per-finding constitutive authority moves across the round boundary without loss; what compresses is in-round chat traffic (down to one decision per round — the round-mode gate itself), not Constitution semantics. The composition trades 1 round of latency between detection and edit landing for clean modality (chat = round-mode decision; sidepanel = per-finding disposition) and bounded in-round chat surface area. RELOCATING A GATE RELOCATES ITS TERMINAL WITH IT, AND WHICH TERMINAL A PROTOCOL REACHED IS THAT PROTOCOL'S OWN CONDITION TO ANSWER — never a condition restated here. What this channel supplies is the INPUT that condition reads: per finding, whether its judgment reached the source protocol and what the judgment was. At review exit, evaluate each invoked protocol's own terminal condition against what it received, report which terminal it granted, and name whatever that condition leaves open as that protocol's unresolved residual. A task this channel holds OPEN because the judgment carried an instruction it does not perform is a judgment DELIVERED — Rule 9 keeps it open so the owed instruction is not closed over, which is a different question from whether the protocol was answered, and that protocol's condition is what says what an unperformed instruction comes to there. Restating a terminal condition here is what this clause exists to prevent: a copy drifts from what it copies, and a review reporting a convergence the protocol's own condition does not grant is exactly the failure relocating the gate created. The review's own exit asserts neither — a review that ended is not a protocol that converged, and leaving the two unseparated would let a relocated gate read as a gate that was passed.
