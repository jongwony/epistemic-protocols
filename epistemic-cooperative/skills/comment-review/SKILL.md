...
Q : Round {k+1} — Which round mode? (Answer after browser comment authoring + sidepanel disposition is complete)
  ...
  In HTML mode this gate carries NO round-mode options. The scan step has no venue there, so ① and ② would translate the same queue the same way — and ①, which consumes the queue whole, would archive the ambiguous comments the absent scan was what handled. The round runs as ② and the gate says so; what it asks is only the round-completion signal.
...
**Trajectory differential** — both options translate this round's queued JSONL into edits NOW, under the same source-protocol classification (so the hermeneutic cycle stays seamless), but differ in whether the scan step also runs (in **HTML mode** the scan step has no venue — the sidepanel and sub-protocol finding-flow are suppressed — so both round modes reduce to apply-only for HTML, and the round-mode axis therefore does not open there at all: the two would be one option under two names, and the one that consumes the queue whole would archive the ambiguous comments the absent scan was what handled. An HTML round runs as `apply` with that basis stated rather than chosen, and what it defers waits on the user's own next comment rather than on a scan round that will not come; see Channel Modality § HTML Anchoring):
