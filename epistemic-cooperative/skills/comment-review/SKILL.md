...
- **Pre-gate material travels with the finding**: a `/sublate` finding's body carries that audit's narrowing whole — the bound claim, what makes it suspect, the evidence with its source, the stake — together with the antithesis and its cited basis. The response popup pre-loads that body above its textarea, so the material sits before the response affordance rather than sharing it, which is what this channel's realization of the gate owes.
