...
- **Submit response** → comment posted to `/feedback` with `[task: <id>]` auto-appended; the next apply step calls `TaskUpdate(status=completed)` and translates the comment into edits — EXCEPT where the linked finding came from an open-verdict protocol (`/sublate`), whose response IS the user's judgment and is recorded rather than executed (apply step, Address-via-comment)
...
2. Classify each line:
   - **Disposition signal** (comment matches `[disposition: <variant>] [task: <task-id>]`): call `TaskUpdate(status=completed)` against the named task and, when the variant implies an edit (e.g., `/gap`'s `Address`, or `/sublate`'s `Withdraw` where the artifact leans on the withdrawn source), translate the variant's semantics into a concrete Edit/Write call. `Dismiss`, `Probe`, and a `/sublate` verdict carrying no instruction close the task without an edit. `Revisit` and `HandOff` are different: each instructs work this apply step does not perform — `Revisit` names a condition only the source protocol's own loop watches, and `HandOff` names a deficit someone has to emit. Record each against the task and surface it in the round report as OWED rather than closing it, so an instruction this channel cannot carry out is named instead of discarded — the same discipline `/sublate` applies to a Revisit its own run ends without meeting.
   - **Address-via-comment** (free comment optionally carrying `[task: <task-id>]`): translate the comment intent into an Edit/Write call; if the link tag is present, additionally call `TaskUpdate(status=completed)` against the named task. **Exception — a finding from an open-verdict protocol**: where the linked task came from a protocol whose Phase 2 judgment is the user's own verdict rather than a variant (`/sublate`), the free comment IS that verdict — record it against the task and close it, and do not translate it into an edit. `/sublate` does not rewrite the sources it audits, so reading its verdict as an edit instruction would have this pipeline perform what that protocol is bound not to do. An edit follows only where the same comment also carries an instruction tag implying one.
