...
10. **Cross-round Constitution preservation** — sub-protocol Phase 2 Constitution gate option sets are not elided; they are materialized as TaskList disposition affordances and judged by the user in the next round's sidepanel. Per-finding constitutive authority moves across the round boundary without loss; what compresses is in-round chat traffic (down to one decision per round — the round-mode gate itself), not Constitution semantics. The composition trades 1 round of latency between detection and edit landing for clean modality (chat = round-mode decision; sidepanel = per-finding disposition) and bounded in-round chat surface area. RELOCATING A GATE RELOCATES ITS TERMINAL WITH IT, AND THE ARMS PARTITION ON WHETHER A JUDGMENT WAS RECEIVED — NEVER ON WHETHER A TASK IS OPEN: at review exit a sub-protocol reaches its own convergence where every audit it admitted received a judgment, and reaches its own declared non-converging exit where any admitted audit received none, those audits named as that protocol's unresolved residual. A task this channel holds OPEN because the judgment carried an instruction it does not perform is a judgment RECEIVED — Rule 9 keeps that task open so the owed instruction is not closed over, which is a different question from whether the source protocol was answered, and each protocol's own terminal condition says what an unperformed instruction comes to there. The review's own exit asserts neither — a review that ended is not a protocol that converged, and leaving the two unseparated would let a relocated gate read as a gate that was passed.
