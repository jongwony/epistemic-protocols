...
Q : Round {k+1} — Which round mode? (Answer after browser comment authoring + sidepanel disposition is complete)
  ① apply + scan — APPLY: translate this round's queued JSONL comments into edits NOW (queued comments include both fresh drag-comments and any sidepanel disposition signals carrying [task: …] tags, which also call TaskUpdate(completed) unless the finding-lifecycle rule's owed-instruction exception holds).
...
**Trajectory differential** — both options translate this round's queued JSONL into edits NOW (so the hermeneutic cycle stays seamless), but differ in whether the scan step also runs (in **HTML mode** the scan step has no venue — the sidepanel and sub-protocol finding-flow are suppressed — so both round modes reduce to apply-only for HTML; see Channel Modality § HTML Anchoring):
- ① `apply + scan` — APPLY: queued JSONL fully consumed + archived; clear comments translated into edits, except a response to an open-verdict protocol's finding, which is the user's judgment and is recorded rather than executed; sidepanel disposition signals (`[disposition: Withdraw] [task: …]` etc.) translated to TaskUpdate(completed) — subject to the finding-lifecycle rule's owed-instruction exception, which leaves a `Revisit` or `HandOff` task open — plus, where the variant implies an edit, the corresponding edit; browser auto-reloads after edits. SCAN: `/inquire`+`/sublate`+`/gap`+`/contextualize` audit runs over the post-apply artifact; each finding is materialized as a TaskList entry whose description carries the source protocol's own judgment shape (e.g., /sublate's free-text verdict plus at most one instruction — Withdraw, Revisit(condition) or HandOff(deficit); /gap's Address/Dismiss/Probe). No audit-finding edits this round; the user judges per finding via the next round's sidepanel.
...
- **Submit response** → comment posted to `/feedback` with `[task: <id>]` auto-appended; the next apply step calls `TaskUpdate(status=completed)` (subject to the finding-lifecycle rule's owed-instruction exception) and translates the comment into edits — EXCEPT where the linked finding came from an open-verdict protocol (`/sublate`), whose response IS the user's judgment and is recorded rather than executed (apply step, Address-via-comment)
- **Cancel popup** → no submission; finding stays open
- **Close without responding** → ask in chat ("mark task &lt;id&gt; done"); the AI calls `TaskUpdate(status=completed)` directly. For an open-verdict protocol's finding this closes the task WITHOUT a judgment having been given, so record it as such and name it in the round report — that protocol's own convergence answers for every audit it admitted, and a finding closed unanswered is one it never received
...
2. Classify each line:
   - **Disposition signal** (comment matches `[disposition: <variant>] [task: <task-id>]`): call `TaskUpdate(status=completed)` against the named task unless the finding-lifecycle rule's owed-instruction exception holds, and, **for an open-verdict protocol's finding, only where the comment also carries the verdict** — the tag names an instruction and never stands in for the judgment, which that protocol requires as free text; a tag-only response is recorded as an instruction given without a judgment and named in the round report, since a disposition that closed on the tag alone would report a judgment the user never made. Then, when the variant implies an edit (e.g., `/gap`'s `Address`, or `/sublate`'s `Withdraw` where the artifact leans on the withdrawn source), translate the variant's semantics into a concrete Edit/Write call. `Dismiss`, `Probe`, and a `/sublate` verdict carrying no instruction close the task without an edit. `Revisit` and `HandOff` fall under the finding-lifecycle rule's owed-instruction exception — recorded, task left open, surfaced in the round report as owed.
   - **Address-via-comment** (free comment optionally carrying `[task: <task-id>]`): translate the comment intent into an Edit/Write call; if the link tag is present, additionally call `TaskUpdate(status=completed)` against the named task, subject to the finding-lifecycle rule's owed-instruction exception. **Exception — a finding from an open-verdict protocol**: where the linked task came from a protocol whose Phase 2 judgment is the user's own verdict rather than a variant (`/sublate`), the free comment IS that verdict — record it against the task and close it, and do not translate it into an edit. `/sublate` does not rewrite the sources it audits, so reading its verdict as an edit instruction would have this pipeline perform what that protocol is bound not to do. An edit follows only where the same comment also carries an instruction tag implying one.
