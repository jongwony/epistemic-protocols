...
Prothesis(U) → Q1(MB(U)) → J_mb → [confirm: MBᵥ | modify(field): MB' → re-present Q1(MB') → J_mb (loop until confirm)] → G(MBᵥ) → C → {P₁...Pₙ}(C, MBᵥ) → Sc({P₁...Pₙ}, Pᵦ) → Pₛ → LensEstablished →
  ...
  converge(trace) → handoff(FramedInquiry) → seam → END(morphism)
  -- END(morphism) = frame's own action ends: it arranges, isolates, executes and synthesizes NOTHING. This bounds what frame does; it does not halt the turn — two different boundaries, and only the first is frame's to declare
  -- seam = the next move proceeds under the source that SETTLED it, in precedence: an arrangement the invoking context has already settled — a user-declared chain naming the next protocol, or a caller carrying its own arrangement for these lenses — settles it first; failing that, the ConductRef this file declares, borne by every SubstrateCorrespondence handoff, settles it and the seam proceeds to /conduct. ConductRef fills an arrangement still UNDERDETERMINED — /conduct's own activation precondition — and never overrides one already fixed. Absent both — the LensReturn branch, which carries no ConductRef — the handoff is the last move
  -- ConductRef = the /conduct nudge that carries isolation + arrangement + reconciliation + synthesis; frame forms lenses and hands them off, never realizing isolation or asserting convergence
...
ConductRef = the /conduct nudge that accompanies every SubstrateCorrespondence handoff (the frame→/conduct advisory edge): it routes the isolation + arrangement (order, independence, reconciliation, termination, routing) + synthesis apparatus to /conduct, which designs it, and the isolated substrate, which runs it — frame names the reference, never realizing isolation, arranging, or synthesizing
...
Phase 3:  LensEstablished → bind_substrate(Pₛ) → discriminate(substrate_specialization_need) → [single lens ∨ no specialized substrate: LensReturn | ≥2 lenses with at least one specialized substrate need: SubstrateCorrespondence(lens↔substrate pairs) ⊕ nudge(ConductRef)] → converge(transformation trace) → handoff(FramedInquiry) → seam → END(morphism)  -- bind each lens's substrate, discriminate by substrate specialization need, emit the framed object as the terminal relay with the /conduct nudge, then end frame's own action (no gate of frame's own, no dispatch by frame, NO isolation, NO synthesis; isolation + arrangement + synthesis is /conduct's to design and the isolated substrate's to run downstream). The seam then carries the settled next move wherever the handoff names one — an arrangement the invoking context already settled where there is one, else the ConductRef on the SubstrateCorrespondence branch — under that protocol's own gates
...
During Phase 3 (Bind Substrate & Handoff):
  ...
  discriminate(substrate_specialization_need):
    single lens (|Pₛ| = 1) ∨ no specialized substrate beyond general-purpose → package the detailed lens(es): emit FramedInquiry = inj₁(LensReturn), hand off; frame's own action ends there, and this branch carries no ConductRef for the seam to proceed to. NO synthesis, NO convergence claim, NO isolated handoff.
    ≥2 lenses with at least one specialized substrate need → pair each lens with ⟨substrate_need + binding_hints + per-perspective directive + channel-need⟩ → emit FramedInquiry = inj₂(SubstrateCorrespondence) with the /conduct nudge (ConductRef), hand off the lens↔substrate pairs; frame's own action ends there and the seam proceeds to /conduct on that ConductRef — unless the invoking context has already settled the arrangement, in which case the seam proceeds under that instead. frame does NOT realize isolation, arrange, execute, or synthesize.
  -- handoff is the completeness boundary: frame records the handoff and its own action ends there — a boundary on frame's morphism, not a halt of the turn. Where the handoff names a settling source the seam carries the next move directly, under that protocol's own gates: an arrangement the invoking context already settled where there is one, and otherwise the ConductRef every SubstrateCorrespondence handoff bears. frame does NOT realize isolation — the isolated execution arrangement (isolation, reconciliation/dialogue, synthesis) is /conduct's to design (the /conduct nudge accompanies every SubstrateCorrespondence handoff) and the substrate's to run (an agent team, a dynamic-workflow, isolated subagents, or plan-mode). Convergence is claimable only by the isolated substrate that ran the lenses, which frame neither performs nor asserts.
...
Phase 3 handoff (extension)      → TextPresent+Proceed (emit the FramedInquiry — detailed lens(es) or lens↔substrate pairs ⊕ /conduct nudge — as the terminal relay; frame's own morphism ends here and it does NOT realize isolation, arrange, execute, or synthesize — it does NOT spawn or call delegate. Ending the morphism is not halting the turn: what proceeds is the seam below, and where the handoff names no settling source — the LensReturn branch, which carries no ConductRef — the seam has nothing to proceed to and the handoff is the last move. The isolation + arrangement + synthesis apparatus is /conduct's to design (via the nudge) and the substrate's to run; when the substrate is the main session (the default) it consumes the framed object, when isolated executors are elected that dispatch is the substrate's action, not frame's morphism)
Λ (track)                        → Internal state update (the problem-to-solve and the framing shifts are what LEAVES this skill: they ride in the FramedInquiry the Phase 3 handoff emits, and per-phase bookkeeping stays in session and does not leave at all. No separate durable entry is written here — it would be a second copy of what the handoff already carries, and a copy can drift from the thing it copies while still reading as authoritative)
seam (extension)                 → TextPresent+Proceed (fires at deactivation/handoff: proceed directly to the next move, citing the source that SETTLED it; every Constitution gate inside this skill and inside the next protocol fires unchanged. Precedence: an arrangement the invoking context has ALREADY settled — a user-declared chain naming the next protocol, or a caller that carries its own arrangement for these lenses and says so — settles it first and the seam proceeds under that; failing that, the composition edge this SKILL.md declares — the ConductRef nudge accompanying a SubstrateCorrespondence handoff — settles it and the seam proceeds to /conduct. ConductRef fills an arrangement that is still UNDERDETERMINED — which is /conduct's own activation precondition — and never overrides one already fixed: a caller whose order, independence, reconciliation, termination and routing are all settled leaves /conduct nothing to determine, so routing there would ask a protocol to re-open a closed question rather than answer an open one, and its own warrant would relay back)
