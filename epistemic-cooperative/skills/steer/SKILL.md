...
`AI proposed verdict: <Confirm | Dismiss | Reorient(implication') | Stop>` — one-paragraph rationale citing cluster evidence, counter-evidence considered, and why the proposed verdict dominates alternatives.
...
8. **Detection with Authority** — AI detects calibration drift candidates with cited move evidence and proposes its own verdict (Phase 3) and disposition (Phase 5) with rationale; the user constitutes per-cluster verdicts and the final approval. AI proposal serves Recognition over Recall — verdict options without an AI-proposed rationale collapse to blank-canvas recall. The verdict and approval acts remain the user's exclusive territory.
