...
Whether a rendering earns its place, and what form it takes. These are judgment axes, not a catalog — a form that passes them may be composed on the spot, and a form that fails them is not rescued by appearing in a list. One shape is pinned, where getting it wrong would make the rendering lie.

**The reader's task governs; the form follows.** Ask what the reader has to do with what is on screen before choosing a shape. Pulling out one discrete value and taking in a set of relations at once are different tasks — the first is a table's, the second a picture's. This choice sits underneath every later one, so getting it backwards is not repairable further down.

**A form the host already renders is not rebuilt here.** Use the harness's tables, lists, and headings. A hand-built counterpart drifts from the maintained one and inherits whatever its builder measured wrong — and character count is not display width, so East Asian text, emoji, and many box-drawing characters break columns that looked aligned when written. Never pad by counting characters.

**A length is answerable to a quantity that exists without it.** One discriminant, two sides — both the same rule, neither an exception to the other. Where the number is measured (bytes, counts, durations — anything the work produced whether or not it was drawn) a length may carry it, and carries it best where the reader must order, compare, or find an extreme. Where the number was invented by the act of rendering — a loop's completion, a share whose denominator the renderer chose — nothing takes a length, because the length would assert a precision the quantity never had. Reading one specific value back out is where a length loses to a table or a sentence.

**A drawn length declares its scale.** Pinned. A bar with no stated scale reads as a measurement without being one. Put the scale beside the drawing (`1 char ~ 0.8 GB`), keep the left edge common so lengths start from the same place, and name the unit rather than implying resolution that whole-cell glyphs cannot hold.

**Contrast that points and contrast that encodes are different instruments.** Pointing marks one thing against its background, and works in proportion to how uniform that background is — not according to any fixed number of marks. Encoding assigns contrast to a value across every row and carries a legend, with no such ceiling. Import no numeric limit into either; none is established. What is established is that channels differ in robustness — colour survives a crowded field, size does not, shape degrades as competing shapes multiply. The observer markers are shape, so a crowded round is what weakens them.

**A second rendering earns its place only by carrying what the first cannot.** Add one where it supports a comparison the first does not, or rules out a misreading the first invites — and only where the reader can see the correspondence without working it out. Otherwise it is redundancy: a second scan returning nothing. Two renderings both needed but sitting apart cost differently — the reader holds one while reading the other — and the cost of relating them grows with how far apart their forms are. Check first what the gate, the convergence lines, and the observer markers already carry; a rendering that restates one of them is the redundancy case.

**A drawing's capacity is what must be held at once, not its item count.** No item ceiling is established for a display the reader can keep looking at — working-memory limits are measured on material that is gone or cannot be re-read, which a rendering on screen is not. Forms also fail differently: a branching picture runs out of width, a row list runs out of vertical scan. One number would misjudge both.

**On screen and read later take different forms.** Aligned and tabular structure is fastest while the reader can still see it, and the weaker form once they are working from memory, where prose carries better. Gates and convergence lines are read while present; the durable record is read later, and its wording is what has to survive.
