...
**Drift tracking**: Vocabulary rendering is restated in each protocol's `## Rules` section as "Plain emit discipline", and round-local salience bundling is restated as its own rule (compiled copies across the runtime contract surface). When either rule body here changes, audit per-protocol restatements to match. Per Plugin Encapsulation, each SKILL.md must be self-contained; the per-protocol inscription is structural, not a reference.
...
**Gate firing precondition** — before rendering a gate, run the reversibility tier first. If the decision is reversible and its direction is clear — or, still reversible, its direction already settled by the project's stated goals, an established convention, a prior declared decision, or a Settled Direction the substrate records — do **not** gate: execute the action and summarize. Settled direction sharpens *clear* on the direction axis only; it never lowers the reversibility tier, so an irreversible decision stays gated however settled its direction. Reserve the gate for a decision that escapes the tier — irreversible, or reversible-but-genuinely-ambiguous with divergent downstream paths. A reversible choice whose direction is clear, settled, or non-divergent, rendered as a gate, is over-gating: the gate surfaces live judgment, it does not re-confirm an action the reversibility tier has already resolved. This precondition governs *decision* gates and realizes — it does not override — each protocol's SKILL.md classification: a verification or floor gate (one that measures the user rather than routing a decision) fires by its SKILL.md classification regardless of reversibility, which has precedence. This precondition is upstream of the rendering spec below — the divider block is *how* a gate looks; this decides *whether* there is one.
...
**Channel boundary — text gate vs genuine tool call.** Rendering a gate as the divider block means you write the divider text and yield the turn — you do *not* additionally call AskUserQuestion for that gate. It does **not** mean transcribing a tool call into your message. A genuine tool invocation — AskUserQuestion, or any tool a skill explicitly calls — always goes through the real tool-use channel; never write its invocation syntax (`<invoke …>`, a bare `call` line, parameter markup, or any function-call XML) as literal text in your output. The "no tool call wrapper" note on the Gate element scopes to the Ink gate's own rendering; it is not license to hand-type a tool call as content. When a skill's SKILL.md explicitly directs a tool (e.g., `forge` or `image-companion` calling AskUserQuestion), invoke it through the tool channel as written — the Ink text gate and a skill-directed tool call are distinct realizations, and neither is ever emitted as raw markup.
