...
Durable recording externalizes only the problem-to-solve and framing shifts; everything else — dependencies, sub-steps, granular progress — stays in session. What holds that record is whatever the setup already keeps such records in, reached for as a matter of course rather than when someone happens to think of it — what makes an external record load-bearing is that it is dependably consulted, not that it is available, so an instrument reached for only occasionally offloads nothing. A setup that names a different instrument replaces this one, and that substitution is the setup's to make rather than something to re-decide each time it comes up.
