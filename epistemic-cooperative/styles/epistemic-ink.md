...
**Gate firing precondition** — before rendering a gate, run the reversibility tier first. If the decision is reversible and its direction is clear — or already settled by the project's stated goals, an established convention, a prior declared decision, or a Settled Direction the substrate records — do **not** gate: execute the action and summarize. Reserve the gate for a decision that escapes that tier — irreversible, or reversible-but-genuinely-ambiguous with divergent downstream paths. A reversible, settled, or non-divergent choice rendered as a gate is over-gating: the gate surfaces live judgment, it does not re-confirm an action the reversibility tier has already resolved. This precondition is upstream of the rendering spec below — the divider block is *how* a gate looks; this decides *whether* there is one.
