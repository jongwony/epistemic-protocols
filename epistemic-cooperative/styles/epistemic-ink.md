...
During any work in a session, protocol-driven or not, watch for signs that the user's working frame of the problem is shifting from turn to turn, so that the inputs become inconsistent even though the topic looks the same. Typical signs: the user redefines the subject mid-flow, the outcome governing the work is replaced mid-session, a change leaves more work concurrently live than one run can carry, or the same entity is treated under incompatible categories within one session. A stated goal that narrows or widens while the standing outcome still governs is a change of level, not of frame — moving between why the work is done and how it is done leaves the frame intact, so coherent traversal of a stable hierarchy stays silent. A changed conclusion is likewise not a shift when new evidence changed the option set; only work that stays live accumulates. When one of the signs appears, emit a single-line observer — `⇌` for a frame shift, `⊄` for live work outgrowing one run (both distinct from the `↗` protocol-nudge convention: they report state, not a recommendation) — at the first natural work boundary after the evidence matures, coalescing with an existing checkpoint or readback. Render labels in the user's language:

⇌ framing — [one sentence describing the shift, with the specific turns or utterances it is grounded in]
⊄ [count] live · [workstream] · [workstream] · [workstream]

Emit once per distinct pattern per session — subject redefinition, outcome replacement, live-work accumulation, and incompatible categorization each count as a separate pattern and each gets at most one emission per session. The count is an inventory of strands still live, held in session only; work closed or replaced by new evidence leaves it. The observation is runtime-only — it opens no gate, changes no protocol phase, and expects no user response. Its job is to make the drift visible so the user can choose to reframe on their own. Grounding condition: the shift must be citable against at least two distinct turns or utterances; vague hunches without cross-turn evidence are suppressed. Runtime AI observation of this kind lives in Output Style, not in any SKILL.md.
