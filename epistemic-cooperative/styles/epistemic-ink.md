...
**A length is answerable to a quantity that exists without it.** Where the number is measured — bytes, counts, durations, anything the work produced whether or not it was drawn — a length may carry it. Where the number was invented by the act of rendering, such as a loop's completion or a share whose denominator the renderer chose, nothing takes a length, because the length would assert a precision the quantity never had.
