...
| 인식론적 형식 리뷰 (category theory / type theory / operational semantics + gap) | `/formal-review` (프로젝트 스킬) | 형식 3-렌즈 **고정** 1-pass 리뷰; 발견 사항을 통합 코멘트 1건으로 게시 |
| 범용 다관점 리뷰 (렌즈를 diff에서 도출) | `/lens-review` (플러그인) | frame이 렌즈를 도출하는 1-pass 리뷰; `/formal-review`가 특수화하는 범용 베이스 |
...
- **인식론적 리뷰**(이 프로젝트 고유 로직)는 `claude-epistemic-review.yml`에서 스킬로 이관되었고, 이후 **범용 베이스와 프로젝트 특수화로 분리**되었다. 고정 형식 3-렌즈 리뷰는 프로젝트 스킬 `/formal-review`(`.claude/skills/formal-review/SKILL.md`)가 담당하고, 렌즈를 diff에서 도출하는 범용 베이스는 플러그인 `/lens-review`(`epistemic-cooperative/skills/lens-review/SKILL.md`)다. 고정 패널은 이 저장소 고유의 형식 리뷰 축이라 프로젝트 레이어에 두고, 범용 능력은 플러그인에 둔다.

## `/formal-review` 파이프라인

`claude-epistemic-review.yml`이 하던 형식 리뷰를 스킬로 옮긴 것으로(고정 형식 3-렌즈), 단계는 다음과 같다. 범용 `/lens-review`도 동일 파이프라인을 쓰되 Phase 2의 렌즈를 frame이 diff에서 도출한다는 점만 다르다:
...
/formal-review [scope?]
  ...
  Phase 2  : 고정 렌즈 리뷰 (격리 → 적대) — /frame을 형식 3-렌즈에 고정(Category Theory ∥ Type Theory ∥ OpSem) + /gap; substrate는 스킬이 직접 기술(/conduct 우회 아님)
...
| 렌즈 패널 | `/formal-review`: 형식 3-렌즈 고정 (Category Theory / Type Theory / Operational Semantics + Gap) / `/lens-review`: frame이 diff에서 도출 | 고정 패널은 이 저장소 형식 리뷰 축이라 프로젝트 스킬에 핀닝; 범용 베이스는 런타임 frame 도출 |
