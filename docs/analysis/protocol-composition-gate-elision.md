...
> 관련 원칙: Interaction Kind Factorization (`.claude/rules/axioms.md`), Semantic Autonomy (`.claude/rules/axioms.md`), ELIDABLE CHECKPOINTS (각 SKILL.md).
...
| 경계 소유권 | 1 | Horismos Phase 2 Qc (classify) | UserSupplies/AIAutonomous/AIPropose 분류 |
...
elidable(G, Composition) ≡
  ...
  ∧ (system_state(G) ∨ answer(G) ⊆ output(prior)     -- AI에 인식론적 접근 있음
     ∨ BoundaryMap(domain(G)) = AIAutonomous)           -- 사용자가 권한 위임
```
...
- `BoundaryMap(domain(G)) = AIAutonomous`: 사용자가 해당 도메인의 의사결정 권한을 AI에 위임
...
| BoundaryMap 값 | 게이트 처분 | 하류 Phase 1 행동 | 근거 |
|----------------|-------------|-------------------|------|
| **User-supplies** | always_gated | 표준 context collection | 사용자가 값을 직접 제공하는 도메인 |
| **AI-proposes** | always_gated | ENRICH-AND-PRESENT: 확장된 context collection + 후보 생성 | AI가 초안 제시, 사용자가 조향하는 도메인 |
| **AI-autonomous** | ELIDE 후보 | RESOLVE-OR-PRESENT: 확장 + 해소 시도 | 사용자가 AI에 위임 — 3축 나머지 충족 시 엘리전 |
| **Dismissed** | prune | N/A | 범위에서 완전 제거 — 게이트 자체가 불필요 |

**다중 도메인 규칙**: 게이트가 복수 도메인에 걸칠 때 max-authority-wins — 하나라도 User-supplies이면 전체 게이트가 always_gated.
...
대상: Qs 게이트, unbounded-regret Qc, UserSupplies Qc, AIPropose Qc.

사용자에게 게이트를 제시하고 응답을 대기한다. 합성 맥락에서도 이 게이트들은 건너뛸 수 없다 — 인식론적 기능이 합성으로 대체되지 않기 때문이다.

### 5.2 elide — 자동 통과 + 로그

대상: Qc + bounded regret + (AIAutonomous 또는 선행 출력에서 응답 도출 가능).
...
Gate G in Composition
  ...
  │
  ├─ domain(G) ∈ UserSupplies ∪ AIPropose?  → present
  │
...
preserves: user authority on UserSupplies ∪ AIPropose ∪ {Qs, unbounded-regret Qc}
```
...
**Semantic Autonomy — Outcome Equivalence corollary 보존**: 같은 프로토콜 정의가 합성 맥락과 단독 실행에서 동일한 인식론적 결과를 산출한다. 엘리전은 상호작용 경로를 변경하지만, 수렴 조건과 산출물 구조는 동일하다 (Semantic Autonomy의 Outcome Equivalence corollary — `.claude/rules/axioms.md` 참조).
...
나머지 42%는 **구조적으로 필수** — Qs 게이트(구성적 콘텐츠), unbounded-regret Qc(비가역성), UserSupplies 경계(사용자 권위). 이 게이트들은 합성이 아무리 정교해도 엘리전할 수 없다.
...
프로토콜 합성에서의 게이트 엘리전은 **자율성의 확장이 아니라 실현 레이어(R(p))의 맥락 적응**이다. Semantic Autonomy의 Outcome Equivalence corollary가 보존되는 한, 동일한 프로토콜 정의가 단독 실행(모든 게이트 제시)과 합성 실행(일부 게이트 엘리전)에서 동일한 인식론적 결과를 산출한다. 최대 엘리전 비율 ~58%는 설정 상호작용의 제거를 의미하며, 나머지 42%의 실질적 상호작용은 구조적으로 보존된다. 핵심 불변량은 "No silent authority transfer" — 사용자가 UserSupplies으로 선언한 도메인이나, 구성적 콘텐츠(Qs), 비가역적 결정(unbounded-regret Qc)에서는 어떤 합성 맥락에서도 사용자 상호작용이 생략되지 않는다.
