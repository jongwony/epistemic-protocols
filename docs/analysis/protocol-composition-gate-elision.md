...
> 관련 원칙: Interaction Kind Factorization (design-philosophy.md), Structural Idempotency (design-philosophy.md), ELIDABLE CHECKPOINTS (각 SKILL.md).
...
**Structural Idempotency 보존**: 같은 프로토콜 정의가 합성 맥락과 단독 실행에서 동일한 인식론적 결과를 산출한다. 엘리전은 상호작용 경로를 변경하지만, 수렴 조건과 산출물 구조는 동일하다.
