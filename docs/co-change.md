...
| Architectural principle addition | `.claude/rules/architectural-principles.md` (new section adjacent to existing architectural principles) + cascading reflection in cited rule files (e.g., `project-profile-calibration.md`, `project-profile.md`) + SKILL.md inscription **only when gate annotations are directly affected** (e.g., new TOOL GROUNDING entry, new Rule, new Phase prose section — substrate boundary touch is one such case). Profile-layer reclassification alone (rule-file changes without gate annotation changes) does NOT trigger SKILL.md updates — those follow per-protocol future stages pending accumulated use evidence. Plugin.json version bumps only for plugins whose SKILL.md actually changes. If axiom layer purification is involved (subsumed paragraph removal), perform removal in the same commit to preserve axiom homogeneity |
| Gate axis purification (single TOOL GROUNDING axis) | ALL SKILL.md TOOL GROUNDING (split `(extension)` entries for conditional specialization) + removal of any obsolete dual-axis annotation section + `.claude/rules/axioms.md` A2 §Configurable boundary + A5 §Interaction Kind Factorization + `.claude/rules/derived-principles.md` (§Loop Continuity / §Full Taxonomy Confirmation) + `.claude/rules/project-profile-calibration.md` + `.claude/rules/project-profile.md` deferral note + verify scripts (`single-axis-soundness` check; removal of any structural check enforcing the obsolete annotation section) + utility skill references (comment-review / steer) + READMEs / agents / styles citing the old regret-axis vocabulary + plugin.json version bumps for affected plugins |
