...
| Gate axis purification (single TOOL GROUNDING axis) | ALL SKILL.md TOOL GROUNDING (split `(extension)` entries for conditional specialization) + removal of any obsolete dual-axis annotation section + `.claude/rules/axioms.md` A2 §Configurable boundary + A5 §Interaction Kind Factorization + `.claude/rules/derived-principles.md` (§Loop Continuity / §Full Taxonomy Confirmation) + `.claude/rules/project-profile-calibration.md` + `.claude/rules/project-profile.md` Stage 1 conjecture + verify scripts (`single-axis-soundness` check; removal of any structural check enforcing the obsolete annotation section) + utility skill references (compose / review-ensemble / artifact-review / steer) + READMEs / agents / styles citing the old regret-axis vocabulary + plugin.json version bumps for affected plugins |
