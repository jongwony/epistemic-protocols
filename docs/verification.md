...
27. **form-feedback-body-identity**: Verifies the `**Form feedback**` rule body is byte-identical across every core protocol SKILL.md and both Ink-derived Output Styles, comparing each against the canonical `epistemic-cooperative/styles/epistemic-ink.md` after stripping the per-file numbered-rule ordinal. Item 16 asserts the label is present; a copy whose body had been emptied or rewritten still satisfied that, so the content this rule turns on could leave a packaged runtime contract without any check noticing. What it validates is **one well-formed rule item read to its real Markdown boundary**, not the first fragment whose prefix looks right — the weaker reading leaks wherever a source can be prefix-shaped, and every leak ends in the same state: a sentence reversing the rule sits in a packaged contract while the check calls all copies identical. Three requirements carry that. **Exactly one occurrence**: every line mentioning `**Form feedback**:` is collected, and zero or more than one fails, naming each line number — a second item can contradict the first while the compared one stays identical. **The shape its own source class publishes**, not the union of both: `N. **Form feedback**: ` in a protocol SKILL.md, `**Form feedback**: ` in an Output Style, unindented with a single space after the ordinal. Detection is loose and validation strict on purpose, so a wrongly-shaped label fails as malformed rather than vanishing into "not found". **Markdown's own boundary**: the label line plus every line the item continues across, up to a blank line, the next `N. ` item, a real ATX heading (`#` through `######` followed by a space or end of line — a `#`-prefixed non-heading is body, not a boundary), or end of file. When the canonical file itself is missing or carries no usable item there is no anchor, so the check reports that and compares nothing rather than promoting an arbitrary stand-in. Scoped to Form feedback alone — the other three rules item 16 requires are deliberately adapted per protocol and are not compared. Fail level; the fail names the diverging file and quotes both excerpts, and no pass line is emitted alongside any fail from this check. Repair: re-sync the named file's rule item to the canonical body, keeping that file's own ordinal
