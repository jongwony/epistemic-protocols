...
27. **form-feedback-body-identity**: Verifies the `**Form feedback**` rule body is byte-identical across every core protocol SKILL.md and both Ink-derived Output Styles, comparing each against the canonical `epistemic-cooperative/styles/epistemic-ink.md` after stripping the per-file numbered-rule ordinal. Item 16 asserts the label is present; a copy whose body had been emptied or rewritten still satisfied that, so the content this rule turns on could leave a packaged runtime contract without any check noticing. The unit compared is the **whole list item** — the label line plus every line Markdown continues it with, up to a blank line, the next numbered item, or a heading — because a sentence appended under the label is rule body, and a label-line comparison would report the copies identical while one of them said the opposite. The label must appear in its published shape (`N. **Form feedback**: ` in a SKILL.md, `**Form feedback**: ` in an Output Style), unindented and with a single space after the ordinal; a variant that is no longer that list item fails as unfindable rather than being normalized back into a match. When the canonical file itself is missing or carries no such item there is no anchor, so the check reports that and compares nothing rather than promoting an arbitrary stand-in. Scoped to Form feedback alone — the other three rules item 16 requires are deliberately adapted per protocol and are not compared. Fail level; the fail names the diverging file and quotes both excerpts, and no pass line is emitted alongside any fail from this check. Repair: re-sync the named file's rule item to the canonical body, keeping that file's own ordinal
