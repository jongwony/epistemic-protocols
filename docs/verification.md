...
26. **gate-integrity-rule**: Verifies every core protocol SKILL.md carries a `(Safeguard tier)`-tagged Gate integrity rule inscribing the mutation-taxonomy kernel — option injection / option deletion / option substitution, distinguished from type-preserving materialization (see `.claude/rules/axioms.md` §Gate Integrity). Fail level. Repair: add the missing tagged rule to the protocol's `## Rules` section
