...
- **GROUND.** The session primitive every Lean block reads, identical across blocks and to the canonical module `lean/EpistemicProtocols/Ground.lean`: a `Turn` carries its origin, form, and content; a `Context` is the list of turns; `fuse` appends one utterance, a turn a person sent; a `Cite` points at one turn with the basis that turn is eligible for; a `Coord` names which bases it admits and leaves support to the model; `Occ` is an open coordinate, optionally with a candidate citation, or a filled one with its citation, admission, and support. A person's statement grounds as an utterance and a person's observation as testimony, so a coordinate that admits only an utterance is filled by a statement; an AI turn grounds nothing.
