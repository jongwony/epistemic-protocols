...
- `*` names the deficits composed in the current session; a pre-declared protocol chain instead specifies a sequence of invocations

New phases must appear in PHASE TRANSITIONS with `[Tool]` suffix AND in TOOL GROUNDING with the capability or internal operation that realizes it (Lean notation: §Lean Notation below). Which parts of this anatomy a static check actually enforces is read from the check itself rather than asserted here — an enforcement-coverage claim on this surface has nothing that re-runs it, so it keeps asserting an earlier reading with present authority.

### Lean Notation

A protocol's Definition block is authored either in the DSL above (a bare fence) or in core Lean 4 (a `lean` fence). The Lean form elaborates: a reference that does not resolve, a proposition stated without proof, or a type that does not check is an error rather than a reading a reviewer has to catch. Its anatomy keeps the DSL's section names and changes what each section is made of.

- **Section markers.** Each section opens a module doc comment carrying the same marker, `/-! ── NAME ──`, closed on that line (` -/`) when the section is code only. Lean requires a declaration before its use, so the order is FLOW, MORPHISM, GROUND, TYPES, any `*-BINDING`, MODE STATE, PHASE TRANSITIONS, LOOP, CONVERGENCE, TOOL GROUNDING, COMPOSITION. A short module doc before FLOW tells the runtime reader how to read the block.
- **Doc-text sections.** FLOW, MORPHISM, `*-BINDING`, LOOP, and COMPOSITION are doc text, and CONVERGENCE opens with its trace obligation as doc text before its stated theorems. MORPHISM keeps its source object, its `→` chain ending in the resolution, and its `requires:` / `deficit:` / `preserves:` / `invariant:` lines at line start — the protocol router and the morphism check read that line shape. Doc text avoids fence lines and the comment tokens `/-` and `-/`, which would close or nest the comment.
- **GROUND.** The session primitive every Lean block reads, identical across blocks and to the canonical module `lean/EpistemicProtocols/Ground.lean`: a `Turn` carries its origin and content; a `Context` is the list of turns; `fuse` appends one utterance, a turn a person sent; a `Grounding` is an origin that may ground — never the assistant, injected text, or an unknown origin; a `Cite` points at one turn with that origin; a `Coord` names which origins it admits and leaves support to the model; `Occ` is an open coordinate, optionally with a candidate citation, or a filled one with its citation, admission, and support. Admission reads only who sent a turn, because that is the one fact the harness supplies. What the turn does — a statement, a request, an instruction, a report of what was observed — is text the model reads, so it lives in `supports`: a coordinate that admits only the person is filled by any turn of theirs whose content settles it, and a coordinate that must not rest on bare assent says so in its `supports`.
- **Code sections.** TYPES, MODE STATE, PHASE TRANSITIONS, and CONVERGENCE are declarations. MODE STATE is the fused context; a field that is a projection of the context is read from it and not declared beside it. A transition is a constructor of a relation or an arm of a structural recursion over the user's utterances, and a CONVERGENCE claim the contract makes is a theorem where it can be proved.
- **Theorems are stated, not proved, in the block.** A proof is verification rather than contract, so the block carries only a theorem's statement: a module doc comment holding a line of prose followed by `theorem name binders : statement`, with no `:=` and no body, placed in the section it is about. The proofs live in the repository's Lake package (`lakefile.toml` at the root; contributor-side, never packaged), in `lean/EpistemicProtocols/<Namespace>/Proofs.lean` — a module (`module` header) importing only its contract and GROUND, so that under Lean's module system its proofs stay private to it while its theorems are public. The contract module it imports, `Contract.<Namespace>`, is generated from the SKILL.md block by `.claude/skills/verify/scripts/lean-contract.js` into the gitignored `lean/.contract/`, with the block's GROUND section replaced by the canonical `EpistemicProtocols.Ground`; the SKILL.md block stays the one source and nothing generated is edited by hand. The proofs module restates each signature because Lean requires one; `lean-definition` checks by elaboration that every statement in the block follows from the proved theorem of its name alone, so a proof carrying a premise the statement lacks fails. A protocol may instead state its theorems in `lean/EpistemicProtocols/<Namespace>/Theorems.lean` — the same doc-comment form, in a module that imports `Contract.<Namespace>` and nothing else, states inside `namespace <Namespace>` alone, and declares nothing — so the runtime block carries only what the model reads; where that module exists the block states no theorem of its own (GROUND's, copied verbatim, stay), and the stated set is read from the module.
- **Judgments.** The model reading the block is the one party the contract leaves open: it settles, by inference over the context, each element the block does not fix. Such a judgment is declared as an `axiom` with a doc comment saying what is judged — a black box to the contract, so that `#print axioms` names the judgments each theorem rests on — or, in a block that has not taken that form, as an `opaque`. The block does not narrow a judgment with finer types than the contract needs, and it adds no `sorry` and no other `axiom`. An axiom judgment's type is shown inhabited by a `Nonempty` instance in the proofs module, so no judgment can assume what nothing inhabits. Only core Lean is used (no Std, no Mathlib). An addition made only because elaboration requires it — an `Inhabited` witness for an `opaque`, a `deriving` clause — must add no meaning and is marked with an `-- elab:` comment.
- **TOOL GROUNDING.** The realization header is a line comment (`-- Realization: …`), `Annot` enumerates the annotation vocabulary below, and `grounding : Op → Annot × String` carries one arm per entry, `| .op => (.annotation, "Tool: description")`. An operation new to PHASE TRANSITIONS is a constructor of `Op` with its arm.

Elaboration runs in two places, with the toolchain pinned by the repository's `lean-toolchain`. The block is elaborated standalone, as a runtime reader loads it. The package is built with `node .claude/skills/verify/scripts/lean-contract.js generate .` followed by `lake build`, and `lean-definition` then elaborates a generated audit per block that re-derives each stated theorem from its proof and reads the elaborated environment: the theorems the proofs module declares must equal the stated set, no project module may declare an axiom other than a block's documented judgments, each judgment's type must have a `Nonempty` instance, and every proof's transitive axioms must lie within `propext`, `Classical.choice`, `Quot.sound`, and the block's own judgments. Token predicates on the block, the canonical GROUND, and each proofs module reject `sorry`, `admit`, `set_option`, `native_decide`, `implemented_by`, `extern`, `unsafe`, and metaprogramming commands; an unaccounted `.lean` file anywhere under `lean/` fails. The static check runs the whole sequence locally when `lean` and `lake` are reachable and in CI (`.github/workflows/lean.yml`).
...
**Qc/Qs runtime distinction**: The Qc/Qs classification is a definition-time property of the gate — Qs expects constitutive response, Qc expects classificatory response. At runtime with Text+Stop realization, this distinction blurs: Qc gate responses can carry constitutive surplus (new meaning beyond the classification). The formal answer type (closed coproduct) captures the classification; the protocol's response interpretation accounts for any additional meaning. Pure interpretation is a `(sense)` operation, while an integration that writes mode state is `(track)`. The protocol's own TOOL GROUNDING fixes that boundary. A non-obvious interpretive contribution may be surfaced through the Output Style `Basis:` marker under `premise/recognition-and-authority.md` §Detection with Authority.
