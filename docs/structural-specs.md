...
- **Form feedback** controls density: each round takes its density from what the current request asked for rather than carrying it over, and a reported symptom ("hard to follow", "too long") is read as a form instruction rather than answered with a question about which form is wanted.
