...
- **Theorems are stated, not proved, in the block.** A proof is verification rather than contract, so the block carries only a theorem's statement: a module doc comment holding a line of prose followed by `theorem name binders : statement`, with no `:=` and no body, placed in the section it is about. The proof lives in the repository at `lean/<plugin>/<skill>.lean`, a contributor file never packaged, whose text is the SKILL.md block verbatim followed by the namespace reopened and every stated theorem proved. The stated signature and the proved one are the same text on purpose; `lean-definition` re-runs that match, so neither side is redundant enough to delete.
...
Elaboration is run by invoking `lean` on the proof file, whose prefix is the block (or, for a block that states no theorem, on the extracted fence); the toolchain is pinned by the repository's `lean-toolchain`, and the `lean-definition` static check does the same, in CI as well as locally. After editing a block, rewrite the proof file's prefix from it before re-running.
