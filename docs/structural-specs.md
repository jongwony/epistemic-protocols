...
**Secondary pattern** (inter-protocol): Five complementary pairs form Pre/Post cycles across distinct epistemic axes — Hermeneia↔Katalepsis (intent), Telos↔Syneidesis (goal), Aitesis↔Epharmoge (context), Prothesis↔Analogia (structure), Prothesis↔Prosoche (action — theoria/praxis). These cycles operate heuristically via Output Style nudge, driven by observed session conditions rather than graph.json structural edges. **Multi-axis pairing**: Prothesis appears in two pairs (structure with Analogia, action with Prosoche) because each pair operates on an independent epistemic axis; protocol membership in multiple Secondary pairs is permitted when the axes are independent. **Asymmetric temporal mode**: most pairs are Pre/Post on observation axes (pre-execution analysis ↔ post-execution verification); the action pair is Pre/During (Prothesis pre-execution lens placement ↔ Prosoche during-execution attention). Backward flow in the action pair: Prosoche's convergence trace (per-task `ExecutionBlind → situated` transformations, plus Wirkungsgeschichte emission of framework-relevant patterns — recurring risk signals, TeamStructure ineffectiveness, halted-task clusters) becomes Vorverständnis for the next Prothesis activation, intaken via Phase 4 LOOP `J=extend` "review execution results". Stage 1 conjecture: action-pair coverage extends Secondary classification to one previously-unpaired protocol (Prosoche); Stage 2 corroboration accumulates when Prothesis re-activations cite Prosoche convergence trace as perspective-formation evidence. Horismos and Periagoge remain Secondary-unpaired by present inscription — separate analysis required; Anamnesis is Tertiary substrate and is exempt from Secondary pairing.
