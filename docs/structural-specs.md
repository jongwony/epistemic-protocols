...
- **Judgments.** An `opaque` declaration is a judgment the runtime model makes, with its doc comment saying what is judged. The block does not narrow that judgment with finer types than the contract needs, and it adds no `sorry` and no project `axiom`. Only core Lean is used (no Std, no Mathlib). An addition made only because elaboration requires it — an `Inhabited` witness for an `opaque`, a `deriving` clause — must add no meaning and is marked with an `-- elab:` comment.
