...
| `diylisis` | `Skill.md` defines `/distill`, closure phases F0–F7, refute-posture zero-memory F5 gate (fresh-context review, session-term watchlist, evidence-cited verdict), channel emit | Short routing cue for session-tethered handoff distillation | Main agent owns F0–F7; F5 realized via packaged `agents/zero-memory-refuter.md` (Claude Code loads plugin agents). Codex boundary: Codex plugins bundle skills/apps/MCP only — no agent bundling — and named custom-agent spawn from tool-backed sessions is limited (openai/codex#15250), so the Codex realization is a generic fresh subagent carrying the F5 instructions from `Skill.md` (self-contained for this purpose); lint checklist where no subagent exists | Refuter finding quality, watchlist completeness, rendering-accident detection |
