...
| `diylisis` | `Skill.md` defines `/distill`, closure phases F0–F7, refute-posture zero-memory F5 gate (fresh-context review, session-term watchlist, evidence-cited verdict), channel emit | Short routing cue for session-tethered handoff distillation | Main agent owns F0–F7; F5 realized via packaged `agents/zero-memory-refuter.md`. Codex note: the plugin cache envelopes `agents/` on install (observed in `~/.codex/plugins/cache/`), but whether Codex registers and spawns cached plugin agents by name is pending a behavior test (named custom-agent spawn from tool-backed sessions has a known limit, openai/codex#15250); the `Skill.md` lint-checklist fallback covers until verified | Refuter finding quality, watchlist completeness, rendering-accident detection |
