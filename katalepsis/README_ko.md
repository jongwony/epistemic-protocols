# Katalepsis — /grasp (κατάληψις)

결과물이나 산출물에 대한 확실한 이해 달성 (κατάληψις: 확고하게 파악함)

> [English](./README.md)

## Katalepsis란?

스토아 철학의 κατάληψις (확고한 파악, 확실한 이해)를 현대적으로 재해석한 프로토콜 — 누가 만들었든 **결과물을 사용자가 따라가며 검증된 이해에 도달**할 수 있게 합니다.

### 핵심 문제

대상이 복잡할 때 — 큰 변경, 남이 쓴 코드, 밀도 높은 논문 — 사용자는 종종 그것을 온전히 파악하지 못합니다 (`ResultUngrasped`). 이해했다고 생각하거나, 어디서부터 시작해야 할지 몰라 압도당할 수 있습니다.

### 해결책

**설명보다 이해 확인**: 일방적으로 설명하는 대신, AI가 사용자의 말에 가까운 의도 기반 진입점을 먼저 제시합니다. 사용자는 이해, 판단, 설명, 수정 중 지금 필요한 경로를 고르고, AI는 그 경로를 실제 산출물에 근거시킨 뒤 점진적인 질문으로 이해를 확인합니다.

### 단순 설명과의 차이

| 차원 | 단순 설명 | Katalepsis |
|------|----------|------------|
| 방향 | AI가 말하고, 사용자가 듣는다 | AI가 검증하고, 사용자가 확인한다 |
| 진입점 | AI가 무엇을 설명할지 결정 | 사용자가 의도 기반 경로 선택 |
| 확인 | 설명 후 가정됨 | 질문을 통해 검증됨 |
| 진행 상황 | 정의되지 않음 | 태스크로 추적됨 |

## 프로토콜 흐름

```
Phase 0: 방향 잡기   → 결과와 사용자 신호에서 이해 의도 추론
Phase 1: 진입점     → 의도 기반 경로 제시, 사용자 선택 (gate interaction)
Phase 2: 근거화     → 산출물 근거를 연결하고 태스크 생성 (record)
Phase 3: 검증 루프  → 점진적으로 이해 확인 (gate interaction + record update)
```

## 사용 시점

**사용**:
- 상당한 코드 변경이 들어온 뒤 — 누가 한 것이든
- 사용자가 "뭘 했어?", "설명해줘", "이해하고 싶어", "이게 이해가 안 돼", "못 따라가겠어"라고 할 때
- 복잡한 리팩토링, 새 기능, 아키텍처 변경

**건너뛰기**:
- 사소한 변경 (오타, 포매팅)
- 사용자가 이미 이해를 보여줌
- 사용자가 명시적으로 설명을 거부

## 사용법

```
/grasp
```

## 갭 유형

| 유형 | 설명 |
|------|------|
| **Expectation** | 사용자가 가정한 동작이 실제와 다름 |
| **Causality** | 사용자가 왜 그렇게 되는지 이해하지 못함 |
| **Scope** | 사용자가 변경의 전체 영향을 보지 못함 |
| **Sequence** | 사용자가 실행 순서를 이해하지 못함 |
| **Horizon** | 현재 entry point의 co-intended but unspoken edge — 사용자가 자신의 프레임 안에서는 이름 붙일 수 없는 지점. evidence-bound·material·unspoken 조건(false-positive guard)을 만족할 때만 surface |
| **Emergent** | 정형 유형에 속하지 않는 gap — 구체적 comprehension deficit에 맞춰 적응 |

## 저자

최종원 (https://github.com/jongwony)
