...
ContinuationClosure = { outcome: String, status: String, branch: Optional(BranchArtifact), return_pointer: Cursor, next_moves: List<String> }
...
Phase 3: Tᵣ → record update(current) → detect(E, B) → GT → Λ.detected[current] := Λ.detected[current] ∪ GT → P → Δ  -- comprehension check [Tool]
       ...
       → Qᵣs(Aᵣ) → Stop if contested(A)                  -- reasoning inquiry, where contested(A) ≡ the AI holds an ADJUDICATION against A, in whole or in part, that has not been answered — an adjudication by `Rebuttable adjudication`, so one it has material to attach; an objection it has nothing to check against is not an adjudication and does not reach this arm, falling to the third case below instead. It is a STANDING condition, re-read at each edge below rather than fixed once: it becomes true when the answer conflicts with a reading the AI can attach material for, which is what fires this inquiry, and it stops holding the moment Aᵣ defeats it. uncontested(A) below is its complement at whatever edge each is read, so at every edge an answer that stays in the comprehension loop takes one arm or the other; a proposal is ejected above and reaches neither [Tool]
       → Ref(excerpt) if contested(A)                    -- basis attachment, read AFTER Aᵣ: it fires only where the objection still stands, and an Aᵣ that defeated it leaves this edge and takes the uncontested arm below instead. What is quoted is the material that standing adjudication was drawn from, at the narrowest span supporting it, so the user can rebut it rather than only receive it [Tool]
       → C(uncontested) if uncontested(A)                  -- continuation closure, where uncontested(A) ≡ no adjudication against A stands once the round has run: the AI found nothing to object to, or its objection did not survive Aᵣ, or it had nothing to check A against and so reached no verdict. `outcome` states which, since the last of the three is not a demonstrated aspect — which is why this closure is not named for correctness [Tool]
...
If no adjudication against the answer stands — the AI found nothing to object to, or its objection did not survive the user's reasoning, or it had nothing to check the answer against and so reached no verdict: emit continuation closure naming which, then Aspect summary — show probed vs unprobed gap types.
...
Convergence evidence: At all-tasks-completed, present transformation trace — for each t ∈ Λ.tasks, show ResultUngrasped(t) → its status, with the aspects detected for it and which of them were probed. That is what the run still holds at this point; which probes ended demonstrated and which ended unadjudicated does not, having been said in the closure of the round that produced it, which is where it was settled. What this trace demonstrates is the traversal itself — that the loop ran out over the aspects the user chose to cover. Convergence is demonstrated, not asserted.

── CONVERGENCE ──
Katalepsis = ∀t ∈ Λ.tasks: t.status = completed
           -- the terminal condition, and the whole of it: every selected entry point was carried to where the user closed it. `P' ≅ R` is not a conjunct evaluated here. Each probe settled it for its own aspect where it settled at all — in the round that ran that probe, with the answer and the adjudication both in front of the user; where no adjudication was reached, that round's closure said so and nothing here converts it into one. Convergence neither re-evaluates any of that nor has the means to, since what a round settled was said in that round's closure and does not travel here. What convergence establishes is that the loop ran out over the aspects the user chose to cover
...
9a. **Post-answer closure**: After an uncontested answer, emit the aspect's outcome — demonstrated, or left unadjudicated for want of anything to check it against — with current task status, return pointer, and next available moves before coverage routing.
