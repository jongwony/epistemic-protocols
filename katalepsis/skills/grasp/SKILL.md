...
GT = Relevant gap types per entry point ⊆ GapType. Relevance is TWO conditions, not one: the gap type must apply to the entry point, AND the AI's reading of R must rest on evidence about R sufficient to adjudicate an answer there. An aspect the AI cannot adjudicate is not relevant and is not emitted — a probe launched from no evidence is a test the user must fail, and refusing to emit conceals strictly more than a gate that would have to name the aspect to route it
...
Phase 3 detect (sense) → Internal analysis (gap type relevance detection per entry point, over both conditions the GT type fixes — applicability to the entry point, and whether the AI's reading of R is grounded enough there to adjudicate an answer. This is where the aspects in play are settled; nothing downstream re-decides it, and nothing names an aspect in order to withhold it)
...
Where the reading is your own inference from the artifact, say so when the adjudication lands — after the answer, never before a probe or re-probe on the same aspect — and hold the conflict as a disagreement rather than a mistake: the reasoning inquiry runs to find which reading is wrong, and it may be yours.
...
- **Adjudication warrant**: Adjudicate an aspect only where your reading of the target rests on evidence about it; detection settles that alongside relevance, so an aspect you cannot adjudicate never becomes a probe and is never named in order to be withheld. Where the reading is your own inference from the artifact, state that firmness with the adjudication after the answer, and hold a conflicting answer as a disagreement in which either reading may be wrong.
