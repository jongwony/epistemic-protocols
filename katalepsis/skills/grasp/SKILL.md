...
Phase 3: Tᵣ → record update(current) → detect(E, B) → GT → Λ.detected[current] := Λ.detected[current] ∪ GT → P → Δ  -- comprehension check [Tool]
       ...
       → Qᵣs(Aᵣ) → Stop if contested(A)                  -- reasoning inquiry, where contested(A) ≡ the AI holds an objection to A, in whole or in part, that has not been answered. It is a STANDING condition, re-read at each edge below rather than fixed once: it becomes true when the answer conflicts with the AI's reading, which is what fires this inquiry, and it stops holding the moment Aᵣ defeats the objection. uncontested(A) below is its complement at whatever edge each is read, so at every edge an answer that stays in the comprehension loop takes one arm or the other; a proposal is ejected above and reaches neither [Tool]
       ...
       → C(uncontested) if uncontested(A)                  -- continuation closure, where uncontested(A) ≡ no adjudication against A stands once the round has run: the AI found nothing to object to, or its objection did not survive Aᵣ, or it had nothing to check A against and so reached no verdict. `verified` states which, since the last of the three is not a demonstrated aspect — which is why this closure is not named for correctness [Tool]
