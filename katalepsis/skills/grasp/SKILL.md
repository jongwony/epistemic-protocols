...
13. **PCME does not apply** (Tool-Aligned Cognitive Compromise family): Comprehension verification is verification-category — the user is the measurement target, and bundling probes per round would change the verification's identity. Verification gates are excluded from PCME's round-scale operational rules per Differential Future Requirement's verification-gate exclusion.
