...
12. **Plain emit discipline**: What the user reads is in their language, not the spec's — formal-block vocabulary stays in the formal block.
13. **Round-local salience bundling**: Each round carries the current judgment, its nearest evidence, and what it implies for the next move; everything else defers.
