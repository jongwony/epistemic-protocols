...
Phase 3: Tᵣ → record update(current) → detect(E, B) → GT → Λ.detected[current] := Λ.detected[current] ∪ GT → P → Δ  -- comprehension check [Tool]
       ...
       → Qᵣs(Aᵣ) → Stop if contested(A)                  -- reasoning inquiry, where contested(A) ≡ the AI has an objection to raise against A, in whole or in part; whether that objection stands is settled after Aᵣ and not here. uncontested(A) below is its exact complement over the answers that stay in the comprehension loop, so every such answer takes one arm or the other; a proposal is ejected above and reaches neither [Tool]
       → Ref(excerpt) if contested(A)                    -- basis attachment, on the same guard the reasoning inquiry above takes. The adjudication is settled HERE, after Aᵣ and in the same turn as this attachment — the guard above marks an objection to raise, not a verdict already reached — so what is quoted is the material that settled adjudication was drawn from, at the narrowest span supporting it, and the user can rebut it rather than only receive it [Tool]
...
Katalepsis = ∀t ∈ Λ.tasks: t.status = completed
           -- the terminal condition, and the whole of it: every selected entry point was carried to where the user closed it. `P' ≅ R` is not a conjunct evaluated here. Each probe settled it for its own aspect, in the round that ran that probe, with the answer and the adjudication both in front of the user — convergence neither re-evaluates that nor has the means to, since what a round settled was said in that round's closure and does not travel here. What convergence establishes is that the loop ran out over the aspects the user chose to cover
