...
- **F1 — 지표 폐쇄(Deictic closure)**: 각 세션-로컬 토큰을 canonical 참조로 정규화합니다. grounding보다 선행하여 grounded 항목이 안정 참조를 가리키도록 합니다. `surface_token → canonical_ref → confidence → unresolved?`를 산출합니다. 먼저 **secret-redaction 스윕**이 실행됩니다 — secret 형태의 내용(credential, token, key, `$(...)` command-substitution 참조)을 disposition 전에 placeholder로 가로채므로, secret이 평범한 KEPT 항목으로 흘러가지 않고 그 값이 새 수신자에게 도달하지 않습니다(leak lint는 DROP된 내용에 대한 backstop로 남습니다).
...
**정직 라벨 규칙**: refuter를 건너뛴 산출물은 "converged /distill"이 아니라 **draft / degraded handoff**입니다. "converged"는 Pass verdict로 고정점에 도달한 tier-(c)에만 허용되며, tier (b)가 PortableHandoff 주장의 하한입니다.
