...
- **F7 — 인증**: **Certificate**를 조립합니다 — 판정 목록, outcome(수리가 필요 없었으면 `AlreadyPortable`, 그렇지 않으면 `Repaired(deltas)`), 그리고 필수 **수신 절차**(역할 선언, 수신측이 실행하는 역참조 단계, 수신측이 재확인하는 주장-수리 발견의 전제 목록)입니다. 수리 델타는 `target`에 제자리 적용됩니다 — 이 프로토콜의 유일한 변형입니다.
...
| Certified | **인증된 `/distill`** | 상시 F5 refuter Pass 1회 + leak / durable-pointer 감사 1회, 고정점 도달. `target`은 Certificate를 가집니다: zero-memory refuter가 Pass에 도달했고, 모든 판정 근거가 ObservedKeep 또는 Resolve로 근거지어졌으며, 감사가 통과했습니다. |

F5 refuter dispatch가 무조건적이므로 — 모든 인증 패스가 전체 refuter를 실행하며, 더 가벼운 선택적 계층이 없음 — 부분-엄밀성 중간 등급은 없습니다: `target`은 F7을 거친 적이 없거나(Draft) 완료된 패스의 Certificate를 가집니다(Certified). 라벨은 `target`을 그 발행된 activation edge와 함께 인증합니다; Diylisis 밖에서의 후속 편집이나 대체된 edge로 디스패치된 인증은 검증된 수신 조건을 깨뜨립니다.
