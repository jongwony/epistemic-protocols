...
- **F7 — 인증**: **Certificate**를 조립합니다 — 인증 대상 레코드 자신의 참조, 인증된 상태의 동일성(최종 Pass가 검토한 바로 그 상태의 리비전 고정 형식 또는 내용 다이제스트 — 수신 절차의 선두 역참조 단계에서 재확인), activation edge(외재화 시에도 자기-식별), 판정 목록, outcome(수리가 필요 없었으면 `AlreadyPortable`, 그렇지 않으면 `Repaired(deltas)`), 그리고 필수 **수신 절차**(역할 선언, 수신측이 실행하는 역참조 단계, 수신측이 재확인하는 주장-수리 발견의 전제 목록)입니다. `target`은 이미 모든 수리 델타를 담고 있습니다 — 해소 추가와 drop-절제가 각각을 분류한 패스에서 제자리 적용되었고, 어느 것도 단언된 주장을 바꾸지 않습니다 — 그래서 F7은 더 이상 편집하지 않습니다: 최종 Pass verdict가 검토한 바로 그 상태를 인증합니다.
