...
- `constituting_utterance`: the verbatim user turns (this turn and any prior turns that defined the task) that triggered this handoff. This is a **comparison input scoped to checklist category 10 only** — it is not a source the recipient reads via the document, and it does not become part of your evidence base for categories 1–9. An empty or absent value is valid input (the handoff was AI-detected with no triggering utterance, Layer 2 activation): treat every judgment-bearing contract field as unconfirmed against it and report category 10 accordingly — do not skip the category.
...
10. **Contract-provenance misfit (contract-provenance backstop)** — the contract's `next_task` and `execution_scope`, compared against `constituting_utterance`, claim more than those words authorize: a parked observation elevated into the primary task, a scope wider than what was asked, a work item created or revived that the utterance never named, a reprioritization the utterance does not support. Judge only what the utterance's own words established, not whether the elevation is reasonable — reasonableness is the user's call, not this review's. Two-question test: (1) does a phrase in `constituting_utterance` authorize this task/scope claim, even by a fair paraphrase? (2) if not, does the document's own contract section show the delta as already confirmed (a note that the field was reviewed and ratified at the F0 gate)? A "no" on both is a finding. NOT a finding: a delta the contract section already marks as confirmed — recompute nothing the F0 gate already settled. Report it at the level of the category; the target of the repair is the contract's own declaration, not the document's content — no amount of added context repairs a misdeclared `next_task`. Advisory: **Defer**, with repair note "contract-provenance misfit — F0 re-declaration required (task/scope confirmed against the user's own words)"; the caller halts and re-declares the contract instead of re-entering this finding as a comprehension-gap residual.
11. **Emergent** — anything else a zero-memory recipient cannot resolve from the document plus its stable references. The named categories are working hypotheses, not an exhaustive set.
...
- `constituting_utterance` is the one exception to "your evidence is exactly what the document grants" — it is a provided comparison input, scoped to checklist category 10 only, never treated as a recipient-available source and never used to resolve categories 1–9. It does not dilute the zero-memory posture: the recipient still gets nothing beyond the handoff and its named sources; you alone see the utterance, to check the contract's authorization against it.
