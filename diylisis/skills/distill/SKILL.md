...
Phase 3 comprehension_gate (observe)   → Read, Task (refute-posture review via the diylisis:zero-memory-refuter subagent — fresh context; inputs are the candidate handoff text, the contract's recipient profile + next task, and a runtime-enumerated session-term watchlist; lint-checklist fallback when subagents are unavailable; author self-simulation excluded)
