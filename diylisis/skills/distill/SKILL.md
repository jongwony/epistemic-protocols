...
ZeroMemoryVerdict ∈ {Pass, Fail(blocking_items: Set(ContextItem))}  -- F5 comprehension verdict; Fail names the items a fresh recipient cannot resolve, which re-enter the residual ledger (reason: comprehension-gap) and block the fixed point. Each Fail blocking item carries cited evidence (quoted token + location + category) and an advisory Gate disposition (Resolve | Route | Drop | Defer — type-preserving materialization of A; the user decides at the Gate). A Pass is valid only with a per-category sweep trace recording what was checked — a bare "all clear" assertion is not a verdict. A multi-file handoff — one whose prose references a sibling handoff file the recipient must also read — is a Fail: a fresh recipient must execute from ONE self-contained handoff (Single Canonical Handoff invariant)
...
Phase 3 comprehension_gate (observe)   → Read, Task (refute-posture fresh-context review — realized by the packaged zero-memory-refuter agent where the platform loads plugin agents, by a generic fresh subagent carrying the same F5 instructions where the platform offers only generic spawn, or by a lint checklist where no subagent exists; inputs are the candidate handoff text, the contract's recipient profile + next task, and a runtime-enumerated session-term watchlist; author self-simulation excluded)
...
9. **Zero-memory comprehension gate (refute posture)**: F5 verifies against a fresh recipient with no session access via the `zero-memory-refuter` subagent — fresh context, refute posture (the premise is that an unresolvable token exists and must be found), inputs limited to the candidate handoff text, the contract's recipient profile and next task, and a runtime-enumerated session-term watchlist (active protocol/skill/agent/tool names, which read as plain prose and evade cooperative review). The verdict is evidence-cited: a Fail names each blocking item with quoted token, location, category, and an advisory Gate disposition; a Pass requires a per-category sweep trace — a bare assertion is rejected as no verdict. Where the platform loads no packaged agents but offers generic subagent spawn, a generic fresh subagent carries the same instructions; where no subagent exists, the same checklist runs as a lint pass. The author's own read is excluded as the verification channel, because an author silently shares the missing context and would pass items a fresh recipient could not resolve. A Fail verdict re-enters its blocking items into the residual ledger (reason: comprehension-gap) and blocks the fixed point: convergence requires a Pass verdict, so a failed comprehension forces another pass rather than emitting a handoff. The reviewer's disposition recommendations are advisory only — disposition authority stays with the user at the Gate.
