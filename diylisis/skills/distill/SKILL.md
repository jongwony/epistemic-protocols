...
3. **Read the CorrectionDelta ledger and fix `now`** — locate the append-only correction-delta record for the session and bind it read-only as `Λ.ledger`. At the same moment, fix `Λ.now` as the invocation timestamp: it is immutable for the duration of this invocation and is the sole time source for every horizon-expiry evaluation (`expired(d)`), so an expiry verdict cannot flip between passes. When no ledger is present, bind `Λ.ledger = Unknown` (still fixing `Λ.now`); F3b will route provenance questions to the Gate rather than inferring KEEP.
...
| Silent-residual ban | Every routed-residual, unresolved token, unknown-provenance item, horizon-expired item, and comprehension-gap item enters the ResidualLedger and must reach surfaced = true before convergence | No residue reaches the recipient unseen |
| Provenance hard line | CorrectedKeep reachable only via an effective matching non-provisional unexpired KEEP CorrectionDelta or a user Resolve at the Gate; ROUTE/DROP via F3a relevance or the Gate | Unknown provenance is surfaced for judgment, never defaulted to KEEP |
...
7. **Provenance hard line**: F3b reads the append-only CorrectionDelta ledger and grants **CorrectedKeep** only on an effective matching non-provisional unexpired `KEEP` record. Every other ledger state — absent, no matching record, provisional-only, a non-KEEP delta, or expired (validity_horizon lapsed) — is **Unknown** and routes to the Gate for user judgment. Provenance authority is the KEEP hard line only: a CorrectedKeep verdict requires an explicit matching non-provisional KEEP delta, ROUTE and DROP dispositions come from F3a relevance or the Gate, and an Unknown item reaches any disposition only through a user answer at the Gate.
