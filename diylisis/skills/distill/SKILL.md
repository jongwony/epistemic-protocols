...
expired(d) = d.validity_horizon ≠ Null ∧ now > d.corrected_at + d.validity_horizon   -- now is fixed once per distillation invocation, when Λ.ledger is bound at Phase 0 (invocation-local constant: verdicts cannot flip between passes, so the hygiene measure stays weakly decreasing); Null horizon never expires. Same comparison idiom as Elenchus freshness (observed_at + horizon < now)
...
A horizon-expired item is framed as re-verification: Resolve re-confirms the correction still holds (under re-distillation, F7 appends a superseding delta with a fresh `corrected_at` and a new horizon only when a constituted basis for one exists); in a fresh (non-re-distillation) handoff the Resolve carries KEEP for this handoff only — no ledger write occurs, so a later distill re-asks.
