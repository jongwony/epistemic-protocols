...
Diylisis(W, recipient?, next_task?) → Detect(W) → tethered? →
  ...
  tethered:  pass_n=1, ledger = read(CorrectionDelta) | unknown, residuals = ∅, loop:
    ...
    F5 comprehension_gate(minimal_complete, contract) → zero_memory_verdict →                    -- refute-posture fresh subagent (zero-memory-refuter) or lint-checklist fallback; no self-simulation
         Fail(blocking_items): parse each verdict Findings row → EvidencedFinding, append to residuals (reason: comprehension-gap, evidence kept), block fixed_point
         Pass:                 no new residual (valid only with the per-category sweep trace; sweep absent → no verdict, re-dispatch)
...
WorkingContext, recipient?, next_task?
  ...
  → gate(minimal_complete, contract) → zero_memory_verdict                     -- F5 refute-posture comprehension against a fresh recipient standard; Fail re-enters blocking items as residuals, no author self-simulation
...
EvidencedFinding = { item: ContextItem; quoted_token: String; location: String; category: String; advisory: A_tag; repair_note: String }  -- one F5 Fail finding: cited evidence (quoted token + location + category), an advisory Gate disposition tag, and a prose repair note; the typed form of one Findings-table row in the refuter verdict
WatchList      = Set(String)  -- session-term watchlist for F5, enumerated at dispatch from the author session's own record: skill/protocol names invoked this session ∪ subagent names dispatched ∪ tool names called ∪ plugin/skill names whose instructions are loaded in the session context. The enumeration reads the live session, never a directory scan of installed-but-unused plugins: the watchlist names what is ACTIVE in the session (and could therefore leak into the handoff as plain prose), not what is installed
ZeroMemoryVerdict ∈ {Pass, Fail(blocking_items: Set(EvidencedFinding))}  -- F5 comprehension verdict; Fail names the findings a fresh recipient cannot resolve, whose items re-enter the residual ledger (reason: comprehension-gap, evidence: the finding) and block the fixed point. A Pass is valid only with a per-category sweep trace recording what was checked — a bare "all clear" assertion is not a verdict, and a Pass without the sweep trace is rejected as no verdict (re-dispatch, never default to Pass). A multi-file handoff — one whose prose references a sibling handoff file the recipient must also read — is a Fail: a fresh recipient must execute from ONE self-contained handoff (Single Canonical Handoff invariant)
...
ledger_emit_clean(d) ≡ for a CorrectionDelta d written to the emitted correction ledger: supersedes references prior ledger-entry ids only (never a discarded-content label) ∧ original_ref, original_claim_hash identify the original claim by stable reference and content hash, never by reproducing a DROP'd item's discriminating content ∧ corrected_claim states the current settled value positively (not a negation or retraction of a DROP'd alternative)  -- a portable ledger records provenance for the KEPT value, not a pointer at the discarded alternative; a pointer-at-discarded — any emitted delta field that names, quotes, or otherwise reproduces a DROP'd item's discriminating content — re-introduces the DROP'd content into the recipient's attention and is a leaked_drop
...
Phase 3 comprehension_gate (observe)   → Read, Task (refute-posture review via the diylisis:zero-memory-refuter subagent — fresh context; inputs are the candidate handoff text, the contract's recipient profile + next task, and the session-term WatchList enumerated per TYPES — skill/protocol names invoked, subagents dispatched, tools called, plugin instructions loaded, read from the live session record; lint-checklist fallback when subagents are unavailable — a weakened realization that loses fresh-context isolation, named as such in the verdict; author self-simulation excluded)
Phase 3 comprehension_gate (track)     → Internal state update (parse the verdict: each Findings row → EvidencedFinding; Fail findings re-enter Λ.residual_ledger as comprehension-gap entries carrying their evidence; Λ.zero_memory_verdict set; a Pass without the per-category sweep trace is rejected as no verdict → re-dispatch)
...
| Zero-memory comprehension | F5 verifies via a refute-posture fresh-context reviewer with a session-term watchlist and an evidence-cited verdict (lint fallback when subagents are unavailable — weakened: no fresh-context isolation, verdict names the fallback); a Fail re-enters blocking items as residuals and blocks the fixed point | A failed gate forces another pass; convergence requires an evidence-cited Pass, so neither author self-simulation nor a bare "all clear" can pass items a fresh recipient could not resolve |
