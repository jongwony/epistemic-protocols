...
emit_candidate(Λ) = the candidate PortableHandoff channels before convergence: contract ∪ prose ∪ task_state ∪ residual_ledger ∪ correction_ledger?  -- all PortableHandoff channels (contract included — F7 emits it too; correction_ledger present only under re-distillation); distinct from the F7 emit(·) action; this is the to-be-emitted content F6 lints
...
ledger_emit_clean(d) ≡ for a CorrectionDelta d written to the emitted correction ledger: original_ref ∧ original_claim_hash ∧ supersedes reference prior ledger-entry ids only (never a discarded-content label) ∧ corrected_claim states the current settled value positively (not a negation or retraction of a DROP'd alternative)  -- a portable ledger records provenance for the KEPT value, not a pointer at the discarded alternative; a pointer-at-discarded re-introduces the DROP'd content into the recipient's attention and is a leaked_drop
...
Λ = { phase: Phase, W: WorkingContext,
      ...
      emitted_deltas: List(CorrectionDelta),              -- F7 re-distillation write side: the CorrectionDelta lines this pass appends to the emitted ledger; each must satisfy ledger_emit_clean (pointer-clean, positively stated)
