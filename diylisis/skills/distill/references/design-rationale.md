...
**Why the origin is an index keyed by `ItemId` rather than a field threaded through the flowing types.** Centralizing construction was not enough on its own: the first version of the constructor took the observation as an argument, which works at the two sites that hold one and is unconstructible everywhere else. `EvidencedFinding` has no `request` field, and the two F6 legs are predicates over judgments and decisions that record no origin — so at a recurrence block or a failed leg repair there was nothing to pass. Threading an origin field through `EvidencedFinding`, `RouteJudgment`, and `DecisionRecord` was evaluated and rejected: three types would carry a field only one consumer reads, and each new flowing type would inherit the obligation. `Λ.ref_origins` instead records the origin once, at the Phase 3 track step — the only moment the request and the row are both in hand — keyed by the `ItemId` that `Λ.repaired_targets` already keys by. Later sites do not carry provenance; they look it up. That is also what lets the constructor be total: an item absent from the index simply has no reference origin, so no append site has to classify itself and none can classify itself wrongly.

**Why one constructor rather than the field-setting rule restated per append site.** The tag alone did not close the defect. Stating on the type that every reference-derived entry carries both fields left the appends free to disagree with it, and they did: the two direct routes set the pair, while a failed F6 leg repair on a route-pointer or ledger-less-decision member — and a recurrence block on a reconciliation-built finding — appended generic entries with neither. Those paths reach the ledger from an F5 observation just as the direct ones do, so an `Item` or `Decision` owner could arrive untagged and become deferrable, restoring the same invisible livelock by a longer route. The repair evaluated first was to add the pair at each of those sites; it was rejected because propagating an invariant by restating it at every site is the failure this specification keeps reproducing — each new append is another chance to forget, and nothing detects the omission. `refResidual` makes the invariant hold by construction instead: an append either goes through it and carries the pair, or has no reference origin and carries neither, and there is no third shape to get wrong. Adding an append site later cannot silently reopen the hole, because the constructor is where the fields come from.
