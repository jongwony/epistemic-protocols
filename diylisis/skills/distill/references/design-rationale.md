...
**Why the `Unresolved` arm reads its wire pair off the row.** `EvidencedFinding` REQUIRES its own `quoted_token`/`location`. `ObservedRefRequest` carries neither, and only the `Item(id)` owner projects one. Deriving the pair from `request.owner` therefore left the arm **unconstructible for the `ContractField`, `Decision`, and `StopClause` owners** — three of the four. Carrying the pair on the row closes that, and makes the arm read its siblings' fields by the same mechanism rather than by a per-owner special case.

**Why the `Item(id)` owner does not read that pair, and why this is not a regression of the row uniformity.** Carrying the pair on every row made the arm CONSTRUCTIBLE at every owner; it did not settle which source is AUTHORITATIVE where two exist. At `Item(id)` two did: the dispatch-carried `id`, and the pair the refuter emitted. `itemId(row)` projects from wire fields, and the recurrence key `itemId(f)` reads the finding's own pair — so with the finding built from the row, the repair routing (`request.owner`) and the recurrence key could name different items, and the synthesis fallback could fire for an item that was tracked all along. Two repairs were evaluated: an equality guard rejecting the verdict on disagreement, and taking the dispatch-carried `id` as the finding's identity outright. The second was chosen — rejection is too strict, because the full `(owner, basis, locator)` key already established that the row answers this request, and a refuter quoting a different mention of the same reference is ordinary rather than a failed answer. Taking the `id` leaves nothing to disagree about. The rows still carry their pair uniformly (a row cannot know its owner arm), so the uniformity the previous decision established is intact; what changed is caller-side precedence, not row shape.

**Editing discipline.** What is forbidden is deciding the partition by reach: no site may present reachability as what selects an arm, because that is how the two predicates drifted apart before. `SKILL.md` states the deciding conditions once, at `resolves_at`; elsewhere it names the arms and may state a specific consequence of one — FLOW's "a session-scoped/ephemeral root beyond reach is `Unresolved`", for instance, is compliant precisely because it says reach does NOT decide it — but no site re-derives the partition from reach, and none carries a second copy of the conditions themselves. The packaged refuter is the one deliberate exception, and it is not a lapse in this discipline but the case the discipline cannot reach: it runs in a fresh context and cannot dereference `SKILL.md`, so it necessarily carries the partition's operative conditions by value (`verification-contract.md#packaged-agent-contract-sync`). That copy is why the drift it can incur is closed mechanically rather than by this rule.
...
**Why the observation arm is a tag on the entry rather than a new `reason` value.** Both F5 reference-observation arms that route to the ledger — Unobservable and Unresolved — append under the same `reason`, `certification-leg` (`resolvability_consumers` is authoritative on this). So when the Qd deferral exclusion was written to spare Unresolved-derived entries, it named a class the ledger could not represent: the two kinds of entry were identical field for field, and the exclusion was unenforceable at the gate that had to apply it.

Two repairs were evaluated. A new `reason` value would have made the distinction a membership class — but `reason` is what the `gate_domain` enumerations are written over, so an enumeration site that failed to name the new value would drop the entry out of the domain entirely: the entry would stop being gated at all. That fails in the dangerous direction, and silently. A tag leaves membership untouched: a site that fails to read `ref_arm` still gates the entry and merely permits a Defer that should have been refused — visible at the gate, and strictly better than an entry nobody sees. The tag was chosen for that asymmetry, not for size.

**Why the origin is projected per shape rather than carried, threaded, or stored.** Centralizing construction was not enough on its own: the first version of the constructor took the observation as an argument, which works at the two sites that hold one and is unconstructible everywhere else — `EvidencedFinding` has no `request` field, and the two F6 legs are predicates over judgments and decisions, so a recurrence block and a failed leg repair had nothing to pass.

Three repairs were evaluated. Threading an origin field through `EvidencedFinding`, `RouteJudgment`, and `DecisionRecord` was rejected: three types would carry a field one consumer reads, and every future flowing type would inherit the obligation. A stored index written at reconciliation was tried and **rejected on evidence, having been implemented first**: it has to be keyed by something the later sites can present, which made it `ItemId` — and `ItemId` is a *mention* identity, not a reference identity. A dispatched reference not quoted in `target` takes its own locator as the token and its declaring field as the location, so two decision records sharing one ledger pointer project a single `ItemId`, and one origin displaces the other. That is the same silent collision `OwnerId.Decision(claim, subject_ref)` was widened to prevent, reappearing one layer up. An index also needed set/clear semantics per arm, left target-native rows (which never reconcile) with no writer, and made a missing entry indistinguishable from a genuine absence of origin.

What replaced it needs no key at all. Every site that appends already holds the object the origin belongs to: an observation row carries `owner`/`basis`/`locator` outright, dispatched or target-native alike; a route-pointer judgment carries its item and its `Reference` locator; a ledger-less decision carries its claim, subject_ref, and ledger locator. `refOriginOf` projects from whichever of those the site hands over, and returns None for the shapes that genuinely have no reference origin. Nothing is stored, so nothing can go stale, be written on the wrong arm, or collide; and the constructor stays total, because the projection — not the site — decides whether an entry has an origin.

**Why one constructor rather than the field-setting rule restated per append site.** The tag alone did not close the defect. Stating on the type that every reference-derived entry carries both fields left the appends free to disagree with it, and they did: the two direct routes set the pair, while a failed F6 leg repair on a route-pointer or ledger-less-decision member — and a recurrence block on a reconciliation-built finding — appended generic entries with neither. Those paths reach the ledger from an F5 observation just as the direct ones do, so an `Item` or `Decision` owner could arrive untagged and become deferrable, restoring the same invisible livelock by a longer route. The repair evaluated first was to add the pair at each of those sites; it was rejected because propagating an invariant by restating it at every site is the failure this specification keeps reproducing — each new append is another chance to forget, and nothing detects the omission. `residualEntry` makes the invariant hold by construction instead: an append either goes through it and carries the pair, or has no reference origin and carries neither, and there is no third shape to get wrong. Adding an append site later cannot silently reopen the hole, because the constructor is where the fields come from.

**Why `ref_key` is populated on both arms.** The same idempotence argument that produced it for Unobservable entries applies unchanged to Unresolved ones: the worklist rebuilds from the contract field or stop clause every pass, so without the key each pass appends a fresh entry for the same reference. The two fields are therefore populated together — an entry either has a reference origin, and carries both, or has none, and carries neither.
