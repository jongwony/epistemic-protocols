...
**Editing discipline.** No site restates `resolves_at`'s conditions. Every description of a worklist outcome names these three arms and points here and at `resolves_at` for what each means — never a "reachable" / "beyond tool reach" paraphrase, which is how the two predicates drifted apart before.
