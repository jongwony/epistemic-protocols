...
- **F5 — Comprehension gate**: verify against a zero-memory recipient via the refute-posture `zero-memory-refuter` subagent (fresh context, session-term watchlist, evidence-cited verdict; lint-checklist fallback — weakened: no fresh-context isolation); author self-simulation is excluded.
