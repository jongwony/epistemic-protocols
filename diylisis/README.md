...
- **F3 — Disposition**: KEEP(inline) | ROUTE(StableRef) | DROP. A settled DROP couples to an excision delta: the dropped item's discriminating content is removed from `target` in place (claim-preserving), so the fresh recipient inherits kept and routed content only — the dropped content and its reason survive session-side.
