...
1. **Scope** — read the premise document(s) for the area you're touching (`premise/README.md` routes by concern), plus `.claude/rules/project-profile.md` and `.claude/rules/editing-conventions.md` for this repo's own bindings; run `/horismos:bound` if multiple domains are in play
