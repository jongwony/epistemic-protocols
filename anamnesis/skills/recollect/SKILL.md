...
--   runtime = claude when Claude-host env is present (`CLAUDE_PROJECT_DIR`, `CLAUDE_ENV_FILE`, `CLAUDE_PLUGIN_ROOT`, or plugin-injected `CODEX_COMPANION_SESSION_ID`); this is decisive evidence for the Claude store.
--   runtime = unresolved when Claude-host env is absent; do not infer Codex from skill load path alone. Ask which recall surface to search: Claude store, Codex store, or both.
...
1. **Track-dispatched scan strategy**:
   ...
   Tool realization (Claude Code + Codex local substrates): `Read/Grep/Glob` over the Store binding declared in TOOL GROUNDING. First resolve the active substrate from Claude-host environment evidence. `CODEX_COMPANION_SESSION_ID` is a Claude-session injection, not Codex runtime evidence. If Claude-host env is present, scan the Claude store first. If Claude-host env is absent, ask the user which recall surface to search before scanning: Claude store, Codex store, or both; disclose the selected source scope in the candidate presentation. Codex scanning reads rollout JSONL under live and archived session roots as SSOT, parses session metadata/message rows structurally, and then enriches matching candidates from `session_index.jsonl`, `history.jsonl`, and `logs/hooks.log` when present. Track-internal ranking composes track-appropriate signals — entropy track: literal precision (corpus rarity) dominates; salience track: Σ-match + marker-profile/title/prompt overlap + temporal neighborhood + adjacent vector discovery.
