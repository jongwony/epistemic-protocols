...
SourceScan       = { skipped_lines: Nat, unverified_user_turns: Nat }   -- how completely the record's own source parsed at capture: transcript lines that failed to parse, and human turns absent from the runtime's own cross-check channel. Either count non-zero ⇒ the record was built from a partially readable source, so the emit says so (Rule 18)
...
AttemptsExhausted = |C[]| > 0 ∧ attempts = max                                 -- candidates-present terminal, guarded on the Refine/Reorient branches (FLOW, PHASE TRANSITIONS): surface best candidate → deactivate, never NullMatch
