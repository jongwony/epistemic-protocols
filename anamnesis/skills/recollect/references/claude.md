...
Bind `Candidate.runtime = claude`. Bind the remaining candidate fields from INDEX frontmatter: `session_id`, optional `cwd`, `cross_refs`, the highest evidence tier over matched artifacts, and `source_scan` from the matched artifact's `source_scan` mapping — absent in an entry written before integrity capture, where Null is neutral in ranking exactly as a missing evidence tier is. A non-zero `omitted_chars` means some extraction that fed this artifact was held to a bound short of the whole source, so the emit names the count rather than presenting the entry as a complete account. It does not locate what was dropped: this realization's bounds cut in different places and one artifact's extractions may read different channels, so what reached the artifact need not be a contiguous opening stretch of the session — a later turn can be present while an earlier one is absent. Read it as a quantity missing, never as a boundary the reading stopped at. Each artifact's count answers for every extraction composed into that artifact and not only for the prompt that named it, so a zero read here is a zero for the whole file — bind the matched artifact's own mapping and do not carry one artifact's count to another. An absent mapping on an artifact whose entry otherwise carries integrity capture means no bounded read fed it, which is the same Null the reader already treats as neutral. `narrative.md` is appended to rather than replaced, so a session indexed more than once carries one frontmatter block per pass, each with the same `started_at` and a later `last_turn_at`; bind from the LAST block, which is the most complete reading. The other five artifacts are overwritten and carry one block. Bind `Candidate.record` to the record's own path, `{config_dir}/projects/{slug}/{session_id}.jsonl`, using the slug of the partition the candidate was FOUND in — not the active transcript's, since the spine scan below is not slug-partitioned and the two differ exactly when the recall is hardest. Legacy entries without evidence metadata remain neutral in ranking.
