...
traversal_scope  = the pair Traverse returned, with each skipped broken-link edge noted   -- reported beside the coverage searched by the NullMatch diagnosis at this scope
...
Confirm at this scope: each claim against SSOT(claim.source) — its OWN record, opened through the runtime reference; a recognizable spans several records and there is no single record for the whole. |claims(O[top])| reads, bounded by what will be rendered and never by the traversed sub-graph
                  -- confirmed ⇒ assert as settled; corrected ⇒ assert the record's value, discard the index reading; unattested ⇒ provisional or omitted, never as fact, and the recognizable is at most medium — so it never reaches the SingleObvious relay on an unread claim
...
SparseEdges          : entry candidates exist but too few shared anchors/keywords/metadata infer edges joining them; Assemble = ∅ with a non-empty entry — |O[]| = 0 as for any scan (SKILL.md FLOW): the probe asks granularity and boundary before any NullMatch, and the NullMatch diagnosis reports traversal_scope
BrokenLinkChain      : inferred edges resolve mostly to records never written (lifecycle gap) — surface traversal_scope as a note, not an error; assembled recognizables may be too thin to recognize
