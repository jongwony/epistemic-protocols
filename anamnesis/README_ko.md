...
**검색보다 인지(Recognition over Retrieval)**: AI가 SSOT(세션 JSONL)와 hypomnesis INDEX를 Salience 차원을 따라 스캔하여, 후보를 소크라테스식 내러티브 지문(origin → direction → outcome)으로 제시하고, 사용자가 직접 매치를 인지합니다 — 또는 인접 벡터를 통해 소크라테스식 probe로 세부 조정하거나, 직교적 회상 차원으로 방향을 재설정합니다. 구조화된 literal match는 source namespace와 claim kind가 회상 trace와 맞을 때만 ranking anchor가 됩니다. 모호한 회상을 인지된 맥락으로 변환합니다.
