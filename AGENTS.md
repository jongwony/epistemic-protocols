...
- The `.claude/rules/` files scoped to protocol editing are read and applied at the moment each one binds — `type-category-convention.md` before creating or changing what a TYPES block carries, and `protocol-repair.md` once a defect has been found and the repair is being written. A host that loads them on the skill path delivers them without this bullet; a host that does not is why the bullet is here.
