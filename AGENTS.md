...
- Two `.claude/rules/` files bind at protocol-editing moments and are read and applied there: `type-category-convention.md` before creating or changing what a TYPES block carries — adding an entry or removing one, widening or narrowing it, giving one a new constructor — and `protocol-repair.md` once a defect has been found and the repair is being written. A host that loads them on the skill path delivers them without this bullet; a host that does not is why the bullet is here.
