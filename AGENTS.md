...
- **Checkpoint policy stays at the meta layer**: where a protocol checkpoint presents the user a typed choice, `SKILL.md` inscribes only when the checkpoint fires, what it presents, and each branch's state transition — never which branch is correct. That resolution is constituted by the user per occasion at protocol runtime. A finding that such a checkpoint "lacks a default" or "should prefer branch X" resolves as relay against this clause rather than opening a gate. The accepted consequence, for such checkpoints, is that silence selects no branch: the transition carries `Stop`, so an unanswered checkpoint suspends the protocol instead of auto-continuing — auto-selecting a branch would itself answer the question the clause reserves for the user. This bounds the *silent* default only: a default surfaced for the user to recognize and override, an option ordering within a presentation format, and a relay emit that converges on non-divergence each leave the user's answer intact and fall outside the clause. The clause presupposes branches with materially different downstream trajectories; a branch that collapses into another is a spec defect to fix, not a policy the user constitutes.
- **Conditional-load cut criterion**: what leaves a `SKILL.md` for its `references/` surface is decided by structural belonging, not by length. A clause belongs on the on-demand surface when a precise pointer reaches it just before it first binds — the pointer is what preserves protocol identity across the split, so a clause moved without one is a defect rather than an optimization. Every clause goes to the earliest loading tier its need requires; a lazier tier is admissible only when the clause is provably not needed before that tier loads. Eligibility takes two questions together: does one invocation traverse only some arms (traversing all arms means not eligible), and is there no rebinding path that re-classifies mid-invocation. A measured recoverable word count is an upper bound on the cut, never a budget — material that *decides* a branch stays inline while only material that *executes* it moves, and a disclosure a consumer reads to judge whether to trust the contract is not a branch clause. Load reduction is neither the justification nor a claimed result: a finding that argues a cut by length, or that ranks candidates by recoverable volume, resolves as relay against this clause. Scope: `SKILL.md` prose and its `references/` surface, the domain the passes constituting this direction ranged over.
