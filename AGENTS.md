...
- **Ledger binding**: this project's canonical ledger is the commit-message history on the default branch. A "then"-record — narrative, provenance, a trade-off, a rejected alternative — routes there at write time, in the commit that lands the change it explains. Issues and pull requests are where a proposal is raised and deliberated, not ledger surfaces: what a deliberation settles reaches the ledger through the commit that lands it, so a record left only in an issue or a PR body has not been written down. What a change in flight is being built toward is not a then-record and does not route here; Intent binding below states where it goes, and the two are disjoint because a ledger entry is read by someone who goes looking, while a change in flight has to reach a session that does not yet know to look.
- **Intent binding**: a change in flight carries its direction — what is wanted, why, and under which constraints — in `intent/<slug>.md` on that change's branch, and nothing auto-loads it. Before changing anything on a branch, read every file under `intent/` except `README.md`, which states the convention and the file's lifecycle.
