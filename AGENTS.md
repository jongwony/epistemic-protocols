...
- **Harness boundary**: this project unfolds the epistemic layer only. Harness concerns — tool schema limits, permission models, execution channels, state mutation — belong to the harness ecosystem, which settles them independently; here they are aggregated rather than developed: observed and recorded, but no rule is authored on this repository's instruction surfaces to govern them. A protocol names its delegation point — where execution hands off — and stops there; the reasoning behind the handoff lives in this clause, not in the protocol.
- **Premise surface boundary**: `premise/` carries only what someone who adopted it without access to this repository can resolve on its own — the general principle rises there, while this project's instance of it, and any claim whose meaning depends on this repository's own artifacts or conventions, stays on a project surface. A project document asserts its own project-specific claim on its own authority; citing `premise/` as the ground for a claim that is this repository's own turns a local decision into a general principle without anyone having decided to, and the citation then reads as support while the premise sentence's only support is the project that wrote it. Detection does not follow from a path-reference sweep, because a leak of this kind wears a concept name rather than a path and passes such a check untouched; what catches it is reading the sentence as that adopter and asking whether it resolves. Repair is subtraction that keeps the obligation: retire the project vocabulary, and where the retired wording carried a general contract nothing else states, restate that contract in its general form rather than dropping it with the vocabulary.
