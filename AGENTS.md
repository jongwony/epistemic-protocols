...
- **Ledger binding**: this project's canonical ledger is the git record — commit messages plus issue/PR bodies. "Then"-records (narrative, provenance, trade-offs, rejected alternatives) route there at write time; state surfaces carry only now-asserting operative content, strictest in always-loaded instruction files. This binding also fixes where a cross-session handoff lands: an ADR-shaped record, or an artifact a `/distill` certification lands for the next session to consume, goes to the ledger — the issue or PR body the work is next read from, or a commit message — never to a document under `docs/`. Both are durable, so durability alone does not choose between them; the ledger is time-indexed and append-only, so a later reader meets the record as history, while a doc reads as current instruction and leaks stale context into unrelated sessions as it ages. `docs/` remains for exposition that is maintained as current.
