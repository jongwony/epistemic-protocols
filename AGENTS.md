...
- Instruction-surface placement — which clause belongs on which surface — routes through `/place`. Its host bindings: this project's ledger is the commit-message history (`## Settled Directions`, Ledger binding), the direction of a change in flight is its unit's record (`## Settled Directions`, Intent binding), and its enforcement channel is the static checks (`## Verification`).
