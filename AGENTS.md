...
- **Checkpoint policy stays at the meta layer**: general principle (a checkpoint spec inscribes only when/what/state-transition, never which branch is correct; silence carries a halt rather than auto-selecting a branch): `premise/tiering-and-scope.md` §Checkpoint Policy Stays at the Meta Layer. This project's instantiation: `SKILL.md` is the spec surface this binds, and the transition it carries on silence is `Stop`.
