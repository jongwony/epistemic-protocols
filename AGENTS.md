...
- **Multi-skill plugin description scope**: a plugin bundling more than one skill (currently: `epistemic-cooperative` only — every other plugin in the marketplace ships exactly one skill) is not required to enumerate every skill's `/command` in its top-level plugin description to satisfy `artifact-self-containment.js`'s `hasRoutingCue` check. Full per-skill enumeration in a shared description does not scale as skills are added to that plugin. This is a deliberate trade-off, not a redundancy removal: pre-install per-skill routing discoverability (the description a user sees browsing the marketplace before installing) is traded away for maintainability; post-install discoverability stays fully enforced via `/catalog` and each skill's own `SKILL.md` (the sibling `hasInvocationCue` check is unaffected). Single-skill plugins are unaffected — their description must still name their one skill's `/command`, and the check keeps enforcing this for them unchanged. (Constituted: gh-address-comments follow-up on PR #656 review.)
