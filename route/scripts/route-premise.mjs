...
const PREMISE_INDEX = [
  ...
  { file: "instruction-authoring.md",
    ...
    at: { moment: "instruction-surface-change", when: "when writing or revising instructions and durable records, before adding to standing instructions that already carry entries, and again on the text once it is written: a clause that enumerates cases is standing in for a criterion it did not state, unless the cases are all there can be." } },
