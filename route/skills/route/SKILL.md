...
-- C is read after the protocol active at the prompt, if any, has read the prompt at its checkpoint by its own contract: what that reading closed is residual in C, what it kept is active(C)
...
active(C) = Option(Protocol)   -- the protocol whose invocation is in C with no converge or deactivate it emitted after it; read off C, never off a session store, and read after that protocol has read the prompt, so a prompt its contract closed it on leaves None
