...
4. **Decide** by the option-set relay test:
   ...
   - **a protocol is active** → the prompt is that protocol's to read first, at the checkpoint where it stopped, the way its contract reads any free response there — an answer, a decline, a withdrawal, a dismissal, or a continuation the contract itself holds. Route reads the context that reading leaves. Where the reading closed the protocol, what it recorded is residual and Route is in the ordinary branches above: the deficit the prompt carries is routed as usual, invoked where one protocol dominates, and the residual is read at the next prompt (Rule #5). Where the contract keeps its checkpoint and hands the prompt to the session — a continuation it resumes on its own — the prompt is likewise the ordinary branches' to route, and the protocol resumes where its contract says once that routing has run. Where the protocol stays active, reading the prompt itself, and the dominant match is a different protocol → **monitor mode**: emit one `↗ /command — reason` line as a finding, invoke nothing, and let the turn continue. Detecting a deficit and switching to it are separate flows: the finding is a line in the transcript, which the user reads where the active protocol next stops for them, and whether to switch is theirs to answer there — as a free response, which the active protocol handles the way its contract handles any free response at that stop. That protocol's contract ends at its own boundary; the finding lives in the transcript, outside it, and adds no path to the contract. A protocol is active when its invocation is in the accumulated context and no converge or deactivate it emitted has followed; its prose alone does not make it active.

```
── FLOW ──
Route(C) → Candidates(listing) → P → Resolve(P) → D → Match(C, D) → M →
  active(C) = None ∧ |M| = 1 ∧ dominant:  call(skill_invocation, id(protocol(M)), deficit_statement(C)) → stop
  active(C) = None ∧ (|M| ≥ 2 ∨ weak):    emit(↗ /command — reason) → stop       -- one line per protocol that fits
  active(C) = None ∧ |M| = 0:             silence → stop                          -- reached by no match and by dom(D) = ∅ alike
  active(C) = Some(A) ∧ |M| = 1 ∧ dominant ∧ protocol(M) ≠ A:  finding(↗ /command — reason) → continue   -- monitor mode: a line in the transcript; no invoke, no stop
  active(C) = Some(A) ∧ (protocol(M) = A ∨ |M| ≠ 1 ∨ weak):      silence → continue                        -- monitor mode: A is already the match, or nothing dominates
-- C is read after the protocol active at the prompt, if any, has read the prompt at its checkpoint by its own contract: what that reading closed is residual in C; a prompt it handed to the session under a continuation it holds is routed with active(C) = None; what it kept is active(C)
-- exactly one branch fires; outside monitor mode its stop ends Route's turn, in monitor mode nothing stops — the turn goes on with A
...
active(C) = Option(Protocol)   -- the protocol whose invocation is in C with no converge or deactivate it emitted after it; read off C, never off a session store, and read after that protocol has read the prompt: a prompt its contract closed it on, or handed to the session under a continuation the contract holds, leaves None for that prompt — resuming a held continuation is the contract's, never Route's
