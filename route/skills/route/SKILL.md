...
6. **Route keeps no state** — everything Route does is done by reading the accumulated context at one prompt and relaying: it holds no stack of parked protocols, records no resumption point, and resumes nothing. A protocol the user turns away from is settled by that protocol's own contract, and Route reads the result. Routing that keeps state across prompts — parking a protocol under a newer one, returning to its checkpoint when the newer one converges — needs a hook layer that carries state, and belongs to a separate plugin built on one, never to this skill.
