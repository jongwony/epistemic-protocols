...
**`UserPromptSubmit` → `scripts/route-evaluator.mjs`** is a second entry on the same event, **silent unless you put a key in the environment**. Armed, it asks a constrained-output evaluator — a model that answers over an answer space the caller declares and writes no prose — which of the installed protocols the session looks like it fits, and adds one line naming them:
