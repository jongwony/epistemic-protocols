...
It is registered as its own hook entry rather than inside `route-prompt.mjs`, so a timeout or an outage here cannot take the static directive down with it. The directive goes out whatever this says, including when it says nothing — which is the point: an evaluator answering about the prompt it was handed cannot see the accumulated context Route actually matches against, and cannot see a deficit that surfaces later in the turn. Its silence is not a finding that the session holds none.
