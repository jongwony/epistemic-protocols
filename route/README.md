...
`hooks/hooks.json` registers four hook entries running three scripts. All write `hookSpecificOutput.additionalContext`, read the payload on stdin without requiring it, and exit 0 on every path. No file is written, no network is touched, and no session content leaves the process.
...
**`PreToolUse` and `PostToolUse` → `scripts/route-tool.mjs`** deliver a premise entry at the tool call that is its moment. The index splits each document's moments by kind. A moment that arrives in conversation — deciding, presenting, declaring — is only recognizable by the reader, so it stays in the session-start line and the reader carries it. A moment that is a tool call is observable by the host: the call carries its tool name and input, and these hooks fire around it. So such a moment goes out here, once, at that call, and leaves the session-start line — a pointer delivered many turns before its moment was found not to be followed at the moment; a document whose moments are all observable has no session-start line at all. The observable moments, each defined beside the index: a change to an instruction surface — the project instruction file, a rule, a principle, a skill, an agent definition, recognized by path shape (`instruction-authoring.md`); a question about to be put to the person, with its options (`recognition-and-authority.md`, `interaction-factorization.md`); work about to be handed to an agent that cannot see the conversation (`delegation-and-subagents.md`), and that agent's report coming back, after the call (`verification-discipline.md`); a turn being scheduled for later (`session-and-handoff.md`); a file about to be replaced, or a shell command that discards work no later command restores — a recursive forced delete, a forced push, a hard reset, a dropped table (`boundaries-and-safety.md`). How a call names what it changes: a file tool names its path outright; a shell command is read for a path together with a marker of writing — a redirect, an in-place editor, a copy or move, a heredoc, a write call — since a command that only reads a file is not a change to it, and that reading is a heuristic that errs toward one short line where a read looked like a write, while a write it does not see is still reached by the instruction-file pointer a project keeps; Codex's `apply_patch` carries the patch in `command`, and a path named there is being changed. Tool names are matched as the hosts currently give them, and an unmatched name costs nothing. On no match the hook writes nothing.

Codex reads the same `hooks/hooks.json` and runs every hook in it. The table stays empty there, since the derivation reads Claude Code's install record, and the premise index — at session start and at an edit alike — goes out only where `premise/` sits beside the plugin root — so Route on Codex is the opener, the directive, the skill's own resolution of each candidate's deficit, and the index where the plugin root is inside a checkout of this repository.
