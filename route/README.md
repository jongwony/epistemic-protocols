...
**`PreToolUse` → `scripts/route-edit.mjs`** delivers a premise entry at the moment a tool call is about to change the surface the entry names. The index splits its entries by the kind of moment each names. A moment that arrives in conversation — deciding, presenting, declaring — is only recognizable by the reader, so its entry goes out at session start and the reader carries it. A moment that is a file operation is observable by the host: the tool call carries the path, and this hook fires before the call runs. So an entry whose moment is a change to an instruction surface — the project instruction file, a rule, a principle, a skill, an agent definition, recognized by path shape — goes out here, once, as the change is made, and leaves the session-start index: a pointer delivered many turns before its moment was found not to be followed at the moment. A file tool names its path outright. A shell command is read for a surface path together with a marker of writing — a redirect, an in-place editor, a copy or move, a heredoc, a write call — since a command that only reads a surface is not the moment; that reading is a heuristic and errs toward one short line where a read looked like a write, while a write it does not see is still reached by the instruction-file pointer the project keeps. Codex's `apply_patch` carries the patch in `command`, and a surface named there is being changed. On no match the hook writes nothing.

Codex reads the same `hooks/hooks.json` and runs all three hooks. The table stays empty there, since the derivation reads Claude Code's install record, and the premise index — at session start and at an edit alike — goes out only where `premise/` sits beside the plugin root — so Route on Codex is the opener, the directive, the skill's own resolution of each candidate's deficit, and the index where the plugin root is inside a checkout of this repository.
