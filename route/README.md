...
The hooks registered in `hooks/hooks.json` all write `hookSpecificOutput.additionalContext`, read the payload on stdin without requiring it, and exit 0 on every path. No file is written, and no transcript is read. Every one of them below except the advisory channel touches no network, so no session content leaves the process. The advisory channel is the exception and ships disabled: enabling it sends the conversation — the user's turns and the replies to them, tool calls excluded — to a configured endpoint on every prompt, which is why it has a switch at all and why the section on it states the cost before the capability.
