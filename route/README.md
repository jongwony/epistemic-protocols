...
`hooks/hooks.json` registers four command hooks. Three write `hookSpecificOutput.additionalContext`; the fourth writes a `Stop` decision, which is the one place here where a hook changes what happens rather than what is in context. All read the payload on stdin and exit 0 on every path — the `Stop` hook requires its payload, for the reason given below. No file is written, no network is touched, and no session content leaves the process.
...
**`Stop` → `scripts/route-stop.mjs`** returns the turn for one reading before it closes. A turn ending is a moment the host recognizes whole — it is the host that ends the turn — so the premise entry marked `atClose` goes out here, the way a matcher-decided moment goes out at its call. The reading it asks for is of the turn's own final paragraph: where that paragraph announces work rather than carrying it out — a promise, a plan, a next step, a question the turn could have answered itself — the work is what the turn owed, and ending on the announcement hands back a direction already settled, which is what `recognition-and-authority.md` separates relay from. Where the work waits on something nameable, saying so in one sentence is the close; where nothing is owed, the turn closes without remarking on the reading.

Nothing is matched. Whether a final paragraph announces or carries, and whether what it waits on is real, is a reading of the turn's content, and the same sentence is owed work when nothing blocks it and is a correct close when something does — a regex over wording would be wrong in both directions at once, and would move the judgment out of the reasoning it belongs to, exactly as reading a shell command's intent would in the tool channel above. So the line goes out at every close and the judgment stays with the reader. The cost is one extra model pass per turn; `stop_hook_active` bounds it to one, per the runtime's own guidance to return success while that flag is set, and on the second pass the close proceeds whatever the reading concluded. Two things withhold the return rather than risk it: a payload the hook cannot parse — which is also a payload whose `stop_hook_active` it cannot read, so defaulting would block with the loop guard gone — and a premise root that does not resolve, since a return with no document to read against is cost with no channel.

Codex reads the same `hooks/hooks.json` and runs every hook in it. The table stays empty there, since the derivation reads Claude Code's install record, and the premise index — at session start and at an edit alike — goes out only where `premise/` sits beside the plugin root — so Route on Codex is the directive, the skill's own resolution of each candidate's deficit, and the index where the plugin root is inside a checkout of this repository. The `Stop` hook runs there too and writes the same decision; whether Codex acts on it is Codex's own reading of the output, and where it does not, the turn closes as though the hook were absent.
