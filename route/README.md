...
**`Stop` → `scripts/route-stop.mjs`** holds the turn's end once when the message that just ended presented a numbered option set, and hands the genuine-choice test back to the model for a second pass. The test itself — whether the options genuinely diverge, or collapse to one answer the analysis already settled with the others standing as foils — binds while the set is being written, and no hook reaches that moment; what a hook reaches is the boundary after the set went out, and this one uses it retrospectively. The set the user reads is the one already sent; the second pass — the collapse to a relay, or one line saying why the set stays open — is appended beneath it. The hook does not judge whether the options diverge: that reading stays the reader's, as the paragraph above says. It judges only what the message's shape shows — a numbered list of bold-led items, or the divider block the epistemic output style renders a checkpoint in, its numbered items counted between the opening rule and the closing one — read off `last_assistant_message`, which the host places in the Stop payload; a numbered list of plain steps or findings is left alone, even one that follows a closed block of another kind, while items that lead with bold are read as a choice wherever they stand, since the shape is what is read and never what the items mean; lines inside a fenced code block do not count, the fence read the same way — off its own opening and closing lines, not off markdown structure. One pass, never a loop: a stop the host marks `stop_hook_active` passes through, as does a payload that carries no `last_assistant_message`. The reason the model continues from carries the test, the axis question beside it — a set whose options differ on something the decision does not turn on is standing on the wrong axis, and the pass names the axis it does turn on — its one exemption — a set built to verify understanding, one correct answer by design, is left as it stands — and the endings, names the user's answer as still theirs, and asks for what changed and nothing else.

Codex reads the same `hooks/hooks.json` and runs every hook in it. Codex holds a stop on `decision: block` the same way; whether its Stop payload carries `last_assistant_message` decides whether the second pass fires there, and without it the hook passes through. The table stays empty there, since the derivation reads Claude Code's install record, and the premise index — at session start and at an edit alike — goes out only where `premise/` sits beside the plugin root — so Route on Codex is the directive, the skill's own resolution of each candidate's deficit, and the index where the plugin root is inside a checkout of this repository.
