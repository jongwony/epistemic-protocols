...
The hooks registered in `hooks/hooks.json` all write `hookSpecificOutput.additionalContext`, read the payload on stdin without requiring it, and exit 0 on every path. No file is written, and no transcript is read. Every one of them below except the advisory channel touches no network, so no session content leaves the process. The advisory channel is the exception: armed, it sends the conversation — the user's turns and the replies to them, tool calls excluded — to a configured endpoint on every prompt, which is why it has a switch at all and why the section on it states the cost before the capability. **Nothing arms it but a key in the environment variable `config/evaluator.json` names**, so an install where that variable is empty touches no network either.
...
Not every run of Latin letters is a word, and the difference is expensive. Letters cost about a fifth of a token each inside real words, because the vocabulary carries words whole — but a hex digest, a base64 blob or a UUID is letters by character class and nothing like a word to the tokenizer, and costs near a full token per character. Before that was accounted for, base64 was estimated at 0.63 of its actual cost, hex at 0.69, a UUID run at 0.80 — and an ordinary Korean conversation quoting short commit SHAs at 0.93. So a run of letters and digits is charged as a word only when it reads like one: no digit inside it, and no length an ordinary word does not reach.
...
**A quarter of the room is held back, and the measured residual is not what sets that.** After the word-run rule the worst underestimate across every sample tried — prose in four scripts, code, JSON, hex, base64, UUIDs, file paths — is three percent, and a margin fitted to three percent would be fitted to the wrong thing. Every time a new kind of content was measured here it underestimated: Korean first, then the per-turn structure, then the code points above the BMP, then base64. The risk is in the content nobody has measured yet, and the residual says nothing about its size. What sets the margin is the cost of being wrong: the retry catches an overshoot, but it catches it by halving the conversation, so a near miss costs half the context rather than a few hundred milliseconds. A quarter leaves about 23k tokens, which carried 330 turns of Korean mixed with paths, digests and code in a live check without a retry.
...
One variable covers both uses, so **scoring the fixtures and arming the session channel are the same consent** — the harness is not carved out of the policy above. What keeps them apart in practice is process scope, not a second name: `TYPESAFE_API_KEY=… node route/scripts/route-evaluator-eval.mjs --live` sets the variable for that command alone, so sessions already running, and sessions started from a shell that never exported it, do not see it. `export` it and every session launched from that shell is armed. Do not try to score the fixtures with the variable unset — the harness refuses to run without a key, so that is not a way to have both.

Withhold consent by one of those two, not by editing `apiKeyEnv` to a dead name. A plugin update restores `config/evaluator.json`, but no update sets the variable that file names. Editing `apiKeyEnv` is the case that fails: that edit is reverted by the same restore, and a key still sitting in the shipped variable is reconnected by it.
...
Ahead of that sits a smaller question the fixtures do not ask: is the arrow wired at all? `scripts/route-smoke.mjs` runs it — every protocol in the checkout is an option the endpoint can answer with, the discovered set is the canonical one by identity rather than by size, the answer space is closed so a key outside it never becomes a name, and the state the channel claims to send is the state the request carries. It needs no key and no adjudication, because "this context shows deficit X" is true by construction when the context is that protocol's own declaration of the situation it resolves. `--live` sends one such probe per protocol and reports, per element, whether that protocol's own name came back. A reach there is the path running end to end, not evidence that the declared text separates a protocol from its neighbours on prose a person would type — that is the fixtures' question, and their labels ship unadjudicated.
