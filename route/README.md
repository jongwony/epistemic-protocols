...
Everything here is read-and-relay at one prompt: the hooks inject text, the skill reads the accumulated context and invokes, nudges, places a finding, or stays silent. Nothing is stored between prompts, so `/route` keeps no stack of parked protocols and no resumption point of its own. When the user turns away from an active protocol mid-run, the prompt is that protocol's to read first, at the checkpoint where it stopped, the way its contract reads any free response there; what that reading leaves — a residual folded at a dismissal or a withdrawal, or a checkpoint the contract itself holds across a continuation — is the protocol's own record, and `/route` reads it at the next prompt like any other residual. A prompt the contract hands to the session under such a continuation is routed as usual, and the protocol resumes where its contract says. Routing that holds state across prompts needs a hook layer that holds state; it is a separate, experimental plugin's job, not a feature this one grows.
