...
theorem settled_by_utterance {c : Context P} {a : Axis} {s : Cite c}
    (ok : (axisCoord (P := P) a).admits s.kind) : s.kind = .utterance := ok

theorem recognized_by_utterance {c : Context P} {s : Cite c}
    (ok : (recognitionCoord (P := P)).admits s.kind) : s.kind = .utterance := ok

theorem placed_by_utterance {c : Context P} {s : Cite c}
    (ok : (placementCoord (P := P)).admits s.kind) : s.kind = .utterance := ok
