...
Convergence evidence: At |remaining| = 0, present transformation trace — for each g ∈ Λ.clarified, show (gap_evidence(g) → resolution(g)) from Λ.history. Convergence is demonstrated, not asserted.
...
14. **No premature convergence**: Do not declare |remaining| = 0 without presenting the convergence evidence trace. "All gaps resolved" as assertion without per-gap evidence = protocol violation
