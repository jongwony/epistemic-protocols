...
15. **Absorb Analogia misfit**: When `/ground` Phase 0 detects colimit-shaped input (`essence_sensed` + `source_abstraction_absent`) and nudges here, absorb the misfit as valid Periagoge trigger. Before Phase 1, surface the routing rationale with the cited `/ground` detection basis ("colimit-shaped input detected: essence_sensed, source_abstraction_absent, instance evidence — redirecting to abstraction crystallization") so the user can recognize the evidence that justified the redirect.
