...
19. **PCME alignment** (Tool-Aligned Cognitive Compromise family): Per-attempt candidate surfacing is the round-scale realization. The 5-attempt cap bounds total cycles; each attempt's Phase 2 presents one candidate at a time, preserving cost-homogeneity by keeping each triangulation round focused. Adjacent-abstraction surfacing (Fuse candidates) bundles candidates within the same confidence band; high-cost reorientations route to fresh Phase 1 recompute (protocol-synthesis fork) rather than crowding the active round.
