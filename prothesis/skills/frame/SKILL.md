...
dispatch : ResolvedValue → ReconciliationBehavior   -- TOTAL homomorphism, defined for EVERY reachable ResolvedValue:
           ...
           dispatch(A ∥ B) = run dispatch(A) and dispatch(B) in parallel
           dispatch(adversarial_refute) = adversarial pass over the current aggregate, bounded by the termination generator — so X ⨾ adversarial_refute is just the composition dispatch(X) then dispatch(adversarial_refute)
dispatch_DT : ResolvedStrategy → ReconciliationBehavior   -- whole-topology projection lifting single-value dispatch over the 4-axis Map (DT and DT_default are consumed through THIS, not through single-value dispatch):
           dispatch_DT(DT) = ⟨ independence: DT[independence] → Phase 4 isolation; reconciliation: dispatch(DT[interaction-mode]) bounded by DT[termination] → Phase 5; routing: DT[routing] → Phase 5 presentation ⟩
...
Δ/D?/Dᵣ behavior = the reconciliation component of dispatch_DT(DT) — i.e. dispatch(DT[interaction-mode]) bounded by DT[termination]:
  ...
  dispatch(X ⨾ adversarial_refute)           → run dispatch(X), then adversarial pass over X's aggregate; termination per termination generator (until_dry_ceiling)
...
L      = Lens { convergence: ∩, divergence: D, assessment: A, topology_provenance: DT }  -- Bottom-line is a presentation-layer projection of A (decision-relevant summary surfaced first), not a field of L; Syn(R', Dᵣ) → (∩, D, A) unchanged; topology_provenance records the resolved DT (intra-protocol, surfaced in Synthesis Basis — not re-exported in the FramedInquiry coproduct)
...
FramedInquiry = inj₁(Lᵣ where m = recommend ∧ LensEstablished) ⊕
        ...
        -- coproduct discriminated by m: inj₁ Mode 1 → Lᵣ (selected lens handoff); inj₂ Mode 2 → L (synthesized inquiry lens). DT is intra-protocol: NOT exported — the resolved topology is recorded in L's Synthesis Basis provenance, not in the coproduct. Consumers branch ingest on the tag, not on field probing.
...
J      = Routing ∈ {extend, add_input, wrap_up, withdraw}  -- Phase 5 routing decision (post-merge)
...
Phase 3:  LensEstablished → [m=recommend: skip] | [m=inquire: DT_default_relay(extension: present DT_default + basis as pre-gate text) → proceed-unless-redirected → loop( SDc(next unsurfaced seed axis, top-priority first: default + basis + per-value differential implications + relevant pre-composed composites + ⨾/∥ affordance) → Stop → VM ∈ {Select | Fuse | Reorient | Sufficient} → update(ResolvedStrategy, surfaced_dims, elicitation_cycle) → [VM=Sufficient: exit | else: auto-advance(relay) to next unsurfaced axis] ) until VM=Sufficient ] → unsurfaced seed axes → Gen default → DT → converge(topology trace)  -- deliberation design: topology elicitation, important-first, compositional, Extension fast-path on default [Tool]
...
Phase 5:  R' → Δ(R') → Δₛ → D?(Δₛ, T, DT)[SendMessage] → Dᵣ → Syn(R', Dᵣ) → L → O(L) → route(DT)[return_list: present | deepen_on_finding: loop/handoff] → Qc(routing) → Stop → J  -- triggers (DT dispatch), cross-dialogue, synthesis, DT-routing, presentation & routing [Tool]
...
Phase 5 D? (dispatch)    → SendMessage tool (DT dispatch: dispatch(independent_first_then_dialogue) → coordinator signals tension topic to peer pair → peer exchange → structured report → conditional hub-spoke, skip if Δₛ = ∅; dispatch(independent_aggregate) → skip; dispatch(dialectical_debate) → unconditional; dispatch(A ⨾ B) → run dispatch(A) then dispatch(B) over A's aggregate; dispatch(A ∥ B) → run both in parallel; X ⨾ adversarial_refute → adversarial pass over aggregate, until_dry_ceiling)
Phase 5 route (extension)    → TextPresent+Proceed (routing axis dispatch: return_list → synthesize + present L (current default); deepen_on_finding → on a surviving high-severity finding, loop back to deepen or hand off downstream — prothesis→aitesis is an advisory+downstream edge per graph.json, surfaced as an advisory handoff, not a hard dependency)
...
- **routing** wires Phase 5 presentation (`return_list` synthesizes and presents; `deepen_on_finding` loops back or hands off on a surviving high-severity finding — the prothesis→aitesis edge is advisory and downstream, so this is an advisory handoff, not a hard dependency).
