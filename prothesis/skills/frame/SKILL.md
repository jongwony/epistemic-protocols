...
Prothesis(U) → Q1(MB(U)) → J_mb → [confirm: MBᵥ | modify(field): MB' → re-present Q1(MB') → J_mb (loop until confirm)] → G(MBᵥ) → C → {P₁...Pₙ}(C, MBᵥ) → Sc({P₁...Pₙ}, Pᵦ) → Pₛ → LensEstablished →
  ...
  -- seam = where the handoff carries a settling source the next move is settled and proceeds under that protocol's own gates: the ConductRef this file declares (borne by every SubstrateCorrespondence handoff), or a user-declared chain. Absent one — the LensReturn branch, which carries no ConductRef — the handoff is the last move
...
Phase 3:  LensEstablished → bind_substrate(Pₛ) → discriminate(substrate_specialization_need) → [single lens ∨ no specialized substrate: LensReturn | ≥2 lenses with at least one specialized substrate need: SubstrateCorrespondence(lens↔substrate pairs) ⊕ nudge(ConductRef)] → converge(transformation trace) → handoff(FramedInquiry) → seam → END(morphism)  -- bind each lens's substrate, discriminate by substrate specialization need, emit the framed object as the terminal relay with the /conduct nudge, then end frame's own action (no gate of frame's own, no dispatch by frame, NO isolation, NO synthesis; isolation + arrangement + synthesis is /conduct's to design and the isolated substrate's to run downstream). The seam then carries the settled next move wherever the handoff names one — the ConductRef on the SubstrateCorrespondence branch, or a user-declared chain — under that protocol's own gates
