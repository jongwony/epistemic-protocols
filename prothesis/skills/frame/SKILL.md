...
Prothesis(U) → Q1(MB(U)) → J_mb → [confirm: MBᵥ | modify(field): MB' → re-present Q1(MB') → J_mb (loop until confirm)] → G(MBᵥ) → C → {P₁...Pₙ}(C, MBᵥ) → Sc({P₁...Pₙ}, Pᵦ) → Pₛ → LensEstablished →
  ...
  -- seam = the next move proceeds under the source that SETTLED it, in precedence: an arrangement the invoking context has already settled — a user-declared chain naming the next protocol, or a caller carrying its own arrangement for these lenses — settles it first; failing that, the ConductRef this file declares, borne by every SubstrateCorrespondence handoff, settles it and the seam proceeds to /conduct. ConductRef fills an UNSETTLED arrangement and never overrides a settled one. Absent both — the LensReturn branch, which carries no ConductRef — the handoff is the last move
...
seam (extension)                 → TextPresent+Proceed (fires at deactivation/handoff: proceed directly to the next move, citing the source that SETTLED it; every Constitution gate inside this protocol and inside the next protocol fires unchanged. Precedence: an arrangement the invoking context has ALREADY settled — a user-declared chain naming the next protocol, or a caller that carries its own arrangement for these lenses and says so — settles it first and the seam proceeds under that; failing that, the composition edge this SKILL.md declares — the ConductRef nudge accompanying a SubstrateCorrespondence handoff — settles it and the seam proceeds to /conduct. ConductRef fills an UNSETTLED arrangement and never overrides a settled one: designing an arrangement the caller has already fixed re-opens a settled question instead of answering an open one)
