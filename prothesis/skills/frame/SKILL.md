...
16. **PCME alignment** (Tool-Aligned Cognitive Compromise family): When multiple analytical frameworks surface for selection, bundle cost-homogeneous frames per round. Frames with disproportionately high evaluation cost (unfamiliar substrate, deep domain knowledge required) route as separate focused rounds; auto-resolvable frame selections under option-set relay test flow through autonomous propagation.
