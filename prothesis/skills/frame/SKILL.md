...
Prothesis(U) → Q1(MB(U)) → MBᵥ → G(MBᵥ) → C → {P₁...Pₙ}(C, MBᵥ) → S → Pₛ → LensEstablished →
  ...
  [≥2 lenses with specialized substrate needs: pair(lens ↔ substrate_need + binding_hints) ⊕ nudge(ConductRef) → SubstrateCorrespondence] →
...
Inquiry
  ...
  → discriminate(substrate_availability) -- single lens ∨ no specialized substrate beyond general-purpose → LensReturn; ≥2 lenses with specialized substrate needs → SubstrateCorrespondence
...
Pₛ     = Selected perspectives (Pₛ = Pᵦ ∪ sel({P₁...Pₙ}), |Pₛ| ≥ 1)
         -- a single lens is valid (→ LensReturn); multiple lenses WITH SPECIALIZED SUBSTRATE NEEDS trigger substrate-correspondence, while multiple lenses with no specialized substrate beyond general-purpose return directly (→ LensReturn). The branch is decided at Phase 3 by substrate availability, not by lens count. No two-lens minimum (no mode to satisfy)
...
During Phase 3 (Bind Substrate & Handoff):
  ...
  discriminate(substrate_availability):
    ...
    ≥2 lenses with specialized substrate needs → pair each lens with ⟨substrate_need + binding_hints + per-perspective directive + channel-need⟩ → emit FramedInquiry = inj₂(SubstrateCorrespondence) with the /conduct nudge (ConductRef), hand off the lens↔substrate pairs, terminate. frame does NOT realize isolation, arrange, execute, or synthesize.
...
Phase 3 discriminate (sense)     → Internal operation (coproduct by substrate availability: single lens ∨ no specialized substrate → LensReturn = inj₁ FramedInquiry; ≥2 lenses with specialized substrate needs → SubstrateCorrespondence (lens↔substrate pairs ⊕ per-perspective directive ⊕ channel-need) with the /conduct nudge = inj₂ FramedInquiry)
...
Each perspective is an **individual option**. Keep each option a single perspective so the user composes any combination directly. The user selects one or more perspectives directly. A single selected lens is valid (it returns directly at Phase 3 as a detailed lens); selecting multiple lenses **with specialized substrate needs** triggers substrate-correspondence at Phase 3, while multiple lenses with no specialized substrate beyond general-purpose return directly as `LensReturn`. The branch is decided at Phase 3 by substrate availability, not by lens count alone. There is no minimum-lens requirement.
...
Lens selection (Phase 2) established *which* perspectives. Phase 3 binds each lens to the substrate it needs, discriminates the framed object by substrate availability, and hands it off — frame does not realize isolation, arrange, execute, or synthesize.
