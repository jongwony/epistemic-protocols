...
9. **Convergence evidence**: as typed by the `Convergence evidence` statement in the LOOP block, together with the Phase 3 converge/handoff `(extension)` entries in TOOL GROUNDING.
...
16. **Seam relay on declared continuation**: as typed by the "seam (extension)" entry in TOOL GROUNDING.
