...
dispatch : ResolvedValue⟨interaction-mode⟩ → ReconciliationBehavior   -- TOTAL over interaction-mode values ONLY (generators + their ⨾/∥ composites). interaction-mode is the SOLE axis that dispatches to a reconciliation behavior; the other three axes do NOT dispatch (single_pass, isolated, return_list, deepen_on_finding have no reconciliation primitive) — they close via the scalar projections below:
           dispatch(Gen⟨interaction-mode⟩) = that interaction-mode generator's primitive reconciliation behavior (per Rule 7 wiring)
...
iso     : Gen(independence) → IsolationBehavior   -- isolated → strict isolation (Phase 4, Rule 3 default); shared → relaxed + partial-Lens degradation
present : Gen(routing) → RoutingBehavior          -- return_list → standard J gate; deepen_on_finding → J gate conditioned (recommend extend(deepen) + advisory aitesis handoff on a surviving high-severity finding)
-- termination does NOT dispatch and has no standalone behavior: Gen(termination) ∈ {single_pass, bounded_rounds, until_dry_ceiling} is INJECTED as the dialogue-stage bound into the interaction-mode dispatch (see dispatch_DT)
dispatch_DT : ResolvedStrategy → DispatchedTopology   -- whole-topology projection lifting single-value dispatch over the 4-axis Map (DT and DT_default are consumed through THIS, not through single-value dispatch); codomain is DispatchedTopology, NOT the single-axis ReconciliationBehavior:
           dispatch_DT(DT) = ⟨ independence: iso(DT[independence]) → Phase 4; reconciliation: dispatch(DT[interaction-mode]) with DT[termination] injected as the dialogue-stage bound → Phase 5; routing: present(DT[routing]) → Phase 5 ⟩   -- TOTAL over the whole strategy: interaction-mode closes via dispatch, independence/routing via iso/present, termination via bound-injection
...
During Phase 3 (Deliberation Design — topology elicitation, m=inquire only):
  ...
  When the user engages refinement, each cycle surfaces the most-decision-relevant UNSURFACED seed axis (top-priority first: interaction-mode, then termination, then independence/routing) with default + basis + per-value differential implications via the single dimension gate (SDc); for the interaction-mode axis (the sole composable axis) the gate additionally surfaces relevant pre-composed composites + a one-line ⨾/∥ composition affordance. One gate type offering moves {Select, Reorient, Sufficient}, plus Fuse on the interaction-mode axis only. After a value-setting move, AUTO-ADVANCE (relay) to the next unsurfaced axis. Each move integrates one TopologyMove and updates MODE STATE:
    ...
    VM = Fuse(compose)  → [interaction-mode axis ONLY — Fuse is not offered on the scalar axes] record interaction-mode → Compose(RVᵢₘ, RVᵢₘ, op) in ResolvedStrategy; surfaced_dims ∪= {interaction-mode}; elicitation_cycle += 1; auto-advance to next unsurfaced axis
...
Phase 3 SDc (constitution)       → present (single topology dimension gate, opened only on user refinement: surface one seed axis — default + basis + per-value differential implications; for the interaction-mode axis ONLY (the sole composable axis) additionally surface relevant pre-composed composites like independent_aggregate ⨾ adversarial_refute as recognizable options + one-line ⨾/∥ composition affordance; per-cycle, top-priority axis first; moves {Select | Reorient | Sufficient}, plus Fuse on interaction-mode only — Sufficient is a move here, no separate satisfaction gate; m=inquire only; Esc key → loop termination at LOOP level)
...
The topology is a small **compositional algebra**: a finite set of working axes, each with a finite set of generator values (`Gen`), composable via two operators (⨾ sequential, ∥ parallel). It is not an open or cycle-emergent space — the seed axes are fixed (interaction-mode, termination, independence, routing) and every reachable topology has a defined behavior via the total `dispatch_DT` (single-value `dispatch` over the composable interaction-mode axis, lifted across the scalar axes by their projections). The whole-topology default `DT_default` assembles each axis's generator default; its dispatch reproduces today's exact Phase 4-5 pipeline (independent inquiry, then bounded peer dialogue only when reconciliation triggers fire).
