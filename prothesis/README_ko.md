...
그리스어 πρόθεσις(앞에 놓기)의 현대적 재해석으로, **분석 렌즈를 사용자 앞에 놓고, 선택된 각 렌즈마다 그 렌즈가 필요로 하는 substrate를 선언**한 뒤 렌즈↔substrate 쌍을 격리 실행으로 넘기는 프로토콜이다. frame은 분석 객체(렌즈)와 그 substrate 필요를 공급하며, 다관점 배열은 `/conduct`의 몫, inquiry 실행과 합성(synthesis)은 substrate의 몫이다. frame은 자기 컨텍스트 안에서 다관점 결과를 합성하지 않는다 — 수렴(convergence)은 렌즈들을 진정한 격리 상태에서 실행한 substrate만이 주장할 수 있다.
...
**Recognition over Recall + Substrate-Correspondence** (frame은 공급하고 handoff할 뿐, 실행도 합성도 하지 않음):
...
- **SubstrateCorrespondence**: substrate 필요를 가진 ≥2개 렌즈는 렌즈↔substrate 쌍이 되어 **격리 호스트 실행**으로 넘겨지며, 비자명 배열에는 `/conduct` 넛지가 붙는다. substrate(에이전트 팀, dynamic-workflow, 격리 subagent, plan mode, 또는 main 세션)가 각 렌즈를 격리 상태에서 실행하고 그 다음에야 수렴을 주장할 수 있다; frame은 handoff에서 정지한다.
...
Phase 3: Bind Substrate & Handoff →
  bind_substrate: 렌즈마다 substrate_need(권위적) + binding_hints(권고적) 선언
  단일 렌즈 ∨ 전문 substrate 불필요 → LensReturn(상세 렌즈) → STOP
  substrate 필요를 가진 ≥2개 렌즈   → SubstrateCorrespondence(렌즈↔substrate 쌍 ⊕
                                     /conduct 배열 참조) → handoff → STOP
--- frame은 실행도 합성도 하지 않음: 각 렌즈는 격리 substrate에서 실행되고,
    비자명 배열은 /conduct로 라우팅 ---
```
