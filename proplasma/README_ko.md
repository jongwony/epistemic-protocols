...
**라우팅 우선순위** (첫 매치가 이김): 이미 있는 대상 설명에 대한 매핑이 의도된 추론을 허가하는지 불확실 → `/ground`; 실증거 필요 → `/inquire`; 후보장이 얇음(1개 이하) → `/ideate`, 프레임 자체가 부재 → `/frame`, 좌표가 외재화 substrate에 암묵 → `/elicit`; 후보 ≥ 2 ∧ 증거 불요 ∧ placeholder로 운반 가능 → **`/preview`**.
