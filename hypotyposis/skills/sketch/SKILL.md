...
Concretum = Text(value) | Artifact(versioned_ref)                  -- Text: narration carried in session text; Artifact: a version in the workspace the run holds for its sketches, reachable as itself through the reference registered at its creation
...
Phase 3: produce(Λ.spec, Λ) → Sk : NonEmptyList(Sketch) → Λ.sketches ++= Sk
       ...
       [|Λ.spec.targets| > 1, conditional] produce_delegate(∥ one sketch per executor, each registered in the workspace) [Tool]
       ...
       -- every sketch records its concretum and a versioned reference at creation; producing one writes only into the workspace the run holds for its sketches
...
Phase 6: account → [a terminal arm] Λ.exit := cause → terminal                          -- all arms [Tool]; the RetainFailed arm returns to Phase 5 with Λ.exit still None
       ...
       -- release: per sketch, the destruction and its verification both read off the concretum — Text lives only in session text and takes Released as recorded; an Artifact is destroyed at its reference and shown absent there → Disposition; a failure retries once, then is declared ReleaseFailed with a handoff, never silent
...
Phase 3 produce (transform)         → artifact write, environment run (sketches made in the workspace the host supplies for them — a capability named here and bound nowhere in this contract: what it must satisfy is that a sketch made there is written only into it, and that each version stays reachable as itself through the reference registered at its creation until account disposes of it; a brief naming a parent revises that retained version, a parentless brief generates from the material its source names or from prior material; Text concreta are session text only. Each sketch proposes as coordinates what it was rendered under that no Settled or Provisional coordinate covered — its brief's own commits and what producing it determined beyond them, since the relayed draft settles nothing — so the user meets those determinations beside the sketch at Qfit rather than only their consequences on the screen)
Phase 3 produce_delegate (dispatch) → delegate (conditional: more than one target; parallel topology: one sketch per executor, each registered in the workspace under its own reference; subordinate to the active runtime policy)
...
withdraw (extension)                → TextPresent+Proceed (explicit free-response exit at any gate: partial trace + residual declared; account enforced; EarlyExit. A hard interrupt yields no turn, so account cannot run: the backstop is the bound the workspace puts on an unreleased version's lifetime, and where it puts none, the sketches an interrupt left behind are outstanding until something accounts for them)
