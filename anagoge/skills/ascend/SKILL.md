...
9. **Convergence evidence**: as typed by the `Convergence evidence` statement in the LOOP block, with the SingleObvious Extension-path folding in the CONVERGENCE block's `elevate_complete` equation.
...
22. **Seam relay on declared continuation**: as typed by the "seam (extension)" entry in TOOL GROUNDING.
