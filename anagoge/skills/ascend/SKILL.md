...
ResumeHandle     = String       -- copy-paste re-entry command for a deposit's session — `cd <cwd> && claude --resume <sid>` (construction binding in TOOL GROUNDING) — or a non-resumable note when cwd is absent
...
Common to all:
- **Source & resume (per deposit)**: for each deposit named in the unit above — the chain's line deposits, the cluster's fragments, or the concept's forging deposits — give where it came from (its partition slug + session id + the deposit's date) and a copy-paste command to jump back into that session: `cd <cwd> && claude --resume <session_id>`. The project is resolved from the cwd at resume time, so both the cwd and the session id are needed; when a deposit has no stored cwd, give its source and note it is not directly resumable rather than emitting a command that would fail. This lets the user re-enter any session the unit is built from, not just read that it exists.
