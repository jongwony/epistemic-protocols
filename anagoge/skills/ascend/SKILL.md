...
--   {config_dir} = CLAUDE_CONFIG_DIR when set, else ~/.claude — bound once per invocation by Phase 0 ResolveConfigDir below, then substituted as an absolute path into every path here before any tool call (a ${...} form written into this block is silently inert: Read/Grep/Glob perform no shell expansion, and a tilde inside ${VAR:-default} stays literal even when quoted)
...
--   Confirm              ↦ Read/Grep the session transcript {config_dir}/projects/{slug}/{sid}.jsonl for the deposit's sid to verify an evidential claim before surfacing it; index-only readings stay provisional. Mirrors the Anamnesis "SSOT ⊕ INDEX" store topology (SSOT authoritative, INDEX derived/lossy).
...
--                          then DISCOVER related deposits across partitions by READ-TIME SEARCH (Read/Grep/Glob over `{config_dir}/projects/*/hypomnesis/`) for shared anchors / keywords / session metadata
...
Phase 0 ResolveConfigDir (observe)   → Bash (`printf '%s\n' "${CLAUDE_CONFIG_DIR-$HOME/.claude}"` — binds {config_dir} before any store path is read; read-only, no side effect. `${VAR-default}` without the colon falls back on UNSET only, matching the harness resolver's `??`)
