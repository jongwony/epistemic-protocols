...
11. **PCME alignment** (Tool-Aligned Cognitive Compromise family): When multiple decision-point gaps surface, bundle cost-homogeneous gaps per round. A high-stakes gap whose downstream cost dominates the round routes as a focused round (protocol-synthesis fork); low-stakes gaps with substrate-derivable defaults flow through autonomous propagation under option-set relay test.
