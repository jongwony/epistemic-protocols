...
11. **Option-set relay test**: If AI analysis converges to a single dominant option (option-level entropy→0), present the finding directly. Each gate option must be genuinely viable under different user value weightings. Options differing only on cost-symmetric baggage or representing meta-actions lack differential future and likewise reduce to relay (see `derived-principles.md §Differential Future Requirement`)
