...
**Action**: At Phase 2, present findings with evidence via Cognitive Partnership Move (Constitution).
</system-reminder>
...
4. **On Other(P)**:
   - **Present** via Cognitive Partnership Move (Constitution) to resolve which protocol P to use
...
2. **Route on context richness**:
   ...
   - **Conflict** (`C.tasks ≠ ∅` + `C.prior`): **Present** via Cognitive Partnership Move (Constitution) 1x — resume existing tasks, refresh from prior, or merge
   - **Confirm boundary** (`C.prior` exists, no tasks): Create tasks from prior protocol output, **present** via Cognitive Partnership Move (Constitution) 1x to confirm materialized task list — crossing the protocol boundary (prior output → Prosoche tasks) is an Extension/Constitution boundary, even when intent was verified upstream. Longer chains carry more context but do not eliminate the boundary crossing
   ...
   - **Confirm** (neither + ¬Fired): Create tasks from arguments, **present** via Cognitive Partnership Move (Constitution) 1x to verify task list — transparent cold start without upstream or prior verification. Phase 0 Classify provides independent downstream risk check.
...
1. **Detect team** at invocation time:
   ...
   - Team exists (`C.team`) → Present the team context as text output:
     ...
     Then **present** via Cognitive Partnership Move (Constitution) to select team structure:
...
2. **Augment** path: AI proposes additional epistemic roles based on task scope. **Present** via Cognitive Partnership Move (Constitution) to confirm/add/remove roles. Spawn confirmed roles (|roles| ≤ 6 cap).
...
**Present** findings with evidence via Cognitive Partnership Move (Constitution).
...
**Stop-as-Gate path**: When a subagent (prosoche-executor or team agent) returns `GATE_DETECTED` output, the main agent parses the findings and surfaces them via Cognitive Partnership Move (Constitution) in Phase 2. The subagent stops execution; the main agent handles user interaction.
...
| Level | When | Format |
|-------|------|--------|
| Light | Advisory-severity signals only | Inline note, no Constitution interaction |
| Medium | Single Gate-severity signal, clear pattern | Constitution interaction with approve/halt options |
| Heavy | Multiple Gate signals, production environment, PromptInjection | Detailed evidence + all five options |

Subagent delegation: intensity is determined by the subagent's risk assessment at execution time. The main agent's intensity applies to Phase 2 surfacing of GATE_DETECTED findings (Gate-severity, see RISK SIGNAL TAXONOMY).
...
| Materialization routing | Context-based auto-routing (resume/confirm_boundary/auto_proceed/confirm/conflict) | Confirmation count bounded by Cognitive Partnership Move (Extension/Constitution); confirm_boundary when C.prior, confirm when ¬Fired ∧ ¬C.prior |
| Stop-as-Gate | Subagent stops on Gate, main agent surfaces | Subagent safety without Constitution interaction access |
...
7. **Recognition over Recall**: Present structured options with differential implications via Cognitive Partnership Move (Constitution); Constitution interactions yield turn before proceeding.
8. **Withdraw honored**: User can withdraw at any Phase 2 checkpoint. Withdraw triggers graceful shutdown (SendMessage shutdown_request to team members, then deactivate); user_esc is ungraceful (no cleanup).
9. **Materialization routing invariant**: Phase -1 Sub-A confirmation count varies inversely with context richness, bounded by Cognitive Partnership Move (Extension/Constitution); routing decisions (resume / confirm_boundary / auto_proceed / confirm / conflict) are automatic per `MATERIALIZATION ROUTING`, not user-configured.
10. **Team coordination invariant**: Phase -1 Sub-B resolves team structure via Cognitive Partnership Move (Constitution) with cap `|roles| ≤ 6` and guard `|retain| ≥ 1`; absence of team falls back to Solo (prosoche-executor handles all tasks).
11. **Upstream routing invariant**: Phase -1 Sub-A0 scans deficit conditions before task materialization with execution-blocking filter; each iteration accumulates `Resolved (ProtocolId ∪ DeficitCondition)` to prevent re-detection of an addressed deficit; transparent when `D[] = ∅`. Upper bound: `|ProtocolId|` iterations.
12. **Context-Question Separation**: All analysis, evidence, and rationale appear as text output preceding the Constitution interaction; the question contains only the essential choice and option-specific differential implications.
13. **Convergence evidence**: Present transformation trace before declaring all `T` resolved; per-task evidence (`ExecutionBlind(t) → situated(t)`) is required, not asserted.
14. **Option-set relay test**: When AI analysis converges to a single dominant option (option-level entropy → 0), present the finding directly as Extension. Each Constitution option must be genuinely viable under different user value weightings; options sharing a downstream trajectory collapse to one, and options lacking on-axis trajectory surface as free-response pathways.
15. **Gate integrity** (Safeguard tier — revisitable as model capability evolves; revision triggers: model upgrade with demonstrated instruction-following improvement, sustained low violation rate across sessions, or successful compression PR demonstrating guard reducibility without outcome loss): The defined option set is presented intact — option injection, deletion, and substitution each violate this invariant. Type-preserving materialization (specializing a generic option while preserving the TYPES coproduct structure) is distinct from mutation.
16. **No bounded-regret loop interruption**: Within the LOOP, verify each Stop represents unbounded regret — genuinely viable alternative paths whose wrong choice creates irreversible divergence. Bounded-regret items (deterministic, single viable path, correctable at next Constitution interaction) proceed as Extension (auto-proceed). Plan-level threshold: when `count(severity ≥ Elevated ∩ signal_type = Irreversibility) > 1` in `T[]`, surface the compound effect as a plan-direction question before individual task processing.

17. **Substrate boundary**: Prosoche scope is the epistemic substrate — risk detection, classification, evaluation, and gate surfacing through Phase -1 to Phase 2. Post-gate execution (the gated action's substrate enforcement, harness permission, network/state mutation) belongs to native harnesses or specialized substrates, delegated by handoff after Phase 3 judgment integration. Detailed substrate semantics in `## Substrate Boundary` section prose.

18. **Adaptive granularity**: Default Meso (per task scan). Gate-severity finding → escalate to Micro (per tool call within task). Task boundary → revert to Meso. Detail in UX Safeguards (Granularity adaptation entry).

19. **Plain emit discipline**: User-facing emit (Phase 2 surfacing prose, convergence traces, gate options, and any text shown to the user) uses everyday language to reduce the user's cognitive load — every emit token should carry decision-relevant meaning, not project-internal overhead. SKILL.md formal-block vocabulary — variable names with subscripts, Greek-rooted terms in narrative, formal type labels inline, and code-style backtick tokens — stays in the formal block. What the user reads is the action, observation, or question in their idiom.

20. **PCME alignment** (Tool-Aligned Cognitive Compromise family): When multiple risk classifications surface for an execution, bundle cost-homogeneous classifications per round. Single high-severity risk routes as a focused round; substrate-resolvable risk patterns pre-committed under Standing authority flow through autonomous propagation. Phase 2 gate integrity over runtime-discovered risks remains constitutive at the substrate floor regardless of round composition.
