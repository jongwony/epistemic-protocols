...
- **Restorability of the emitted records is the consuming session's own leg**: the emitted entries are execution-face authorship — completion predicates and verification commands whose validity the user judged at the Phase 2 gate. Whether a fresh consumer can actually work from those records is observable only in that consumer's session, so `/attend` does not audit it from here. Record-restorability discipline is never compiled into `/attend` itself — that would re-mix a concern this protocol is deliberately narrowed away from.
