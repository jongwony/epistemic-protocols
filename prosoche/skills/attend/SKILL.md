...
15. **Gate integrity** *(Safeguard tier)*: The defined option set is presented intact — option injection, deletion, and substitution each violate this invariant. Type-preserving materialization (specializing a generic option while preserving the TYPES coproduct structure) is distinct from mutation. Tier note: classified Safeguard per `Gate Integrity (Operational Guards)`; revisitable as instruction-following improves (PR #270 compression precedent).
