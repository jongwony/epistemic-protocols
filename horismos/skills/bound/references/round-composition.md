...
Analysis, evidence, rationale, the BoundaryMap snapshot, and the `default_for_residual` with derivation cite all appear as text before the gate; the question contains only the essential question, options carry only option-specific differential implications. Embedding context in question fields = protocol violation.
