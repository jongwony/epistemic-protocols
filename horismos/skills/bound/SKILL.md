...
Phase 1: T, cycle_n → Ctx(T, cycle_n) → candidates[cycle_n] → (cycle_n = 1: candidates[1] := candidates[1] ∪ Λ.B_seed) → (Sub-D[cycle_n], auto_resolved?)   -- per-cycle context collection; cycle-1 anchor selection runs over candidates[1] = newly-surfaced ∪ Λ.B_seed (seed joins BEFORE Sub-D binds — step 2; a Reopen(d) seed stays an anchor candidate even when the re-scan misses it), then auto-resolve check [Tool]
       → (cycle_n = 1) non-anchored B_seed remainder → Λ.residual ∪ Λ.domains_touched; Λ.B_seed := ∅   -- cycle-1 B_seed consumption (track): non-anchored remainder residualizes (step 5), cleared after consumption (consumed once)
...
Phase 1       (track)        → Internal state update (cycle 1 only: Λ.B_seed candidates enter anchor selection alongside newly-surfaced domains; non-anchored remainder → Λ.residual ∪ Λ.domains_touched — partition invariant preserved; Λ.B_seed := ∅, consumed once)
