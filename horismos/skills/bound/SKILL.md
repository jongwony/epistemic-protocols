...
18. **PCME alignment** (Tool-Aligned Cognitive Compromise family): One-anchor-per-cycle (Rule 7) is the round-scale realization. Cognitive-cost homogeneity holds by anchoring one domain per cycle while residual accumulates for Phase 4 bulk classification. Phase 1 auto-resolve flows through autonomous propagation for substrate-determined classifications. The free-response termination affordance is the protocol-synthesis fork — a single high-cost satisfaction signal routes to Phase 4 without crowding the per-cycle gate.
