...
Phase 1: T, cycle_n → Ctx(T, cycle_n) → (Sub-D[cycle_n], auto_resolved?)                                      -- per-cycle context collection + auto-resolve check [Tool]
       → (cycle_n = 1) anchor candidates ∪= Λ.B_seed; non-anchored B_seed remainder → Λ.residual ∪ Λ.domains_touched; Λ.B_seed := ∅   -- cycle-1 B_seed consumption (track): seed enters anchor selection, non-anchored remainder residualizes (step 5), cleared after consumption (consumed once)
