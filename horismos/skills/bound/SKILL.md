...
Horismos(T, B_prior?) → Probe(T, context) → B_provisional → boundary_readout → present provisional whole →
  ...
  Phase 0b: sync_kind_route(B_provisional, T, context) → KindRouteMap →
    ...
    atomic: certify(KindBinding, local_claims) → DeficitFitCertificate →
      ...
      pass: init
  Phase 1: enrich the whole map and requested focus → reconcile current sources and dependencies →
    boundary_readout(B, kind, focus, context) → BoundaryEssence
    already_settled: final_readout → DefinedBoundary → converge
    otherwise: Phase 2 Qs(BoundaryEssence) → Stop → A
  Phase 3: read the whole utterance → RoundAnswer →
    ambiguous handling: Qc(live readings and consequences) → Stop → confirmed_intent ∈ J
    Withdraw: reconcile current-kind effects, preserve the correction and unresolved remainder → deactivate
    active answer changing the boundary question: retain requested focus → Phase 0b
    otherwise integrate grounded changes; reconcile affected entries →
...
── MORPHISM ──
TaskScope, B_prior?
  → provisional_whole(task, context) → present_provisional_whole
...
claimed_by(c) = S for certificate(S); {BoundaryUndefined} for user(Own); {d} for user(Route(d)); ∅ for user(Unattributable)
...
A = String                                         -- the complete user utterance after Phase 2; inspect, correct, accept, entrust, defer, and finish can coexist within it
...
integrate : (BoundaryMap, RoundAnswer, context) → BoundaryMap
                 -- apply only the response's grounded commitments and corrections, citing it; adopt exactly the displayed proposals denoted by an informed acceptance or the user's stated replacements. Reconcile dependencies before terminal assessment. Unaccepted proposals and open material remain unsettled.
...
── PHASE TRANSITIONS ──
Phase 0: T, B_prior? → inspect context → Probe(T, context) → B_provisional → boundary_readout → present the provisional whole without a turn yield [Tool]
...
Phase 0b: B_provisional, T, context → sync_kind_route → KindRouteMap
       ...
       → status = pass: initialize B by grounding B_provisional and the applicable prior; derive focus from outstanding examination requests in context, preserving a re-entry request; use ∅ only when none is pending; cycle_n := 1 → Phase 1
       → prior grounding: read the prior's setting record and revisions; admit only same-kind, current supported settlements. Keep still-relevant unresolved domains even if current discovery omitted them. A kind-mismatched prior is advisory; an unavailable setting source supplies no settlement. Never alter the prior record.
Phase 1: B, focus, context → enrich(B, focus) → reconcile(B, context) → Λ.boundary_map := B' [Tool]
       → boundary_readout(B', captured_kind, focus, context) → BoundaryEssence [Tool]
       → already_settled(B', context): final_readout → DefinedBoundary → converge
       → otherwise: Phase 2
Phase 2: BoundaryEssence → Qs(current arrangement, sufficient as shown or open/correct an axis) → Stop → Λ.answer [Tool]
       → Phase 3
Phase 3: read_answer(Λ.answer, B, context) → (RoundAnswer?, viable futures)
       ...
       → bind the confirmed RoundAnswer to the original utterance and its confirming source
       → otherwise no recognized answer: preserve B; render the question from the complete utterance → Phase 2
       ...
       → an active answer changing the boundary question: preserve B with its original kind and setting sources as prior/advisory, carry the utterance's outstanding examination into focus, rebuild and present B_provisional from the user-revised concern → Phase 0b before integrating any new-kind settlement
       → otherwise integrate(B, RoundAnswer, context) → reconcile → Λ.boundary_map := B' [Tool]
       → Continue: derive focus from the utterance; cycle_n := cycle_n + 1 → Phase 1
       → Finish ∧ completion_ready(B', RoundAnswer, context): final_readout → DefinedBoundary → converge
...
Each Phase 1 takes the scope and focus from current ground. A requested axis can reveal subdecisions, provisional concrete choices, constraints, or dependencies; insert them into B and show how they affect the whole. A focus is not a fixed level or mode, and an unchanged source supplies no reason to re-ask a settled decision.
...
Phase 0b attribute / initialize (track) → Internal state update (record the actual Attribution, or initialize the sole current map and cycle after a pass; derive focus from unserved requests, preserving a request carried through kind re-entry)
...
Phase 0b route / unattributable exit (extension) → TextPresent+Proceed (relay deficit and basis with the local command hint where one exists; a user-named deficit without a hint is emitted bare)
Phase 1 enrich (observe) → record read, artifact read, artifact search (collect or derive the evidence and tentative concrete content needed for the requested examination, respecting the observation boundary; unavailable evidence stays explicit)
...
Phase 1 boundary_readout (observe) → record read, artifact read (render the whole map and focused detail beside their current sources; classify content as tentative or settled by its actual setting act)
...
Phase 3 confirmation (constitution) → present (only when several live handling futures remain; show their actual consequences before obtaining confirmation)
Phase 3 focus / question revision (track) → Internal state update (for an active answer, carry the user's unserved focus through kind dispatch into the next whole-map read; for Withdraw, preserve the correction in the partial record and deactivate)
...
Λ = { phase: Phase, T: TaskScope, B_prior: Optional(SeededPrior),
      ...
      focus: Set(Domain), cycle_n: Nat, answer: Optional(A),
