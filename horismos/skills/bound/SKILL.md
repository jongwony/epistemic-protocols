...
Horismos(T, B_prior?) → Probe(T) → Bᵢ? →
  ...
  |Bᵢ| > 0: Phase 0b sync_kind_route(T) → KindRouteMap →                                            -- up-front KIND dispatch (dispatch-first: kind settles before downstream consumers read it)
    [single_dominant_kind(KindRouteMap): relay(captured_kind + basis) (extension) → proceed-unless-redirected   -- entropy→0 fast-path (option-set relay test): one kind dominates, foils route away → NO turn yield
    ...
    certify(KindBinding, graph.json) → DeficitFitCertificate                                         -- fail-closed: status ≠ pass BLOCKS value-space binding (split / route_if / narrow-disambiguation first)
...
TaskScope, B_prior?
  ...
  → certify(KindBinding, registry) → DeficitFitCertificate         -- {owner, in_scope_if, route_if[], evidence, status}; fail-closed — status ≠ pass BLOCKS bind_value_space; checks captured kind's positive_predicate against the sibling-deficit registry (graph.json deficit/edge graph)
...
DeficitFitCertificate = { owner: Deficit, in_scope_if: String, route_if: List<RoutePair>, evidence: Set(Evidence), status ∈ {pass, route, ambiguous} }
                 -- fail-closed: status ≠ pass BLOCKS bind_value_space. Generated by fitting KindBinding.positive_predicate against the documented sibling-deficit scopes — the deficit inventory + edge topology in .claude/skills/verify/graph.json read together with each sibling protocol's deficit: declaration (its SKILL.md deficit: line + the CLAUDE.md protocol-pair table).
...
Phase 0b: T → sync_kind_route(T) → KindRouteMap → [single_dominant_kind: relay(captured_kind + basis) → proceed-unless-redirected (extension, NO Stop) | else: Stop → captured_kind (constitution)]
...
Phase 0b sync_kind_route relay branch: single_dominant_kind(KindRouteMap) = true → relay captured_kind + basis, proceed-unless-redirected (extension, NO turn yield) → continue to bind_kind   -- entropy→0 (option-set relay test): one kind dominates, foils route away; user may still redirect the kind at the next turn; certify stays fail-closed
...
Pre-loop dispatch (Phase 0b, one-shot — runs once before the per-cycle loop opens):
  ...
  Extension fast-path (single-dominant-kind relay): when single_dominant_kind(KindRouteMap) holds — exactly one hypothesis' positive_predicate is unambiguously satisfied, every other hypothesis' route_away_if holds, and no emergent/naming capture is indicated (option-set entropy→0 per the option-set relay test) — present the captured kind + its basis (the dominating predicate + the foils' route-away) as relay text and proceed-unless-redirected, recording Λ.captured_kind with ZERO turn yields (mirrors /conduct's CT_default relay). The mandatory Constitution gate fires only when single_dominant_kind is false — ≥2 kind hypotheses stay viable under different readings, OR an emergent/naming capture is needed — which is the default when in doubt: the kind seeds a frozen multi-consumer signal and genuine framing ambiguity is common, so relay is the NARROW exception.
  bind_kind → certify (fail-closed) → bind_value_space. The captured kind FIXES BoundaryClassification for the whole activation; the per-cycle loop classifies object_ref(kind) over the frozen BoundaryClassification. BOTH branches (relay and gate) feed this same pipeline — the relay collapses only the kind-capture turn yield; a route/ambiguous certificate still re-syncs the full Constitution gate. Re-sync on ambiguous/non-atomic; route_away (deactivate) when a sibling deficit owns the kind.
...
Phase 0b sync_kind_route_relay (extension) → TextPresent+Proceed (RELAY branch — fires when single_dominant_kind = true: exactly one KindHypothesis dominates (positive_predicate unambiguously satisfied, every other hypothesis' route_away_if holds, no emergent/naming situation), so option-set entropy → 0 per the option-set relay test — the kind hypotheses reduce to ONE analytically-correct kind with the others as foils. Present the captured kind + its basis as relay text and proceed-unless-redirected, recording Λ.captured_kind WITHOUT a turn yield; the user may still redirect the kind at the next turn (gated-interaction freedom inherent in turn structure). Mirrors /conduct's Phase 2 CT_default_relay. The fail-closed certify STILL runs on this branch — a route/ambiguous DeficitFitCertificate re-syncs the full Constitution gate, so the relay never bypasses deficit-fit)
Phase 0b certify (sense)     → Internal analysis (extension — fail-closed DeficitFitCertificate; deterministic check of KindBinding.positive_predicate against the documented sibling-deficit scopes (the deficit inventory + edge topology in .claude/skills/verify/graph.json read together with each sibling protocol's deficit: declaration): owner = BoundaryUndefined when in-scope; status = pass | route | ambiguous; basis = the registry deficit/edge fit, cited at Phase 2 cycle 1's first surfacing)
...
Dispatch-first rationale: BoundaryMap is a multi-consumer router (9 downstream advisory consumers read it, per graph.json). The captured kind must settle BEFORE the per-cycle loop opens — hence bound carries the up-front Phase 0b sync rather than emerging the kind cycle-by-cycle: downstream consumers reading a stable kind cannot tolerate a kind that shifts mid-loop. The DeficitFitCertificate (checked against graph.json's deficit/edge registry) keeps a misfit kind from entering the shared map — a kind a sibling deficit owns is routed away (deactivate) instead of polluting the multi-consumer signal.
...
**Action**: At Phase 0b, settle the boundary kind — relay the captured kind as Extension when one hypothesis dominates (entropy→0, single-dominant-kind fast-path), otherwise present the KindRouteMap gate via Cognitive Partnership Move (Constitution); the fail-closed deficit-fit certificate runs before value-space binding on either branch. At Phase 2, present highest-impact boundary-undefined domain for user classification by the uniform settlement disposition via Cognitive Partnership Move (Constitution).
</system-reminder>
...
3. **Scan for B_prior seed (scoped detection, kind-aware)**: inspect current session context for a prior BoundaryMap emitted by an earlier `/bound` invocation in this same session. The detection target is structurally a Horismos `DefinedBoundary` artifact (BoundaryMap structure: `object_ref(kind) → BoundaryClassification` entries with cited basis — always `domain → disposition`) emitted at a previous `/bound` convergence — NOT a `/recollect` recall artifact and NOT a hypomnesis store entry. `/recollect`-recalled classifications stay in the "Cross-session enrichment" advisory channel; they do not populate `Λ.B_prior`. If a `/bound` emit is detected AND the current task scope is a refinement or feedback iteration of the prior scope, bind `Λ.B_prior` to that BoundaryMap **together with the prior map's KIND** (the boundary kind it was produced over); otherwise leave `Λ.B_prior = ⊥`. Capturing the prior map's kind here is what lets Phase 0b step 4 decide whether the seed answers the SAME boundary question as this invocation's captured kind — same-kind is checked at seed time (step 4), not at detection time, because the captured kind is not yet settled at Phase 0.
...
### Phase 0b: Kind Dispatch + Deficit-Fit Certificate (Constitution + fail-closed gate)
...
1. **Dispatch the boundary kind (Constitution gate, with single-dominant-kind relay)** — Surface up to three recognizable kind hypotheses plus an emergent/naming free-response path. Each hypothesis carries a positive predicate, cited evidence, a differential future (what the loop classifies and how downstream consumers read it under this kind), and route-away conditions. The named kinds are **priors (recognition seeds), NOT a closed coproduct** (Rule 5: no fixed taxonomy) — the user may recognize a seed, name an emergent kind, or extend/replace a seed.
   ...
   - **Single-dominant-kind relay (Extension fast-path)**: Before opening the gate, apply the option-set relay test (Rule 13) to the kind hypotheses. When exactly ONE hypothesis dominates — its positive predicate is unambiguously satisfied by the framing AND every other hypothesis' route-away condition holds AND no emergent/naming capture is indicated (option-set entropy → 0: one kind is analytically correct, the others are foils) — do NOT open the gate. Instead present the captured kind + its basis (the dominating predicate and the foils' route-away) as **relay text** and **proceed-unless-redirected**, recording `Λ.captured_kind` without a turn yield (mirrors `/conduct`'s `CT_default` relay). The user may still redirect the captured kind at the next turn. This is **narrow**: the kind seeds a frozen multi-consumer signal (9 downstream advisory consumers), so relay ONLY when one kind unambiguously dominates — when ≥2 hypotheses stay viable under different readings, or an emergent/naming capture is needed, **default to the mandatory Constitution gate**.
...
2. **Recognition over Recall**: Present structured options with differential futures via Cognitive Partnership Move (Constitution); Constitution interactions yield turn for response. Phase 2 binds to `A ∈ BoundaryClassification` (the uniform settlement disposition `{UserSupplies, AIPropose, AIAutonomous, Dismiss}`, same for every kind) plus a complete BoundaryMap snapshot with provisional `default_for_residual` visible (so ImplicitTermination is informed); Phase 4 binds to `FinalGateAnswer = FinalGateDisposition` (the uniform `{UserSupplies, AIAutonomous} ⊆ BoundaryClassification`) (per TYPES).
