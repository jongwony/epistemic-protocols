...
Horismos(T, B_prior?) → Probe(T) → Bᵢ? →
  ...
  |Bᵢ| > 0: Phase 0b sync_kind_route(T) → KindRouteMap → Stop → captured_kind                       -- up-front KIND dispatch (dispatch-first: kind settles before downstream consumers read it)
...
Pre-loop dispatch (Phase 0b, one-shot — runs once before the per-cycle loop opens):
  ...
  bind_kind → certify (fail-closed) → bind_value_space. The captured kind FIXES BoundaryClassification for the whole activation; the per-cycle loop classifies object_ref(kind) over the frozen BoundaryClassification. Re-sync on ambiguous/non-atomic; route_away (deactivate) when a sibling deficit owns the kind.
...
Phase 0b sync_kind_route (constitution) → present (mandatory; up-front KindRouteMap — ≤3 recognizable kind hypotheses each with positive_predicate + evidence + differential_future + route-away conditions, plus an emergent/naming free-response path; named kinds are PRIORS not a closed set; user captures the kind by recognizing a seed, naming an emergent, or extending/replacing a seed; Esc → deactivate)
...
Λ = { phase: Phase, T: TaskScope,
      ...
      captured_kind: Optional(Kind),                   -- the kind the dispatch settled on (recognize-seed | name-emergent | extend/replace-seed)
...
AI probes for boundary-undefined domains before execution OR user calls `/bound`. Probing is silent (Phase 0); the boundary **kind** is then dispatched up-front via Cognitive Partnership Move (Constitution) and fail-closed deficit-fit certificate (Phase 0b); per-domain classification by the uniform settlement disposition requires user interaction (Phase 2) unless the domain is auto-resolved from substrate — a definitive boundary assignment found in Phase 1 records the classification and skips Phase 2 for that cycle.
...
**Action**: At Phase 0b, present the KindRouteMap to settle the boundary kind (fail-closed deficit-fit certificate before value-space binding). At Phase 2, present highest-impact boundary-undefined domain for user classification by the uniform settlement disposition via Cognitive Partnership Move (Constitution).
</system-reminder>
...
Then **present** (the question is phrased for the captured kind; the four disposition options are the same for every kind — bracketed text fills each option with this kind's content, e.g. for ownership "who decides", for direction/priority "which way", for scope "how wide", for type/concept "which category"):
