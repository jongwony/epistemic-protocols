...
- **Emit-scan backstop** — re-check the *actual* candidate output for any straggling copy of an **already-detected** value (an incompletely-scrubbed or re-materialized occurrence) and deterministically scrub it (find-replace the known value — terminating). The backstop verifies intent against output; it does **not** extend detection — a value never recognized as a secret is out of reach of both layers, surfaced honestly as a Known Limitation rather than silently claimed-handled.
