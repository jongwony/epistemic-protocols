...
converge iff (Phase 3 Sufficient ∨ substrate_exhaustion) ∧ coverage_complete(work_unit_map) ∧ ¬user_esc
  ...
  Phase 3 Sufficient:       residual filled by autonomous_pack (each remaining region cut at its natural joints, Extension-default), surfaced for recognition; emits once packing empties the residual — every verdict left to packing is a size verdict it settles autonomously by a further cut or a merge
  substrate_exhaustion:     Phase 1 surfaces no new region — if residual = ∅, emit directly; if residual ≠ ∅, autonomous_pack the residual, surface, finalize over the packed map, then emit — likewise once the residual is empty
...
4. **Routing**:
   - If `TerminationIntent = Sufficient`: autonomous-pack the residual (each remaining region cut at its natural joints, Extension-default per the project's Extension calibration), surface each cut for recognition, assert the three invariants over the whole map, emit WorkUnitMap, converge — emitting once packing empties the residual, which it does autonomously because every verdict left to it is a size verdict settled by a further cut or a merge.
