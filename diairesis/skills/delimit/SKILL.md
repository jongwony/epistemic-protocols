...
Diairesis(WB) → Probe(WB) → granularity_underdetermined? →
  ...
  underdetermined:         init_loop_state: cycle_n=1, cut_set=∅, work_unit_map=∅, residual=regions(WB), committed=∅, regions_touched=regions(WB), joints=∅, invariant_status=⊥, history=∅, loop:
    ...
    Phase 3 (parse(A) → (cut_disposition?, termination?)) →
      ...
      Sufficient: → autonomous_pack(residual) (track) → [residual ≠ ∅ after packing: it re-enters as the next cycle's Anchor (reason surfaced as relay) → cycle_n += 1, loop] | (work_unit_map'', residual := ∅) → surface (extension) → finalize(work_unit_map'') → emit WorkUnitMap → converge
...
ExternalWBS × ExecutionHorizon × ContextLifecycle
  ...
  → fit(unit, horizon, lifecycle) → SpanFit       -- per-unit span-fit predicate: a unit fits one span iff its work volume completes within one execution span under the declared horizon and context lifecycle (the capacity judgment /delimit owns). Whether the receiving span can then work from the unit's externalized record is that span's own reception concern, observable only there — /delimit cuts, it does not certify what a future span will find
  ...
  → finalize(WorkUnitMap'')                       -- WorkUnitMap'' = WorkUnitMap' with the residual autonomous_pack'ed at its natural joints (Extension-default, residual → ∅); finalize asserts the three invariants hold over the whole packed map — autonomous_pack itself commits only Fits units (re-checking span_fit, settling any size verdict by a further cut or a merge, and returning any still-non-empty residual to the cycle loop), while a committed non-Fits unit exists only by an explicit user-accepted override recorded on its fit field, which span_fit counts as satisfied
  → WorkUnitMap
...
SpanFit ∈ {Fits, Overflows, Underfills}  -- the per-unit fit verdict against (horizon × lifecycle): Fits = one span; Overflows = too big for one span (split); Underfills = too small to warrant its own span (merge). A capacity verdict only — where the cut falls is the separate natural_joint invariant, and how well a future span receives the unit is that span's own concern, not a verdict /delimit can issue from here
...
A      = CutDisposition ∈ {AcceptCut, MoveCut(joint), SplitUnit(joint), MergeUnits(neighbor)}  -- the per-cycle user answer over the proposed cut; the coproduct is the PARSE domain, held intact per gate integrity — Qc presentation is verdict-relevant: a disposition appears as a peer option only when it carries a live repair trajectory for the current SpanFit verdict (a merge offered against an Overflows verdict is a foil, excluded from presentation per the option-set viability rule, while a free response naming it still parses; a parsed excluded disposition executes normally — integrate runs it under the standard commit enforcement, a unit committing only on Fits or an explicit recorded override, so a repair that leaves the verdict standing re-surfaces the unit next cycle with its verdict-matched proposal — presentation exclusion is never answer rejection)
         -- AcceptCut: adopt the proposed unit as a committed WorkUnit; on a non-Fits verdict this is an explicit user override — the unit commits carrying its recorded verdict on WorkUnit.fit (never silently upgraded to Fits)
...
TerminationIntent = parsed natural-language signal that the partition is complete  ∈ {Sufficient}
         -- Sufficient: commit the current map; the AI autonomously packs every remaining residual region at its natural joints (Extension-default), re-checking span_fit per packed unit — autonomous_pack commits only Fits units, and every non-Fits verdict left to it is a size verdict it can settle autonomously (an Overflows residual takes a further interior cut; an Underfills residual merges with an adjacent region) — a region packing cannot resolve this way (e.g. an Underfills region with no residual neighbor to merge) stays in residual and re-enters the cycle loop as the next Anchor, the reason surfaced as relay — and the three invariants are asserted over the whole map before emit
...
Phase 1 → converge (autonomous residual): Anchor empty ∧ residual ≠ ∅ → autonomous_pack(residual) (track) → [residual ≠ ∅ after packing: it becomes the next cycle's Anchor (reason surfaced as relay) → cycle_n += 1, Phase 1] | (work_unit_map'', residual := ∅) → surface (extension) → finalize(work_unit_map'') → emit WorkUnitMap  -- substrate exhausted; AI completes residual cuts at natural joints, surfaced as relay; packed carrier + finalize, same shape as the Sufficient path
...
Phase 3 → Phase 1: (¬termination ∧ ¬Esc) ∨ (Sufficient ∧ autonomous_pack left the residual non-empty — it becomes the next Anchor, the reason surfaced as relay) → re-derive next-cycle anchor frame → cycle_n += 1  -- continue the partition loop
...
converge iff (Phase 3 Sufficient ∨ substrate_exhaustion) ∧ coverage_complete(work_unit_map) ∧ ¬user_esc
  coverage gate:            convergence presupposes the three invariants hold over the whole map. span_fit is COMMIT-ENFORCED (a unit moves to committed only once it Fits, or the user explicitly accepts a non-Fits unit — the override recorded on the unit's fit field) and natural_joint holds BY CONSTRUCTION (every cut sits on a joint), so both are maintained invariants that already hold over the committed map at convergence — they need no separate convergence gate. coverage_complete is therefore the sole remaining HARD convergence gate (a map leaving work outside every unit is not a valid WorkUnitMap and cannot emit)
  Phase 3 Sufficient:       residual filled by autonomous_pack (each remaining region cut at its natural joints, Extension-default), surfaced for recognition; emits only once packing empties the residual — every verdict left to packing is a size verdict it settles autonomously by a further cut or a merge, and a residual still non-empty after packing re-enters the cycle loop as the next Anchor before any emit
  substrate_exhaustion:     Phase 1 surfaces no new region — if residual = ∅, emit directly; if residual ≠ ∅, autonomous_pack the residual, surface, finalize over the packed map, then emit — likewise only once the residual is empty; a residual still non-empty after packing re-enters the cycle loop
...
Phase 1 Pack (sense)         → Internal analysis (the packing search: propose the cut-set fitting each region against horizon × lifecycle; the per-unit Fits predicate is the capacity judgment against that budget; basis cited at Phase 2 surfacing)
...
3. **Anchor selection** — surface the highest-leverage uncut region as `Anchor[cycle_n]`: the region whose cut most constrains the rest of the partition (a large Overflows region, or a hard dependency seam). Pair it with the proposed cut and its `SpanFit` (the capacity verdict — a unit Fits when its work volume completes within one execution span under the declared horizon and context lifecycle).
...
- **Span-fit verdict**: Fits / Overflows / Underfills, with its basis (the horizon × lifecycle estimate)
...
**Verdict-relevant presentation**: the options above are the full menu; a given cycle presents as peers only the dispositions carrying a live repair trajectory for the current span-fit verdict — an Overflows verdict presents SplitUnit (and AcceptCut-as-override), an Underfills verdict presents MergeUnits, and MoveCut appears wherever a different joint is a live alternative. The full coproduct remains the parse domain: a free response naming an unpresented disposition still parses AND executes — integrate runs it under the standard commit enforcement (a unit commits only once it Fits or carries an explicit recorded override), so a repair that leaves the verdict standing simply re-surfaces the unit next cycle with its verdict-matched proposal; exclusion from presentation is a foil judgment for the current verdict, never a rejection of the user's named answer — gate integrity binds the parse domain, not the per-verdict peer set.
...
2. **Integrate** (only when a `cut_disposition` is present — skipped on Esc, which binds no cut, and on a pure-termination Sufficient that carries no cut) — apply the cut to `Λ.cut_set` and `Λ.work_unit_map` as reference entries (issue ids, never copied content); move the anchor region from `Λ.residual` to `Λ.committed` only once its unit Fits (or the user explicitly accepts a non-Fits unit) — otherwise it stays in `Λ.residual` for a later cycle; append `(region, cut, fit, A)` to `Λ.history`.
   - **AcceptCut**: commit the proposed unit as a WorkUnit; on a non-Fits verdict the acceptance is an explicit override — the unit commits with its verdict recorded on `fit`, never silently upgraded to Fits.
...
4. **Routing**:
   - If `TerminationIntent = Sufficient`: autonomous-pack the residual (each remaining region cut at its natural joints, Extension-default per the project's Extension calibration), surface each cut for recognition, assert the three invariants over the whole map, emit WorkUnitMap, converge — emitting only when packing empties the residual: a region packing leaves non-empty re-enters the cycle loop as the next Anchor (the reason Sufficient could not complete it surfaced as relay), and the emit proceeds once it is re-cut.
...
| Per-unit span-fit | `SpanFit` is the capacity verdict against horizon × lifecycle; shown with basis at Phase 2 | AcceptCut/SplitUnit/MergeUnits recognized from a fit verdict, not guessed |
| Commit only Fits | A unit moves to committed only when it Fits (or is explicitly accepted); a non-Fits unit stays in residual | No WorkUnitMap silently misreports fit: an accepted non-Fits unit rides with its recorded verdict |
...
5. **The packing judgment is the only new logic**: Diairesis composes `/inquire` (scope insufficiency) and `/bound` (joint candidates), and feeds `/conduct` (downstream). Its one irreducible contribution is the partition/packing search — finding the cut-set satisfying the three invariants. It builds no new subsystem; everything else is composition wired as advisory graph edges.
6. **Three packing invariants**: a valid WorkUnitMap satisfies span_fit (each unit fits one span), natural_joint (every cut falls at a joint), and coverage_complete (the units cover the whole body with no work orphaned). span_fit is enforced at commit — a unit moves to committed only when it Fits, or by an explicit user acceptance that records the non-Fits verdict on the unit; coverage_complete is the HARD convergence gate; natural_joint is surfaced and refined per cycle.
