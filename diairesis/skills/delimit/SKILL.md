...
Diairesis(WB) → Probe(WB) → granularity_underdetermined? →
  ...
  underdetermined:         init_loop_state: cycle_n=1, cut_set=∅, work_unit_map=∅, residual=regions(WB), committed=∅, regions_touched=regions(WB), joints=∅, invariant_status=⊥, history=∅, loop:
    Phase 1 Scan(WB, cycle_n) [per-cycle re-scan] → joints(region) → Pack(joints, horizon, lifecycle) → (Anchor[cycle_n], proposed_cut, SpanFit) →
      ...
      Anchor empty ∧ residual ≠ ∅: → autonomous_pack(residual) (track) → [Unportable remainder: residual ≠ ∅ after packing → it re-enters as the next cycle's Anchor with a MoveCut proposal (reason surfaced as relay) → cycle_n += 1, loop] | (work_unit_map'', residual := ∅) → surface (extension) → finalize(work_unit_map'') → emit WorkUnitMap → converge  -- substrate exhausted; AI completes the residual cuts at their natural joints (Extension-default), surfaced as relay; same packed-carrier + finalize shape as the Sufficient path
    ...
    Phase 3 (parse(A) → (cut_disposition?, termination?)) →
      ...
      Sufficient: → autonomous_pack(residual) (track) → [Unportable remainder: residual ≠ ∅ after packing → it re-enters as the next cycle's Anchor with a MoveCut proposal (reason surfaced as relay) → cycle_n += 1, loop] | (work_unit_map'', residual := ∅) → surface (extension) → finalize(work_unit_map'') → emit WorkUnitMap → converge
...
ExternalWBS × ExecutionHorizon × ContextLifecycle
  ...
  → fit(unit, horizon, lifecycle) → SpanFit       -- per-unit span-fit predicate (composes /distill's portability standard: a unit fits one span iff BOTH (a) its work volume completes within one execution span under the declared horizon and context lifecycle — the capacity leg, which /distill does not measure — and (b) once externalized to a single substrate-owned record, that record would certify portable for the receiving span — the boundary Role in that counterfactual is the receiving span session's continuing-collaborator role, fixed by the cut itself, not a free parameter)
  ...
  → finalize(WorkUnitMap'')                       -- WorkUnitMap'' = WorkUnitMap' with the residual autonomous_pack'ed at its natural joints (Extension-default, residual → ∅); finalize asserts the three invariants hold over the whole packed map — autonomous_pack itself commits only Fits units (re-checking span_fit, returning any Unportable residual to the cycle loop), while a committed non-Fits unit exists only by an explicit user-accepted override recorded on its fit field, which span_fit counts as satisfied
  → WorkUnitMap
...
SpanFit ∈ {Fits, Overflows, Underfills, Unportable}  -- the per-unit fit verdict against (horizon × lifecycle): Fits = one span; Overflows = too big for one span (split); Underfills = too small to warrant its own span (merge); Unportable = the capacity leg holds but the portability leg fails — the counterfactual externalized record would not certify portable for the receiving span: a boundary defect, not a size defect, disposed by MoveCut (re-cut at a joint whose record can certify), never by split/merge (an explicit user-accepted override may still commit it, the verdict recorded on the unit and the portability test deferred to the receiving span's certification). Composes /distill's portability-certification standard as the Fits predicate
...
A      = CutDisposition ∈ {AcceptCut, MoveCut(joint), SplitUnit(joint), MergeUnits(neighbor)}  -- the per-cycle user answer over the proposed cut; the coproduct is the PARSE domain, held intact per gate integrity — Qc presentation is verdict-relevant: a disposition appears as a peer option only when it carries a live repair trajectory for the current SpanFit verdict (a size repair against a boundary defect is a foil, excluded from presentation per the option-set viability rule, while a free response naming it still parses; a parsed excluded disposition executes normally — integrate runs it under the standard commit enforcement, a unit committing only on Fits or an explicit recorded override, so a repair that leaves the verdict standing re-surfaces the unit next cycle with its verdict-matched proposal — presentation exclusion is never answer rejection)
         -- AcceptCut: adopt the proposed unit as a committed WorkUnit; on a non-Fits verdict this is an explicit user override — the unit commits carrying its recorded verdict on WorkUnit.fit (never silently upgraded to Fits), and an accepted Unportable defers the portability test to the receiving span's actual /distill certification
...
TerminationIntent = parsed natural-language signal that the partition is complete  ∈ {Sufficient}
         -- Sufficient: commit the current map; the AI autonomously packs every remaining residual region at its natural joints (Extension-default), re-checking span_fit per packed unit — autonomous_pack commits only Fits units (an Overflows residual takes a further interior cut, still autonomous; an Unportable residual is a boundary judgment autonomy cannot settle: it re-enters the cycle loop as the next Anchor with a MoveCut proposal, the reason Sufficient could not complete it surfaced as relay) — and the three invariants are asserted over the whole map before emit
InvariantStatus = { span_fit: Bool, natural_joint: Bool, coverage_complete: Bool }  -- the three packing invariants over the committed cut-set; span_fit holds when each committed unit Fits or carries an explicit user-accepted override (the verdict recorded on WorkUnit.fit); coverage_complete is HARD (full coverage, no orphaned work — a map leaving work outside every unit is not a valid WorkUnitMap)
...
Phase 1 → converge (autonomous residual): Anchor empty ∧ residual ≠ ∅ → autonomous_pack(residual) (track) → [Unportable remainder: residual ≠ ∅ after packing → it becomes the next cycle's Anchor with a MoveCut proposal (reason surfaced as relay) → cycle_n += 1, Phase 1] | (work_unit_map'', residual := ∅) → surface (extension) → finalize(work_unit_map'') → emit WorkUnitMap  -- substrate exhausted; AI completes residual cuts at natural joints, surfaced as relay; packed carrier + finalize, same shape as the Sufficient path
...
Phase 3 → Phase 1: (¬termination ∧ ¬Esc) ∨ (Sufficient ∧ autonomous_pack left an Unportable remainder — it becomes the next Anchor with a MoveCut proposal, the reason surfaced as relay) → re-derive next-cycle anchor frame → cycle_n += 1  -- continue the partition loop
Phase 3 → converge (Sufficient): TerminationIntent = Sufficient ∧ autonomous_pack empties the residual → (work_unit_map'', residual := ∅) → surface (extension) → finalize(work_unit_map'') → assert InvariantStatus (coverage_complete hard) → emit WorkUnitMap
...
converge iff (Phase 3 Sufficient ∨ substrate_exhaustion) ∧ coverage_complete(work_unit_map) ∧ ¬user_esc
  coverage gate:            convergence presupposes the three invariants hold over the whole map. span_fit is COMMIT-ENFORCED (a unit moves to committed only once it Fits, or the user explicitly accepts a non-Fits unit — the override recorded on the unit's fit field, an accepted Unportable deferring the real portability test to the receiving span's /distill certification) and natural_joint holds BY CONSTRUCTION (every cut sits on a joint), so both are maintained invariants that already hold over the committed map at convergence — they need no separate convergence gate. coverage_complete is therefore the sole remaining HARD convergence gate (a map leaving work outside every unit is not a valid WorkUnitMap and cannot emit)
  Phase 3 Sufficient:       residual filled by autonomous_pack (each remaining region cut at its natural joints, Extension-default), surfaced for recognition; emits only once packing empties the residual — an Unportable remainder re-enters the cycle loop as the next Anchor (MoveCut proposal) before any emit
  substrate_exhaustion:     Phase 1 surfaces no new region — if residual = ∅, emit directly; if residual ≠ ∅, autonomous_pack the residual, surface, finalize over the packed map, then emit — likewise only once the residual is empty; an Unportable remainder re-enters the cycle loop
...
Phase 2 Qc (constitution)    → present (conditional: ¬SingleDominantCut(Anchor, proposed_cut) ∨ SpanFit ≠ Fits; per-cycle partition gate over CutDisposition {AcceptCut, MoveCut, SplitUnit, MergeUnits} presented verdict-relevant (peer options = dispositions with a live trajectory for the current SpanFit; the full coproduct stays the parse domain) + the anchor region + its proposed cut + SpanFit basis + current cut-set snapshot + cycle_n + free-response Sufficient termination affordance; Esc → loop termination, not an Answer)
...
autonomous_pack (track)      → Internal state update (fires from Phase 3 on Sufficient, or directly from the Phase 1 substrate-exhaustion terminal: complete the residual cuts at their natural joints — Extension-default per the project's Extension calibration — moving each region from residual to committed; span_fit re-checked per packed unit: only Fits commits — an Overflows residual takes a further autonomous interior cut, an Unportable residual re-enters the cycle loop as the next Anchor with a MoveCut proposal instead of committing)
...
Composition (compose, do not reimplement): Diairesis carries ONE new operation — the packing/partition judgment (search for the cut-set satisfying the three invariants). Everything else composes existing protocols and is wired as advisory graph edges:
  ...
  • /distill (Diylisis) — the per-unit fit predicate's portability leg: a unit "fits one span" iff its work volume completes within one execution span (the capacity leg, judged by /delimit from horizon and lifecycle) AND, once externalized to a single substrate-owned record, that record would certify portable for the receiving span (boundary Role = the receiving span session's continuing-collaborator role, fixed by the cut) — the portability leg is exactly /distill's portability-certification standard. SpanFit = Fits composes both legs.
...
3. **Anchor selection** — surface the highest-leverage uncut region as `Anchor[cycle_n]`: the region whose cut most constrains the rest of the partition (a large Overflows region, or a hard dependency seam). Pair it with the proposed cut and its `SpanFit` (composing `/distill`'s portability-certification standard as the Fits predicate — a unit Fits when its work volume completes within one execution span AND, once externalized to a single substrate-owned record, that record would certify portable for the receiving span, the cut-fixed boundary Role).
...
Options:
1. **AcceptCut** — commit the proposed unit as a WorkUnit (on a non-Fits verdict: an explicit override, recorded on the unit)
...
**Verdict-relevant presentation**: the options above are the full menu; a given cycle presents as peers only the dispositions carrying a live repair trajectory for the current span-fit verdict — a size repair has no trajectory against a boundary defect, and vice versa (an Unportable verdict presents AcceptCut-as-override and MoveCut, not SplitUnit/MergeUnits). The full coproduct remains the parse domain: a free response naming an unpresented disposition still parses AND executes — integrate runs it under the standard commit enforcement (a unit commits only once it Fits or carries an explicit recorded override), so a size cut that leaves a boundary defect standing simply re-surfaces the unit next cycle with its MoveCut proposal; exclusion from presentation is a foil judgment for the current verdict, never a rejection of the user's named answer — gate integrity binds the parse domain, not the per-verdict peer set.
...
2. **Integrate** (only when a `cut_disposition` is present — skipped on Esc, which binds no cut, and on a pure-termination Sufficient that carries no cut) — apply the cut to `Λ.cut_set` and `Λ.work_unit_map` as reference entries (issue ids, never copied content); move the anchor region from `Λ.residual` to `Λ.committed` only once its unit Fits (or the user explicitly accepts a non-Fits unit) — otherwise it stays in `Λ.residual` for a later cycle; append `(region, cut, fit, A)` to `Λ.history`.
   - **AcceptCut**: commit the proposed unit as a WorkUnit; on a non-Fits verdict the acceptance is an explicit override — the unit commits with its verdict recorded on `fit`, an accepted Unportable deferring the portability test to the receiving span's `/distill` certification.
...
3. **Check invariants** — assert `InvariantStatus` over the committed cut-set: span_fit (each committed unit Fits or carries an explicit user-accepted override, its verdict recorded on the unit), natural_joint (each cut sits on a joint), coverage_complete (the committed ∪ residual covers the whole body with no work orphaned). coverage_complete is HARD — the map cannot emit while it fails.
4. **Routing**:
   - If `TerminationIntent = Sufficient`: autonomous-pack the residual (each remaining region cut at its natural joints, Extension-default per the project's Extension calibration), surface each cut for recognition, assert the three invariants over the whole map, emit WorkUnitMap, converge — emitting only when packing empties the residual: an Unportable remainder re-enters the cycle loop as the next Anchor with a MoveCut proposal (the reason Sufficient could not complete surfaced as relay), and the emit proceeds once it is re-cut.
...
| Commit only Fits | A unit moves to committed only when it Fits (or is explicitly accepted); a non-Fits unit stays in residual | No WorkUnitMap silently misreports fit: an accepted non-Fits unit rides with its recorded verdict, an accepted Unportable deferring to the receiving span's /distill certification |
...
6. **Three packing invariants**: a valid WorkUnitMap satisfies span_fit (each unit fits one span), natural_joint (every cut falls at a joint), and coverage_complete (the units cover the whole body with no work orphaned). span_fit is enforced at commit — a unit moves to committed only when it Fits, or by an explicit user acceptance that records the non-Fits verdict on the unit (an accepted Unportable defers the portability test to the receiving span's /distill certification); coverage_complete is the HARD convergence gate; natural_joint is surfaced and refined per cycle.
...
12. **Gate integrity** (Safeguard tier): The defined option set (`A ∈ CutDisposition`) is the PARSE domain, held intact — injection, deletion, and substitution against that domain each violate this invariant. Qc presentation is verdict-relevant per the option-set viability rule (Rule 11): a disposition with no live repair trajectory for the current SpanFit verdict is excluded from presentation as a foil, while a free response naming it still parses and executes under the standard commit enforcement — presentation narrowing is not option deletion, which would remove the constructor from the parse domain itself. Type-preserving materialization (filling `MoveCut`/`SplitUnit`/`MergeUnits` with a concrete joint or neighbour while preserving the coproduct) is distinct from mutation. The free-response Sufficient termination lives in Phase 2 prose, not in the typed coproduct.
...
16. **Seam relay on declared continuation**: when a user-declared chain or a composition edge this SKILL.md declares names the next protocol, the between-protocol seam after Diairesis's convergence is relay (Extension) — proceed directly, citing the settling source (the chain declaration or the named edge). This governs only the seam BETWEEN protocols; every Constitution gate inside Diairesis and the next protocol fires unchanged, and the user can redirect at any turn.
