---
name: delimit
description: "Delimit a large body of work into right-sized execution units before its method is conducted. Searches for the cut-set over an external work-breakdown structure such that each unit fits one execution span, every cut falls at a natural joint, coverage is complete, and no work is orphaned — then emits a WorkUnitMap that references the WBS without owning it, ready to flow into /conduct. Type: (GranularityUnderdetermined, AI, DELIMIT, ExternalWBS × ExecutionHorizon × ContextLifecycle) → WorkUnitMap. Alias: Diairesis(διαίρεσις)."
---
...
Diairesis(WB) → Probe(WB) → granularity_underdetermined? →
  ...
  underdetermined:         init_loop_state: cycle_n=1, cut_set=∅, work_unit_map=∅, residual=regions(WB), loop:
    Phase 1 Scan(WB, cycle_n) [per-cycle re-scan] → joints(region) → Pack(joints, horizon, lifecycle) → (Anchor[cycle_n], proposed_cut, SpanFit) →
      Anchor empty ∧ residual = ∅: → DefinedPartition → converge          -- every region cut, nothing left
      Anchor empty ∧ residual ≠ ∅: → autonomous_pack(residual) (extension) → surface → converge  -- substrate exhausted; AI completes the residual cuts at their natural joints (Extension-default), surfaced as relay
      else:                        → Phase 2
    ...
    Phase 3 (parse(A) → (cut_disposition, termination?)) →
      integrate(cut_disposition, cut_set, work_unit_map) → (cut_set', work_unit_map')
      check_invariants(work_unit_map') → InvariantStatus                  -- span_fit ∧ natural_joint ∧ coverage_complete ∧ no_orphan over committed cuts
      Sufficient: → autonomous_pack(residual) (extension) → finalize(work_unit_map', residual) → DefinedPartition → converge
      Esc:        → ungraceful deactivate (residual uncut, partition abandoned)
...
ExternalWBS × ExecutionHorizon × ContextLifecycle
  ...
  → pack(joints, horizon, lifecycle) → CutSet     -- THE IRREDUCIBLE CORE: search for a cut-set whose every unit fits one span, every cut sits on a joint, coverage is complete, no work orphaned
  ...
  → check(WorkUnitMap') → InvariantStatus         -- span_fit ∧ natural_joint ∧ coverage_complete ∧ no_orphan
  → finalize(WorkUnitMap', residual)              -- autonomous_pack the residual at its natural joints (Extension-default), then assert the four invariants hold over the whole map
  → WorkUnitMap
...
Anchor = the highest-leverage uncut Region surfaced this cycle (the region whose cut most constrains the remaining partition) — paired with its AI-proposed cut and SpanFit
...
TerminationIntent = parsed natural-language signal that the partition is complete  ∈ {Sufficient}
         -- Sufficient: commit the current map; the AI autonomously packs every remaining residual region at its natural joints (Extension-default), and the four invariants are asserted over the whole map before emit
InvariantStatus = { span_fit: Bool, natural_joint: Bool, coverage_complete: Bool, no_orphan: Bool }  -- the four packing invariants over the committed cut-set; coverage_complete ∧ no_orphan are HARD convergence conditions (a map with orphaned work is not a valid WorkUnitMap)
WorkUnitMap = { units: Set(WorkUnit), wbs_ref: WBSRef }  -- the result: the set of execution cuts plus the WBS reference they view. Consumed by /conduct (Hyphegesis) as its WorkProspect — Diairesis cuts the units, /conduct orders and conducts them
...
Phase 0 → Phase 1: granularity_underdetermined(WB) = true → init loop state (cycle_n=1, cut_set=∅, work_unit_map=∅, residual=regions(WB))
...
Phase 1 → converge: Anchor empty ∧ residual = ∅ → DefinedPartition directly (every region cut)
Phase 1 → converge (autonomous residual): Anchor empty ∧ residual ≠ ∅ → autonomous_pack(residual) (extension) → surface → DefinedPartition  -- substrate exhausted; AI completes residual cuts at natural joints, surfaced as relay
...
Phase 3: (parse(A) → (cut_disposition, termination?)) → integrate(cut_disposition, cut_set, work_unit_map) → (cut_set', work_unit_map') → check_invariants(work_unit_map') → InvariantStatus
Phase 3 → Phase 1: ¬termination ∧ ¬Esc → re-derive next-cycle anchor frame → cycle_n += 1  -- continue the partition loop
Phase 3 → converge (Sufficient): TerminationIntent = Sufficient → autonomous_pack(residual) (extension) → finalize(work_unit_map', residual) → assert InvariantStatus (coverage_complete ∧ no_orphan hard) → emit WorkUnitMap
...
Packing search (the irreducible core): `pack(joints, horizon, lifecycle)` proposes the cut-set by fitting each candidate region against the span budget (horizon × lifecycle) — an Overflows region invites an interior cut (split), an Underfills region invites a merge with a neighbour, a Fits region anchors a unit. The search seeks the cut-set satisfying all four invariants; the user constitutes each cut, so the search proposes and the gate disposes.

Convergence evidence: At convergence, present the transformation trace — per committed WorkUnit, (Anchor region → Cut at joint → SpanFit → disposition), plus the residual disposition (each autonomous_pack cut with its joint + Extension-default basis), plus the InvariantStatus (all four invariants, with the coverage/no-orphan check shown). The WorkUnitMap is presented as a separate session-text artifact carrying the WBS reference. Convergence is demonstrated, not asserted.

── CONVERGENCE ──
converge iff (Phase 3 Sufficient ∨ substrate_exhaustion) ∧ coverage_complete(work_unit_map) ∧ no_orphan(work_unit_map) ∧ ¬user_esc
  coverage/no-orphan gate:  convergence presupposes the four invariants hold over the whole map — coverage_complete ∧ no_orphan are HARD (a map leaving work outside every unit is not a valid WorkUnitMap and cannot emit); span_fit ∧ natural_joint are surfaced and refined per cycle
...
Phase 1 Pack (transform)     → Internal analysis (the packing search: propose the cut-set fitting each region against horizon × lifecycle; compose /distill's zero-memory carriability as the per-unit Fits predicate; basis cited at Phase 2 surfacing)
...
Phase 3 integrate (track)    → Internal state update (apply the settled cut to cut_set + work_unit_map as reference entries; move the anchor region from residual to committed; the WBS is never copied)
Phase 3 check (sense)        → Internal analysis (assert the four packing invariants over the committed cut-set; coverage_complete ∧ no_orphan are hard convergence gates)
...
converge (extension)         → TextPresent+Proceed (per-unit cut trace + residual disposition trace + InvariantStatus + WorkUnitMap as a separate session-text artifact carrying the WBS reference; proceed — the map flows to /conduct)
...
Λ = { phase: Phase, work_body: WB,
      ...
      invariant_status: InvariantStatus,    -- the four packing invariants over the committed cut-set
...
Composition (compose, do not reimplement): Diairesis carries ONE new operation — the packing/partition judgment (search for the cut-set satisfying the four invariants). Everything else composes existing protocols and is wired as advisory graph edges:
...
| Trigger | Effect |
|---------|--------|
| Sufficient signal (free-response: the partition is complete) | Autonomous-pack the residual at natural joints, assert the four invariants, emit WorkUnitMap |
...
4. If granularity is underdetermined: initialize loop state (`cycle_n = 1`, `cut_set = ∅`, `work_unit_map = ∅`, `residual = regions(WB)` the single whole-body region) and proceed to Phase 1.
...
Parse the answer, apply the settled cut, and check the four invariants.
...
2. **Integrate** — apply the cut to `Λ.cut_set` and `Λ.work_unit_map` as reference entries (issue ids, never copied content); move the anchor region from `Λ.residual` to `Λ.committed`; append `(region, cut, fit, A)` to `Λ.history`.
   - **AcceptCut**: commit the proposed unit as a WorkUnit.
   - **MoveCut(joint)**: re-delimit the region's boundary at the named joint; re-fit.
   - **SplitUnit(joint)**: cut the region at the interior joint into two units; the second enters `residual` for its own cycle.
   - **MergeUnits(neighbor)**: merge the region with the named adjacent region into one unit; remove the absorbed region from `residual`.
3. **Check invariants** — assert `InvariantStatus` over the committed cut-set: span_fit (each committed unit Fits), natural_joint (each cut sits on a joint), coverage_complete (the committed ∪ residual covers the whole body), no_orphan (no work outside any region). coverage_complete ∧ no_orphan are HARD — the map cannot emit while either fails.
4. **Routing**:
   - If `TerminationIntent = Sufficient`: autonomous-pack the residual (each remaining region cut at its natural joints, Extension-default per the project's Extension calibration), surface each cut for recognition, assert the four invariants over the whole map, emit WorkUnitMap, converge.
...
| Packing search | `pack(joints, horizon, lifecycle)` proposes the cut-set against the four invariants | The one new operation; the user disposes each proposed cut |
...
| Hard coverage / no-orphan | `coverage_complete ∧ no_orphan` are convergence gates | A map orphaning work cannot emit; every piece of the body lands in a unit |
...
| WorkUnitMap artifact | Emitted as separate session text carrying the WBS reference | The cut-set flows to `/conduct` as its work prospect |
...
5. **The packing judgment is the only new logic**: Diairesis composes `/inquire` (scope insufficiency), `/bound` (joint candidates), `/distill` (per-unit span-fit), and feeds `/conduct` (downstream). Its one irreducible contribution is the partition/packing search — finding the cut-set satisfying the four invariants. It builds no new subsystem; everything else is composition wired as advisory graph edges.
6. **Four packing invariants**: a valid WorkUnitMap satisfies span_fit (each unit fits one span), natural_joint (every cut falls at a joint), coverage_complete (the units cover the whole body), and no_orphan (no work outside any unit). coverage_complete ∧ no_orphan are HARD convergence conditions; span_fit ∧ natural_joint are surfaced and refined per cycle.
...
**Cross-session enrichment**: Prior `/delimit` partitions accumulated in the hypomnesis store, and any context surfaced when `/recollect` has been invoked this session, bias the packing search toward cut granularities the user has found productive for similar work. This is a heuristic input; the constitutive judgment — where each cut falls — remains with the user at Phase 2.
