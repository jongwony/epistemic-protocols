...
**Secondary pattern Pre/Post pairs** (per `docs/structural-specs.md` §Pattern over Vocabulary): the linear cluster ordering above is the activation default; orthogonal to it, five inter-protocol Pre/Post pairs operate across distinct epistemic axes — Hermeneia↔Katalepsis (intent), Telos↔Syneidesis (goal), Aitesis↔Epharmoge (context), Prothesis↔Analogia (structure), Prothesis↔Prosoche (action — theoria/praxis). Pair backward flow operates via Output Style nudge rather than graph.json structural edges (action-pair Wirkungsgeschichte intake at Prothesis Phase 4 LOOP `J=extend` "review execution results" is the inscribed exception — see `prothesis/skills/frame/SKILL.md` and `prosoche/skills/attend/SKILL.md`).
