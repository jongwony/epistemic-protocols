...
**Utility skills**: Epistemic Cooperative (`/catalog`, `/report`, `/onboard`, `/probe`, `/dashboard`, `/compose`, `/introspect`, `/sophia`, `/curses`, `/write`, `/artifact-review`, `/review-ensemble`, `/steer`, `/misuse`), Verify (`/verify`). Triggers, flows, and detailed descriptions in each plugin's SKILL.md.
