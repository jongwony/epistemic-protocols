...
**Utility skills**: Epistemic Cooperative (`/catalog`, `/report`, `/onboard`, `/probe`, `/dashboard`, `/compose`, `/introspect`, `/sophia`, `/curses`, `/write`, `/comment-review`, `/review-ensemble`, `/goal-research`, `/steer`, `/misuse`, `/crystallize`, `/rehydrate`, `/dispatch`, `/realign`), Verify (`/verify`). Triggers, flows, and detailed descriptions in each plugin's SKILL.md.
...
**Realign utility (project northstar realignment via issue-landscape audit)**: `/realign` (epistemic-cooperative) is a Periagoge family specialization on a different territory than `/steer` — it audits the current open issue landscape for direction-drift signals (NorthstarExtended, NorthstarNarrowed, NorthstarReoriented, NorthstarStale, plus an Aligned-no-signal class and Emergent), surfaces per-cluster evidence for user-constituted verdict, then writes an updated northstar declaration line in the project guide (default `CLAUDE.md`) via Circular Return. Realign is a utility skill, not a 13th core protocol — intentionally absent from `graph.json`. Specializes `/induce` along two axes (instances constrained to open issues observed in the current backlog; output constrained to writable project-guide northstar inscription). Forms a **functional dual pair with `/steer`** on the dispatch read territory: Steer writes the calibration profile (Cognitive Partnership Move axis), Realign writes the mission-direction northstar (project-guide axis); dispatch reads both, modifies neither. Architectural inscription (graph.json edges, advisory lineage to `/induce`, optional dispatch auto-suggest from high skip-ratio observations) is deferred pending accumulated cross-session use evidence. Pre-write timestamped backup is mandatory — rollback is a single `mv` command. Phase 5 RouteToOperationalLayer disposition handles cases where the proposed northstar overflows a single line, implies graph.json edits, or cascades terminology to multiple downstream documents.
...
21 static checks: json-schema, notation, directive-verb, xref, structure, tool-grounding, version-staleness, graph-integrity, spec-vs-impl, morphism-anatomy, cross-ref-scan, onboard-sync, precedence-linear-extension, partition-invariant, catalog-sync, gate-type-soundness, artifact-self-containment, single-axis-soundness, agents-symlinks-sync, workflow-paths-sync, language-purity. `artifact-self-containment` validates the packaged runtime contract view (`Skill.md` + plugin description metadata + packaged support entries) rather than source prose alone. `single-axis-soundness` enforces `TOOL GROUNDING`'s `(constitution)`/`(extension)` markers as the sole runtime annotation axis across live `*.md` files. `agents-symlinks-sync` verifies the `.agents/skills/` Agent Skills cross-tool view (Cursor / GitHub Copilot / Devin / OpenCode / Codex / Gemini per [agentskills.io](https://agentskills.io)) stays in sync with plugin sources (repair: `scripts/sync-agents-symlinks.sh`). `workflow-paths-sync` verifies `.github/workflows/claude-epistemic-review.yml` `paths` declares every protocol plugin so new protocols cannot silently miss the multi-perspective epistemic review trigger. `language-purity` surfaces Korean characters in project text files at warn-level under a Stage 1 surface posture; whitelist preserves intentional Korean regions (Korean READMEs, release notes, landing-page i18n, repo docs, Korean commit conventions, `graph.json` `satisfies` field, plugin-level `docs/`/`references/`). Details: [docs/verification.md](docs/verification.md)
...
- **Comment Review**: No delegation—main agent handles all pipeline phases. Composes /inquire × /gap × /contextualize; sub-protocol delegation rules apply when invoked. Channel-loop runs as a background Bun process (not Task delegation).
