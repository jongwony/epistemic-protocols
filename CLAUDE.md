...
- **Key**: Phase 0 calls `AskUserQuestion` for Mission Brief confirmation; Phase 1 gathers context guided by MBᵥ; Phase 2 for perspective selection (multiSelect: individual options; user-supplied Pᵦ auto-included, novel perspectives only proposed); Phase 5 classification preview + sufficiency check with `act` option; Phase 6-7 classify findings and execute fixes with peer verification; loop until user satisfied or ESC
