...
- **Key**: User-initiated; Phase 0 categorizes AI work; Phase 1 calls `AskUserQuestion` for entry point selection; Phase 2 uses `TaskCreate` for tracking; Phase 3 calls `AskUserQuestion` for comprehension verification (4 options: confirm / scenario example / view actual code / explain reasoning) with `TaskUpdate` and on-demand `Read`; user proposals ejected via `TaskCreate` to maintain comprehension focus
