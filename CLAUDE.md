...
│   └── skills/                        # report, onboard, dashboard, introspect, catalog, compose, sophia, curses, write, comment-review, review-ensemble, steer, misuse
...
**Secondary pattern hermeneutic spirals** (per `docs/structural-specs.md` §Pattern over Vocabulary): the linear cluster ordering above is the activation default; orthogonal to it, five inter-protocol spiral revolutions operate across distinct epistemic axes — Hermeneia↔Katalepsis (intent), Telos↔Syneidesis (goal), Aitesis↔Epharmoge (context), Prothesis↔Analogia (structure), Prothesis↔Prosoche (action — theoria/praxis). Each revolution has a forward arc and a backward Wirkungsgeschichte arc. Forward arc topology varies by axis: most use implicit temporal precedence; structure axis uses a dyadic graph.json advisory edge (`prothesis → analogia`); action axis uses an explicit triadic graph.json cascade `prothesis → aitesis → prosoche` (frame → inquire → attend). Backward arc consistently via Output Style nudge — action-axis Wirkungsgeschichte intake at Prothesis Phase 4 LOOP `J=extend` "review execution results" is the inscribed exception (see `prothesis/skills/frame/SKILL.md` and `prosoche/skills/attend/SKILL.md`); the action axis's triadic-forward / dyadic-backward asymmetry is intentional (framework patterns flow back to theoria directly without re-passing through Aitesis preparation).
...
19 static checks: json-schema, notation, directive-verb, xref, structure, tool-grounding, version-staleness, graph-integrity, spec-vs-impl, cross-ref-scan, onboard-sync, precedence-linear-extension, partition-invariant, catalog-sync, gate-type-soundness, artifact-self-containment, single-axis-soundness, agents-symlinks-sync, workflow-paths-sync. `artifact-self-containment` validates the packaged runtime contract view (`Skill.md` + plugin description metadata + packaged support entries) rather than source prose alone. `single-axis-soundness` enforces `TOOL GROUNDING`'s `(constitution)`/`(extension)` markers as the sole runtime annotation axis across live `*.md` files. `agents-symlinks-sync` verifies the `.agents/skills/` Agent Skills cross-tool view (Cursor / GitHub Copilot / Devin / OpenCode / Codex / Gemini per [agentskills.io](https://agentskills.io)) stays in sync with plugin sources (repair: `scripts/sync-agents-symlinks.sh`). `workflow-paths-sync` verifies `.github/workflows/claude-epistemic-review.yml` `paths` declares every protocol plugin so new protocols cannot silently miss the multi-perspective epistemic review trigger. Details: [docs/verification.md](docs/verification.md)
