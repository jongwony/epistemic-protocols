...
- **framing-readout-enforcement** — the glyph denylist is a regression guard over the surfaces where completion bars were observed, scoped by decision. The guard kernel is a drift guard — it catches the sentence going missing when the element is rewritten, and reaches no further, since no presence test separates prose that binds from prose that only looks like it. It pins a positive statement rather than a prohibition because the Output Style body is injected every turn, which is runtime motivating prose under `premise/instruction-authoring.md` §Prohibition Base Rate and White Bear Avoidance.
