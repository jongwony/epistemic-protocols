...
- **lean-definition** — a Definition block authored in Lean 4 (`docs/structural-specs.md` §Lean Notation) is a contract only if it elaborates, so reference resolution for such a block is the elaborator's rather than `gate-answer-reference`'s, which reads the DSL's `→ Stop →` arrows. Token predicates always run: no `sorry`, `admit`, `axiom`, `set_option`, `native_decide`, `implemented_by`, `extern`, `unsafe`, or metaprogramming command in the block, the canonical GROUND, or a proofs module; no `theorem` proved inside the block; GROUND identical to `lean/EpistemicProtocols/Ground.lean`; a proofs module importing only its contract and GROUND; where a protocol keeps a Theorem module (`lean/EpistemicProtocols/<Namespace>/Theorems.lean`), that module importing its contract and nothing else, stating inside that namespace alone, declaring nothing (the audit imports it, so an axiom or proof there is read from the environment), and being the one place the protocol's theorems are stated; no theorem stated before GROUND, where no audit reads it; no unaccounted `.lean` file under `lean/`. With `lean` and `lake` reachable it elaborates the block standalone, generates the contract modules (`lean-contract.js`), runs `lake build`, and reads a per-block audit from elaboration: each stated theorem must follow from the proved theorem of its name alone, the proofs module's declared theorems (read from the environment, private ones included) must equal the stated set (read from the Theorem module where one exists), no project module may declare an axiom, and transitive axioms stay within `propext`, `Classical.choice`, `Quot.sound`. Text matching cannot establish those, which is why they are read from elaboration. With no toolchain it warns that elaboration was not run rather than passing; the `lean` workflow makes one reachable, so CI never takes the warn path. Whether the model realizes the elaborated contract at runtime stays with `/realize`.
