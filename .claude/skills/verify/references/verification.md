...
- **framing-readout-enforcement** — the glyph denylist is a regression guard over the surfaces where completion bars were observed, scoped by decision. The guard kernel is a presence test over live prose only: the kernel sentence inside the Cognitive work element's own span, comment regions and fenced blocks removed first. It pins a positive statement rather than a prohibition because the Output Style body is injected every turn, which is runtime motivating prose under `premise/instruction-authoring.md` §Prohibition Base Rate and White Bear Avoidance.
