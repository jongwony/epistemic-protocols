...
- **lean-definition** — a Definition block authored in Lean 4 (`docs/structural-specs.md` §Lean Notation) is a contract only if it elaborates, so reference resolution for such a block is the elaborator's rather than `gate-answer-reference`'s, which reads the DSL's `→ Stop →` arrows. The check always rejects `sorry` and a declared `axiom` from the block's code, and elaborates the block with a core Lean toolchain when one is reachable; with none, it warns that elaboration was not run rather than passing. Whether the model realizes the elaborated contract at runtime stays with `/realize`.
...
| `lean-definition` | Prove or remove the `sorry`; restate an `axiom` as an `opaque` judgment or a parameter; fix the elaboration error the message quotes (extract the fenced block and run `lean` on it) |
