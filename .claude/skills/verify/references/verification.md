...
- **lean-definition** — a Definition block authored in Lean 4 (`docs/structural-specs.md` §Lean Notation) is a contract only if it elaborates, so reference resolution for such a block is the elaborator's rather than `gate-answer-reference`'s, which reads the DSL's `→ Stop →` arrows. The check always rejects `sorry` and a declared `axiom` from the block's code and its proof file, and a `theorem` proved inside the block: a proof is verification, so the block states the signature in a doc comment and `lean/<plugin>/<skill>.lean` proves it. The proof file must open with the block verbatim and prove exactly the stated signatures, matched by name and whitespace-normalized text, so the runtime statement and the checked one cannot drift. It then elaborates the proof file with a core Lean toolchain when one is reachable; with none, it warns that elaboration was not run rather than passing. The `lean` workflow runs the verifier with the pinned toolchain, so CI never takes the warn path. Whether the model realizes the elaborated contract at runtime stays with `/realize`.
...
| `lean-definition` | Prove or remove the `sorry`; restate an `axiom` as an `opaque` judgment or a parameter; move an in-block proof to `lean/<plugin>/<skill>.lean`; after editing the block, rewrite the proof file's prefix from it and align each stated signature with its proof; fix the elaboration error the message quotes (`lean lean/<plugin>/<skill>.lean`) |
