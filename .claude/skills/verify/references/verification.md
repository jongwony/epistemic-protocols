...
- **framing-readout-enforcement** — the Ink framing readout is defined against a completion bar, and the ban is scoped to the two surfaces where a bar glyph can only be one: the core protocol files, whose loop completion is the invented quantity §Rendering Judgment's length discriminant refuses, and the per-turn Ink bodies, which state the rendering rules rather than demonstrating them. §Rendering Judgment does sanction a drawn length for a *measured* quantity — so this check is not a claim that such a length is disallowed anywhere, and whether one declares its scale correctly is context-dependent meaning that routes to `/realize` or review under `AGENTS.md`'s static-verification boundary. The guard kernel is anchored *inside* the Cognitive work element rather than anywhere in the file, so the invariant cannot migrate into a comment or an opposite instruction and still pass.
