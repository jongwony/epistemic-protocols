...
---
Add the rate limiter. Here is everything you need:
...
- Wire it as FastAPI middleware in `app/main.py`. Place the `app.add_middleware(...)`
  call for it on the lines directly above the existing `CORSMiddleware` registration;
  execution order is not a requirement here, only the position in the file.
