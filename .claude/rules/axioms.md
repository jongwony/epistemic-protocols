...
**Configurable relay/constitution boundary**: The relay/constitution boundary is not fixed at protocol definition time — User can shift it by creating or revoking Standing authority. TOOL GROUNDING entries map to this model: `(extension)` markers identify gates delegated to Standing authority (relay-eligible); `(constitution)` markers identify gates requiring Active authority. Conditional Constitution-to-Extension specialization is recorded as separate `(extension)` entries within the same phase.

**A5 coordination**: Standing/Active is an authority-source dimension; A5's regret dimension (bounded/unbounded) is coextensive with the operational classification — Extension implies bounded regret, Constitution implies unbounded regret. The two dimensions are conceptually distinguishable at the meta/design layer (per Tier Factorization §axis_α × axis_β observed instances) but collapse at the runtime annotation layer; TOOL GROUNDING's `(extension)`/`(constitution)` markers are the authoritative single axis. Conditional elision is recorded as separate `(extension)` entries within the same phase rather than as a parallel regret-axis annotation.
...
**Transitional state caveat (bounded-zone Constitution gates)**: The coextension claim above (Extension ↔ bounded regret, Constitution ↔ unbounded regret) holds at the fully-migrated single-axis state. In the current transitional state, *bounded-zone Constitution gates* — Telos GoalContract, Horismos BoundaryMap, Syneidesis decision pattern, Hermeneia Phase 2 articulation — carry bounded regret via retry semantics: incorrect output is correctable at the next protocol invocation. They remain `(constitution)` at runtime as capability statements pending Stage 2 corroboration toward split-Extension migration. See `project-profile-calibration.md §Scope Boundary` for the bounded-zone enumeration. The consistency guard for `(extension)` assignment — that an entry may only be assigned `(extension)` when regret under elision is bounded — applies prospectively to new entries; the bounded-zone Constitution gates remain Constitution until per-protocol migration converges.
