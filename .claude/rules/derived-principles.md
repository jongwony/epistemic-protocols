...
**Falsifiability and counter-evidence**: Outcome Equivalence is falsifiable when a realization fails the completeness assumption. Known counter-example (historical, resolved in frame v7): Prothesis v6 TOOL GROUNDING `Await` depended on SubagentStop (a Claude Code substrate guarantee); platforms without equivalent teammate-termination semantics partially refuted Outcome Equivalence at that realization boundary. Frame v7 removed the execution surface (compile → handoff → STOP), resolving the instance; the pattern remains the falsifiability template. When such a failure occurs, A4 survives intact (the definition remains autonomous); Outcome Equivalence fails locally (that specific realization does not preserve outcomes).
