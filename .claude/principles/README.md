...
| False-positive gating guard family — architectural-inscription vs type-refinement disambiguation | `epharmoge/skills/contextualize/SKILL.md` Rule 17 (Significant requires demonstrable behavioral impact) + `epistemic-cooperative/skills/probe/SKILL.md` Rule 17 (Structural-change calibration) + CLAUDE.md `/probe` section inline distinction — issue #295 N=3 family member (`/contextualize` retained; `/introspect` and `/curses` resolved via PR #294) | Stage 2 monitoring: false-positive gating instance frequency in `/contextualize` Phase 0 across architectural-inscription-adjacent PR contexts; reduction of ex-post Rule 15 retraction incidents in subsequent Epharmoge invocations as guard-effectiveness signal; family N+1 instance accumulation as systematic post-vocabulary calibration sweep readiness signal. **Promotion-readiness threshold**: 3 false-positive-free `/contextualize` activations in architectural-inscription-adjacent PR contexts post-Rule 17 inscription, OR 1 documented false-positive recurrence as retraction signal (triggers re-expansion of Rule 17 with concrete corrective vocabulary). Surface re-expansion if `/contextualize` continues to over-classify type-refinement-only PRs as Significant despite Rule 17 inscription. | 2026-05-01 |
