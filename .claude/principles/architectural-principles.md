...
A **binding** routing relation — one that constrains where an activation may proceed, as opposed to a suggestion the user is free to disregard — is not carried by this observational layer. It is not inscribed as a named destination either. Composition is by type: each protocol declares the deficit it takes and the resolution it produces, and where a protocol carries the COMPOSITION operator those domains and codomains compose with the resolution emergent from session context. A protocol therefore states its own scope and identity — the guard, transition, or consumption point where it actually binds, read from the session's own context — and does not inscribe which protocol comes next, nor condition its activation on whether a sibling has run. Both would fix at authoring time an answer that varies with the context accumulated by the time the question is live: whether a sibling's earlier run actually discharged the matter depends on what that run produced, which is knowable only then. Where two protocols' scopes would both reach a situation, what settles it is the activation each protocol already gates on its own: every protocol is user-invocable, and AI-guided activation surfaces for user constitution before it proceeds. No separate cross-protocol collision mechanism is defined, and none is wanted — one would be the same pre-inscribed structure under another name, and it would have to live outside every protocol's own formal blocks to do its job. A pre-inscribed suppression or routing rule answers in the user's absence what that activation already puts to them, which is what makes it scaffolding rather than structure. Each protocol's formal blocks are the sole authority for what constrains its own activation; there is no separate cross-protocol structural registry.
