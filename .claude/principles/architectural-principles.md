...
A **binding** routing relation — one that constrains where an activation may proceed, as opposed to a suggestion the user is free to disregard — is not carried by this observational layer. It lives protocol-locally, stated as a condition on the owning protocol's own scope and read from the session's context: a precondition guard's `requires:` clause, a transition's entry condition, a consumption-time read of a prior artifact. A protocol describes its own scope and identity, and never names a sibling's activation as a condition on its own — where two protocols would otherwise ask the same question, each declines because its own scope condition finds the matter already settled in context. That routes the relation through context and utterance, which stay current, rather than through a sibling's name, which goes stale the moment either protocol moves. Each protocol's formal blocks are the sole authority for what constrains its own activation; there is no separate cross-protocol structural registry.
