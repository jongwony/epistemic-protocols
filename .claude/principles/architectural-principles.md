...
A **binding** routing relation — one that constrains where an activation may proceed, as opposed to a suggestion the user is free to disregard — is not carried by this observational layer. It lives protocol-locally: inscribed at the specific guard, transition, or consumption point in the owning protocol's own SKILL.md where it actually binds (a precondition guard's `requires:` clause, a suppression check at dispatch, a consumption-time read of a prior artifact). Each protocol's formal blocks are the sole authority for what constrains its own activation; there is no separate cross-protocol structural registry.
