...
- **Runtime motivational prose** — Rules, Phase prose, agent system prompts, and other sections that direct what the LLM does at execution time. The prohibited target becomes the foreground attractor during application, so prefer positive rationale ("X IS Y because Z"). Use negative prohibition only when the boundary meaning collapses without it (the rewrite test below).
- **Diagnostic substrate** — Anti-patterns, tests, checklists framed as failure cases, audit findings, and review vocabulary. The section's explicit role is naming failure modes for detection, regression coverage, or review — the negative form *is* the section's content, not a runtime instruction. Negative or failure-case wording is compliant by purpose here, provided the section role is visible (e.g., the heading `## Anti-patterns` or equivalent framing makes the diagnostic intent explicit).

**Rewrite test** (load-bearing check, applies to both placements): a White Bear rewrite is valid when the positive restatement preserves both **directive force** (the instruction still binds the same behavior) and **boundary meaning** (the discriminant boundary or detection signal survives). If removing the prohibition would erase either — the *attractor vs discriminant* signal above, or a diagnostic detection cue — preserve the original phrasing or relocate the failure case into a diagnostic section. The placement distinction provides relocation as an option when in-place rewrite would lose meaning.
