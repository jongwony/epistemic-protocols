...
accepted(a)        = ∃ r ∈ Σ.dispositions : aspect(r.mismatch) = a ∧ r.disposition = Keep ∧ r.judgment ∈ {Some(Upheld), None}
                     ...
                     -- and Adapt is out of its reach until the protocol is re-entered. Re-entry is the correction path, shared with
                     -- judgment_relay_overruled: a fresh Σ makes accepted(a) false again and the aspect returns to the gate
...
| Both Phase 1 relays exit the same way | a relay-closed aspect leaves `pending(Σ)` and reaches no further Qc this run; re-entering Epharmoge on the same result rebuilds `Σ` and returns it to the gate | One correction path covers a disputed retraction and a disputed carry alike |
