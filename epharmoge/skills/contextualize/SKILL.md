...
pending(Σ) = Set(Mismatch) where registered task status ≠ completed  -- a routed/ambiguous mismatch never enters pending(Σ); only certificate-passing mismatches are registered
             ...
             -- status↔ledger equivalence: task(m).status = completed ⟺ ∃ r ∈ Σ.dispositions : r.mismatch = m. The dispose step writes the record and completes the task as one action (Phase 2, TOOL GROUNDING dispose), which is what keeps the two readings of "resolved" the same reading
...
Judgment    ∈ {Upheld, Overruled}                              -- EPISTEMIC axis: does the flagged aspect genuinely fail to fit X?
                                                               -- an ESTABLISHED verdict only: reachable via a user answer at Qc or via judgment_relay's cited basis.
                                                               -- No internal producer may write it — see DispositionRecord's judgment_by, which carries `unjudged` for exactly that reason
...
well_formed(j, d) =                                            -- governs the USER ANSWER space only (what Qc may present); the ledger has its own constraint below
...
DispositionRecord = { mismatch: Mismatch, judgment: Option(Judgment), judgment_by ∈ {user, relay, unjudged},
         ...
         --   judgment_by = relay     ⟹ judgment = Some(Upheld) ∧ basis ≠ ""                -- judgment_relay, basis cited
...
Phase 1 → keep_all_remaining (bulk exit): the user declares the remaining mismatches acceptable as-is rather than answering them one at a time  -- guard: pending(Σ) ≠ ∅ ∧ the declaration is a free-response at Qc (not a peer option in the presented set — it disposes of the whole axis instead of taking a position on one mismatch). EVERY m ∈ pending(Σ) is closed with DispositionRecord { judgment = None, judgment_by = unjudged, disposition = Keep, assigned_by = user, basis = the bulk declaration } and TaskUpdate(m, completed); judgment is None because a bulk acceptance disposes without judging each mismatch, and recording Upheld or Overruled per item would invent verdicts the user never gave. R_final := Some(Λ.R); deactivate. Convergence follows from disposed(a) exactly as the per-mismatch path does [Tool]
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by, disposition = d, assigned_by = user, basis } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs — this ordering is what makes the exactly-one ledger invariant hold on the withdrawal path] →
...
User can exit at Phase 1 by declaring the remaining mismatches acceptable as-is (Phase 1 → keep_all_remaining, which closes each with an unjudged Keep), or by Esc.
...
Convergence evidence: At adjudicated(Λ.R, X), present the transformation trace ranging over Σ.dispositions — ONE ledger, because every close writes a record there regardless of who assigned it (user, certificate, or loop), so the trace no longer has to be assembled from parallel lists. For each r ∈ Σ.dispositions show (aspect(r.mismatch) → r.judgment → r.disposition), reading:
  ...
  (unjudged) → Keep             : closed by a bulk keep-all-remaining declaration — disposed without an individual verdict
...
1. **`(Overruled, Keep)`**: the flagged aspect fits after all — `R_final := Some(Λ.R)`, no adaptation, no re-scan
...
| Early exit | User can keep all remaining as-is at any Phase 1 — compiled as `Phase 1 → keep_all_remaining`, closing each with an unjudged Keep | Full control over review depth, with the ledger showing they were accepted rather than judged |
