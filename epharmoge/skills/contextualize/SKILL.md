...
merge  = Mismatch × Mismatch → Mismatch, combining two readings of ONE claim. The carried element is the left side and stays the element
                 ...
                 --   unrepaired : the disjunction — a set-only stamp (below), so a claim stamped on either side stays stamped
                 --   certificate, value_space : the carried element's, untouched, and for one reason — both are fitted to kind_binding, which did not move.
                 --                At Phase 0 the element may be carrying neither yet, and the certify and bind_value_space legs reach it as they reach every
                 --                other element of Mᵢ; at the re-scan a merged element already holds both from its own registration and is not in Mₑ, so
                 --                nothing re-runs them on it. The field list is TOTAL: merge names every field of Mismatch and leaves none to the caller
