...
(R, X)
  ...
  → surface(fit_scoped_mismatch, mode) -- present the mismatch with its fit basis and evidence. The mode is per AXIS, not per mismatch: an axis the evidence has settled surfaces as_report (basis cited, turn NOT yielded), an axis still open surfaces as_inquiry (the gate fires and yields). A settled Upheld reports the judgment and still inquires the disposition; a settled Overruled reports both and inquires neither. Writing one mode into this step would fix by the step's NAME what only the axis's state can fix
...
pending(Σ) = Set(Mismatch) where registered task status ≠ completed  -- a routed/ambiguous mismatch never enters pending(Σ); only certificate-passing mismatches are registered
             ...
             -- status↔ledger equivalence, SCOPED TO REGISTERED MISMATCHES: ∀ m registered, task(m).status = completed ⟺ ∃ r ∈ Σ.dispositions : r.mismatch = m. Every close of a REGISTERED mismatch writes the record and completes the task as ONE action — the Phase 2 dispose step (TOOL GROUNDING dispose), the Phase 1 judgment_relay_overruled close, and the keep_all_remaining bulk close alike — which is what keeps the two readings of "resolved" the same reading. A close that did either half alone would break the equivalence wherever it fired
             -- the scoping is not a hedge: a certificate-assigned close (Route, Residual) fires BEFORE registration, so its mismatch has a ledger entry and NO task at all — there is nothing to complete, and an unscoped ⟺ would be false for exactly the mismatches the fail-closed certificate is designed to keep out of pending(Σ). The Ledger invariant under MODE STATE is the unscoped statement that does hold: every close appends exactly one record, registered or not
...
judgment_settled : Mismatch × F → Option(Judgment)             -- judgment-level entropy → 0 with a citable basis (Rule 10); evaluated against the EVALUATED TARGET Λ.R
              ...
              Some(Overruled) when the evidence admits no reading under which aspect(m) FAILS to stand — the aspect carries an
                              ...
                              the no-contrary-reading bar. Membership is where the evidence is FOUND, never the condition itself: AssessFit places an
                              entry by classifying, and classifying is a weaker act than adjudicating, so an entry whose evidence still admits a reading
                              under which the aspect fails to stand yields None and the judgment stays the user's
              None            otherwise: the evidence admits more than one reading, so the judgment is the user's
              -- TWO-SIDED by construction. The Overruled arm is the reader Λ.fit_map.fit_justifications otherwise lacks: AssessFit
...
ApplicabilityVerdict = { target: R_final, dispositions: List(DispositionRecord), scan_count: Nat }
         ...
         --                  keep_all_remaining, Phase 0 → confirm_no_mismatch / deactivate, or the ESC exit). judgment_relay_overruled is
         --                  NOT among them: it closes a mismatch, and reaches a terminal only by EMPTYING pending(Σ) — at which point the
         --                  LOOP convergence step already listed does the binding. A close whose terminality depends on pending(Σ) is
         --                  listed by that condition, never by its own name
         ...
         --                                          which is why the counter exists at all rather than accumulating unread
         -- The last two are bound identically at EVERY terminal — they read accumulated state rather than anything path-specific — so a
         --   terminal states only its own R_final and assembles the rest from Σ. A terminal that bound target alone would satisfy the
         --   declared type with an empty list and a zero count, which is exactly the shape of a clause no step produces
...
Phase 1: Mᵢ_passed → TaskCreate[all certificate-passing initial mismatches] → pending(Σ) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →  -- register all certificate-passing initial mismatches, surface selected mismatch with fit basis [Tool]; reached only when Mᵢ_passed ≠ ∅. The three outcomes take DIFFERENT paths; the gate is not on the common path:
         [Some(Overruled)] judgment_relay_overruled (below) closes Mₛ — NO Qc, no Stop, no turn yielded; control returns to SelectNext over the remaining pending(Σ)
         [Some(Upheld)]    judgment_relay_upheld (below) settles the judgment half, then Qc(Mₛ scoped by the CURRENT Λ.fit_map, evidence, judgment_state) → Stop → A = (j, d) for the disposition
         [None]            Qc(Mₛ scoped by the CURRENT Λ.fit_map, evidence, judgment_state) → Stop → A = (j, d)  -- the full two-axis set, both halves user-answered
Phase 1 → judgment_relay_upheld (mismatch-local): judgment_settled(Mₛ, Λ.fit_map) = Some(Upheld) — the evidence for ¬warranted(aspect(Mₛ), Λ.R, X) admits no reading under which the aspect stands warranted; judgment-level entropy → 0 with a citable basis (Rule 10)  -- relay j := Upheld with the basis cited: judgment_state := Some((Upheld, basis)), which the close downstream reads as judgment_by = relay. Drop the Overruled/Keep pairing from the presented set and CONTINUE to Qc for the disposition; the turn is NOT yielded for the judgment half. This NEVER authorizes adapt or discard: the repair axis is still open and two of its three values change the artifact, so Qc still fires and still yields turn (invariant: relay closes only what leaves the artifact unchanged). When judgment_settled(Mₛ, Λ.fit_map) = None, judgment_state := None, the full two-axis set is presented, and j is user-answered (judgment_by = user)
Phase 1 → judgment_relay_overruled (mismatch-local, CLOSES the mismatch): judgment_settled(Mₛ, Λ.fit_map) = Some(Overruled) — the evidence admits no reading under which aspect(Mₛ) FAILS to stand, read off that aspect's own AspectFit entry in Λ.fit_map.fit_justifications and the evidence it cites. The entry is where that evidence LIVES, not the trigger: membership whose evidence still admits a contrary reading leaves judgment_settled = None and the gate fires as usual (TYPES). well_formed(Overruled, d) leaves d = Keep as the sole pairing, so the answer set has collapsed to one and no gate is owed (Rule 10)  -- REQUIRED STEP FIRST, not a courtesy: report the flagged aspect, that it is being retracted, and the cited fit evidence, as text. That report is what leaves the basis in front of the user while Λ.R is still untouched, so a retraction they disagree with costs one sentence to reopen — it is what makes this close correctable, and therefore relay rather than a silent decision. THEN close Mₛ with DispositionRecord { judgment = Some(Overruled), judgment_by = relay, disposition = Keep, assigned_by = relay, basis = the cited fit evidence } AND TaskUpdate(Mₛ, completed) as ONE step (the same ordering discipline as dispose, so Mₛ leaves pending(Σ) before anything else runs). Λ.R is untouched, no Mₑ, no re-scan, the turn is NOT yielded — and R_final is NOT bound here, for the same reason the Phase 2 Keep arm does not bind it: this close is terminal only WHEN it empties pending(Σ), which is a fact about pending(Σ) at that moment and not a property of the kind of close this is, and while any mismatch is still pending a later Adapt would advance Λ.R past whatever was bound. The binding stays with the step that owns it — the convergence step (LOOP), reached from here as retraction convergence exactly when this close empties pending(Σ). Continue to SelectNext over the remaining pending(Σ) — or, when this close empties it, to convergence (CONVERGENCE: retraction convergence) [Tool]
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, basis = that relayed basis or the user's stated one } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs — this ordering is what makes the exactly-one ledger invariant hold on the withdrawal path. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1 instead, which is why assigned_by is user here] →
         ...
         [d = Keep] non-mutating adjudication — no adapt, no Mₑ, no re-scan; Λ.R stands as whatever earlier Adapt dispositions left it as, and R_final is NOT bound here: this arm may be answered with other mismatches still pending, and a later Adapt would advance Λ.R past whatever was bound. The single binding happens at the convergence step (LOOP)
         [d = Discard(replacement)] → Phase 2 → withdraw (below)
         -- only Adapt mutates the evaluated target AS A SUCCESSION of it (Λ.R := R'), which is what triggers the re-scan + registration-time certify of emerged mismatches. Discard also touches the artifact — the discard transform withdraws it — but produces no successor to re-evaluate, which is why it terminates instead of re-scanning [Tool]
...
After Phase 2, re-scan is disposition-keyed, not answer-keyed:
  ...
  Keep                  → non-mutating; breeds no Mₑ; no re-scan; binds no R_final (the convergence step does)
...
Recompute the fit map over pending(Σ) and re-bind Λ.fit_map to it before selecting the next surfaced mismatch, even when Mₑ_passed = ∅. FIT-MAP SUCCESSION, the exact counterpart of TARGET SUCCESSION: EVERY reader takes Λ.fit_map at the moment it reads — selection, judgment, and the gate presentation alike — never a fit map computed against an earlier Λ.R. What binds is the reading moment, not the kind of act at it; naming reader kinds instead of all readers is exactly how a reader gets left behind. A relay on stale fit evidence would retract a mismatch on grounds that no longer describe the current target, and without a gate; a gate presenting stale fit evidence would rest the user's disposition on adaptation options computed for a target that no longer exists.
CHECKED IN THIS ORDER, and the order is load-bearing rather than stylistic:
...
Step 1 runs first so a re-registered aspect can never be read past: disposed(a)'s second conjunct already excludes it, and this ordering means the two conditions cannot both hold and leave the winner unstated. THIS is the step that fixes R_final on the ordinary path, and it is the only one: a Keep answered mid-loop must NOT bind it, because a later Adapt still advances Λ.R and the verdict would carry the pre-adaptation target. The withdrawal path is excluded because Phase 2 → withdraw has already bound R_final to the replacement, which is precisely where the two come apart
...
disposed(a)        = (∃ r ∈ Σ.dispositions : aspect(r.mismatch) = a) ∧ a ∉ { aspect(m) : m ∈ pending(Σ) }
                     ...
                     -- SECOND CONJUNCT — DISPOSITION IS TARGET-RELATIVE, the ledger is not. A record is written about the target as it stood
                     -- when the close happened; TARGET SUCCESSION then advances Λ.R past it. So an aspect adapted at one cycle and re-registered
                     -- at the next — the ordinary case of an adaptation that did not fully land — would otherwise read as disposed from its own
                     -- stale record while its new task sits in pending(Σ), and adjudicated(Λ.R, X) would hold with a Constitution gate still owed.
                     -- Re-registration un-disposes the aspect, which is the same reading progress(Λ) already takes: resolved_tasks counts the
                     -- registered task, and a re-registered aspect carries a NEW, uncompleted one
                     ...
                     -- label, a documented low-probability assumption, not foreclosed by the type. That collision is DISTINCT from re-registration
                     -- above: there two different aspects share a label, here one aspect legitimately returns
...
--   the proxy is sound because of the status↔ledger equivalence at pending(Σ), which is scoped to registered mismatches for the same reason
--   this proxy is: a completed task and a written DispositionRecord are the same event seen from two sides, since dispose performs both as one
--   step wherever a task exists. Certificate-assigned closes carry no task and are covered by disposed(a) directly, not by this proxy.
--   Without that equivalence the two readings of "resolved" could drift apart
...
**Fit-map scope**: Support for mismatch selection and adaptation direction, and the place a judgment's cited evidence is found — it classifies, and never creates, suppresses, or decides (Rule 15; open-condition bound at UX Safeguards: Fit-map cap).

### Phase 1: Mismatch Surfacing

**Register all certificate-passing mismatches (`Mᵢ_passed`) as Tasks** (TaskCreate), then take the next pending mismatch selected by `SelectNext`. If the fit evidence has already settled its judgment as `Overruled`, report the retraction and its cited basis and close it as relay (`judgment_relay_overruled`) — no gate is owed, because `well_formed` leaves one pairing and it edits nothing. Otherwise **present** it via Cognitive Partnership Move (Constitution). Routed/ambiguous mismatches never enter `pending(Σ)` (fail-closed certificate).
...
2. **`(Upheld, Keep)`**: the mismatch stands and is accepted anyway — the result is left as whatever earlier adaptations made it, not the original; note the fitness assumption accepted, no re-scan (`R_final` is bound once, at convergence, so a later adaptation still counts). Recorded distinctly from arm 1: this is an accepted residual, not a retracted finding
...
| Relay retraction is not immune | a relay-assigned Keep (`assigned_by = relay`) carries no user acceptance, so it grants no immunity across TARGET SUCCESSION: after an Adapt advances Λ.R, the aspect is evaluated again against the new target and the new Λ.fit_map | The retraction was the AI's, and it was classified relay precisely because the user can still re-derive and re-open it — immunity would foreclose the correction the classification rests on |
...
1. **AI-guided, user-judged**: AI detects the applicability mismatch; the judgment is the user's wherever it turns on what they weigh, presented via Cognitive Partnership Move (Constitution) at Phase 1. Where the evidence instead fixes one answer and its basis is cited to the user, the AI relays that answer — which is not the AI taking the judgment but the AI having none to take, and is why a relayed close still reports its basis
