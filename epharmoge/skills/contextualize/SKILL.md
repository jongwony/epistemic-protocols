...
AI detects applicability mismatch after execution OR user calls `/contextualize`. Detection is silent (Phase 0). Surfacing requires user interaction via Cognitive Partnership Move (Constitution) wherever the disposition is still open (Phase 1) — which is every path but one: `judgment_relay_overruled` surfaces the retraction and its cited basis as relay and proceeds without yielding the turn, because `well_formed` has already closed the disposition to a `Keep` that edits nothing.
...
**Action**: At Phase 1, present mismatch evidence via Cognitive Partnership Move (Constitution) — except where the fit evidence has already settled the judgment as Overruled, which is reported as relay and closes without a gate (`judgment_relay_overruled`).
</system-reminder>
