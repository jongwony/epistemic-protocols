...
Mismatch = { id: MismatchId, aspect: String, description: String, evidence: String, severity: Severity, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate, value_space: Option(ValueSpace) }
                 ...
                 --   clause). kind_binding is IDENTITY and cannot move. unrepaired is stamped once at the re-scan that produced it and never
                 --   re-stamped
...
identity(m: Mismatch) = (m.kind_binding.positive_predicate, m.kind_binding.evidence)
                 -- WHAT A SAME-CLAIM READING IS TAKEN FROM, and the only thing it may be taken from: the claim ITSELF rather than a name for
                 --   it, together with what that claim stands on — which is what makes the reading a verification against the binding rather
                 --   than one more label comparison
                 -- NOT A KEY, and the pair is never settled the same by string equality: the predicate is authored afresh at each detection, so
                 --   one claim can be worded two ways and two claims can be worded alike. WHETHER TWO READ AS THE SAME CLAIM IS SETTLED AT THE
                 --   TURN, on the accumulated context and what the user has actually said this run. The contract fixes what the reading is taken
                 --   from and never what it comes out as — what counts as the same problem is the question this protocol exists to put to the
                 --   user, and it is also the reading that moves as the model reading it gets better, so freezing it here would pin it to one
                 --   moment's capability
                 -- WHERE THE READING DOES NOT RESOLVE, take the one that leaves the mismatch in front of the user: it registers and is surfaced.
                 --   The two errors are not symmetric — reading two claims as one closes a mismatch the user was never shown, which is the
                 --   failure this keying exists to remove, while reading one claim as two costs a question that was owed anyway. Same direction
                 --   as the certificate's fail-closed order and as re-registration un-disposing an aspect (disposed(a), CONVERGENCE)
                 -- WHAT IT DOES: matches a re-detected mismatch to the registration it succeeds — accepted(i) and
...
pending    = Set(Mismatch) where entry(m).status ≠ completed  -- a view over the carrier C, reached at Λ.carrier, which is where registration status lives; Σ holds the ledger and the scan count and never held this. A routed/ambiguous mismatch never enters pending; only certificate-passing mismatches are registered
             ...
             -- NOT keyed on identity, and this is the load-bearing half: identity(m) (TYPES) matches a re-detected mismatch to the registration it SUCCEEDS, and a match discharges nothing. A re-registration always opens its own entry and sits in pending until its own close — whether the return reads as the same claim on unmoved ground (unrepaired(m): the prior disposition did not take) or as a residue, that claim narrowed onto ground the adaptation moved (owed its own surfacing).
...
accepted(i)        = ∃ r ∈ Σ.dispositions : identity(r.mismatch) reads as the same claim as i ∧ r.disposition = Keep ∧ r.judgment ∈ {Some(Upheld), None}
                     ...
                     -- and carries; a residue — that claim narrowed onto ground the adaptation moved — is a DIFFERENT registration and
...
6. If certificate-passing mismatches remain: record `Mᵢ_passed` with aspect, description, evidence, severity (per Rule 12 — behavioral-impact qualifier assessed against current-session task graph), kind_binding, certificate, and fit-map placement — proceed to Phase 1
...
- Re-evaluate the advanced target `Λ.R` against `X` for remaining AND **newly emerged** mismatches. `Mₑ` excludes only what already sits in `pending` — an aspect closed earlier in this run re-registers, and Phase 1 closes it by `keep_carried_forward` wherever the user already accepted it
