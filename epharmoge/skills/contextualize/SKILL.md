...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) [split where non-atomic] → init_occurrence(m) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → ∀m ∈ Mᵢ_passed: m.value_space := Some(bind_value_space(m)) → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
...
Mismatch = { id: MismatchId, aspect: String, description: String, evidence: String, severity: Severity, origin: Origin, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate, value_space: Option(ValueSpace) }
                 ...
                 --   is narrower than the field: a mismatch IN pending(Σ) always carries Some, because registration follows bind_value_space on both scans. Qc and
...
identity(m: Mismatch) = (m.kind_binding.positive_predicate, m.kind_binding.evidence)
                 ...
                 --   locates a predecessor and never completes a new registration's task (status↔ledger equivalence, pending(Σ))
...
pending(Σ) = Set(Mismatch) where registered task status ≠ completed  -- a routed/ambiguous mismatch never enters pending(Σ); only certificate-passing mismatches are registered
             ...
             -- NOT keyed on identity, and this is the load-bearing half: identity(m) (TYPES) matches a re-detected mismatch to the registration it SUCCEEDS, and a match discharges nothing. A re-registration always opens its own task and sits in pending(Σ) until its own close — whether the return matched an earlier record exactly (unrepaired(m): the prior disposition did not take) or only on the predicate (a residue owed its own surfacing). Reading this equivalence over a record written for an earlier registration is what let a re-detected mismatch be declared complete without ever being surfaced
...
RepairSite     = the locus a resolution projects onto Λ.R when it is EXECUTED — produced by adapt/discard from a disposition the user has already
                 answered, and by nothing earlier. It is a projection of a settled answer, never an input to one
                 -- IT TYPES NOTHING UPSTREAM: it does not individuate a mismatch, does not decide how many mismatches there are, does not enter
                 --   identity(m), does not gate registration, and does not appear in any convergence predicate. A mismatch is individuated by what
                 --   it claims and what that claim stands on; where a repair would land is not part of that and asking it first is the failure
                 --   Rule 23 names
                 -- carried as a runtime value of the executing step, so it holds no field in Mismatch, KindBinding, or DispositionRecord
...
disposed(a)        = (∃ r ∈ Σ.dispositions : aspect(r.mismatch) = a) ∧ a ∉ { aspect(m) : m ∈ pending(Σ) }
                     ...
                     -- close, so the return opens its own task and sits in pending(Σ), which is what fails the second conjunct while it is open.
...
- Re-evaluate the advanced target `Λ.R` against `X` for remaining AND **newly emerged** mismatches. Bind and split each detected mismatch first, then form `Mₑ`: it excludes only what already sits in `pending(Σ)`, matched by `identity(m)` rather than by label. **A detection that IS already pending is not discarded** — it refreshes that registration's current reading (`aspect`, `description`, `evidence`, `severity`) and its task (`subject`, `description`, `activeForm`), keeping `id`, `kind_binding`, `origin` and `unrepaired`; skipping that leaves a registration recorded under a reading no longer flagged, and `disposed` never closes for the one that is (LOOP, Rule 22). For what does enter `Mₑ`, so a residue whose evidence has moved is not the pending one and registers on its own. A mismatch closed earlier in this run re-registers too — the match against the ledger tells the protocol what the return **is**, never that it is already done — and Phase 1 closes it by `keep_carried_forward` only where the user's `Keep` answered that same identity
...
| Each Phase 1 relay says where its correction lives | a relay-closed mismatch leaves `pending(Σ)` and reaches no further Qc this run — that registration, not every mismatch sharing its label. A disputed CARRY reopens by re-entering on the same result (`accepted(i)` reads `Σ`, rebuilt at activation). A disputed RETRACTION has no such reset — `judgment_settled` reads `Λ.fit_map`, derived from `(Λ.R, X)`, so the same input reproduces the same close, and correcting it is the user's interrupt | The user is told what to reach for, rather than trying a reset that moves only one of the two guards |
