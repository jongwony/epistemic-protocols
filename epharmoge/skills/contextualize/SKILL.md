...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
      ...
      [d = Adapt(direction)] adapt → R' → Λ.R := R' → Eval(Λ.R, X) → bind_kind each [split per site] → match_identity each → Mₑ (those whose identity(m) is not already pending) → ∀m ∈ Mₑ: stamp origin from that match → certify(m, local_claims) → keep(status = pass) → Mₑ_passed → Register(Mₑ_passed) → AssessFit(Λ.R, X, pending) → F' → (loop: back to Phase 1 above — judgment half, then disposition half — until contextualized)
...
Origin ∈ {Initial, Emerged(aspect), Persisting}                -- mismatch provenance: initial scan, spawned by adapting parent aspect, or a mismatch a closed record
                                                               ...
                                                               -- record shares this binding's predicate AND site, Emerged(adapted_aspect) where none does.
                                                               -- Persisting stays a causal NEGATIVE — that site was flagged and closed before the latest Adapt, so
                                                               -- it is outside the Emerged chain. A fresh instance of a known kind at a NEW site is Emerged, since
                                                               -- nothing was ever closed there
...
KindBinding    = { label: Axis, positive_predicate: String, evidence: Set(Evidence), origin ∈ {seed, emergent}, atomicity ∈ {atomic, non-atomic} }
                 -- captures the mismatch as a kind. ONE positive_predicate per binding, so a detector output asserting two claims is bound as TWO
                 --   mismatches — a predicate question never reaches atomicity, which decides one thing only: how far the evidence spreads
                 -- atomicity RANGES OVER evidence rather than being judged over the mismatch as a whole. It is READ OFF the binding:
                 --     atomic     ⟺ |sites(kind_binding)| ≤ 1   -- all the evidence sits at one site; one adaptation reaches all of it
                 --     non-atomic ⟺ |sites(kind_binding)| > 1   -- the evidence spans sites an adaptation can reach separately
                 --   → split BEFORE certify, ONE sub-mismatch per site carrying that site's evidence alone (no registration, no surfacing on a
                 --   compound mismatch). An empty evidence set is atomic here and fails closed downstream — certify finds no claim it supports,
                 --   lands ambiguous, and the one bounded re-assessment leaves it Residual, so nothing registers on no evidence
Site           = an independently-adaptable locus of Λ.R: a place an adaptation can repair while the other loci in the same evidence set still stand.
                 -- Loci no adaptation can separate — one edit necessarily repairs them together — are ONE site
site           : Evidence → Site                               -- projection, read off the evidence item's own content against Λ.R; adds no field to Evidence
sites(k: KindBinding) = { site(e) : e ∈ k.evidence }           -- the site spread atomicity is read from, and the second half of Persisting's stamping test
...
Mᵢ     = Identified mismatches from Eval(R, X)                 -- origin = Initial; each bind_kind'd (split per site where non-atomic) + certified at registration (Phase 0)
Mₑ     = The re-scan registration set from Eval(R', X)         -- each bind_kind'd (split per site where non-atomic) + certified at re-scan registration (Phase 2).
                                                               -- ORIGIN-SPLIT at stamping time, decided by identity(m) against Σ.dispositions:
                                                               --   Emerged(adapted_aspect) — no closed record shares this binding's predicate at this site; the latest Adapt spawned it
                                                               --   Persisting              — one does, so this site was flagged and closed across the succession
...
Phase 0 → split (pre-certify): KindBinding.atomicity = non-atomic  -- |sites(kind_binding)| > 1: the binding's evidence spans loci an adaptation can reach separately → split into ONE sub-mismatch per site, each carrying that site's evidence alone, and re-run bind_kind + certify on each (same Phase 0 pass, before any pass/route/defer decision); recursive until atomic, and terminating because each split strictly decreases |sites|. A non-atomic mismatch is split pre-registration, never deferred or registered as a compound. This is what makes a PARTIAL adaptation legible: each site is its own registration, so an adaptation reaching some of them closes exactly those and leaves the rest standing in pending(Σ) — no re-registration to get right, and the per-site multiplication it costs the user is what the bulk keep-all-remaining exit absorbs
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split per site where non-atomic] → match_identity(m) against pending(Σ) and Σ.dispositions → Mₑ := those whose identity(m) ∉ { identity(p) : p ∈ pending(Σ) } → ∀m ∈ Mₑ: stamp origin(m) from that match (Persisting / Emerged(adapted_aspect)) [BEFORE certify: the match that decides the stamp has already run, and every m ∈ Mₑ is closed one way or another — gating the stamp on the certificate filter would leave each route/Residual close holding a Mismatch whose origin no step ever produced] → certify(m, local_claims) → (status = pass) → Mₑ_passed → TaskCreate[all Mₑ_passed] → pending(Σ) → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
...
Phase 2 → split (pre-certify): KindBinding.atomicity = non-atomic  -- |sites(kind_binding)| > 1 on an emerged mismatch → split into ONE sub-mismatch per site, each carrying that site's evidence alone, re-bind_kind + certify each (same re-scan pass); never deferred/registered as a compound. The split runs BEFORE the pending exclusion and the origin stamping, both of which read identity(m) — a compound's identity would answer for sites it no longer owns
...
unrepaired(m)      = ∃ r ∈ Σ.dispositions : identity(r.mismatch) = identity(m) ∧ r.disposition = Adapt(_)
                     ...
                     -- by adapting. Nothing at that site moved, so the adaptation did not take. This is what re-scanning alone cannot say — a
...
bind_kind (sense)   → Internal analysis (capture each detected mismatch as a KindBinding {label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity}. ONE positive_predicate per binding, so a detector output asserting two claims is bound as two mismatches rather than one compound — atomicity is never asked a predicate question. atomicity is then READ OFF the evidence set: atomic where all of it sits at one independently-adaptable site, non-atomic where it spans more (|sites(kind_binding)| > 1), and a non-atomic binding is split into one sub-mismatch per site before certify. Runs before certify, before the pending exclusion, and before the origin stamping — all three read the binding)
match_identity (sense) → Internal analysis (at the Phase 2 re-scan, after bind_kind + split and before TaskCreate: compare identity(m) — the binding's positive_predicate together with the evidence set it stands on — against pending(Σ) and against every record in Σ.dispositions. Settles three readings and nothing else: whether the return is already pending (excluded from Mₑ), how its origin is stamped (Persisting where a closed record shares the predicate and site, Emerged(adapted_aspect) otherwise), and unrepaired(m) where the matched record closed by adapting. It never completes the new registration's task — a record answers only for the registration it closed, and reading a match as a close is the defect this comparison replaces)
...
3. **Bind + certify each detected mismatch at registration (fail-closed)**: For each candidate mismatch, set `m.kind_binding = { label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity }` — one `positive_predicate` per binding, so a candidate asserting two claims is bound as two mismatches. Then read `atomicity` **off the evidence set** rather than judging it over the mismatch as a whole: `atomic` where all the evidence sits at one independently-adaptable site, `non-atomic` where it spans more. If `atomicity = non-atomic`, **split before certify**, one sub-mismatch per site. Then `m.certificate = certify(m.kind_binding, local_claims)` where the local claims are epharmoge's own claim and its route claims, both inscribed in this SKILL.md (TYPES: `OwnClaim`, `RouteClaim`); the certificate reads nothing outside this file. Fit the predicate against every one of them and collect `claimed_by`, the set of claims the evidence supports:
   ...
   - **`atomicity = non-atomic`** — the binding's evidence spans more than one independently-adaptable site (`|sites(kind_binding)| > 1`) → **split** into one atomic sub-mismatch per site, each carrying that site's evidence alone, and re-run bind_kind + certify on each (same Phase 0 pass). Each site then registers, is judged, and is disposed of on its own, so an adaptation that reaches only some of them closes exactly those.
