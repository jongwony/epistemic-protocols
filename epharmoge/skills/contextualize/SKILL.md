...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) [split where non-atomic] → assign m.id → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
      ...
      [d = Adapt(direction)] adapt → R' → Λ.R := R' → Eval(Λ.R, X) → bind_kind each [split where non-atomic] → match_identity each → Mₑ (those whose identity(m) is not already pending) → each detection that DOES match a pending one refreshes that registration's current reading (its id is unchanged) and registers nothing → ∀m ∈ Mₑ: assign m.id → stamp origin from that match (Emerged carries Mₛ.id), and unrepaired from THIS cycle's own Mₛ → certify(m, local_claims) → keep(status = pass) → Mₑ_passed → Register(Mₑ_passed) → AssessFit(Λ.R, X, pending) → F' → (loop: back to Phase 1 above — judgment half, then disposition half — until contextualized)
...
Phase 1: Mᵢ_passed → TaskCreate[all certificate-passing initial mismatches, each carrying its id] → pending(Σ) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →  -- register all certificate-passing initial mismatches, surface selected mismatch with fit basis [Tool]; reached only when Mᵢ_passed ≠ ∅. TWO-LEVEL DISPATCH — the JUDGMENT half resolves first, then the DISPOSITION half; the gate is not on the common path:
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split where non-atomic] → match_identity(m) against pending(Σ) and Σ.dispositions → Mₑ := those whose identity(m) ∉ { identity(p) : p ∈ pending(Σ) } → ∀ detected m whose identity DOES match a pending p: REFRESH p's current reading from m (p.aspect, p.description, p.evidence, p.severity := m's) and TaskUpdate p's existing task to the refreshed subject/description [Tool] — that task was created from the reading just superseded and is a separate copy outside Σ, so refreshing the pending object alone leaves the user's own tracked list showing a target two successions back — and register nothing: the registration is p's and stays p's, while what the user is shown about it tracks the advanced target [REQUIRED, not cosmetic: disposed(a) is aspect-keyed, so a pending object left holding a superseded aspect writes its record under a name no longer flagged — the currently-flagged name then has no record and no pending entry, disposed fails for it permanently, and adjudicated can never close. p.kind_binding is untouched: the identity match is what established there is nothing there to move. p.origin and p.unrepaired are untouched: provenance is stamped once] → ∀m ∈ Mₑ: assign m.id (the next handle in this run's order; a detection that only refreshed a pending registration took none, so ids name occurrences the protocol carries and nothing else) → stamp origin(m) from that match (Persisting / Emerged(Mₛ.id) — the parent is the OCCURRENCE this cycle adapted, named by its handle so the chain resolves to one row) and unrepaired(m) := (identity(m) = identity(Mₛ)) — Mₛ being the mismatch THIS cycle's dispose just closed by adapting, and no other record [NOT any matching Adapt in the ledger: the ledger holds every Adapt this run has made, and a claim one of them legitimately removed can be re-introduced later by an unrelated Adapt, at which point that older record still matches and reports a failure that did not happen. Only the Adapt this re-scan is the re-scan OF can have failed to land here] [BEFORE certify: the match that decides the stamp has already run, and every m ∈ Mₑ is closed one way or another — gating the stamp on the certificate filter would leave each route/Residual close holding a Mismatch whose origin no step ever produced] → certify(m, local_claims) → (status = pass) → Mₑ_passed → TaskCreate[all Mₑ_passed] → pending(Σ) → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
...
Phase 2 → split (pre-certify): KindBinding.atomicity = non-atomic  -- an emerged binding bundling two distinct aspects → split into ONE atomic sub-mismatch per claim, each carrying the evidence that claim stands on, re-bind_kind + certify each (same re-scan pass); never deferred/registered as a compound. The split runs BEFORE the pending exclusion and the origin stamping, both of which read identity(m) — a compound's identity would answer for claims it no longer owns
