...
A      = Answer = (j: Judgment, d: Disposition) where d ∈ {Adapt(direction), Keep, Discard(replacement)}
         ...
         --   once the judgment settles Overruled
...
DispositionRecord = { mismatch: Mismatch, judgment: Option(Judgment), judgment_by ∈ {user, relay, unjudged},
         ...
         --     Reachable ONLY from judgment_relay_overruled, which reports the retraction and its basis before writing this
...
Convergence evidence: At adjudicated(Λ.R, X), present the transformation trace ranging over Σ.dispositions — ONE ledger, since every close writes a record there whoever assigned it. For each r ∈ Σ.dispositions show (aspect(r.mismatch) → r.judgment → r.disposition), reading:
