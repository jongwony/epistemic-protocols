...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
      ...
      [d = Adapt(direction)] adapt → R' → Λ.R := R' → Eval(Λ.R, X) → bind_kind each [split per site] → match_identity each → Mₑ (those whose identity(m) is not already pending) → ∀m ∈ Mₑ: certify(m, local_claims) → keep(status = pass) → Mₑ_passed → stamp origin from that match → Register(Mₑ_passed) → AssessFit(Λ.R, X, pending) → F' → (loop: back to Phase 1 above — judgment half, then disposition half — until contextualized)
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split per site where non-atomic] → match_identity(m) against pending(Σ) and Σ.dispositions → Mₑ := those whose identity(m) ∉ { identity(p) : p ∈ pending(Σ) } → ∀m ∈ Mₑ: certify(m, local_claims) → (status = pass) → Mₑ_passed → stamp origin(m) from that match (Persisting / Emerged(adapted_aspect)) → TaskCreate[all Mₑ_passed] → pending(Σ) → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
...
| Each Phase 1 relay says where its correction lives | a relay-closed mismatch leaves `pending(Σ)` and reaches no further Qc this run — that registration, not every mismatch sharing its label. A disputed CARRY reopens by re-entering on the same result (`accepted(a)` reads `Σ`, rebuilt at activation). A disputed RETRACTION has no such reset — `judgment_settled` reads `Λ.fit_map`, derived from `(Λ.R, X)`, so the same input reproduces the same close, and correcting it is the user's interrupt | The user is told what to reach for, rather than trying a reset that moves only one of the two guards |
