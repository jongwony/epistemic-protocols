...
merge  = Mismatch × Mismatch → Mismatch, combining two readings of ONE claim. The carried element is the left side and stays the element
                 --   id, kind_binding : the carried element's, untouched — the same occurrence taking on more of what is known about it
                 ...
                 --   certificate, value_space : the carried element's, untouched — and the incoming detection's binding is dropped rather than combined,
                 --                which is what keeps them sound. THE ARM'S OWN PRECONDITION IS THE GROUND: the certificate fits kind_binding.positive_predicate
                 --                against the local claims, and this arm was taken because the run judged the detection to state THAT predicate's claim. A
                 --                detection stating a claim the carried certificate would not fit is a detection the run judged different, and it takes the
                 --                other arm, where it is certified on its own. So identity does not move under merging, and the next detection is judged
                 --                against a stable claim rather than one that drifts as content accumulates. What the detection contributes is its READING —
                 --                what this claim now looks like on the target in front of the run — never a redefinition of the claim
                 --                At Phase 0 the element may be carrying neither field yet, and the certify and bind_value_space legs reach it as they reach
                 --                every other element of Mᵢ; at the re-scan a merged element already holds both from its own registration, which is why it is
                 --                not in Mₑ. The field list is TOTAL: merge names every field of Mismatch and leaves none to the caller
...
Mismatch = { id: MismatchId, aspect: String, description: String, evidence: String, severity: Severity, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate, value_space: Option(ValueSpace) }
                 ...
                 --   READING and are combined; kind_binding is IDENTITY and cannot move; unrepaired is neither, being a monotone stamp that merge
                 --   takes the disjunction of
...
Mᵢ     = What Phase 0 carries: Eval(R, X) bound, split, and absorbed  -- one element per claim, each certified at registration. Eval(R, X) = ∅ ⟺ Mᵢ = ∅, the fold neither creating nor emptying
Mₑ     = What the Phase 2 fold OPENED — the elements absorb added to pending rather than merged into it  -- each certified at re-scan registration; a detection that merged carries no separate member, its content being in the element it merged into
