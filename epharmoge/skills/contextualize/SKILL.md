...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) [split where non-atomic] → init_occurrence(m) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed → ∀m ∈ Mᵢ_passed: m.value_space := Some(bind_value_space(m)) →
...
identity(m: Mismatch) = (m.kind_binding.positive_predicate, m.kind_binding.evidence)
                 ...
                 --   locates a predecessor and never completes a new registration's entry (status↔ledger equivalence, pending(Σ))
...
V              = bind_value_space : Mismatch → ValueSpace       -- the mismatch's answer constructors; generated ONLY after certificate.status = pass; frozen for the cycle (relay / dead-signal test applied)
                 ...
                 --   value space) — and written to that mismatch's own value_space field, never at selection: judgment_relay_overruled and keep_carried_forward both
...
SelectNext = Set(Mismatch) × F × Σ → Mₛ
           -- priority: severity(Critical > Significant > Minor), then FitRank, then oldest registered mismatch
...
registered = the mismatches C carries  -- certificate-passing only; a Route/Residual close fires before registration and never enters C
entry(m)   = the Entry C carries for m's CURRENT registration, reached by reading C at Λ.carrier when the durable record itself is wanted  -- keyed by the registration, not by the aspect: a Persisting mismatch re-registering after an Adapt appends a FRESH entry rather than reusing its closed one, which is what stops pending(Σ) from reading a stale completed status off a target the run has moved past (the same reason disposed(a) treats re-registration as un-disposing)  -- within a run the status↔ledger equivalence below settles its status from Σ.dispositions, so no read is owed per cycle; there is no per-mismatch entry and no per-mismatch identifier to hold
pending(Σ) = Set(Mismatch) where entry(m).status ≠ completed  -- a routed/ambiguous mismatch never enters pending(Σ); only certificate-passing mismatches are registered
             ...
             -- status↔ledger equivalence, SCOPED TO REGISTERED MISMATCHES AND TO THE REGISTRATION AT HAND: ∀ m registered, entry(m).status = completed ⟺ Σ.dispositions holds the record that CLOSED THIS REGISTRATION of m. Every close of a REGISTERED mismatch writes the disposition and completes its entry as ONE action — the Phase 2 dispose step (TOOL GROUNDING dispose), the Phase 1 judgment_relay_overruled close, the keep_carried_forward close, and the keep_all_remaining bulk close alike — and that single action is what binds a record to the one registration it answers for
             -- NOT keyed on identity, and this is the load-bearing half: identity(m) (TYPES) matches a re-detected mismatch to the registration it SUCCEEDS, and a match discharges nothing. A re-registration always opens its own entry and sits in pending(Σ) until its own close — whether the return matched an earlier record exactly (unrepaired(m): the prior disposition did not take) or only on the predicate (a residue owed its own surfacing). Reading this equivalence over a record written for an earlier registration is what let a re-detected mismatch be declared complete without ever being surfaced
             -- a certificate-assigned close (Route, Residual) fires BEFORE registration, so its mismatch has a ledger entry and no tracked entry to complete; the unscoped statement that holds for it is the Ledger invariant under MODE STATE
...
Phase 0 → split (pre-certify): KindBinding.atomicity = non-atomic  -- a compound mismatch bundles two distinct aspects → split into atomic sub-mismatches and re-run bind_kind → init_occurrence → certify on each (same Phase 0 pass, before any pass/route/defer decision) — each child is a distinct occurrence and is initialized as one, while the compound never is, init_occurrence running after the split for exactly that reason; recursive until atomic, and terminating because each split strictly decreases the number of bundled aspects. A non-atomic mismatch is split pre-registration, never deferred or registered as a compound. PARTIAL adaptation stays legible without any splitting — identity(m) carries the evidence, so an adaptation reaching only part of it leaves a residue whose evidence has moved; that residue is a DIFFERENT identity, so it is not excluded from Mₑ, and it registers and surfaces on its own account (LOOP)
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND record update(Λ.carrier, entry(Mₛ).status := completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split where non-atomic] → match_identity(m) against pending(Σ) and Σ.dispositions → Mₑ := those whose identity(m) ∉ { identity(p) : p ∈ pending(Σ) } → ∀ detected m whose identity DOES match a pending p: REFRESH p's current reading from m (p.aspect, p.description, p.evidence, p.severity := m's) and record update(Λ.carrier, entry(p) := the refreshed reading) [Tool] — the entry was written from the reading just superseded and is a separate copy outside Σ, so refreshing the pending object alone leaves the durable record showing a target two successions back — and register nothing: the registration is p's and stays p's, while what the user is shown about it tracks the advanced target [REQUIRED, not cosmetic: disposed(a) is aspect-keyed, so a pending object left holding a superseded aspect writes its record under a name no longer flagged — the currently-flagged name then has no record and no pending entry, disposed fails for it permanently, and adjudicated can never close. p.kind_binding is untouched: the identity match is what established there is nothing there to move. p.id, p.origin and p.unrepaired are untouched: a refreshed registration is the same occurrence carrying a newer reading] → ∀m ∈ Mₑ: init_occurrence(m) [TYPES: id — a detection that only refreshed a pending registration took none, so ids name occurrences the protocol carries and nothing else; origin from the match, Persisting / Emerged(Mₛ.id), the parent being the OCCURRENCE this cycle adapted so the chain resolves to one row; unrepaired := (identity(m) = identity(Mₛ)) — Mₛ being the mismatch THIS cycle's dispose just closed by adapting, and no other record [NOT any matching Adapt in the ledger: the ledger holds every Adapt this run has made, and a claim one of them legitimately removed can be re-introduced later by an unrelated Adapt, at which point that older record still matches and reports a failure that did not happen. Only the Adapt this re-scan is the re-scan OF can have failed to land here]] [BEFORE certify: the match that decides the stamp has already run, and every m ∈ Mₑ is closed one way or another — gating the stamp on the certificate filter would leave each route/Residual close holding a Mismatch whose origin no step ever produced] → certify(m, local_claims) → (status = pass) → Mₑ_passed → ∀m ∈ Mₑ_passed: m.value_space := Some(bind_value_space(m)) [same registration leg as Phase 0, Rule 17] → record update(Λ.carrier, add all Mₑ_passed) → pending(Σ)   -- Λ.carrier is Some here by construction: this arm is reached only through a Qc answer, and Qc is reached only after the Phase 1 registration that binds it → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
...
Phase 2 → withdraw (run-terminal): d = Discard(replacement) ∧ j = Upheld  -- the user upholds the mismatch and WITHDRAWS the evaluated result instead of adapting it. discard(replacement, Λ.R) [mutating, TOOL GROUNDING discard → artifact write: the artifact is actually withdrawn and the replacement put in its place, or removed outright when replacement = None] → THEN R_final := replacement (already Option(Result): None when the withdrawal leaves nothing in its place); Λ.withdrawn := Some((Mₛ, replacement)); then EVERY mismatch REMAINING in pending(Σ) — Mₛ already left it at the dispose step above, so it cannot be closed twice — is closed with DispositionRecord { judgment = None, judgment_by = unjudged, disposition = Moot, assigned_by = loop, judgment_basis = "", disposition_basis = the withdrawal record } and record update(Λ.carrier, entry(m).status := completed for each). NO re-scan; deactivate. The adjudication claim stays on Λ.R (the object actually evaluated): every flagged aspect of it carries a ledger entry, and NO claim is made about the replacement
...
CHECKED IN THIS ORDER:
  1. If pending(Σ) non-empty: return to Phase 1 (SelectNext by severity, then FitRank, then oldest registered mismatch).
...
disposed(a)        = (∃ r ∈ Σ.dispositions : aspect(r.mismatch) = a) ∧ a ∉ { aspect(m) : m ∈ pending(Σ) }
                     ...
                     -- close, so the return opens its own entry and sits in pending(Σ), which is what fails the second conjunct while it is open.
...
--   record in the carrier and are covered by disposed(a) directly
...
bind_kind (sense)   → Internal analysis (capture each detected mismatch as a KindBinding {label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity}; non-atomic mismatch → split before certify. Runs before certify, before the pending exclusion, and before the origin stamping — all three read the binding)
init_occurrence (track) → Internal state update (writes id, origin and unrepaired onto a detected mismatch, at the one position every scan path states: after split, before certify. It is the sole producer of all three — no other step writes them, and no reader of a DispositionRecord can reach a mismatch that skipped it, which is what the single position buys. It opens no second occurrence for an identity already initialized this scan or already pending. It performs no user interaction and touches no artifact)
...
-- INITIAL BINDING at activation: Λ.R := the result under review; Λ.deferred := ∅; Λ.withdrawn := None; Λ.reopen_focus := None; Λ.carrier := None;
...
After the user's answer `(judgment, disposition)`, **first** append the mismatch's one `DispositionRecord` to `Σ.dispositions` and complete its record in the carrier — as a single step, so it leaves `pending(Σ)` before any arm below runs — **then** act on the disposition:
...
3. **`(Upheld, Adapt(direction))`**: apply the user-directed adaptation through artifact write → `R'`, **advance the evaluated target** (`Λ.R := R'`), then re-scan (below)
...
- If every registered mismatch is completed: execution complete — the verdict carries `R_final` together with every disposition
