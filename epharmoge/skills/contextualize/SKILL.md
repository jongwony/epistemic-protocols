...
bind_kind (sense)   → Internal analysis (capture each detected mismatch as a KindBinding {label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity}; non-atomic mismatch → split before certify. Runs before certify, before the pending exclusion, and before the unrepaired stamping — all three read the binding)
