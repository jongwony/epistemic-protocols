...
DispositionRecord = { mismatch: Mismatch, judgment: Option(Judgment), judgment_by ∈ {user, relay, unjudged},
         ...
         --   needs no field of its own here. What changed is which predicates read it: accepted(i), unrepaired(m) and the Origin stamping read the
         --   binding; none of them reads the label
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split where non-atomic] → match_identity(m) against pending(Σ) and Σ.dispositions → Mₑ := those whose identity(m) ∉ { identity(p) : p ∈ pending(Σ) } → ∀m ∈ Mₑ: stamp origin(m) from that match (Persisting / Emerged(adapted_aspect)) [BEFORE certify: the match that decides the stamp has already run, and every m ∈ Mₑ is closed one way or another — gating the stamp on the certificate filter would leave each route/Residual close holding a Mismatch whose origin no step ever produced] → certify(m, local_claims) → (status = pass) → Mₑ_passed → TaskCreate[all Mₑ_passed] → pending(Σ) → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
...
unrepaired(m)      = ∃ r ∈ Σ.dispositions : identity(r.mismatch) = identity(m) ∧ r.disposition = Adapt(_)
...
match_identity (sense) → Internal analysis (at the Phase 2 re-scan, after bind_kind + split and before TaskCreate: compare identity(m) — the binding's positive_predicate together with the evidence set it stands on — against pending(Σ) and against every record in Σ.dispositions. Settles three readings and nothing else: whether the return is already pending (excluded from Mₑ), how its origin is stamped (Persisting where a closed record shares the predicate, Emerged(adapted_aspect) otherwise), and unrepaired(m) where the matched record closed by adapting. It never completes the new registration's task — a record answers only for the registration it closed, and reading a match as a close is the defect this comparison replaces)
