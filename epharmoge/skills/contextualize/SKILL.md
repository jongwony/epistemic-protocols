...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
      ...
      [d = Adapt(direction)] adapt → R' → Λ.R := R' → Eval(Λ.R, X) → bind_kind each [split where non-atomic] → match_identity each → Mₑ (those whose identity(m) is not already pending) → ∀m ∈ Mₑ: stamp origin from that match → certify(m, local_claims) → keep(status = pass) → Mₑ_passed → Register(Mₑ_passed) → AssessFit(Λ.R, X, pending) → F' → (loop: back to Phase 1 above — judgment half, then disposition half — until contextualized)
...
Mismatch = { aspect: String, description: String, evidence: String, severity: Severity, origin: Origin, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate }
                 ...
                 --   load-bearing. Initial and Mₑ mismatches with no matching Adapt record are stamped false
Origin ∈ {Initial, Emerged(aspect), Persisting}                -- mismatch provenance: initial scan, spawned by adapting parent aspect, or a mismatch a closed record
                                                               ...
                                                               -- Persisting stays a causal NEGATIVE — that claim was flagged and closed before the latest Adapt, so
                                                               -- it is outside the Emerged chain. A claim no record ever closed is Emerged, since nothing was
                                                               -- ever closed for it
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending(Σ) before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split where non-atomic] → match_identity(m) against pending(Σ) and Σ.dispositions → Mₑ := those whose identity(m) ∉ { identity(p) : p ∈ pending(Σ) } → ∀ detected m whose identity DOES match a pending p: REFRESH p's current reading from m (p.aspect, p.description, p.evidence, p.severity := m's) and register nothing — the registration is p's and stays p's, while what the user is shown about it tracks the advanced target [REQUIRED, not cosmetic: disposed(a) is aspect-keyed, so a pending object left holding a superseded aspect writes its record under a name no longer flagged — the currently-flagged name then has no record and no pending entry, disposed fails for it permanently, and adjudicated can never close. p.kind_binding is untouched: the identity match is what established there is nothing there to move. p.origin and p.unrepaired are untouched: provenance is stamped once] → ∀m ∈ Mₑ: stamp origin(m) and unrepaired(m) from that match (Persisting / Emerged(adapted_aspect); unrepaired where the matched record closed by adapting) [BEFORE certify: the match that decides the stamp has already run, and every m ∈ Mₑ is closed one way or another — gating the stamp on the certificate filter would leave each route/Residual close holding a Mismatch whose origin no step ever produced] → certify(m, local_claims) → (status = pass) → Mₑ_passed → TaskCreate[all Mₑ_passed] → pending(Σ) → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
