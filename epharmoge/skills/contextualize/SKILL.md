...
Mismatch = { id: MismatchId, aspect: String, description: String, evidence: String, severity: Severity, origin: Origin, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate, value_space: Option(ValueSpace) }
                 ...
                 --   clause). kind_binding is IDENTITY and cannot move. origin and unrepaired are PROVENANCE, stamped once at the re-scan that
                 --   produced them and never re-stamped
                 ...
                 --   The invariant readers depend on is narrower than the field — a mismatch IN pending(Σ) always carries Some, since registration
...
Origin ∈ {Initial, Emerged(parent: MismatchId), Persisting}    -- mismatch provenance: initial scan, spawned by adapting a parent OCCURRENCE, or a mismatch a closed record
                                                               ...
                                                               -- WHAT Persisting ESTABLISHES is what its key checks and no more: some closed record asserts this
                                                               -- same claim, so the claim has been flagged before. It does NOT establish that the latest Adapt did
                                                               -- not produce this occurrence — the key is the predicate alone. Read it no further than that
                                                               -- Persisting stays OUT OF the Emerged chain because that chain reads which adaptation SPAWNED a
                                                               -- follow-on, which this stamp does not answer. What an adaptation LEFT is carried by the
                                                               -- per-registration trace (CONVERGENCE) instead
...
pending(Σ) = Set(Mismatch) where entry(m).status ≠ completed  -- a routed/ambiguous mismatch never enters pending(Σ); only certificate-passing mismatches are registered
             ...
             -- NOT keyed on identity, and this is the load-bearing half: identity(m) (TYPES) matches a re-detected mismatch to the registration it SUCCEEDS, and a match discharges nothing. A re-registration always opens its own entry and sits in pending(Σ) until its own close — whether the return matched an earlier record exactly (unrepaired(m): the prior disposition did not take) or only on the predicate (a residue owed its own surfacing).
