...
Phase 1: Mᵢ_passed → TaskCreate[all certificate-passing initial mismatches] → pending(Σ) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →  -- register all certificate-passing initial mismatches, surface selected mismatch with fit basis [Tool]; reached only when Mᵢ_passed ≠ ∅. The three outcomes take DIFFERENT paths; the gate is not on the common path:
         ...
         [Some(Upheld)]    judgment_relay_upheld (below) settles the judgment half, then Qc(F-scoped Mₛ, evidence, judgment_state) → Stop → A = (j, d) for the disposition
         [None]            Qc(F-scoped Mₛ, evidence, judgment_state) → Stop → A = (j, d)  -- the full two-axis set, both halves user-answered
...
Recompute the fit map over pending(Σ) and re-bind Λ.fit_map to it before selecting the next surfaced mismatch, even when Mₑ_passed = ∅. FIT-MAP SUCCESSION, the exact counterpart of TARGET SUCCESSION: every selection and judgment reader takes Λ.fit_map, never a fit map computed against an earlier Λ.R — a relay reading stale fit evidence would retract a mismatch on grounds that no longer describe the current target, and it would do so without a gate.
