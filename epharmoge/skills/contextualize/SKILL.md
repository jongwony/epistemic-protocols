...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map, Σ) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
      ...
      [d = Adapt(direction)] adapt → R' → Λ.R := R' → Eval(Λ.R, X) → bind_kind each [split where non-atomic] → match_identity each → Mₑ (those whose identity(m) is not already pending) → each detection that DOES match a pending one refreshes that registration's current reading and registers nothing → ∀m ∈ Mₑ: stamp origin from that match, and unrepaired from THIS cycle's own Mₛ → certify(m, local_claims) → keep(status = pass) → Mₑ_passed → Register(Mₑ_passed) → AssessFit(Λ.R, X, pending) → F' → (loop: back to Phase 1 above — judgment half, then disposition half — until contextualized)
...
Mismatch = { aspect: String, description: String, evidence: String, severity: Severity, origin: Origin, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate }
...
Origin ∈ {Initial, Emerged(aspect), Persisting}                -- mismatch provenance: initial scan, spawned by adapting parent aspect, or a mismatch a closed record
                                                               ...
                                                               -- record shares this binding's predicate, Emerged(adapted_aspect) where none does.
...
Mₑ     = The re-scan registration set from Eval(R', X)         -- each bind_kind'd (split where non-atomic) + certified at re-scan registration (Phase 2).
                                                               ...
                                                               --   Emerged(adapted_aspect) — no closed record shares this binding's predicate; the latest Adapt spawned it
