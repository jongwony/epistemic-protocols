...
entry(m)   = the Entry C carries for m ∈ registered, reached by reading C at Λ.carrier when the durable record itself is wanted  -- within a run the status↔ledger equivalence below settles its status from Σ.dispositions, so no read is owed per cycle; there is no per-mismatch entry and no per-mismatch identifier to hold
...
record({
  ...
  description: "one line per mismatch: [Mismatch:aspect] description | evidence and context | severity | status"
})
```
