...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) [split where non-atomic] → init_occurrence(m) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → ∀m ∈ Mᵢ_passed: m.value_space := Some(bind_value_space(m)) → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
      ...
      [accepted(identity(Mₛ)): keep_carried_forward → report the carry + why they accepted it → close Mₛ as Keep with the judgment read off judgment_state (relayed Upheld, else unjudged), relay-assigned → NO Qc, no turn yielded, Λ.R untouched → next pending, or converge]
      Qc(Mₛ scoped by the CURRENT Λ.fit_map, judgment_state) → Stop → A = (j, d) →
      [d = Adapt(direction)] adapt → R' → Λ.R := R' → Eval(Λ.R, X) → bind_kind each [split where non-atomic] → match_identity each → Mₑ (those whose identity(m) is not already pending) → each detection that DOES match a pending one refreshes that registration (its reading AND its task; id, kind_binding, origin, unrepaired unchanged) and registers nothing → ∀m ∈ Mₑ: init_occurrence(m) → certify(m, local_claims) → keep(status = pass) → Mₑ_passed → ∀m ∈ Mₑ_passed: m.value_space := Some(bind_value_space(m)) → Register(Mₑ_passed) → AssessFit(Λ.R, X, pending) → F' → (loop: back to Phase 1 above — judgment half, then disposition half — until contextualized)
...
invariant: certificate-before-registration  -- DeficitFitCertificate.status = pass strictly precedes registering a mismatch into pending (shared meta-backbone order, at registration of both Mᵢ and Mₑ)
...
MismatchId = a run-local handle naming ONE OCCURRENCE the protocol carries — assigned in ascending order at the moment the occurrence is kept
                 (Phase 0, every detected mismatch after split; Phase 2, every m ∈ Mₑ after split), and never reassigned or reused within the run
                 -- WHAT IT NAMES, against the two names already here: identity(m) says WHICH CLAIM this is, aspect says HOW IT READS right now, and
                 --   this says WHICH OCCURRENCE OF THAT CLAIM the reader is looking at. The three are independent and each answers a question the
                 --   other two cannot — Rule 22 states the triple
                 ...
                 --   mismatches are the same claim — that stays identity(m), and reading a handle as identity would reinstate the naming defect this
                 --   protocol removed. It is a handle for pointing, and pointing only
                 -- WHAT IT IS FOR is that the reader can act on a row: say THIS one and not the other under the same aspect, carry that reference out
                 --   of the run, and see two readings the ledger supports and a label cannot — that a row is an earlier occurrence returned (its
                 --   identity matches one already carried), and how many occurrences one claim has accumulated across the run. Both are framing
                 --   signals in the sense Chain discovery already uses, never scores and never progress: how many times a claim has come back says
                 --   the run is circling, and says nothing about how near it is to converging
                 -- A detection that only REFRESHES a pending registration is not a new occurrence and takes no id; the registration keeps its own (LOOP)
init_occurrence = the ONE step that turns a detected mismatch into an occurrence the protocol carries: Mismatch → Mismatch, producing every field no earlier step
                 could produce. FIXED POSITION, and every block that scans states it at that position: AFTER split (so each atomic child is initialized on its own and
                 no compound ever is) and BEFORE certify (so a certificate-assigned close — Route, Residual — already holds a complete Mismatch, and a Moot close later
                 inherits one). It writes exactly three fields, each by one rule that reaches every path:
                 --   NO SECOND OCCURRENCE for an identity this scan has already initialized, or one already pending: the same claim found twice
                 --                is one claim, and this is the step where that is decidable, since it is the only one every detection passes
                 --                through before becoming something the protocol carries. WHICH reading the surviving occurrence carries is read
                 --                at that turn from what the detections actually say — the protocol fixes that there is one, not which
                 --   id        := the next handle in this run's order
                 --   origin    := Initial on a Phase 0 scan (including a Reopen-focused one, which re-enters that pipeline); on the Phase 2 re-scan, read off the
                 --                identity match — Persisting where a closed record shares this binding's predicate, Emerged(Mₛ.id) where none does
                 --   unrepaired := (identity(m) = identity(Mₛ)) — false on a Phase 0 scan for want of an Mₛ, by the same rule rather than by a separate default
                 -- WHY ONE OPERATION AND NOT A LINE AT EACH SITE: these fields are required by every reader of a closed record, while the sites that produce a Mismatch
                 --   are several and keep growing. A rule restated per site is a rule that holds wherever someone remembered it, and the gap does not announce itself —
                 --   the close still writes, and the missing field is only found by whoever reads the record later
Mismatch = { id: MismatchId, aspect: String, description: String, evidence: String, severity: Severity, origin: Origin, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate, value_space: Option(ValueSpace) }
                 -- ASSEMBLED ACROSS THE REGISTRATION PIPELINE, not built by the scan: Eval detects an aspect and its reading, and bind_kind,
                 --   init_occurrence, certify and bind_value_space each add what only they can produce, in the order Rule 17 fixes. So the whole
                 ...
                 --   and a DISPLAY NAME only: which registration a mismatch IS is settled by identity(m) below, never by this label. No predicate
                 --   here reads it; it is shown, and it is carried because the schema is shared
                 ...
                 -- THE FIELDS SPLIT THREE WAYS, and a re-detection treats each differently. aspect, description, evidence and severity are the
                 --   CURRENT READING: they say how this mismatch is described and weighed right now, so where a re-detected mismatch matches a
                 --   registration already pending, the pending object takes the fresh reading (LOOP, Mₑ clause). kind_binding is IDENTITY and cannot
                 --   move — a match is what established there is nothing to refresh. origin and unrepaired are PROVENANCE, stamped once at the
                 --   re-scan that produced them and never re-stamped; re-deriving provenance later is what the unrepaired entry below rules out
                 -- value_space is Option-valued because the occurrence EXISTS BEFORE ITS SPACE DOES: bind_value_space runs only on a passing certificate, so a
                 --   mismatch closed by certificate (Route, Residual) is complete with None and never had a space to carry. The invariant every reader depends on
                 --   is narrower than the field: a mismatch IN pending always carries Some, because registration follows bind_value_space on both scans. Qc and
                 --   both Phase 1 relay closes read it off the mismatch they are closing, which is what associates a bound space with the occurrence it was bound for
                 -- unrepaired: Bool is a STAMP, not a derived flag — see the unrepaired(m) entry under CONVERGENCE for why the distinction is
                 --   load-bearing. ONE RULE PRODUCES IT ON EVERY PATH, which is what makes the field total: a detected mismatch is stamped
                 --   unrepaired := (identity(m) = identity(Mₛ)) against the Mₛ this cycle's dispose closed by adapting. At Phase 0 there is no Mₛ,
                 --   so every initial mismatch — including every split child and every re-assessed deferral — takes false from that same rule for
                 --   want of an antecedent, rather than from a default no step writes
Origin ∈ {Initial, Emerged(parent: MismatchId), Persisting}    -- mismatch provenance: initial scan, spawned by adapting a parent OCCURRENCE, or a mismatch a closed record
                                                               -- ALREADY covers, re-registered across TARGET SUCCESSION. Which of the two a return is stamped is
                                                               -- decided by identity(m) against Σ.dispositions, never by the label: Persisting where a closed
                                                               -- record shares this binding's predicate, Emerged(id of the occurrence just adapted) where none does.
                                                               -- THE PAYLOAD NAMES AN OCCURRENCE, NOT A LABEL. One aspect can carry several occurrences
                                                               -- once identity keys the registration, so a parent named by label leaves the reader with
                                                               -- several candidate rows and no way to choose — which is the defect Rule 22 removes,
                                                               -- reappearing inside a payload
                                                               -- WHAT Persisting ESTABLISHES is what its key actually checks: some closed record asserts this same
                                                               -- claim, so this claim has been flagged before. WHAT IT DOES NOT ESTABLISH is that the latest Adapt
                                                               -- did not produce this occurrence — the key is the predicate alone, and predicate-alone matching does
                                                               -- not separate a residue from a fresh occurrence of a claim that was closed and has come back. Rule 22
                                                               -- calls a predicate-only match a residue, which settles how it is HANDLED and does not verify which of
                                                               -- the two it is; a stamp may not assert past its own key, and nothing here re-runs the stronger reading
                                                               -- SO WHAT KEEPS Persisting OUT OF THE Emerged CHAIN is not a causal negative but what the chain is FOR:
                                                               -- it reads which adaptation SPAWNED a follow-on, and this stamp does not answer that question either
                                                               -- way. What an adaptation LEFT is a different question, carried by the per-registration trace
                                                               -- (CONVERGENCE), so the movement stays visible on a surface whose key can support the claim
                                                               -- A claim no record ever closed is Emerged, since nothing was ever closed for it
...
KindBinding    = { label: Axis, positive_predicate: String, evidence: Set(Evidence), origin ∈ {seed, emergent}, atomicity ∈ {atomic, non-atomic} }
                 ...
                 --   protocol deliberately leaves open
                 -- captures the mismatch as a kind; if atomicity = non-atomic (the mismatch bundles two distinct aspects) → split BEFORE certify
                 --   (no registration, no surfacing on a compound mismatch)
                 ...
                 --   is RepairSite (below), which is projected from a disposition already answered and types no mismatch (Rule 23)
                 -- an empty evidence set fails closed downstream — certify finds no claim it supports, lands ambiguous, and the one bounded
                 --   re-assessment leaves it Residual, so nothing registers on no evidence
identity(m: Mismatch) = (m.kind_binding.positive_predicate, m.kind_binding.evidence)
                 -- the mismatch's VERIFIED identity, and the only thing that settles whether a re-detected mismatch is one a record already closed.
                 --   positive_predicate is the claim ITSELF rather than a name for it, and kind_binding.evidence is what that claim stands on —
                 --   which is what makes this a verification against the binding rather than one more label comparison
                 -- WHAT IT DOES: matches a re-detected mismatch to the registration it succeeds — the Origin stamping (Mₑ below), accepted(i) and
                 --   unrepaired(m) (CONVERGENCE), and the pending exclusion the LOOP's Mₑ clause states
                 ...
                 --   locates a predecessor and never completes a new registration's task (status↔ledger equivalence, pending)
...
DeficitFitCertificate = { own_claim: OwnClaim, route_claims: List<RouteClaim>, claimed_by: Set(Deficit), evidence: Set(Evidence), status ∈ {pass, route, ambiguous} }
                 -- fail-closed: status ≠ pass BLOCKS registration into pending AND surfacing this mismatch for answer. Generated at registration time (Phase 0 for Mᵢ, Phase 2 re-scan for Mₑ) by fitting KindBinding.positive_predicate against own_claim and every route_claim inscribed below — the certificate reads nothing outside this SKILL.md
                 ...
                 -- status = route: claimed_by = {d} for a single route_claim's routed_deficit d (backward misfit) → emit d as the typed handoff, drop the mismatch from registration (it never enters pending)
...
V              = bind_value_space : Mismatch → ValueSpace       -- the mismatch's answer constructors; generated ONLY after certificate.status = pass; frozen for the cycle (relay / dead-signal test applied)
                 -- PRODUCED at registration on both scans, between the pass filter and Register — the third leg of Rule 17's strict order (KindBinding, certificate,
                 --   value space) — and never at selection: judgment_relay_overruled and keep_carried_forward both reason about which pairings well_formed leaves,
                 --   and each can close a mismatch before it is ever selected, so a space bound at selection would not exist where they read it
...
AspectFit = { target: Mismatch, evidence: String }
           ...
           -- keyed to the mismatch, never to its aspect label (Rule 22): one aspect can carry several registrations at once, so an entry found
           --   by label could hand judgment_relay_overruled another registration's evidence — and that relay closes without a gate
...
Mᵢ     = Identified mismatches from Eval(R, X)                 -- origin = Initial; each bind_kind'd (split where non-atomic) + certified at registration (Phase 0)
Mₑ     = The re-scan registration set from Eval(R', X)         -- each bind_kind'd (split where non-atomic) + certified at re-scan registration (Phase 2).
                                                               -- ORIGIN-SPLIT at stamping time, decided by identity(m) against Σ.dispositions:
                                                               --   Emerged(Mₛ.id)         — no closed record shares this binding's predicate; the latest Adapt spawned it, and the payload
                                                               --                             is the id of the occurrence that Adapt closed, so the chain names a row rather than a name
                                                               --   Persisting              — one does, so this claim was flagged and closed across the succession
                                                               -- The stamping reads the binding, so bind_kind and the split both run BEFORE it
Register = { m ∈ Set(Mismatch) : certificate(m).status = pass } → Set(Task) [Tool: TaskCreate] -- registration of ONLY certificate-passing mismatches as tracked tasks; status ≠ pass blocks registration (fail-closed)
           -- each registration opens its OWN task, a re-registration included: the ledger match that identifies a return locates the record it
           -- succeeds and never stands in for its close, so a returning mismatch is tracked from scratch until it is closed on its own account
pending    = Set(Mismatch) where registered task status ≠ completed — a view over the REGISTERED TASKS, which is where task status lives; Σ holds the ledger and the scan count and never held this  -- a routed/ambiguous mismatch never enters pending; only certificate-passing mismatches are registered
             ...
             -- status↔ledger equivalence, SCOPED TO REGISTERED MISMATCHES AND TO THE REGISTRATION AT HAND: ∀ m registered, task(m).status = completed ⟺ Σ.dispositions holds the record that CLOSED THIS REGISTRATION of m. Every close of a REGISTERED mismatch writes the record and completes the task as ONE action — the Phase 2 dispose step (TOOL GROUNDING dispose), the Phase 1 judgment_relay_overruled close, and the keep_all_remaining bulk close alike — and that single action is what binds a record to the one registration it answers for
             -- NOT keyed on identity, and this is the load-bearing half: identity(m) (TYPES) matches a re-detected mismatch to the registration it SUCCEEDS, and a match discharges nothing. A re-registration always opens its own task and sits in pending until its own close — whether the return matched an earlier record exactly (unrepaired(m): the prior disposition did not take) or only on the predicate (a residue owed its own surfacing). Reading this equivalence over a record written for an earlier registration is what let a re-detected mismatch be declared complete without ever being surfaced
...
judgment_settled : Mismatch × F → Option((Judgment, basis: String))   -- judgment-level entropy → 0 with a citable basis (Rule 10); evaluated against the EVALUATED TARGET Λ.R
              ...
              Some((Overruled, basis)) when it admits no reading under which aspect(m) FAILS to stand — read off m's own AspectFit entry in
                              Λ.fit_map.fit_justifications, located by identity(m); basis = that entry's evidence
...
ResolutionForm = the axis Disposition ranges over — the SHAPE a resolution takes, answering what becomes of the result. Kind (kind_binding.label
                 ...
                 where that lands. THREE ORDERED AXES, and the order is the point (Rule 23)
RepairSite     = THE NAME OF THE THIRD AXIS — where a resolution lands on Λ.R when adapt or discard executes it. A NAME AND NOT A CARRIER:
                 ...
                 -- what the axis forbids is upstream use: it does not individuate a mismatch, does not enter identity(m), does not gate
                 --   registration, and appears in no convergence predicate. Asking it first is the failure Rule 23 names
A      = Answer = (j: Judgment, d: Disposition) where d ∈ {Adapt(direction), Keep, Discard(replacement)}
         -- A ∈ V; drawn from Mₛ.value_space, which registration guaranteed is Some (local_value_space = the two-axis space above)
...
DispositionRecord = { mismatch: Mismatch, judgment: Option(Judgment), judgment_by ∈ {user, relay, unjudged},
         ...
         -- ONE ledger entry per closed mismatch, answering for THAT registration and no other. routed / residual / moot are VIEWS over the ledger
         -- the mismatch field carries its kind_binding, so identity(r.mismatch) is readable straight off the ledger and the discriminating value
         --   needs no field of its own here. What changed is which readings rest on it: accepted(i) reads the binding directly, and the
         --   Origin and unrepaired stamps are both decided by matching on it at the re-scan; none of the three reaches for the label. The two
         --   stamps are then read back off the record as written, since the turn that could decide them has passed (CONVERGENCE, unrepaired(m))
...
ApplicabilityVerdict = { target: R_final, dispositions: List(DispositionRecord), unresolved: Set(Mismatch), scan_count: Nat }
         ...
         -- The last three are bound identically at EVERY terminal, so a terminal states only its own R_final and assembles the rest from Σ
...
EarlyExit = ApplicabilityVerdict where user_esc ∧ unresolved ≠ ∅  -- non-convergent early exit: R_final := Some(Λ.R) (the evaluated target as it stands, adapted or not), dispositions as closed so far, and unresolved := pending — the registrations the exit left open. They get no ledger entry, since an exit closes nothing, so this field is the only place they survive. The two terminals are told apart by it rather than by narration: ContextualizedExecution requires pending = ∅

── PHASE TRANSITIONS ──
Phase 0: R → Eval(R, X) → Mᵢ? → ∀m ∈ Mᵢ: bind_kind(m) [split where non-atomic] → init_occurrence(m) [TYPES: id, origin = Initial, unrepaired = false — before certify, so a route/Residual close holds a complete Mismatch] → certify(m, local_claims) → (status = pass) → Mᵢ_passed → AssessFit(R, X, Mᵢ_passed) → F → Λ.fit_map := F  -- applicability checkpoint + registration-time KIND dispatch (fail-closed) + fit map (silent); certify runs WITHIN Phase 0, at registration, not as a separate phase
...
Phase 0 → route_away (mismatch-local): certify(m).status = route        -- a local route claim holds the mismatch (backward misfit) → emit that claim's routed_deficit (GapUnnoticed, ContextInsufficient, BoundaryUndefined — with its command hint), close m with DispositionRecord { judgment = None, judgment_by = unjudged, disposition = Route(routed_deficit), assigned_by = certificate, judgment_basis = "", disposition_basis = the matched local route claim }, drop m from registration (m never enters pending); scan continues with remaining mismatches
Phase 0 → split (pre-certify): KindBinding.atomicity = non-atomic  -- the binding bundles two distinct aspects, so it asserts more than one claim → split into ONE atomic sub-mismatch per claim, each carrying the evidence that claim stands on, and re-run bind_kind → init_occurrence → certify on each (same Phase 0 pass, before any pass/route/defer decision) — each child is a distinct occurrence and is initialized as one, while the compound never is, init_occurrence running after the split for exactly that reason; recursive until atomic, and terminating because each split strictly decreases the number of bundled aspects. A non-atomic mismatch is split pre-registration, never deferred or registered as a compound. WHAT IT SPLITS ON is claim multiplicity and nothing else: one claim standing on evidence that lies in several places is ONE mismatch and is not split here, because how far a repair must reach is a question for the disposition and not for registration (Rule 23). PARTIAL adaptation stays legible without any splitting — identity(m) carries the evidence, so an adaptation reaching only part of it leaves a residue whose evidence has moved; that residue is a DIFFERENT identity, so it is not excluded from Mₑ, and it registers and surfaces on its own account (LOOP)
Phase 0 → defer (mismatch-local): certify(m).status = ambiguous  -- |claimed_by| ≠ 1, several claims holding or none; Λ.deferred := Λ.deferred ∪ {m} (the write the "no deferred-pending" guard reads; m is removed from Λ.deferred the moment its bounded re-assessment resolves, on every arm — pass, route, or Residual) → ONE narrowed-scope re-assessment at the fixed Phase-0 (R, X) (AI-side, no user interaction — Phase 0 stays silent), then re-certify. the bound is ONE attempt, because a silent phase cannot ask and a re-read of the same ground has no stopping rule of its own; what that attempt reaches is read at the turn: pass (→ registered), route (→ Route(routed_deficit) disposition), or — if it STAYS ambiguous — Residual: close m with DispositionRecord { judgment = None, judgment_by = unjudged, disposition = Residual, assigned_by = certificate, judgment_basis = "", disposition_basis = the unresolved attribution as this re-assessment found it — several claims standing, or none } (never registered into pending, adjudicated by disposed(aspect(m)), surfaced as residual, non-blocking). deferred-pending therefore always clears (no Phase-0 loop)
Phase 0 → deactivate (no in-scope mismatch): Mᵢ ≠ ∅ ∧ Mᵢ_passed = ∅ ∧ adjudicated(Λ.R, X) ∧ no deferred-pending ∧ pending = ∅  -- mismatches WERE detected but EVERY one carries a certificate-assigned disposition, Route(routed_deficit) or Residual (discharged via disposed(a), CONVERGENCE) — no mismatch the own claim holds enters the disposition loop → trivial convergence: R_final := Some(Λ.R), emit the routed deficits with their command hints (/gap, /inquire, /bound) and deactivate without adapting Λ.R. DEFERRED mismatches (atomic, status = ambiguous) do NOT satisfy this path: each first gets its ONE bounded re-assessment via Phase 0 → defer, resolving to pass (→ registered into pending), route (→ Route(routed_deficit)), or Residual. Trivial convergence fires once NONE remain deferred-pending and every flagged aspect is disposed
Phase 1: Mᵢ_passed → ∀m ∈ Mᵢ_passed: m.value_space := Some(bind_value_space(m)) [the third leg of the registration pipeline Rule 17 fixes in strict order — KindBinding, then the passing certificate, then the value space — so it runs HERE, between the pass filter and registration, and not at selection: every reader downstream needs it to already exist. Both Phase 1 relay closes reason about which pairings well_formed leaves, and Qc draws A ∈ V from it] → TaskCreate[all certificate-passing initial mismatches, each carrying its id] → pending → SelectNext(pending, Λ.fit_map) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →  -- register all certificate-passing initial mismatches, surface selected mismatch with fit basis [Tool]; reached only when Mᵢ_passed ≠ ∅. TWO-LEVEL DISPATCH — the JUDGMENT half resolves first, then the DISPOSITION half; the gate is not on the common path:
         ...
         [accepted(identity(Mₛ))] keep_carried_forward (below) closes Mₛ with the carried Keep — NO Qc, no Stop, no turn yielded; control returns to SelectNext. The JUDGMENT it records is whatever judgment_state holds for THIS cycle, never the prior record's
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND TaskUpdate(Mₛ, completed) as ONE step, so Mₛ leaves pending before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split where non-atomic] → match_identity(m) against pending and Σ.dispositions → Mₑ := those whose identity(m) ∉ { identity(p) : p ∈ pending } → ∀ detected m whose identity DOES match a pending p: REFRESH p's current reading from m (p.aspect, p.description, p.evidence, p.severity := m's) and TaskUpdate p's existing task to the refreshed subject, description AND activeForm [Tool] — each of the three embeds the reading — that task was created from the reading just superseded and is a separate copy outside Σ, so refreshing the pending object alone leaves the user's own tracked list showing a target two successions back — and register nothing: the registration is p's and stays p's, while what the user is shown about it tracks the advanced target [REQUIRED, not cosmetic: disposed(a) is aspect-keyed, so a pending object left holding a superseded aspect writes its record under a name no longer flagged — the currently-flagged name then has no record and no pending entry, disposed fails for it permanently, and adjudicated can never close. p.kind_binding is untouched: the identity match is what established there is nothing there to move. p.origin and p.unrepaired are untouched: provenance is stamped once] → ∀m ∈ Mₑ: init_occurrence(m) [TYPES: id — a detection that only refreshed a pending registration took none, so ids name occurrences the protocol carries and nothing else; origin from the match, Persisting / Emerged(Mₛ.id), the parent being the OCCURRENCE this cycle adapted so the chain resolves to one row; unrepaired := (identity(m) = identity(Mₛ)) — Mₛ being the mismatch THIS cycle's dispose just closed by adapting, and no other record [NOT any matching Adapt in the ledger: the ledger holds every Adapt this run has made, and a claim one of them legitimately removed can be re-introduced later by an unrelated Adapt, at which point that older record still matches and reports a failure that did not happen. Only the Adapt this re-scan is the re-scan OF can have failed to land here] [BEFORE certify: the match that decides the stamp has already run, and every m ∈ Mₑ is closed one way or another — gating the stamp on the certificate filter would leave each route/Residual close holding a Mismatch whose origin no step ever produced] → certify(m, local_claims) → (status = pass) → Mₑ_passed → ∀m ∈ Mₑ_passed: m.value_space := Some(bind_value_space(m)) [same registration leg as Phase 0, Rule 17] → TaskCreate[all Mₑ_passed] → pending → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
...
Phase 2 → withdraw (run-terminal): d = Discard(replacement) ∧ j = Upheld  -- the user upholds the mismatch and WITHDRAWS the evaluated result instead of adapting it. discard(replacement, Λ.R) [mutating, TOOL GROUNDING discard → Edit/Write: the artifact is actually withdrawn and the replacement put in its place, or removed outright when replacement = None] → THEN R_final := replacement (already Option(Result): None when the withdrawal leaves nothing in its place); Λ.withdrawn := Some((Mₛ, replacement)); then EVERY mismatch REMAINING in pending — Mₛ already left it at the dispose step above, so it cannot be closed twice — is closed with DispositionRecord { judgment = None, judgment_by = unjudged, disposition = Moot, assigned_by = loop, judgment_basis = "", disposition_basis = the withdrawal record } and TaskUpdate(completed). NO re-scan; deactivate. The adjudication claim stays on Λ.R (the object actually evaluated): every flagged aspect of it carries a ledger entry, and NO claim is made about the replacement
...
Phase 2 → split (pre-certify): KindBinding.atomicity = non-atomic  -- an emerged binding bundling two distinct aspects → split into ONE atomic sub-mismatch per claim, each carrying the evidence that claim stands on, then re-bind_kind → (pending exclusion) → init_occurrence → certify each (same re-scan pass); never deferred/registered as a compound. A child that lands in Mₑ therefore reaches certify already initialized, so a Route or Residual close on it holds a complete Mismatch like any other. The split runs BEFORE the pending exclusion and init_occurrence, both of which read identity(m) — a compound's identity would answer for claims it no longer owns
Phase 2 → defer (mismatch-local): certify(m).status = ambiguous  -- |claimed_by| ≠ 1; Λ.deferred := Λ.deferred ∪ {m} (same write as the Phase 0 arm, same removal on resolution) → emerged (atomic) mismatch whose claims do not resolve to one → ONE bounded narrowed-scope re-assessment at the fixed re-scan (R', X) (one attempt, on the same ground as the Phase 0 arm) → pass (→ registered) / route (→ Route(routed_deficit)) / Residual (closed with assigned_by = certificate), re-certify BEFORE registration
...
A relay-assigned Keep never reaches Phase 2 at all: judgment_relay_overruled and keep_carried_forward each close their mismatch inside Phase 1, non-mutating, breeding no Mₑ and triggering no re-scan, and control returns directly to SelectNext over the remaining pending.
PENDING IDENTITIES ARE UNIQUE, and the Mₑ exclusion below is what maintains it: no two registrations in pending carry the same identity(m). init_occurrence maintains it on every scan: it opens no second occurrence for an identity already initialized this scan or already pending, so several detections of one claim become one occurrence, and a detection matching something pending refreshes that registration instead of adding a second. Without this the exclusion and the refresh both become "which one", and an adaptation could close one of two identical pending registrations while a failed re-scan silently refreshed the other.
Mₑ is the Eval output — bound and split FIRST, since everything below reads the binding — minus what already sits in pending, matched by identity(m): a mismatch whose predicate or evidence has moved is not the pending one and registers on its own. A detection that DOES match a pending registration is not simply dropped: it carries this target's reading of a claim registered against an earlier one, so the pending object takes that reading (aspect, description, evidence, severity) and its registered task is updated to match (subject, description AND activeForm — each embeds the reading, so refreshing only some of them leaves the user's list half-advanced), and nothing is registered — and it keeps its OWN id, kind_binding, origin and unrepaired, since a refreshed registration is the same occurrence carrying a newer reading, not a second one. Dropping it whole is what leaves a registration describing a target two successions back — and since disposed(a) is keyed on the aspect, a stale one there is not merely a display defect but a convergence one. A mismatch CLOSED earlier in this run is not excluded from Mₑ either: it re-registers, which un-disposes its aspect (disposed(a), CONVERGENCE) so no record written for an earlier registration can carry a terminal. The ledger match locates the record a return succeeds and discharges nothing (status↔ledger equivalence, pending); what it settles is how the return is STAMPED and what the user is told about it — origin = Persisting where a closed record shares the predicate, so it stays outside the Emerged chain the convergence output reads, Emerged(Mₛ.id) otherwise — the parent named by the occurrence this cycle adapted — and unrepaired(m) where identity(m) = identity(Mₛ). Phase 1 closes it by keep_carried_forward only where the user's Keep answered THIS identity; a residue sharing only the predicate is a different registration and reaches the gate.
Bind + certify each newly emerged mismatch at registration (fail-closed): only certificate-passing emerged mismatches (Mₑ_passed) are registered into pending; a routed mismatch is closed with Route(routed_deficit) and handed to that deficit (GapUnnoticed, ContextInsufficient, BoundaryUndefined — command hints /gap, /inquire, /bound); a non-atomic mismatch is split into one atomic sub-mismatch per bundled claim and re-certified; an atomic ambiguous mismatch gets its one bounded re-assessment at the fixed re-scan (R', X) → pass / route / Residual, before registration. AssessFit classifies tracked mismatches but never suppresses them.
...
CHECKED IN THIS ORDER:
  1. If pending non-empty: return to Phase 1 (SelectNext by severity, then FitRank, then oldest registered task).
...
On user ESC: R_final := Some(Λ.R); present partial transformation trace ranging over Σ.dispositions as accumulated so far, AT THE SAME GRANULARITY the converged trace uses — one row per registration grouped under its aspect (Convergence evidence, above): a run that stops mid-move is where the move most needs to survive, so this is the last exit that may collapse it, then declare remaining pending mismatches as unresolved residual — they receive no DispositionRecord, which is why an exit yields EarlyExit rather than ContextualizedExecution (disposed(a) fails for them).
Convergence evidence: At adjudicated(Λ.R, X), present the transformation trace ranging over Σ.dispositions — ONE ledger, since every close writes a record there whoever assigned it. ONE ROW PER REGISTRATION, GROUPED UNDER THE ASPECT, EACH ROW HEADED BY ITS id: an aspect that registered more than once across TARGET SUCCESSION carries a record per registration, and each keeps its own row in close order under that aspect rather than collapsing into the last one. Where consecutive rows under one aspect stand on different evidence, each says what its own registration rested on, since that is what separates them — the aspect name does not. The id is what a reader points AT: it is the one thing on the row that is unique to it, so a decision taken about a row (carry this one on, split it out, leave it) survives being written down away from the trace.
  WHY NOT COLLAPSE THEM: what a single final row erases is not detail, it is a MOVE. A partial adaptation narrows the claim — the registration that follows stands on less than the one before it — so the last close disposes of the narrowed claim, and reporting only that close states the narrow disposition as if it answered the whole. The user is then left with no sign that there was an earlier framing at all, and re-entering on this result (the protocol's own way of getting a verdict re-checked) reads this trace to decide where to pick the work back up. A move that is invisible cannot be returned to. The collapse was reaching for reader burden, and that is a real cost, but it is paid on the wrong side: an extra row is read once, while a move erased is unrecoverable from the artifact.
  This is the same signal Chain discovery carries for emerged mismatches — which adaptation opened which follow-on — held for the case where the follow-on is what an adaptation LEFT rather than what it spawned.
  Each row also carries the two readings the ledger supports and a label cannot. RETURNED: where this row's identity matches an occurrence already closed, the row names the MOST RECENTLY CLOSED such occurrence — one id, chosen by close order, so the reading is decidable when several already share that identity. The reader sees "this is id 5 come back" rather than a second thing that happens to read alike, and the ledger settles it because identity(m) is the comparison, not the aspect. Where more than one preceded it, naming them all would answer a different question, and CIRCLING below is the one that answers it. CIRCLING: where one identity has accumulated several occurrences across the run, the count is stated once beside them. A FRAMING SIGNAL AND NOT A SCORE, on the same terms as Chain discovery: it says the run keeps returning to this claim, and says NOTHING about how near convergence is — Rule 22 already holds that registration cardinality settles no identity question, and this readout does not smuggle it back as one.
  Each row still notes how many targets an acceptance was carried across and whether that registration was stamped unrepaired — an adaptation the user asked for that did not take is the one thing a converged run must not report as simply resolved. That stamp answers only for the registration it was written on; a partial adaptation leaves a residue with its own row, and the earlier Adapt row is what says the adaptation happened. For each registration show (id → aspect → judgment → disposition) at its close, reading:
...
disposed(a)        = (∃ r ∈ Σ.dispositions : aspect(r.mismatch) = a) ∧ a ∉ { aspect(m) : m ∈ pending }
                     ...
                     -- close, so the return opens its own task and sits in pending, which is what fails the second conjunct while it is open.
                     -- The label is safe HERE for a reason worth stating: the Ledger invariant puts every flagged mismatch in exactly one of two
                     -- places, closed-with-its-own-record or pending, so an empty second conjunct at this label already gives every identity
                     -- flagged under it a record of its own. Nothing here assumes aspects stay stable or uniquely labelled across the R→R'
                     -- trajectory — that assumption is what let one label's earlier record discharge a registration it was never written for
unrepaired(m)      = m.unrepaired                              -- READS THE STAMP the re-scan wrote; it is NOT re-derived from the ledger here
                     -- a FAILED REPAIR: m came back from the re-scan carrying the same predicate on the same evidence as a record that closed it
                     -- by adapting. Nothing the claim stands on moved, so the adaptation did not take. This is what re-scanning alone cannot say — a
                     -- re-scan reports that the mismatch is there, not that an answer already given failed to land
                     -- WHY A STAMP AND NOT A LEDGER QUERY: the ledger only appends, so a query over it answers about all of history, and history
                     -- cannot tell a failed repair from a re-introduced one. An Adapt closes identity I; the next re-scan does not find I, so that
                     -- adaptation landed; later a different Adapt re-introduces I. I's Adapt record is still there — and is still the most recent
                     -- record for I — so any query form, history-wide or latest-record, reports a failure that did not happen. The one turn at which
                     -- the question is decidable is the re-scan IMMEDIATELY FOLLOWING the close, where the absence or return of I is observable, and
                     -- that is the turn match_identity already runs (TOOL GROUNDING). So the answer is written there and read back afterwards
                     -- MOVING THE QUESTION TO THAT TURN IS ONLY HALF OF IT. A stamp written there but decided against "any matching Adapt record"
                     -- re-admits the same counterexample one layer down, since the ledger reachable at that turn still holds every earlier Adapt. The
                     -- stamp is therefore decided against Mₛ ALONE — the mismatch this cycle's dispose closed by adapting, which is the only
                     -- adaptation this re-scan is testing (PHASE TRANSITIONS)
                     -- BOTH LATER READS ARE READ-BACKS: the Phase 1 gate materializes option 3 away from a direction that already failed (Rule 11),
                     -- and the convergence trace reports a directed adaptation that did not take. Neither turn can decide the question itself
                     -- READABLE AT THE RE-SCAN because the Phase 2 ORDER clause writes the record BEFORE Eval runs: the record this reads against
                     -- is already in Σ by the time the return is detected
...
accepted(i)        = ∃ r ∈ Σ.dispositions : identity(r.mismatch) = i ∧ r.disposition = Keep ∧ r.judgment ∈ {Some(Upheld), None}
                     -- the user's own acceptance of THIS mismatch: answered at Qc, or declared in the bulk keep-all-remaining exit.
                     -- Read as the guard on Phase 1 → keep_carried_forward, which closes a re-registered accepted mismatch without a gate
                     -- IDENTITY-KEYED rather than label-keyed, and the two come apart exactly where it matters: a return whose predicate AND
                     -- evidence both match is the thing they answered and carries, while a residue sharing only the predicate is a DIFFERENT
                     -- registration and reaches the gate on its own. Carrying that one on the strength of a shared label would answer for the
                     -- user about a mismatch they were never shown — the same silence the label produced wherever it was left deciding which record
                     -- answered for which registration
                     -- RUN-SCOPED, and monotone WITHIN a run: Σ is bound fresh at activation (MODE STATE, INITIAL BINDING) and the ledger
                     -- only appends, so once this holds for an identity it holds for the rest of the run — that REGISTRATION reaches no further
                     -- Qc, and Adapt is out of its reach until the protocol is re-entered. A residue of the same kind is not covered by it: a
                     -- different identity is judged and disposed on its own. THIS guard's exit: re-entering on the same result
                     -- rebuilds Σ, so accepted(i) is false again and the mismatch returns to the gate. judgment_relay_overruled does NOT share
...
trivial convergence (all-disposed at registration): when Mᵢ ≠ ∅ but Mᵢ_passed = ∅ AND every flagged aspect is closed with a certificate-assigned disposition, Route(routed_deficit) or Residual — aspect-keyed via disposed(a) over the flagged aspects — (no deferred-pending, pending = ∅), adjudicated(Λ.R, X) holds by disposed(a) for every flagged aspect (and warranted for the rest) — Λ.R is unadapted, R_final := Some(Λ.R), and contextualized(Λ.R) holds. This is the Phase 0 → deactivate (all-routed) path. DEFERRED mismatches carry no ledger entry yet, so disposed(a) does not cover them and they do not satisfy this. Distinct from the no-mismatch case (Mᵢ = ∅, every aspect warranted from the start) — here aspects were flagged but every one is held by a local route claim rather than by the own claim
gateless convergence: when either Phase 1 relay — judgment_relay_overruled or keep_carried_forward — closes the last mismatch in pending, adjudicated(Λ.R, X) follows from disposed(a) for every flagged aspect, and R_final := Some(Λ.R) — the ordinary non-withdrawal result equation, reached without a final gate. Neither relay close performs an adaptation, so Λ.R stands as whatever earlier Adapt dispositions left it as
...
--   the proxy rests on the status↔ledger equivalence at pending, scoped to registered mismatches. Certificate-assigned closes carry no
--   task and are covered by disposed(a) directly
progress(Λ) = 1 if |total_tasks| = 0 else |resolved_tasks| / |total_tasks|   -- resolved_tasks = registered tasks their OWN close completed; that close writes its DispositionRecord in the same action, so this counts closes rather than ledger matches, and a re-registration is a NEW task that stays unresolved until it is closed on its own account (matches the ContextualizedExecution resolution contract)   -- total_tasks = 0 (Mᵢ = ∅, or Mᵢ≠∅∧Mᵢ_passed=∅ trivial convergence via routing) is fully converged, not undefined — the Mᵢ = ∅ leg only after the Rule 9 zero-mismatch confirmation (AcceptNoMismatch, or Reopen whose focused re-scan stays ∅); otherwise NON-MONOTONE: may regress when re-scan over the mutated R' registers newly certified mismatches (transformative-revalidation signature)
...
bind_kind (sense)   → Internal analysis (capture each detected mismatch as a KindBinding {label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity}. atomicity carries the backbone's meaning — how many distinct aspects this binding bundles — so a detector output asserting two claims lands non-atomic and is split into one atomic sub-mismatch per claim before certify. It is never asked how far a repair would reach: one claim standing on evidence in several places is atomic, and where a repair lands is projected later from an answered disposition (RepairSite, Rule 23). Runs before certify, before the pending exclusion, and before the origin stamping — all three read the binding)
init_occurrence (track) → Internal state update (writes id, origin and unrepaired onto a detected mismatch, at the one position every scan path states: after split, before certify. It is the sole producer of all three — no other step writes them, and no reader of a DispositionRecord can reach a mismatch that skipped it, which is what the single position buys. It performs no user interaction and touches no artifact)
bind_value_space (track) → Internal state update (generates a mismatch's answer space from its passing certificate and writes it to that mismatch's own value_space field, which is what associates the space with the occurrence its readers will look it up on; the third leg of the registration pipeline, running between the pass filter and registration on BOTH scans — Mᵢ_passed at Phase 0 and Mₑ_passed at the Phase 2 re-scan — per the strict order Rule 17 fixes. Every downstream reader needs it already bound: both Phase 1 relay closes reason about which pairings well_formed leaves, and Qc draws A ∈ V from it)
match_identity (sense) → Internal analysis (at the Phase 2 re-scan, after bind_kind + split and before TaskCreate: compare identity(m) — the binding's positive_predicate together with the evidence set it stands on — against pending and against every record in Σ.dispositions. Settles four readings and nothing else: whether the return is already pending (excluded from Mₑ — and where it is, the pending object takes this detection's current reading, LOOP), how its origin is stamped (Persisting where a closed record shares the predicate, Emerged(Mₛ.id) otherwise), and whether unrepaired is stamped, which is settled against Mₛ alone rather than against the records this comparison scans: identity(m) = identity(Mₛ) and nothing weaker. It reads no id and assigns none — an id names an occurrence and never decides whether two occurrences are the same claim, which is this comparison's whole job (TYPES, MismatchId). THIS TURN IS WHERE unrepaired IS DECIDED and the only one where it can be — the ledger only appends, so asking it later cannot separate a repair that failed from an identity a later Adapt re-introduced — and Mₛ is the only Adapt this turn is testing, so widening the comparison to the ledger would put that same conflation back inside the stamp (CONVERGENCE, unrepaired(m)). It never completes the new registration's task — a record answers only for the registration it closed, and reading a match as a close is the defect this comparison replaces)
certify (extension) → Internal analysis (fail-closed DeficitFitCertificate; fit of KindBinding.positive_predicate against the own claim and the route claims inscribed in this SKILL.md, reading nothing outside this file: claimed_by = {ApplicationDecontextualized} when the own claim holds alone; status = pass | route | ambiguous; basis = the cited claim fit, shown at the mismatch's Phase 1 surfacing. Relay (Extension): the fit is grounded in a citable source, and an unclear fit returns status = ambiguous → defer. Runs at registration time — within Phase 0 for Mᵢ, within Phase 2 re-scan for Mₑ — BEFORE the mismatch enters pending)
...
adapt  (transform) → Edit, Write (Adapt(direction) disposition: result adaptation based on user direction. THIS STEP IS WHERE RepairSite IS PRODUCED — the answered direction is projected onto Λ.R here and nowhere earlier, so the locus exists only once the disposition it comes from has been given, Rule 23)
discard (transform) → Edit, Write (Discard(replacement) disposition: withdraw the result and put the replacement in its place, or remove it outright when the withdrawal leaves nothing behind. Produces RepairSite on the same terms as adapt above)
                    -- (transform): tool call that changes existing artifacts; medium-agnostic (files, analysis text, generated content)
                    -- RepairSite is a runtime value of these two steps and is written to no field: it reaches no binding, no record, and no
                    --   predicate, which is what keeps the three axes ordered rather than merely named (Rule 23)
...
--   Λ.withdrawn := None; Λ.reopen_focus := None; Λ.judgment_state := None; Σ := { dispositions = [], scan_count = 0 }.
--   Λ.fit_map is bound by the Phase 0 pass (Λ.fit_map := F)
...
-- Certificate invariant: ∀ m ∈ pending : m.certificate.status = pass (fail-closed — routed/ambiguous mismatches never enter pending)
-- Ledger invariant: every mismatch leaving pending, and every mismatch closed at registration, appends exactly one DispositionRecord (no close without a record; no record without a close). PER REGISTRATION: a mismatch that registers twice leaves pending twice and appends two records, one per close, which is why every flagged mismatch is in exactly one of two places at any moment — closed with its own record, or pending

── COMPOSITION ──
*: product — (D₁ × D₂) → (R₁ × R₂). Mismatch-domain resolution emergent via session context.
certificate-before-registration ∘ transformative-revalidation: the fail-closed certificate order composes with the NON-MONOTONE loop — when an Adapt disposition mutates R and Eval(R', X) breeds emergent mismatches Mₑ, EACH Mₑ is independently bind_kind'd + certified at re-scan registration (Phase 2) before it can enter pending. So only in-scope (ApplicationDecontextualized-owned) Mₑ_passed register, even as R mutates across cycles; the certificate gate holds under mutation-induced revalidation. A return that a closed record already covers passes the same gate on its own account: the ledger match identifies which registration it succeeds and never substitutes for that gate or for the close, so mutation cannot smuggle an unsurfaced mismatch through on an earlier registration's record.
...
3. **Bind, initialize, certify each detected mismatch at registration (fail-closed)** — the order and the steps are the formal ones (FLOW, PHASE TRANSITIONS): `bind_kind` → split where non-atomic → `init_occurrence` (which is the sole producer of `id`, `origin` and `unrepaired`, and runs before certify so that a certificate-assigned close already holds a complete occurrence) → `certify` → on a pass, `bind_value_space` into that mismatch's own `value_space` → registration. What follows explains the certificate outcomes; it does not restate the pipeline, and where this prose and a formal block appear to differ the formal block is the contract. For each candidate mismatch, set `m.kind_binding = { label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity }`. `atomicity` asks the backbone's question and only that one — how many distinct aspects this binding bundles: `atomic` where it asserts one claim, `non-atomic` where it bundles more. If `atomicity = non-atomic`, **split before certify**, one sub-mismatch per bundled claim. It is never asked how far a repair would have to reach: a single claim standing on evidence that lies in several places is `atomic` and is not split, because repair locus is projected later from an answered disposition (Rule 23). Then `m.certificate = certify(m.kind_binding, local_claims)` where the local claims are epharmoge's own claim and its route claims, both inscribed in this SKILL.md (TYPES: `OwnClaim`, `RouteClaim`); the certificate reads nothing outside this file. Fit the predicate against every one of them and collect `claimed_by`, the set of claims the evidence supports:
   ...
   - **`atomicity = non-atomic`** — the binding bundles two distinct aspects, so it asserts more than one claim → **split** into one atomic sub-mismatch per claim, each carrying the evidence that claim stands on, and re-run bind_kind + certify on each (same Phase 0 pass). A **partial** adaptation stays legible without this: `identity(m)` carries the evidence, so adapting part of what one claim rests on leaves a residue whose evidence has moved — a different identity, which registers and surfaces on its own account at the next re-scan.
...
6. If certificate-passing mismatches remain: record `Mᵢ_passed` with aspect, description, evidence, severity (per Rule 12 — behavioral-impact qualifier assessed against current-session task graph), the fields `init_occurrence` already wrote (`id`, `origin=Initial`, `unrepaired=false`), kind_binding, certificate, the `value_space` bound at the pass, and fit-map placement — proceed to Phase 1
...
- Done. One thing to verify about applicability:
  ...
  - **Returned unrepaired** — only where `unrepaired(m)` holds: [the adaptation they directed earlier, and that this re-scan found the same claim standing on the same evidence, so that direction did not land]. Materialize option 3 with a different direction rather than the one that already failed; that is type-preserving materialization of `Adapt`, not a change to the option set (Rule 11)
...
**When the user already accepted this mismatch** (`keep_carried_forward` fired — a closed record with the SAME identity carries a prior `Keep` the user answered, and it re-registered after an Adapt advanced the target; a residue sharing only the predicate is a different mismatch and reaches the gate instead): no gate is presented — they have already answered this. Report it — the aspect, that their earlier acceptance is being carried to the current target, and the reason they gave for accepting it — then close it against the current target and move on. What carries is **what they wanted done**, not the verdict: the record takes its judgment from this cycle, so the close claims nothing about the current target that this cycle did not establish.
...
After the user's answer `(judgment, disposition)`, **first** append the mismatch's one `DispositionRecord` to `Σ.dispositions` and complete its task — as a single step, so it leaves `pending` before any arm below runs — **then** act on the disposition:
...
- Re-evaluate the advanced target `Λ.R` against `X` for remaining AND **newly emerged** mismatches. Bind and split each detected mismatch first, then form `Mₑ`: it excludes only what already sits in `pending`, matched by `identity(m)` rather than by label. **A detection that IS already pending is not discarded** — it refreshes that registration's current reading (`aspect`, `description`, `evidence`, `severity`) and its task (`subject`, `description`, `activeForm`), keeping `id`, `kind_binding`, `origin` and `unrepaired`; skipping that leaves a registration recorded under a reading no longer flagged, and `disposed` never closes for the one that is (LOOP, Rule 22). For what does enter `Mₑ`, so a residue whose evidence has moved is not the pending one and registers on its own. A mismatch closed earlier in this run re-registers too — the match against the ledger tells the protocol what the return **is**, never that it is already done — and Phase 1 closes it by `keep_carried_forward` only where the user's `Keep` answered that same identity
- **Bind + certify each emerged mismatch at registration (fail-closed)** — this happens at the **Phase 2 re-scan** (AFTER the Adapt disposition has already mutated R into R'), temporally separated from the Phase-0 certification of Mᵢ: for each `m ∈ Mₑ`, set `m.kind_binding`, then run `init_occurrence` — one step writing `id`, `origin` and `unrepaired` from the identity match — `origin=Persisting` where a closed record shares this binding's predicate and `origin=Emerged(Mₛ.id)` where none does, naming the occurrence this cycle adapted rather than its label, and `unrepaired = (identity(m) = identity(Mₛ))` — true exactly where this return is the very mismatch this cycle closed by adapting, the adaptation the user directed having not taken, and decided against `Mₛ` rather than against any matching ledger record, since only this cycle's Adapt is the one this re-scan tests (CONVERGENCE) — and only then set `m.certificate = certify(m.kind_binding, local_claims)` (same local claims, same `pass | route | ambiguous` statuses as Phase 0). The stamp runs over **every** `m ∈ Mₑ` and reads the match rather than the certificate: every member of `Mₑ` is closed one way or another, so binding the stamp to registration would leave each certificate-assigned close holding a `Mismatch` whose `origin` no step ever produced. Then register only the certificate-passing members (`Mₑ_passed`); a `status = route` mismatch is closed with disposition `Route(routed_deficit)` and handed to that deficit (GapUnnoticed, ContextInsufficient, BoundaryUndefined — command hints `/gap`, `/inquire`, `/bound`); a non-atomic mismatch is split into one atomic sub-mismatch per bundled claim and re-certified; an atomic `status = ambiguous` mismatch gets its one bounded re-assessment at the fixed re-scan `(R', X)` → pass / `Route(routed_deficit)` / `Residual`, before registration. Do not filter the certificate-passing set by fit-map category.
...
**Chain discovery**: The `origin = Emerged(parent)` field records the causal chain by the parent occurrence's `id`, so a child resolves to exactly one trace row even where several occurrences share an aspect, and ranges over Emerged members ALONE. The reason is what this reading is FOR rather than a claim about `Persisting`: it answers which adaptation SPAWNED a follow-on, and a predicate-share stamp does not answer that (TYPES, `Origin`). What an adaptation LEFT is a different question and is carried by the per-registration convergence trace, so nothing goes unreported by the split — the two readings divide the ground rather than either covering less of it. This enables:
...
| Acceptance carried, never assumed | a mismatch the user accepted re-registers against the advanced target and closes by `keep_carried_forward` — reported, no gate. The carry is keyed on `identity(m)`, so a residue sharing only the label reaches the gate instead. The DISPOSITION carries; the judgment comes from the current target or is left unclaimed | The user is not re-asked what they already answered, is still asked about what they were never shown, and every terminal rests on a record written for the registration it closes |
...
| Each Phase 1 relay says where its correction lives | a relay-closed mismatch leaves `pending` and reaches no further Qc this run — that registration, not every mismatch sharing its label. A disputed CARRY reopens by re-entering on the same result (`accepted(i)` reads `Σ`, rebuilt at activation). A disputed RETRACTION has no such reset — `judgment_settled` reads `Λ.fit_map`, derived from `(Λ.R, X)`, so the same input reproduces the same close, and correcting it is the user's interrupt | The user is told what to reach for, rather than trying a reset that moves only one of the two guards |
...
8. **Convergence evidence**: Present transformation trace before declaring `adjudicated(Λ.R, X)` over the evaluated target; per-mismatch evidence is required, and **per-mismatch means per REGISTRATION, not per aspect label** — an aspect that registered more than once keeps a row per registration, in close order, grouped under that aspect. Collapsing them to the final close would report a narrowed claim's disposition as the whole claim's and erase that the framing moved, which is the one thing the trace is read for when deciding whether to re-enter. **This binds both exits** — the partial trace an ESC presents carries the same granularity, since a run that stops mid-move is where the move most needs to survive. Where the run ended in a withdrawal, the trace also states that the verdict's target is the replacement and that no fit claim is made about it
...
20. **A prior acceptance is relayed forward, never assumed away**: where the user has already answered `Keep` for a mismatch and that same mismatch — matched by `identity(m)`, the binding's predicate together with the evidence it stands on, never by its label — re-registers against an advanced target, the DISPOSITION is settled by their own earlier utterance — a distinct relay ground from Rule 10's, which turns on evidence collapsing the option set. **This rule's compiled control path is `keep_carried_forward`** (PHASE TRANSITIONS Phase 1, TOOL GROUNDING), guarded by `accepted(identity(Mₛ))` and reached at the disposition half, after the judgment half has resolved against the current target. **The carry is disposition-only.** A judgment is a claim about how a specific target stands, so it expires with the target it was made about: the record this close writes takes its verdict from this cycle's `judgment_state` — a relayed `Upheld` with its own cited basis, or none at all — and a prior verdict never stands in for one. What transfers is the user's preference about what to do, which is theirs to hold across targets — across TARGETS, not across every mismatch that happens to share a label. A residue of the same kind, whose evidence has moved, is a registration they have not been shown, and carrying their answer onto it would answer for them. The carry-forward writes its own record against the target it closes and reports first the reason the user gave for ACCEPTING the aspect — the ledger keeps a ground per axis for exactly this reader, so a carry can never cite why the mismatch stood in place of why the user accepted it — and so no terminal ever rests on a record written about a target the run has moved past. **Where the reopen lands**: `accepted(i)` is run-scoped and monotone within the run, so a carry the user disputes is reopened by re-entering Epharmoge on the same result — a fresh `Σ` returns the mismatch to the gate — and NOT by a further turn inside the run, where the aspect has already left `pending` and reaches no Qc. This exit is **this guard's alone**: `judgment_relay_overruled` reads `Λ.fit_map` rather than `Σ`, so a rebuilt ledger leaves it firing on the same evidence, and correcting a disputed retraction is the user's interrupt rather than any re-entry. Suppressing the aspect at detection instead would reach the same silence with neither the fresh record nor this exit.
...
22. **Three names, three questions — and none of them substitutes for another**: `identity(m)` says WHICH CLAIM this is (the binding's `positive_predicate` with the `Set(Evidence)` it stands on), `id` says WHICH OCCURRENCE of that claim the reader is looking at, and `aspect` says HOW IT READS on the current target. Each answers something the other two cannot, and every defect this rule guards against is one name doing another's work: a label deciding sameness, a handle counted as if it settled sameness, or an identity asked to say which of its occurrences is meant. **The identity half — verified against the binding, never read off the label**: whether a re-detected mismatch is one a record already closed is settled by `identity(m)` — the binding's `positive_predicate` together with the `Set(Evidence)` it stands on — and never by `aspect`, which is a display name. The predicate is the claim itself rather than a name for it, which is what makes this a verification rather than one more label comparison. Two consequences, and the protocol needs both. **Upstream**: because identity is the claim together with the evidence it stands on, a **partial** adaptation needs no splitting to stay legible — adapting part of what a claim rests on leaves a residue whose evidence has moved, and a moved evidence set is a different identity, so the residue is not excluded from `Mₑ` and registers and surfaces on its own account. Splitting is reserved for what `atomicity` actually asks — a binding that bundles two distinct aspects (Rule 17(b)) — and never for how far a repair would have to reach, which is settled after the disposition is answered and not before (Rule 23). **Downstream**: where a return does re-register, the ledger match tells the protocol what it IS and never that it is done — a record answers for the registration it closed and for no other, so the return opens its own task. A match on predicate AND evidence is the same claim on unmoved ground, and where that record closed by adapting, `unrepaired(m)` says the adaptation did not take: the one thing a re-scan cannot say on its own, since a re-scan reports that the mismatch is there, not that an answer already given failed to land. A match on the predicate alone is a residue — a registration the user has not been shown, owed its own surfacing rather than a carried `Keep` (Rule 20). Registration cardinality is not identity: how many times a mismatch has registered settles nothing here, because the question is whether this registration is that one, and only the binding answers it. That the convergence trace REPORTS how many occurrences a claim has accumulated is not a counter-example but the same distinction from the other side — the count is a framing signal the reader weighs, and it enters no predicate that decides sameness, disposal, or convergence. **Where the return does NOT re-register** — its identity is one already pending — the detection is still not discarded: it carries the advanced target's reading of a claim registered against an earlier one, so the pending object takes that reading (`aspect`, `description`, `evidence`, `severity`) and its registered task is updated to match it (`subject`, `description`, `activeForm` — all three carry the reading), while its `kind_binding`, `id`, `origin` and `unrepaired` stay put: the match established there is nothing to move in the first, and the other three name the occurrence and where it came from, which a newer reading does not change. A registration left describing a target two successions back is not merely showing the user a stale sentence: `disposed(a)` is keyed on the aspect, so a superseded one there writes its record under a name no longer flagged, and the name that IS flagged then has neither a record nor a pending entry and can never be discharged. **`origin` and `unrepaired` do not refresh**, and the reason is the same one that makes the refresh necessary — they are provenance, answerable only at the turn that produced them. `unrepaired` in particular is stamped, never re-derived, and is decided against `Mₛ` alone: the ledger only appends, so asking it afterwards cannot tell a repair that failed from an identity some later `Adapt` re-introduced, and the one turn where the difference is observable is the re-scan the close is followed by. Moving the question to that turn is only half of it — a stamp decided there against *any* matching `Adapt` record re-admits the same conflation, because the ledger reachable at that turn still holds every earlier one. Only the adaptation this re-scan is the re-scan OF can have failed to land in it.

23. **Kind, then resolution form, then repair locus — and never the reverse**: three axes, answered in that order. What KIND of misfit this is (`kind_binding.label` and the predicate under it) is settled at detection; what FORM its resolution takes (`ResolutionForm`, the axis `Disposition` ranges over) is settled next — at Qc where the user answers, and by the certificate, a relay, or the loop on the closes that reach no gate, exactly as `Disposition` names its producers; WHERE the change lands (`RepairSite`) is projected when `adapt` or `discard` executes, and at no earlier point — those two are the only dispositions that touch the artifact, and both are user-answered, so a projected locus always stands on an answer someone gave. The order is the rule, not a sequence that happens to be convenient. **Repair locus types nothing upstream of itself**: it does not individuate a mismatch, does not decide how many mismatches there are, does not enter `identity(m)`, does not gate registration, and appears in no convergence predicate. A mismatch is individuated by what it claims and what that claim stands on; where a repair would land is a property of an answer nobody has given yet, so reading it back into the deficit asks the user's question on their behalf and answers it by counting. **What this forbids concretely**: letting `atomicity` — a shared backbone field asking how many distinct aspects a binding bundles — be re-pointed at how far the evidence spreads across places an edit could reach. That re-pointing splits one claim into several registrations before the user has said anything about how they want it handled, multiplies what they are asked, and leaves the same field name meaning one thing here and another in the protocols this schema is shared with, where nothing checkable can see the divergence because the field list itself still matches. **What stays available**: a resolution may still be partial, and it stays legible without any of that — identity carries the evidence, so a partial adaptation leaves a residue that is a different identity and surfaces on its own account (Rule 22, upstream half).
