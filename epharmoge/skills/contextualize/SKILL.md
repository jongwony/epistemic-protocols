...
absorb = HOW A SCAN'S DETECTIONS BECOME WHAT THE PROTOCOL CARRIES: Set(Mismatch) × Set(Mismatch) → Set(Mismatch), folding the detections into the carried set ONE AT A
                 ...
                 --                MEETS, so several detections of one claim fold to one and the order they arrived in decides nothing about what the set ends up
                 --                holding — there is no last write to win
...
Phase 2 → split (pre-certify): KindBinding.atomicity = non-atomic  -- emerged compound mismatch → split into atomic sub-mismatches, then re-bind_kind → absorb → certify each (same re-scan pass); never deferred/registered as a compound. The split runs BEFORE the pending exclusion and init_occurrence, both of which read identity(m)
