...
Epharmoge(R, X) → Eval(R, X) → Mᵢ? →
  ...
  Mᵢ ≠ ∅: ∀m ∈ Mᵢ: bind_kind(m) [split where non-atomic] → init_occurrence(m) → keep(Some) → certify(m, local_claims) → keep(status = pass) → Mᵢ_passed → ∀m ∈ Mᵢ_passed: m.value_space := Some(bind_value_space(m)) →
    ...
    Mᵢ_passed ≠ ∅: AssessFit(R, X, Mᵢ_passed) → F → Register(Mᵢ_passed) → SelectNext(pending, Λ.fit_map) → Mₛ → judgment_settled(Mₛ, Λ.fit_map)? →
      ...
      [d = Adapt(direction)] adapt → R' → Λ.R := R' → Eval(Λ.R, X) → bind_kind each [split where non-atomic] → match_identity each → Mₑ (those that read as no claim already pending) → each detection that DOES match a pending one refreshes that registration (its reading AND its entry; id and kind_binding unchanged, unrepaired taking this re-scan's stamp on it, set-only) and registers nothing → ∀m ∈ Mₑ: init_occurrence(m) → keep(Some) → certify(m, local_claims) → keep(status = pass) → Mₑ_passed → ∀m ∈ Mₑ_passed: m.value_space := Some(bind_value_space(m)) → Register(Mₑ_passed) → AssessFit(Λ.R, X, pending) → F' → (loop: back to Phase 1 above — judgment half, then disposition half — until contextualized)
...
MismatchId = a run-local handle naming ONE OCCURRENCE the protocol carries — assigned in ascending order at the moment the occurrence is kept
                 ...
                 --   mismatches are the same claim — that stays identity(m). It is a handle for pointing, and pointing only (Rule 23)
...
init_occurrence = the ONE step that turns a detected mismatch into an occurrence the protocol carries: Mismatch → Option(Mismatch), producing every field no earlier step
                 ...
                 inherits one). It writes exactly two fields, each by one rule that reaches every path:
                 ...
                 --                THAT READING IS WRITTEN ONTO THE SURVIVOR (aspect, description, evidence, severity), the same fields a refreshed
                 --                pending registration takes (LOOP, Mₑ clause). The answer is the run's; that it is written down is not, and a
                 --                judgment left nowhere is a judgment nobody made — the gate would surface whichever reading the pass reached first
                 --                THAT ARM RETURNS None, which is what the Option carries: a detection that opens no occurrence is a value
                 --                here rather than a drop rule the caller has to invent, and each scan pass carries the Some results forward
                 --   id        := the next handle in this run's order
                 --   unrepaired := (m reads as the same claim as Mₛ, off identity) — false on a Phase 0 scan for want of an Mₛ, by the same rule rather than by a separate default
Mismatch = { id: MismatchId, aspect: String, description: String, evidence: String, severity: Severity, unrepaired: Bool, kind_binding: KindBinding, certificate: DeficitFitCertificate, value_space: Option(ValueSpace) }
                 ...
                 --   CURRENT READING: a re-detected mismatch matching a registration already pending refreshes them on that registration (LOOP, Mₑ
                 --   clause). kind_binding is IDENTITY and cannot move. unrepaired is neither — it is a MONOTONE STAMP: every re-scan decides it
                 --   afresh against that turn's Mₛ and it lands on whatever registration carries the detection, so a later re-scan can raise it and
                 --   none lowers it
                 -- value_space is Option-valued: a mismatch closed by certificate (Route, Residual) is complete with None, never having had a space.
                 ...
                 --   THE RULE IS ONE; ITS LANDING SITE IS WHEREVER THE DETECTION GOES. A detection that opens an occurrence takes the stamp there
                 --   (init_occurrence); a detection that folds into a registration already pending takes it onto that registration (LOOP, Mₑ clause).
                 --   Both are needed because a detection can read as the claim just adapted while also reading as one already pending, and the fold
                 --   would otherwise carry the answer nowhere. Set-only, never cleared: what it records happened
...
KindBinding    = { label: Axis, positive_predicate: String, evidence: Set(Evidence), origin ∈ {seed, emergent}, atomicity ∈ {atomic, non-atomic} }
                 ...
                 --   is RepairSite (below), which is projected from a disposition already answered and types no mismatch (Rule 22)
...
identity(m: Mismatch) = (m.kind_binding.positive_predicate, m.kind_binding.evidence)
                 ...
                 -- WHO READS IT: accepted(i) and unrepaired(m) (CONVERGENCE), and the pending exclusion the LOOP's Mₑ clause states
...
Mᵢ     = Identified mismatches from Eval(R, X)                 -- each bind_kind'd + certified at registration (Phase 0)
...
pending    = Set(Mismatch) where entry(m).status ≠ completed  -- a view over the carrier C, reached at Λ.carrier, which is where registration status lives; Σ holds the ledger and the scan count and never held this. A routed/ambiguous mismatch never enters pending; only certificate-passing mismatches are registered
...
ResolutionForm = the axis Disposition ranges over — the SHAPE a resolution takes, answering what becomes of the result. Kind (kind_binding.label
                 ...
                 where that lands. THREE ORDERED AXES, and the order is the point (Rule 22)
RepairSite     = THE NAME OF THE THIRD AXIS — where a resolution lands on Λ.R when adapt or discard executes it. A NAME AND NOT A CARRIER:
                 ...
                 --   does not gate registration, and appears in no convergence predicate. Asking it first is the failure Rule 22 names
...
── PHASE TRANSITIONS ──
Phase 0: R → Eval(R, X) → Mᵢ? → ∀m ∈ Mᵢ: bind_kind(m) [split where non-atomic] → init_occurrence(m) [TYPES: id, unrepaired = false — before certify, so a route/Residual close holds a complete Mismatch; None where this scan already initialized an occurrence for the identity — that detection's reading lands on the surviving occurrence (TYPES, init_occurrence) — and the pass continues with the Some results] → certify(m, local_claims) → (status = pass) → Mᵢ_passed → AssessFit(R, X, Mᵢ_passed) → F → Λ.fit_map := F  -- applicability checkpoint + registration-time KIND dispatch (fail-closed) + fit map (silent); certify runs WITHIN Phase 0, at registration, not as a separate phase
Phase 0 → confirm_no_mismatch: Mᵢ = ∅ → Qz(zero_mismatch_finding) → Stop → ZeroMismatchConfirmation  -- true zero-mismatch case (distinct from the Mᵢ≠∅∧Mᵢ_passed=∅ trivial-convergence-by-routing case below); AcceptNoMismatch → R_final := Some(Λ.R), deactivate (execution stands as-is, Rule 9); Reopen(aspect) → reopen_focus := aspect → re-scan Eval focused on that aspect → reopen_focus := None (cleared after the focused re-scan, either arm); [Mᵢ ≠ ∅] re-enter the Phase 0 pipeline above (bind_kind → init_occurrence → certify → Mᵢ_passed → bind_value_space → AssessFit → F) and proceed to Phase 1; [Mᵢ still ∅] relay(finding) → R_final := Some(Λ.R), deactivate (one attempt per aspect) [Tool]
...
Phase 2: A = (j, d) → dispose(Mₛ, j, d) → [ORDER: dispose appends DispositionRecord{ mismatch = Mₛ, judgment = Some(j), judgment_by = relay when judgment_state = Some((Upheld, _)) for this cycle else user, disposition = d, assigned_by = user, judgment_basis = that relayed basis when the verdict was relayed, else the ground the user gave for the VERDICT, disposition_basis = the ground the user gave for the DISPOSITION } to Σ.dispositions AND record update(Λ.carrier, entry(Mₛ).status := completed) as ONE step, so Mₛ leaves pending before any arm below runs. Reached only from Qc; the relay-assigned close is written by judgment_relay_overruled at Phase 1] →
         [d = Adapt(direction)] adapt(direction, Λ.R) → R' [mutating — transformative revalidation] → Λ.R := R' [TARGET SUCCESSION] → Eval(Λ.R, X) → ∀ detected m: bind_kind(m) [split where non-atomic] → match_identity(m) against pending and Σ.dispositions → Mₑ := those that read as no claim already pending → ∀ detected m whose identity DOES match a pending p: REFRESH p's current reading from m (p.aspect, p.description, p.evidence, p.severity := m's) and record update(Λ.carrier, entry(p) := the refreshed reading) [Tool] — the entry is a separate copy outside Σ, so refreshing the pending object alone leaves the durable record showing a superseded target — and register nothing: the registration is p's and stays p's [REQUIRED, not cosmetic: disposed(a) is aspect-keyed, so a registration left holding a superseded aspect can never be discharged. p.kind_binding and p.id are untouched, and p.unrepaired takes this re-scan's stamp by the same rule every detection takes it by — set where this detection reads as the claim the dispose just closed by adapting, and never cleared, so a repair that came back is recorded on whatever registration carries the detection forward] → ∀m ∈ Mₑ: init_occurrence(m) [TYPES: id — a detection that only refreshed a pending registration took none, so ids name occurrences the protocol carries and nothing else; None where this re-scan already initialized an occurrence for the identity, that detection's reading landing on the surviving occurrence as it does for a refreshed registration (TYPES, init_occurrence), and Mₑ carries the Some results; unrepaired := (m reads as the same claim as Mₛ, off identity) — Mₛ being the mismatch THIS cycle's dispose just closed by adapting, and no other record (CONVERGENCE, unrepaired(m))] [BEFORE certify: every m ∈ Mₑ is closed one way or another, so the stamp is not gated on the certificate filter] → certify(m, local_claims) → (status = pass) → Mₑ_passed → ∀m ∈ Mₑ_passed: m.value_space := Some(bind_value_space(m)) [same registration leg as Phase 0, Rule 17] → record update(Λ.carrier, add all Mₑ_passed) → pending   -- Λ.carrier is Some here by construction: this arm is reached only through a Qc answer, and Qc is reached only after the Phase 1 registration that binds it → AssessFit(Λ.R, X, pending) → F' → Λ.fit_map := F' → Σ.scan_count += 1
...
Phase 2 → split (pre-certify): KindBinding.atomicity = non-atomic  -- emerged compound mismatch → split into atomic sub-mismatches, then re-bind_kind → (pending exclusion) → init_occurrence → certify each (same re-scan pass); never deferred/registered as a compound. The split runs BEFORE the pending exclusion and init_occurrence, both of which read identity(m)
...
ONE OCCURRENCE PER CLAIM IN pending is what every scan is directed to keep, not a property the contract can guarantee — sameness is judged, and a judgment carries no invariant. init_occurrence opens no second occurrence for a claim it has already initialized this scan or one already pending, so several detections of one claim become one occurrence, and a detection matching something pending refreshes that registration instead of adding a second.
Mₑ is the Eval output — bound and split FIRST, since everything below reads the binding — minus what already sits in pending, read off identity(m): a mismatch that does not read as the pending one registers on its own. A detection that DOES match a pending registration is not simply dropped: it carries this target's reading of a claim registered against an earlier one, so the pending object takes that reading (aspect, description, evidence, severity) and its entry in C is updated to match, and nothing is registered — and it keeps its OWN id and kind_binding, since a refreshed registration is the same occurrence carrying a newer reading, not a second one, while unrepaired takes this re-scan's stamp on that registration — set where this detection reads as the claim the dispose just closed by adapting, never cleared. A mismatch CLOSED earlier in this run is not excluded from Mₑ either: it re-registers, which un-disposes its aspect (disposed(a), CONVERGENCE) so no record written for an earlier registration can carry a terminal. The ledger match locates the record a return succeeds and discharges nothing (status↔ledger equivalence, pending); what it settles is whether unrepaired is stamped — where m reads as the same claim as Mₛ. Phase 1 closes it by keep_carried_forward only where the user's Keep answered THIS identity; a residue read as a claim of its own is a different registration and reaches the gate.
...
disposed(a)        = (∃ r ∈ Σ.dispositions : aspect(r.mismatch) = a) ∧ a ∉ { aspect(m) : m ∈ pending }
                     ...
                     -- close, so the return opens its own entry and sits in pending, which is what fails the second conjunct while it is open.
...
accepted(i)        = ∃ r ∈ Σ.dispositions : identity(r.mismatch) reads as the same claim as i ∧ r.disposition = Keep ∧ r.judgment ∈ {Some(Upheld), None}
                     ...
                     -- IDENTITY-KEYED rather than label-keyed: a return that reads as the claim they answered is the thing they answered
                     -- and carries; a residue read as a claim of its own is a DIFFERENT registration and
                     -- reaches the gate on its own
                     -- RUN-SCOPED: Σ is bound fresh at activation (MODE STATE, INITIAL BINDING) and the ledger only appends, so within the
                     -- run the Keep record this reads is never withdrawn and nothing un-accepts what the user accepted. Whether a later
                     -- detection reads as covered by that record is that turn's reading (identity, TYPES). Where it does, that REGISTRATION reaches no further
...
bind_kind (sense)   → Internal analysis (capture each detected mismatch as a KindBinding {label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity}. atomicity carries the backbone's meaning — how many distinct aspects this binding bundles — so a detector output asserting two claims lands non-atomic and is split into one atomic sub-mismatch per claim before certify. It is never asked how far a repair would reach: one claim standing on evidence in several places is atomic, and where a repair lands is projected later from an answered disposition (RepairSite, Rule 22). Runs before certify, before the pending exclusion, and before the unrepaired stamping — all three read the binding)
init_occurrence (track) → Internal state update (writes id and unrepaired onto a detected mismatch, or returns None where the detection opens no occurrence, at the one position every scan path states: after split, before certify. It is the sole producer of the handle; the stamp is the re-scan's own and lands here or on a refreshed registration (TYPES, unrepaired). It opens no second occurrence for an identity already initialized this scan or already pending, and on that arm it carries the detection's reading onto the occurrence that survives (TYPES, init_occurrence) rather than dropping it. It performs no user interaction and touches no artifact)
...
match_identity (sense) → Internal analysis (at the Phase 2 re-scan, after bind_kind + split and before registration: compare identity(m) — the binding's positive_predicate together with the evidence set it stands on — against pending and against every record in Σ.dispositions. Settles two readings and nothing else: whether the return is already pending (excluded from Mₑ — and where it is, the pending object takes this detection's current reading, LOOP), and whether unrepaired is stamped, which is settled against Mₛ alone rather than against the records this comparison scans: m read as the same claim as Mₛ and nothing weaker. It reads no id and assigns none (TYPES, MismatchId). THIS TURN IS WHERE unrepaired IS DECIDED and the only one where it can be (CONVERGENCE, unrepaired(m)). It never completes the new registration's entry — a record answers only for the registration it closed)
...
discard (transform) → artifact write (Discard(replacement) disposition: withdraw the result and put the replacement in its place, or remove it outright when the withdrawal leaves nothing behind)
                    ...
                    --   predicate reads it. That is what keeps the three axes ordered rather than merely named (RepairSite, Rule 22)
...
3. **Bind + certify each detected mismatch at registration (fail-closed)**: For each candidate mismatch, set `m.kind_binding = { label, positive_predicate, evidence, origin ∈ {seed, emergent}, atomicity }`; if `atomicity = non-atomic` (the mismatch bundles two distinct aspects), **split before certify**. Then `init_occurrence(m)`, which writes the occurrence's `id` and `unrepaired` and returns `None` where this scan already opened an occurrence for the claim — on that arm the detection's reading is written onto the surviving occurrence rather than dropped, and the pass carries the `Some` results on. Then `m.certificate = certify(m.kind_binding, local_claims)` where the local claims are epharmoge's own claim and its route claims, both inscribed in this SKILL.md (TYPES: `OwnClaim`, `RouteClaim`); the certificate reads nothing outside this file. Fit the predicate against every one of them and collect `claimed_by`, the set of claims the evidence supports:
   ...
   - **`atomicity = non-atomic`** — compound mismatch (two distinct aspects) → **split** into atomic sub-mismatches and re-run bind_kind → init_occurrence → certify on each (same Phase 0 pass).
   - **`status = ambiguous`** (atomic) — `|claimed_by| ≠ 1`, either because several claims hold or because none does on the evidence at hand → ONE narrowed-scope re-assessment at the fixed Phase-0 `(R, X)` (AI-side; no user question — Phase 0 stays silent), then re-certify. the bound is one attempt, because a silent phase cannot ask and a re-read of the same ground has no stopping rule of its own: pass / route / (if still ambiguous) **Residual** (unattributable, never registered, surfaced as residual, non-blocking). Which of the three the re-assessment reaches is read at that turn from the session's own context and the user's wording; it is not fixed in advance here.
...
- Re-evaluate the advanced target `Λ.R` against `X` for remaining AND **newly emerged** mismatches. Bind and split each detection FIRST, since everything below reads the binding, then judge each against `pending`: a detection judged to be a claim already pending refreshes that registration's reading and its carrier entry and registers nothing, keeping its own `id` and `kind_binding` — while `unrepaired` takes this re-scan's stamp on that registration, raised where the detection reads as the claim the dispose just closed by adapting and never cleared. `Mₑ` excludes only what already sits in `pending` — an aspect closed earlier in this run re-registers, and Phase 1 closes it by `keep_carried_forward` wherever the user already accepted it
- **`init_occurrence`, then certify each emerged mismatch at registration (fail-closed), then bind its value space** — `init_occurrence` stamps `unrepaired` here, against the mismatch this cycle's dispose closed by adapting and no other record, and returns `None` for a detection that opens no occurrence — whose reading lands on the occurrence that survives, the same write a refreshed pending registration takes. This happens at the **Phase 2 re-scan** (AFTER the Adapt disposition has already mutated R into R'), temporally separated from the Phase-0 certification of Mᵢ: for each `m ∈ Mₑ`, set `m.kind_binding` and `m.certificate = certify(m.kind_binding, local_claims)` (same local claims, same `pass | route | ambiguous` statuses as Phase 0). Register only certificate-passing members (`Mₑ_passed`); a `status = route` mismatch is closed with disposition `Route(routed_deficit)` and handed to that deficit (GapUnnoticed, ContextInsufficient, BoundaryUndefined — command hints `/gap`, `/inquire`, `/bound`); a non-atomic compound mismatch is split into atomic sub-mismatches and re-certified; an atomic `status = ambiguous` mismatch gets its one bounded re-assessment at the fixed re-scan `(R', X)` → pass / `Route(routed_deficit)` / `Residual`, before registration. Do not filter the certificate-passing set by fit-map category.
...
| Each Phase 1 relay says where its correction lives | a relay-closed mismatch leaves `pending` and reaches no further Qc this run. A disputed CARRY reopens by re-entering on the same result (`accepted(i)` reads `Σ`, rebuilt at activation). A disputed RETRACTION has no such reset — `judgment_settled` reads `Λ.fit_map`, derived from `(Λ.R, X)`, so the same input reproduces the same close, and correcting it is the user's interrupt | The user is told what to reach for, rather than trying a reset that moves only one of the two guards |
