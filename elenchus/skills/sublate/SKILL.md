...
ClaimAudit     = { source: Source, kind_binding: KindBinding, certificate: DeficitFitCertificate, value_space: ValueSpace }
...
bind_value_space : ClaimAudit → ValueSpace              -- the audit's disposition constructors; generated ONLY after certificate.status = pass, and frozen for the cycle
ValueSpace     = the audit's disposition space (local_value_space; elenchus-local instantiation point) ⊆ Disposition
                 -- the certified ClaimRef is the DOMAIN of this function: it sets what each constructor REFERS TO for this audit — what a Revised refinement would rewrite, which external reference a Bounded would name, which measurement a Conditional waits on, which protocol a Routed hands to. Those are read off the bound claim, never authored ahead of it
                 -- Confirmed and Discarded are members for every audit: any certified claim admits both surviving its antithesis and being withdrawn, so no binding removes them. The remaining constructors are members exactly where the bound claim gives them an on-axis trajectory, which is Rule 8's dead-signal test applied to a space that is already bound rather than to an unbound guess at it
                 -- membership is fixed here and presented intact at Phase 2 (Rule 11): binding the space is not the same act as mutating a presented option set
...
Phase 0 bind_value_space (track)       → Internal state update (generate the audit's disposition space ONLY after certificate.status = pass and freeze it for the cycle. The certified ClaimRef sets what each constructor refers to for this audit; Confirmed and Discarded are members for every audit, and the rest are members exactly where the bound claim gives them an on-axis trajectory — Rule 8's dead-signal test applied to a bound space. What is bound here is presented intact at Phase 2, per Rule 11)
...
10. **Closed coproduct discipline**: Disposition is a closed coproduct of seven named variants plus Emergent. The Other option permits free-response, which the AI maps to the closest variant or surfaces as a candidate Emergent variant for that audit — it does not bypass the coproduct. What `bind_value_space` produces is a subset of this same coproduct, not a second one: it selects which variants have an on-axis trajectory for the bound claim and what each of them refers to, and every member it selects is drawn from the seven. So the coproduct stays closed and the answer space stays derived — a variant is never invented for an audit, and a concrete disposition is never authored for one before its claim is certified.
