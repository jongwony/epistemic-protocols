...
W → identify(W) → S_cand → ∀s ∈ S_cand: bind_kind(s) → k [split where non-atomic] → certify(k, local_claims) → keep(status = pass) → bind_value_space → S_high →
  S_high = ∅: emit VettedContext (extension) → deactivate   -- trivial convergence, two causes reported apart: no audit-candidate source at all (Λ.formed = ∅, dispositions empty), or candidates every one of which this protocol handed elsewhere (dispositions = the certificate-assigned records, so NOT empty)
...
converge                (extension)    → TextPresent+Proceed (per-claim disposition trace over the judged audits + the handed/unattributable list, both PROJECTED from J by assigned_by; proceed with VettedContext)
...
5. **Split a compound binding** — if `atomicity = non-atomic` (the candidate is being read as authority for two distinct claims), **split before certify** into one audit per claim and re-run the binding on each. A compound is never admitted: one disposition slot answering for two claims collects a single judgment for both and records it against a thesis the user was never shown whole. **Formation completes here**, once atomicity is settled: each atomic binding — the original where it already was atomic, or each child of a split — becomes an audit with its own ref and is added to `Λ.formed` at that point, and from then the protocol answers for it whatever becomes of it. The compound that produced children never became an audit, so its ref never enters the domain and never has to be discharged.
...
9. If `S_high = ∅`: deactivate trivially, reporting the two causes apart — "no candidate warranted vetting" (nothing was ever formed) and "every candidate was handed elsewhere" (every formed audit carries a certificate-assigned record) are different findings, and only the first emits an empty disposition table
