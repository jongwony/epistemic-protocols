...
Phase 0 → defer (audit-local): certify(a).status = ambiguous  -- |claimed_by| ≠ 1, several claims holding or none; Λ.unattributed := Λ.unattributed ∪ {a} → ONE bounded narrowed-scope re-assessment at the fixed Phase-0 detection state (AI-side, no user question) → pass (→ a is admitted to S_high) / route (→ Phase 0 → route_away) / still ambiguous (→ record J(a.ref) := { disposition = Unattributable(reason), assigned_by = certificate, basis = what the attribution left unresolved — several claims standing, or none }, and report it as a non-blocking residual; a stays in Λ.formed and is discharged by that record). a leaves Λ.unattributed on every arm, so nothing stays parked
...
If ∃ r ∈ Λ.formed : J(r).disposition = Deferred(c) ∧ trigger_met(c): re-run the Phase 0 pipeline on the audit a bearing r — bind_kind → split where non-atomic → certify → bind_value_space — then return to Phase 1 with it as fresh ContextSuspect; new antithesis under updated evidence. The trigger fired because conditions moved, so the claim may have moved with them: re-binding is what keeps the antithesis aimed at the claim as it now stands, and re-certifying is what keeps the fail-closed order holding across the loop rather than only on the first pass. Where the re-certification no longer passes, a leaves S_high and its prior Deferred record is REPLACED by the certificate-assigned one — J(a.ref) := { disposition = Handed(d) or Unattributable(reason), assigned_by = certificate, basis = the cited fit }. a stays in Λ.formed, so the convergence domain does not shrink and dom(J) = Λ.formed keeps holding; what changes is which record discharges it. A loop-time split forms new audits, each with its own ref added to Λ.formed and each pending until judged.
...
Convergence evidence: At vetted(V), present transformation trace — for each a ∈ S_high, show (claim(a) → antithesis(a) → disposition(a)), naming the source each claim was read off. Where one source yielded several audits, each is shown on its own line with its own claim, since that split is what the user was asked to judge separately. Report apart from the trace every audit whose record carries assigned_by = certificate, with the deficit it was handed to or what its attribution left unresolved, and the cited basis — those never entered S_high, so the trace does not range over them, and that list is a PROJECTION of J by who assigned each record rather than a second account kept beside it. Convergence is demonstrated, not asserted.
...
── CONVERGENCE ──
vetted(V): see TYPES
certificate gate: every audit in S_high carried certificate.status = pass (fail-closed, at admission), AND every audit the pipeline formed carries a record in J — routed and unattributable candidates never entered S_high but are discharged in the ledger, so a VettedContext is assembled from audits the own claim holds (claimed_by = {ContextSuspect}), each fit-certified and each atomic. The pass certifies LOCAL admissibility — elenchus's own gate over elenchus's own activation — not the absence of a claim anywhere in the wider protocol set
...
Phase 0 certify_unattributable (extension) → TextPresent+Proceed (conditional: certificate.status stays ambiguous after its one bounded re-assessment → report the candidate as a non-blocking residual with what the attribution left unresolved — several claims standing, or none — and record the Unattributable disposition against the audit's ref (assigned_by = certificate, basis = what the attribution left unresolved), then drop it from admission. Non-blocking: it never entered S_high, so no antithesis is owed for it — but it is in Λ.formed, so convergence answers for it through that record rather than by omitting it)
...
-- formed: WRITTEN as each audit is formed at Phase 0 (and again for each child of a split, at Phase 0 or in the loop), READ by unresolved(Λ), vetted(V) and progress(Λ). It is the convergence DOMAIN and never the working set: an audit the certificate hands elsewhere leaves S_high but stays here, which is what makes the ledger answer for it
...
-- Formed-domain invariant: every audit the pipeline forms is added to Λ.formed at formation and never removed — S_high may shrink across the loop, Λ.formed may only grow
...
6. **Convergence evidence**: Present transformation trace (claim → antithesis → disposition, naming the source each claim is read off) before declaring the working context vetted; per-audit evidence is required, not asserted. Audits the certificate handed elsewhere or left unattributable are listed apart from the trace, since no antithesis was ever put to them — but they carry records like any other, and that list is a projection of the ledger rather than a second account kept beside it.
